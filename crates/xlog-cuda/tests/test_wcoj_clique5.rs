...
/// (2,3), (2,4), (3,4) — 10 edges for K=5; 15 for K=6.
...
fn k5_fixture_u32() -> Vec<Vec<(u32, u32)>> {
    let mut edges: Vec<Vec<(u32, u32)>> = Vec::with_capacity(10);
    for i in 0u32..5 {
        for j in (i + 1)..5 {
...
fn k5_fixture_u64() -> Vec<Vec<(u64, u64)>> {
    let mut edges: Vec<Vec<(u64, u64)>> = Vec::with_capacity(10);
    for i in 0u64..5 {
        for j in (i + 1)..5 {
            edges.push(vec![(i + 1, j + 1)]);
        }
...
fn download_k5_u32(buf: &CudaBuffer) -> Vec<[u32; 5]> {
    ...
    let mut cols: Vec<Vec<u8>> = (0..5).map(|_| vec![0u8; n * 4]).collect();
    for (c, col_bytes) in cols.iter_mut().enumerate().take(5) {
        unsafe {
            sys::cuMemcpyDtoH_v2(
...
fn download_k5_u64(buf: &CudaBuffer) -> Vec<[u64; 5]> {
    let n = match buf.cached_row_count() {
        Some(c) => c as usize,
        None => {
            let mut count = [0u32; 1];
    ...
    let mut cols: Vec<Vec<u8>> = (0..5).map(|_| vec![0u8; n * 8]).collect();
    for (c, col_bytes) in cols.iter_mut().enumerate().take(5) {
        unsafe {
            sys::cuMemcpyDtoH_v2(
                col_bytes.as_mut_ptr() as *mut _,
...
fn run_k5_test_u32_or_symbol(col_type: ScalarType) {
    ...
    for col_idx in 0..5 {
        assert_eq!(
            ...
            Some(col_type),
            "K=5 ({:?}) col {} schema must be preserved",
            col_type,
            col_idx
        );
    }
...
#[test]
fn clique5_u32_round_trips_against_cpu_oracle() {
    run_k5_test_u32_or_symbol(ScalarType::U32);
}

#[test]
fn clique5_symbol_round_trips_against_cpu_oracle() {
    run_k5_test_u32_or_symbol(ScalarType::Symbol);
}

#[test]
fn clique5_u64_round_trips_against_cpu_oracle() {
    let Some(fix) = make_runtime_fixture() else {
        eprintln!("Skipping: CUDA runtime unavailable");
        return;
    };
    let host_edges = k5_fixture_u64();
    let raw_bufs: Vec<CudaBuffer> = host_edges
        .iter()
    ...
    for col_idx in 0..5 {
        assert_eq!(
            out.schema.column_type(col_idx),
            Some(ScalarType::U64),
            "K=5 (U64) col {} schema must be preserved",
            col_idx
        );
    }
}
