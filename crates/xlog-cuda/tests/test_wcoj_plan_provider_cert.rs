...
// ---------------------------------------------------------------
// Test 4: U64 triangle — same seam as Test 1, but every relation
//          is U64-typed. Locks that the planner accepts U64
//          (which is in `WCOJ_SUPPORTED_KEY_TYPES`) and the GPU
//          U64 provider entries produce the same row set as the
//          CPU oracle on the same fixture.
// ---------------------------------------------------------------

fn logic_u64_relation(rows: &[(u64, u64)]) -> LogicRefRelation {
    LogicRefRelation {
        schema: vec![ScalarType::U64, ScalarType::U64],
        rows: rows
            .iter()
            .map(|(a, b)| vec![RefValue::U64(*a), RefValue::U64(*b)])
            .collect(),
    }
}

fn upload_binary_u64_unsorted(memory: &Arc<GpuMemoryManager>, rows: &[(u64, u64)]) -> CudaBuffer {
    let n = rows.len() as u32;
...
fn download_triples_u64(buf: &CudaBuffer) -> Vec<(u64, u64, u64)> {
    ...
    let mut out: Vec<(u64, u64, u64)> = (0..n)
        .map(|i| {
            (
...
fn oracle_rows_to_triples_u64(rows: Vec<Vec<RefValue>>) -> Vec<(u64, u64, u64)> {
    let mut out: Vec<(u64, u64, u64)> = rows
        .into_iter()
        .map(|r| match (&r[0], &r[1], &r[2]) {
            (RefValue::U64(a), RefValue::U64(b), RefValue::U64(c)) => (*a, *b, *c),
            other => panic!("unexpected oracle row shape: {other:?}"),
        })
        .collect();
    out.sort();
...
#[test]
fn plan_provider_cert_u64_triangle_matches_cpu_oracle() {
    // Same triangle topology + planner contract as Test 1, but
    // every relation carries U64-typed schema and keys above
    // u32::MAX. Locks that the GPU U64 provider entries
    // produce identical row sets to `evaluate_rule_typed`.
    let r = rule_with(
        atom("tri", vec![var("X"), var("Y"), var("Z")]),
        vec![
            pos("e1", vec![var("X"), var("Y")]),
    ...
    let mut store: RefRelationStore = BTreeMap::new();
    store.insert("e1".to_string(), logic_u64_relation(&e1_sorted));
    store.insert("e2".to_string(), logic_u64_relation(&e2_sorted));
    store.insert("e3".to_string(), logic_u64_relation(&e3_sorted));

    // ----- planner layer -----
    let plan = plan_rule(&r, &store).expect("plan must succeed for U64 triangle");
    match &plan {
        RulePlan::MultiwayCandidate {
            head_predicate,
            hypergraph,
        ...
        other => panic!(
            "expected MultiwayCandidate for U64 triangle, got {other:?} — \
             planner and provider would disagree on this fixture"
        ),
    }
    ...
    let oracle_rows = evaluate_rule_typed(&r, &store, &AppearanceOrder)
        .expect("evaluate_rule_typed must succeed for the same fixture the planner accepted");
    let cpu_triples = oracle_rows_to_triples_u64(oracle_rows);
    ...
    let buf_e1_raw = upload_binary_u64_unsorted(&fix.memory, &raw_e1);
    let buf_e2_raw = upload_binary_u64_unsorted(&fix.memory, &raw_e2);
    let buf_e3_raw = upload_binary_u64_unsorted(&fix.memory, &raw_e3);

    let layout_stream = fix.pool.acquire().expect("layout stream");
    let buf_e1 = fix
        .provider
        .wcoj_layout_u64_recorded(&buf_e1_raw, layout_stream)
        .expect("layout e1 u64");
    let buf_e2 = fix
        .provider
        .wcoj_layout_u64_recorded(&buf_e2_raw, layout_stream)
        .expect("layout e2 u64");
    let buf_e3 = fix
        .provider
        .wcoj_layout_u64_recorded(&buf_e3_raw, layout_stream)
        .expect("layout e3 u64");

    let triangle_stream = fix.pool.acquire().expect("triangle stream");
    let result = fix
        .provider
        .wcoj_triangle_u64_recorded(&buf_e1, &buf_e2, &buf_e3, triangle_stream)
        .expect("triangle u64");
    let gpu_triples = download_triples_u64(&result);

    assert_eq!(
        gpu_triples, cpu_triples,
        "planner accepted U64 MultiwayCandidate but GPU output disagrees with CPU oracle"
    );
}
