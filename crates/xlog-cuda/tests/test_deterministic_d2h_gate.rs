...
/// Targeted strict-gate coverage for `JoinType::LeftOuter`.
///
...
/// `deterministic_d2h_violation_count()`.
///
/// Together with the inner-join integration test, this proves the
/// metadata-read reclassification covers both `hash_join_inner_v2*`
/// and `hash_join_left_outer_*` count paths under the strict gate.
#[test]
