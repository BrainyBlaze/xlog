...
// result-set correctness, partial-match drop+reuse,
// all-unmatched drop+reuse, empty-right legacy fallback,
// legacy-manager rejection. Build side comes from a cached
// JoinIndexV2; only the probe (left) side is packed on
// launch_stream.
// ───────────────────────────────────────────────────────────

/// Result-set correctness for indexed LeftOuter CSM. Builds
/// left/right with overlapping key spaces so both the matched
/// branch (lk < RKEYS) AND the unmatched-left branch
...
/// Drop+reuse stress with PARTIAL-match graphs.
/// Drops left + right + the index AFTER the call returns; the
...
fn provider_hash_join_left_outer_csm_v2_with_index_recorded_partial_match_survives_drop_and_reuse()
{
    ...
    // Match the shape of the all-unmatched test (which passes
    // with these params): LROWS=256/RROWS=128. The indexed
    // LeftOuter CSM has additional unmatched-mask +
    // compact-tail + Phase-E concat allocations per iter, but
    // higher per-iter LROWS pushes the CUDA mempool harder
    // and gets address recycling visible inside the 512-iter
    // window. RKEYS < LKEYS keeps the partial-match shape.
    const LROWS: usize = 256;
    const RROWS: usize = 128;
    const LKEYS: u32 = 64;
    const RKEYS: u32 = 32;
    ...
    for _ in 0..ITERATIONS {
        ...
        // Drop ALL inputs (including the cached index) WITHOUT host sync.
        drop(left);
        drop(right);
        drop(index);
    ...
    assert_eq!(
        bad_output, 0,
        "indexed CSM left_outer produced corrupted output in {}/{} iterations \
         (reuse_observed={})",
        bad_output, ITERATIONS, reuse_observed,
    );
}
...
fn provider_hash_join_left_outer_csm_v2_with_index_recorded_all_unmatched_survives_drop_and_reuse()
{
    ...
    for _ in 0..ITERATIONS {
        ...
        let result = provider
            ...
            .expect("indexed CSM left_outer all-unmatched");
    ...
    assert_eq!(
        bad_output, 0,
        "indexed CSM left_outer (all-unmatched) produced corrupted output in {}/{} iterations \
         (reuse_observed={})",
        bad_output, ITERATIONS, reuse_observed,
    );
}

// Empty-right is intentionally NOT covered for the indexed
// CSM variant: `build_join_index_v2` rejects empty relations
// ("Cannot build join index for empty relation"), so callers
// cannot construct a `JoinIndexV2` for an empty right buffer.
// In practice callers detect empty right ahead of the index
