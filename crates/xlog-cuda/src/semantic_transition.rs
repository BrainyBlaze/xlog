...
use crate::memory::{CudaColumn, DeviceAllocationProvenance, DeviceMemoryView, DeviceRead, GpuMemoryReservation};
use crate::provider::resident_schedule::{validate_execution_domain, ResidentExecutionDomain};
use crate::semantic_hypergraph::{material_bytes, material_u32, material_u64, SemanticMaterialReader, SemanticRootMaterial};
use crate::{
...
pub trait SemanticTaskProgram: fmt::Debug + Send + Sync {
    fn observe(&self, provider: Arc<CudaKernelProvider>)
...
impl SemanticTaskScoring {
    fn words(self) -> [u64; 6] {
        [self.correct_weight, self.all_correct_weight, self.work_weight,
         self.improvement_weight, self.refusal_weight, self.spent_weight].map(u64::from)
    }
    ...
    fn validate(self) -> Result<(), SemanticTransitionError> {
        ...
        let value_span = 2 * u128::from(self.correct_weight)
            + u128::from(self.all_correct_weight) + 6 * u128::from(self.work_weight);
        let return_bound = value_span * u128::from(self.improvement_weight)
            + 2 * u128::from(self.refusal_weight) + 14 * u128::from(self.spent_weight);
        if return_bound > i64::MAX as u128 {
            return Err(publication_input_error("task scoring can overflow the signed return"));
        }
        Ok(())
    }
}
...
impl SemanticTaskEvaluationSpec {
    fn validate_records(
        ...
        if self.admissible_truth_masks.iter().any(|mask| *mask == 0 || *mask & !15 != 0) {
            return Err(invalid("truth eligibility requires a nonempty four-valued mask"));
        }
        for index in self.statement_records {
            let record = records.records.get(index as usize)
                .ok_or_else(|| invalid("query statement is outside the admission"))?;
            let predicate = records.predicates.iter()
        ...
        if self.allowed_support_records.len() > records.supports.len() {
            return Err(invalid("support selections exceed the admitted support bank"));
        }
        for &index in &self.allowed_support_records {
            let support = records.supports.get(index as usize)
                .ok_or_else(|| invalid("support selection is outside the admission"))?;
            if !self.statement_records.contains(&support.statement) {
                return Err(invalid("support belongs to a statement outside the task scope"));
            }
        }
        Ok(())
    }
...
fn feedback_statement_payload(
    ...
    let append = |payload: &mut Vec<u8>, index: u32| -> Result<(), SemanticTransitionError> {
        ...
        let argument_begin = atom.arguments.first().map_or(atom.bytes.len(), |range| range.start);
        let value_begin = argument_begin.checked_sub(8)
            .ok_or(SemanticTransitionError::InvalidTaskBank)?;
        let values = atom.bytes.get(value_begin..)
            .ok_or(SemanticTransitionError::InvalidTaskBank)?;
...
impl TaskEvaluationBinding {
    pub(crate) fn bind(
        ...
        let statements = [
            admission.statement_key(spec.statement_records[0])
                .map_err(SemanticTransitionError::Semantic)?,
            admission.statement_key(spec.statement_records[1])
                .map_err(SemanticTransitionError::Semantic)?,
        ];
        let statement_bytes = BTreeMap::from([
            (spec.statement_records[0], feedback_statement_payload(
                admission.records(), &admission.encoded_records, spec.statement_records[0],
            )?),
            (spec.statement_records[1], feedback_statement_payload(
                admission.records(), &admission.encoded_records, spec.statement_records[1],
            )?),
        ]);
        ...
        for &index in &spec.allowed_support_records {
            ...
            let statement = admission.statement_key(record.statement)
                .map_err(SemanticTransitionError::Semantic)?;
        ...
        if observation.program_source.is_empty() || observation.result_bytes.is_empty() {
            return Err(publication_input_error("native task observer omitted source or result custody"));
        }
        Ok(Self {
...
impl ActionCodebooks {
    fn support_layout(&self) -> (Vec<std::ops::Range<usize>>, usize) {
        (self.components.iter().map(|component| {
            component.offset as usize..component.offset as usize + component.cardinality as usize
        }).collect(), self.input_cells)
    }
    ...
    fn derive(admission: &SemanticAdmission, owner: u64) -> Result<Self, SemanticTransitionError> {
        ...
        for (record_index, (record, encoded)) in records.records.iter().zip(&admission.encoded_records).enumerate() {
        ...
        let operand_views = operands
            ...
            .map(|(value, ty, sort, record, argument, view)| {
                words.extend([bytes.len() as u64, value.len() as u64, ty, sort, record, argument]);
                bytes.extend(value);
                view
            })
            .collect();
        ...
        let qualifier_views = qualifiers
            ...
            .map(|(refs, source)| {
                ...
                words.extend([offset as u64, (bytes.len() - offset) as u64, u64::from(source)]);
...
fn canonical_text_null(receipt: &SemanticTransitionReceipt) -> bool {
    receipt.kind == COMPONENT_KIND_TEXT as u32 && receipt.choice == TEXT_NULL as u32
        && receipt.legal_count == 1 && receipt.active_count == 1
        && receipt.active_fields == 0 && receipt.null_fields == 0
        && receipt.p == [1, 0, 0] && receipt.q == [1, 0, 0]
        && receipt.factor_denominator == [1, 0, 0]
        && receipt.cdf_start == 0 && receipt.cdf_end == 1 << 63 && receipt.mass == 1 << 63
}
...
impl SemanticStateRole {
    pub fn from_code(code: u64) -> Option<Self> {
        const ROLES: [SemanticStateRole; 55] = [
            SemanticStateRole::PrefixSource, SemanticStateRole::TokenProvenanceRecords,
            SemanticStateRole::PrefixIdentity, SemanticStateRole::AttentionKeys,
            SemanticStateRole::AttentionValues, SemanticStateRole::ConvolutionState,
            SemanticStateRole::RecurrentState, SemanticStateRole::ValueRunningMax,
            SemanticStateRole::ValueDenominator, SemanticStateRole::ValueNumerator,
            SemanticStateRole::EditRunningMax, SemanticStateRole::EditDenominator,
            SemanticStateRole::EditNumerator, SemanticStateRole::CompletionCoverage,
            SemanticStateRole::RawFeedback, SemanticStateRole::FeedbackStatements,
            SemanticStateRole::FeedbackSupportProvenance, SemanticStateRole::SlowWeights,
            SemanticStateRole::FastAdapters, SemanticStateRole::SymbolicHeads,
            SemanticStateRole::OptimizerState, SemanticStateRole::LossScaler,
            SemanticStateRole::TrainingSchedule, SemanticStateRole::Gradients,
            SemanticStateRole::GradientAccumulation, SemanticStateRole::OtherRng,
            SemanticStateRole::ReplayEntries, SemanticStateRole::ReplayPayload,
            SemanticStateRole::TrainingView, SemanticStateRole::IntentEntries,
            SemanticStateRole::IntentPayload, SemanticStateRole::Acknowledgements,
            SemanticStateRole::AttemptReceipt, SemanticStateRole::GoalState,
            SemanticStateRole::WorldLineage, SemanticStateRole::EditJournal,
            SemanticStateRole::Environment, SemanticStateRole::AuthorityEnvelope,
            SemanticStateRole::AuthorityDecisions, SemanticStateRole::CheckpointLineage,
            SemanticStateRole::SchemaContract, SemanticStateRole::TokenizerContract,
            SemanticStateRole::RuntimeContract, SemanticStateRole::ModelContract,
            SemanticStateRole::TopologyContract, SemanticStateRole::PositionTable,
            SemanticStateRole::ActionCatalogue, SemanticStateRole::ActionCodebooks,
            SemanticStateRole::ProofRecords, SemanticStateRole::LearningCopyReset,
            SemanticStateRole::ActiveAttentionKeys, SemanticStateRole::ActiveAttentionValues,
            SemanticStateRole::ActiveConvolution, SemanticStateRole::ActiveRecurrent,
...
impl SemanticModelContractLayout {
    fn validate(self, record_bytes: u64) -> Result<(), SemanticTransitionError> {
        let spans = [(self.schema_begin, self.schema_bytes), (self.schema_digest_offset, 32),
            (self.generation_offset, 8), (self.numerical_digest_offset, 32), (self.identity_offset, 32)];
        for (index, &(begin, bytes)) in spans.iter().enumerate() {
            if bytes == 0 || begin > record_bytes || bytes > record_bytes - begin
                || spans[..index].iter().any(|&(other, length)| begin < other + length && other < begin + bytes) {
                return Err(publication_input_error("model contract spans must be nonempty, disjoint and within its actual bytes"));
            }
        }
        Ok(())
    }
...
impl ModelMemoryGeometry {
    fn location(&self, role: u64, index: u64) -> Result<(usize, usize), SemanticTransitionError> {
        let view = self.views.iter().find(|view| (view.role, view.index) == (role, index))
            .ok_or_else(|| publication_input_error("model tensor lacks its producer storage view"))?;
        let storage = usize::try_from(view.storage).ok().and_then(|index| self.storages.get(index))
            .ok_or_else(|| publication_input_error("model view names an absent storage object"))?;
        let allocation = usize::try_from(storage.allocation).ok()
            .filter(|&index| index < self.allocation_bytes.len())
            .ok_or_else(|| publication_input_error("model storage names an absent backing allocation"))?;
        let offset = storage.byte_offset.checked_add(view.byte_offset)
            .and_then(|offset| usize::try_from(offset).ok())
            .ok_or_else(|| publication_input_error("model storage view offset overflows"))?;
        Ok((allocation, offset))
    }

    fn validate(&self, layouts: &BTreeMap<(u64, u64), SemanticTensorLayout>) -> Result<(), SemanticTransitionError> {
        let mut allocations = BTreeSet::new();
        for storage in &self.storages {
            let capacity = usize::try_from(storage.allocation).ok()
            ...
            if usize::try_from(*capacity).is_err()
                || storage.byte_offset.checked_add(storage.span_bytes).is_none_or(|end| end > *capacity) {
                return Err(publication_input_error("model storage exceeds its complete backing allocation"));
            }
        ...
        for view in &self.views {
            ...
            if !matches!(view.role, 18..=25) || previous.is_some_and(|previous| previous >= key) {
                return Err(publication_input_error("model views must enumerate unique model tensor coordinates in order"));
            }
            previous = Some(key);
            let layout = layouts.get(&key)
                .ok_or_else(|| publication_input_error("model storage view lacks its original tensor layout"))?;
            ...
            if layout.logical_axis != u64::MAX || !(offset as u64).is_multiple_of(layout.element_bytes)
                || view.byte_offset.checked_add(span).is_none_or(|end| end > storage.span_bytes) {
                return Err(publication_input_error("typed model view exceeds or misaligns its declared storage"));
            }
            storages.insert(view.storage);
        }
        if allocations.len() != self.allocation_bytes.len() || storages.len() != self.storages.len()
            || self.views.len() != layouts.values().filter(|layout| matches!(layout.role, 18..=25)).count() {
            return Err(publication_input_error("model memory map omits a tensor or contains unreachable storage"));
        }
        Ok(())
    }

    fn encode_into(&self, bytes: &mut Vec<u8>) -> Result<(), SemanticTransitionError> {
        for count in [self.allocation_bytes.len(), self.storages.len(), self.views.len()] {
            material_u32(bytes, u32::try_from(count).map_err(|_| SemanticTransitionError::GenerationExhausted)?);
        }
        for &extent in &self.allocation_bytes { material_u64(bytes, extent); }
        for storage in &self.storages {
            for field in [storage.allocation, storage.byte_offset, storage.span_bytes] { material_u64(bytes, field); }
        }
        for view in &self.views {
            for field in [view.role, view.index, view.storage, view.byte_offset] { material_u64(bytes, field); }
        }
        Ok(())
    }

    fn digest(&self, layouts: &BTreeMap<(u64, u64), SemanticTensorLayout>)
        ...
        let model = layouts.values().filter(|layout| matches!(layout.role, 18..=25));
        material_u64(&mut bytes, model.clone().count() as u64);
        for layout in model {
            for field in [layout.role, layout.index, layout.element_bytes, layout.scalar_type,
                layout.rank, layout.logical_axis].into_iter().chain(layout.dimensions).chain(layout.strides_bytes) {
    ...
    fn decode_from(reader: &mut SemanticMaterialReader<'_>) -> Result<Self, SemanticTransitionError> {
        let allocations = reader.count(8).map_err(SemanticTransitionError::Semantic)?;
        let storages = reader.count(24).map_err(SemanticTransitionError::Semantic)?;
        let views = reader.count(32).map_err(SemanticTransitionError::Semantic)?;
        let mut geometry = Self::default();
        for _ in 0..allocations { geometry.allocation_bytes.push(reader.u64().map_err(SemanticTransitionError::Semantic)?); }
        for _ in 0..storages {
...
fn prepare_model_memory(allocations: &[PreparedSemanticTensor], storages: Vec<SemanticModelStorage>,
    views: Vec<SemanticModelView>, tensors: &[PreparedSemanticTensor])
    -> Result<ModelMemoryGeometry, SemanticTransitionError> {
    let mut geometry = ModelMemoryGeometry { allocation_bytes: Vec::new(), storages, views };
    for (index, allocation) in allocations.iter().enumerate() {
        let layout = allocation.layout;
        if layout.role != 0 || layout.index != index as u64 || layout.rank != 1
            || layout.scalar_type != 1 || layout.element_bytes != 1 || layout.logical_axis != u64::MAX
            || layout.strides_bytes != [1, 0, 0, 0] || allocation.logical_begin != 0 || allocation.logical_end != 0 {
            return Err(publication_input_error("model backing producer must be its complete ordinal U8 vector"));
        }
        let bytes = tensor_layout_bytes(&layout)? as u64;
        let end = allocation.data.checked_add(bytes).ok_or(SemanticTransitionError::GenerationExhausted)?;
        ...
        for (prior, &prior_bytes) in allocations[..index].iter().zip(&geometry.allocation_bytes) {
            if bytes != 0 && prior_bytes != 0 && allocation.data < prior.data + prior_bytes && prior.data < end {
                return Err(publication_input_error("declared model backing allocations overlap"));
            }
        }
        geometry.allocation_bytes.push(bytes);
    }
    let layouts = tensors.iter().map(|tensor| ((tensor.layout.role, tensor.layout.index), tensor.layout)).collect();
    geometry.validate(&layouts)?;
    for tensor in tensors.iter().filter(|tensor| matches!(tensor.layout.role, 18..=25)) {
        ...
        if allocations[allocation].data.checked_add(offset as u64) != Some(tensor.data) {
            return Err(publication_input_error("original tensor does not occupy its producer-declared backing view"));
        }
    }
    Ok(geometry)
}
...
impl SemanticParentBinding {
    ...
    pub fn bind_initial_sources(
        ...
        if self.recovered_instance.is_some() || self.provenance_records != 0
            || self.records.iter().any(|record| record.role == SemanticStateRole::TokenProvenanceRecords)
            || self.role_counts[SemanticStateRole::TokenProvenanceRecords as usize - 1] != 1 {
            return Err(publication_input_error("fresh source provenance must be constructed by the controller"));
        }
        let (source, prefix, record) = initial_source_ledger(
            self.source, self.prefix.clone(), sources, mapping, authority,
            self.authority_generation, capacity_records,
        )?;
        self.source = source;
        self.prefix = prefix;
...
fn initial_source_ledger(
    ...
    generation: u64,
    capacity: u64,
) -> Result<([SemanticTextSlot; 32], Vec<SemanticTextSlot>, SemanticStateRecord), SemanticTransitionError> {
    let invalid = publication_input_error;
    let source_count = sources.len();
    let sources = sources.iter().map(|source| (source.identity.as_str(), source))
        .collect::<BTreeMap<_, _>>();
    if sources.len() != source_count || sources.values().any(|source| source.identity.is_empty()
        || source.origin.is_empty() || source.tokens.is_empty()
        || source.tokens.iter().any(|&token| token >= TEXT_CARDINALITY as u64 || token == MASK_TOKEN as u64)) {
        return Err(invalid("observed sources require unique identities and complete valid token payloads"));
    }
    if authority.is_empty() || generation == 0
        || capacity < (mapping.len() as u64).checked_add(32).ok_or_else(|| invalid("source ledger capacity overflows"))? {
        return Err(invalid("source ledger requires complete authority and append capacity"));
    }
    let mut positions = BTreeMap::new();
    for entry in mapping {
        let source = sources.get(entry.source.as_str()).ok_or_else(|| invalid("source mapping names an unadmitted payload"))?;
        let offset = usize::try_from(entry.offset).map_err(|_| invalid("source token offset overflows"))?;
        if offset >= source.tokens.len() || positions.insert(entry.logical_position, entry).is_some() {
            return Err(invalid("source mapping has invalid payload, offset, or duplicate logical position"));
        }
    }
    let authority_digest = Identity256::from_bytes(Sha256::digest(authority).into());
    let mut bytes = Vec::with_capacity(mapping.len().checked_mul(size_of::<TokenProvenance>())
        .ok_or_else(|| invalid("source ledger extent overflows"))?);
    let mut seen = BTreeSet::new();
    for (physical_slot, slot) in prefix.iter_mut().map(|slot| (u64::MAX, slot))
        .chain(ring.iter_mut().enumerate().map(|(index, slot)| (index as u64, slot))) {
        if slot.kind != 1 {
            if slot.kind > 2 || slot.provenance != 0 || slot.provenance_record != 0 {
                return Err(invalid("MASK or PAD cannot carry initial SOURCE provenance"));
            }
            continue;
        }
        let entry = positions.get(&slot.logical_position)
            .ok_or_else(|| invalid("every initial FILLED position requires its exact source mapping"))?;
        let source = sources[entry.source.as_str()];
        let token = source.tokens[entry.offset as usize];
        if slot.provenance != 1 || slot.provenance_record != 0 || slot.token != token
            || !seen.insert(slot.logical_position) {
            return Err(invalid("initial FILLED token differs from its admitted SOURCE payload or position"));
        }
        ...
        let logical = unsafe { std::slice::from_raw_parts((&record as *const TokenProvenance).cast::<u8>(), size_of::<TokenProvenance>()) };
        ...
        bytes.extend_from_slice(unsafe { std::slice::from_raw_parts((&record as *const TokenProvenance).cast::<u8>(), size_of::<TokenProvenance>()) });
    }
    if seen.len() != mapping.len() {
        return Err(invalid("source mapping includes a MASK, PAD, or absent logical position"));
    }
    let capacity_bytes = usize::try_from(capacity).ok()
        .and_then(|count| count.checked_mul(size_of::<TokenProvenance>()))
        .ok_or_else(|| invalid("source ledger capacity overflows"))?;
    Ok((ring, prefix, SemanticStateRecord {
        role: SemanticStateRole::TokenProvenanceRecords, index: 0, bytes, capacity_bytes,
    }))
}
...
fn continuation_service_layouts(first_index: usize, capacity: u64)
    -> Result<[SemanticTensorLayout; 6], SemanticTransitionError> {
    let first_index = u64::try_from(first_index).ok().filter(|index| index.checked_add(5).is_some())
        .ok_or_else(|| publication_input_error("continuation service roster overflow"))?;
    if capacity < 34 { return Err(publication_input_error("continuation active capacity lacks its source and feedback slots")); }
    let layout = |offset, scalar_type, element_bytes, rank, dimensions| {
        canonical_tensor_layout(SemanticTensorLayout {
            role: 0, index: first_index + offset, scalar_type, element_bytes, rank, dimensions,
            logical_axis: u64::MAX, strides_bytes: [0; 4],
        })
    };
    Ok([
...
struct TextBinding { rows: u64, count: u64, selected: u64 }
...
enum TextBindingParent {
    Published { bank: usize, header: PublicationHeader },
...
impl TextBindingStorage {
    fn published_parent(&self) -> Result<(usize, PublicationHeader), SemanticTransitionError> {
        match &self.parent {
            ...
            TextBindingParent::Prepared(_) => Err(publication_input_error("a prepared invocation has no host-observed parent header")),
        }
    }
    fn descriptor(&self) -> TextBinding { TextBinding {
        rows: self.inputs[0].data, count: self.inputs[1].data, selected: self.inputs[2].data,
    } }

    fn record(&self, recorder: &mut LaunchRecorder) {
        for input in &self.inputs {
            if let Some(source) = &input.source { recorder.read(source); }
        }
    }

    fn continuation_inputs(&self, kind: SemanticTransitionKind, authority_bytes: u64) -> ContinuationInputs {
        ContinuationInputs { text: self.descriptor(), active_rows: self.inputs[3].data,
            active_row_count: self.inputs[4].data, numerical_admissibility: self.inputs[5].data,
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum SemanticTransitionKind { Proposal, Recompute, Drain }

impl SemanticTransitionKind {
    fn code(self) -> u64 {
        match self { Self::Proposal => 1, Self::Recompute => 2, Self::Drain => 3 }
    }
}
...
fn validate_publication_counts(counts: &[u64; 55]) -> Result<usize, SemanticTransitionError> {
    ...
    for (index, &count) in counts.iter().enumerate() {
        ...
        if (!is_tensor_role(role) && count != if role == 16 { 2 } else { 1 })
            ...
            || (role == 18 && count == 0)
        {
            return Err(publication_input_error("publication role count omits or duplicates a required owner"));
        }
        total = total.checked_add(count).ok_or_else(|| publication_input_error("role count overflow"))?;
    }
    for (left, right) in [(4, 5), (6, 7), (4, 51), (5, 52), (6, 53), (7, 54)] {
        if counts[left - 1] != counts[right - 1] {
            return Err(publication_input_error("paired cache and active tensor layer counts differ"));
        }
    }
...
fn tensor_layout_bytes(layout: &SemanticTensorLayout) -> Result<usize, SemanticTransitionError> {
    if layout.rank > 4
        || !matches!((layout.scalar_type, layout.element_bytes), (1, 1) | (2, 4) | (3, 8) | (4, 2) | (5, 2) | (6, 4) | (7, 8) | (8, 1))
        || (layout.logical_axis != u64::MAX && layout.logical_axis >= layout.rank)
        || layout.dimensions[layout.rank as usize..].iter().any(|&n| n != 0)
        || layout.strides_bytes[layout.rank as usize..].iter().any(|&n| n != 0)
    {
        return Err(publication_input_error("invalid tensor rank, scalar type, or unused dimensions"));
    }
    if layout.dimensions[..layout.rank as usize].contains(&0) { return Ok(0); }
    let mut axes: Vec<_> = (0..layout.rank as usize).filter(|&i| layout.dimensions[i] > 1).collect();
    ...
    for axis in axes {
        ...
        if stride < span || stride % layout.element_bytes != 0 {
            return Err(publication_input_error("overlapping or unaligned tensor layout"));
        }
        span = stride.checked_mul(layout.dimensions[axis] - 1).and_then(|n| span.checked_add(n))
...
fn empty_model_tensor_layout(layout: &SemanticTensorLayout) -> bool {
    matches!(layout.role, 18..=25) && layout.logical_axis == u64::MAX
        && tensor_layout_bytes(layout).is_ok_and(|bytes| bytes == 0)
}

fn prefix_capacity_layout(mut layout: SemanticTensorLayout, capacity: u64) -> Result<SemanticTensorLayout, SemanticTransitionError> {
    if !matches!(layout.role, 4 | 5) || layout.rank != 4 || layout.logical_axis != 2
        || layout.dimensions[0] != 1 || layout.dimensions[2] > capacity
    { return Err(publication_input_error("prefix cache layout must be batch-one rank-four with position axis two")); }
    layout.dimensions[2] = capacity;
    canonical_tensor_layout(layout)
}

fn canonical_tensor_layout(mut layout: SemanticTensorLayout) -> Result<SemanticTensorLayout, SemanticTransitionError> {
    if layout.rank > 4 { return Err(publication_input_error("canonical tensor rank is invalid")); }
    let mut stride = layout.element_bytes;
    for axis in (0..layout.rank as usize).rev() {
        layout.strides_bytes[axis] = stride;
        stride = stride.checked_mul(layout.dimensions[axis].max(1)).ok_or_else(|| publication_input_error("canonical tensor stride overflow"))?;
    }
    tensor_layout_bytes(&layout)?;
    Ok(layout)
}

fn validate_active_capacity(layout: &SemanticTensorLayout, capacity: u64) -> Result<(), SemanticTransitionError> {
    tensor_layout_bytes(layout)?;
    let axis = if matches!(layout.role, 51 | 52) {
        if layout.rank != 4 || layout.dimensions[0] != 1 { return Err(publication_input_error("active attention must be batch-one rank-four")); }
        2
    } else if matches!(layout.role, 53 | 54) { 0 }
    else { return Err(publication_input_error("active capacity requires an active tensor role")); };
    if layout.logical_axis != axis || layout.dimensions[axis as usize] != capacity || capacity == 0
        || layout.dimensions[..layout.rank as usize].contains(&0)
        || !matches!(layout.scalar_type, 4..=6)
    { return Err(publication_input_error("active tensor row axis must have the exact window plus feedback capacity")); }
    Ok(())
}

fn continuation_capacity_layout(mut model: SemanticTensorLayout) -> Result<SemanticTensorLayout, SemanticTransitionError> {
    tensor_layout_bytes(&model)?;
    if !continuation_role(model.role) || !is_tensor_role(model.role) {
        return Err(publication_input_error("continuation layout has no forward tensor owner"));
    }
    let active = matches!(model.role, 51..=54);
    let suffix = matches!(model.role, 4 | 5);
    let axis = if active { if matches!(model.role, 51 | 52) { 2 } else { 0 } }
        else if suffix { 2 } else { u64::MAX };
    if model.logical_axis != axis || (axis != u64::MAX && axis >= model.rank) {
        return Err(publication_input_error("continuation tensor has the wrong original row axis"));
    }
    if suffix {
        model.dimensions[2] = 0;
        prefix_capacity_layout(model, 32)
    } else { canonical_tensor_layout(model) }
}

fn continuation_tensor_layout(actual: &SemanticTensorLayout, begin: u64, end: u64,
    ...
    if destination != expected {
        return Err(publication_input_error("continuation tensor differs from its cold fixed-capacity layout"));
    }
    ...
    let capacity = if expected.logical_axis == u64::MAX { 0 }
        else { expected.dimensions[expected.logical_axis as usize] };
    if (begin, end) != (0, capacity) {
        return Err(publication_input_error("continuation transport differs from its original fixed-capacity output"));
    }
    Ok(destination)
}

/// Cold copy geometry only: no tensor value is inspected or copied through host memory.
fn tensor_copy_plan(source: &SemanticTensorLayout, destination: &SemanticTensorLayout) -> Result<Vec<(u64, u64, usize)>, SemanticTransitionError> {
    tensor_layout_bytes(source)?;
    tensor_layout_bytes(destination)?;
    if source.rank != destination.rank || source.scalar_type != destination.scalar_type
        || source.element_bytes != destination.element_bytes
        || (0..source.rank as usize).any(|axis| source.dimensions[axis] > destination.dimensions[axis])
    { return Err(publication_input_error("tensor snapshot layout does not fit its owned destination")); }
    if source.dimensions[..source.rank as usize].contains(&0) { return Ok(Vec::new()); }
    let mut first_outer = source.rank as usize;
    let mut chunk = source.element_bytes;
    while first_outer > 0 {
        let axis = first_outer - 1;
        if source.strides_bytes[axis] != chunk || destination.strides_bytes[axis] != chunk { break; }
        chunk = chunk.checked_mul(source.dimensions[axis]).ok_or_else(|| publication_input_error("copy chunk overflow"))?;
        first_outer = axis;
    }
    let count = source.dimensions[..first_outer].iter().try_fold(1u64, |count, &dim| count.checked_mul(dim))
        .ok_or_else(|| publication_input_error("copy row count overflow"))?;
    let mut result = Vec::new();
    result.try_reserve(usize::try_from(count).map_err(|_| publication_input_error("copy row count exceeds host extent"))?)
...
unsafe fn semantic_tensor_metadata(actual: &crate::dlpack::DLTensor)
    -> Result<TensorMetadata<'_>, SemanticTransitionError> {
    if !(0..=4).contains(&actual.ndim) || (actual.ndim != 0 && actual.shape.is_null()) {
        return Err(publication_input_error("semantic tensor requires an original scalar through rank-four shape"));
    }
    let rank = actual.ndim as usize;
    Ok(TensorMetadata {
        device_type: actual.device.device_type, device_id: actual.device.device_id, dtype: actual.dtype,
        // SAFETY: a nonempty shape was checked above and is owned by the caller.
        shape: if rank == 0 { &[] } else { unsafe { std::slice::from_raw_parts(actual.shape, rank) } },
        strides: if rank == 0 || actual.strides.is_null() { None } else {
            // SAFETY: DLPack's retained producer owns ndim stride elements.
            Some(unsafe { std::slice::from_raw_parts(actual.strides, rank) })
        },
        data: actual.data as usize as u64, byte_offset: actual.byte_offset,
    })
}

fn publication_input_error(detail: &str) -> SemanticTransitionError {
    SemanticTransitionError::InvalidInput { detail: detail.into() }
}
...
fn validate_tensor_metadata(
    ...
    let invalid = || publication_input_error("semantic tensor metadata differs from its exact model contract");
    ...
    if actual.device_type != crate::dlpack::K_DLCUDA || actual.device_id != device_id
        || rank > 4 || declared.rank != rank as u64
        || actual.dtype.code != dtype_code || actual.dtype.bits != bits || actual.dtype.lanes != 1
        ...
        || declared.dimensions[rank..].iter().any(|&dimension| dimension != 0)
        || declared.strides_bytes[rank..].iter().any(|&stride| stride != 0)
    {
        return Err(invalid());
    }
    ...
    if dimensions != declared.dimensions { return Err(invalid()); }
    let mut strides = [0u64; 4];
    if let Some(actual_strides) = actual.strides {
        if actual_strides.len() != rank { return Err(invalid()); }
        for (index, &stride) in actual_strides.iter().enumerate() {
            strides[index] = u64::try_from(stride).ok()
                .and_then(|stride| stride.checked_mul(element_bytes)).ok_or_else(invalid)?;
        }
    } else {
    ...
    if strides != declared.strides_bytes { return Err(invalid()); }
    ...
    let bytes = if empty {
        ...
        let mut axes = (0..rank).filter(|&axis| dimensions[axis] > 1).collect::<Vec<_>>();
        axes.sort_by_key(|&axis| strides[axis]);
        let mut span = element_bytes;
        for axis in axes {
            if strides[axis] < span { return Err(invalid()); }
            span = strides[axis].checked_mul(dimensions[axis] - 1)
                .and_then(|bytes| span.checked_add(bytes)).ok_or_else(invalid)?;
        }
        span
    };
    if !actual.byte_offset.is_multiple_of(element_bytes)
        || (bytes != 0 && actual.data == 0)
    {
        return Err(invalid());
    }
    let pointer = actual.data.checked_add(actual.byte_offset).ok_or_else(invalid)?;
...
unsafe fn policy_score_cotangent_metadata(actual: &crate::dlpack::DLTensor, device_id: i32)
    -> Result<(u64, usize), SemanticTransitionError> {
    // SAFETY: the caller retains the original shape/stride descriptor arrays.
    let (rows, scalar, data, bytes) = unsafe { crate::dlpack::dlpack_tensor_metadata(device_id, actual) }
        .map_err(|error| publication_input_error(&format!("selected-score cotangent metadata: {error}")))?;
    if rows != COMPONENT_COUNT as u64 || scalar != xlog_core::ScalarType::F64
        || data.checked_add(bytes as u64).is_none() {
        return Err(publication_input_error("selected-score cotangents require exact contiguous FP64 storage"));
    }
    Ok((data, bytes))
}
...
fn prepare_semantic_tensors(
    ...
    let mut pending = RetainedTensorAdmission { inputs: Some(inputs), stream: Arc::clone(&stream), ready: false };
    let mut geometry = Vec::new();
    for input in pending.inputs.as_ref().expect("retained original producer roster") {
        let pointer = input.tensor.as_ptr();
        if pointer.is_null() {
            return Err(publication_input_error("semantic tensor managed owner is null"));
        }
        ...
        let (data, bytes) = validate_tensor_metadata(&input.layout, &metadata, provider.device().ordinal() as i32)?;
    ...
    pending.ready = true;
    let inputs = pending.inputs.take().expect("retained original producer roster");
    ...
    for (input, (data, bytes)) in inputs.into_iter().zip(geometry) {
        let (source, empty_owner) = if bytes == 0 {
            ...
            let column = unsafe { CudaColumn::dlpack(data, bytes, Arc::clone(&stream), input.tensor) };
...
fn complete_tensor_handoff<T: Send + 'static>(provider: &CudaKernelProvider, inputs: Vec<T>) -> Result<Vec<T>, SemanticTransitionError> {
    let stream = Arc::clone(provider.device().inner().stream());
    let mut pending = RetainedTensorAdmission { inputs: Some(inputs), stream, ready: false };
    ...
    pending.stream.synchronize().map_err(|error| runtime_error("tensor producer handoff", error))?;
    pending.ready = true;
    Ok(pending.inputs.take().expect("retained tensor admission owns its inputs"))
}
...
#[derive(Clone, Copy)]
enum PublicationBankField { Source, PrefixExtent, RingHead, Terminal }

impl PublicationBankField {
    fn layout(self) -> (std::ops::Range<usize>, Vec<i64>, Vec<i64>) {
        let (offset, bytes, shape, strides) = match self {
            Self::Source => (std::mem::offset_of!(PublicationBank, source),
                size_of::<[SourceSlot; 32]>(), vec![32, 8], vec![8, 1]),
            Self::PrefixExtent => (std::mem::offset_of!(PublicationBank, header)
                + std::mem::offset_of!(PublicationHeader, prefix_extent),
                size_of::<u64>(), vec![1], vec![1]),
            Self::RingHead => (std::mem::offset_of!(PublicationBank, header)
                + std::mem::offset_of!(PublicationHeader, ring_head),
                size_of::<u64>(), vec![1], vec![1]),
            Self::Terminal => (std::mem::offset_of!(PublicationBank, header)
...
fn tensor_table_capacity(layout_count: usize, row_count: u64) -> Result<usize, SemanticTransitionError> {
    layout_count.checked_mul(size_of::<SemanticTensorLayout>())
        .and_then(|n| n.checked_add(size_of::<TensorLayoutTableHeader>()))
        .and_then(|n| usize::try_from(row_count).ok()?.checked_mul(size_of::<SemanticActiveRow>())?.checked_add(n))
        .ok_or_else(|| publication_input_error("tensor layout table extent overflow"))
}

fn tensor_table_bytes(layouts: &[SemanticTensorLayout], computed: bool, rows: &[SemanticActiveRow]) -> Result<Vec<u8>, SemanticTransitionError> {
    if !computed && !rows.is_empty() { return Err(publication_input_error("uncomputed active cache cannot contain rows")); }
    let length = tensor_table_capacity(layouts.len(), rows.len() as u64)?;
    let mut bytes = Vec::new();
    bytes.try_reserve_exact(length).map_err(|error| runtime_error("tensor layout table allocation", error))?;
    bytes.extend(publication_abi_bytes(&[TensorLayoutTableHeader { abi: 1, layout_count: layouts.len() as u64,
        active_computed: u64::from(computed), active_row_count: rows.len() as u64 }]));
    bytes.extend(publication_abi_bytes(layouts));
    bytes.extend(publication_abi_bytes(rows));
    Ok(bytes)
}

fn decode_tensor_table(bytes: &[u8]) -> Result<(TensorLayoutTableHeader, Vec<SemanticTensorLayout>, SemanticActiveRows), SemanticTransitionError> {
    if bytes.len() < size_of::<TensorLayoutTableHeader>() { return Err(SemanticTransitionError::ObservationMismatch); }
    // SAFETY: checked complete integer-only header; byte storage need not align.
    let header = unsafe { bytes.as_ptr().cast::<TensorLayoutTableHeader>().read_unaligned() };
    let count = usize::try_from(header.layout_count).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
    if header.abi != 1 || header.active_computed > 1 || (header.active_computed == 0 && header.active_row_count != 0)
        || tensor_table_capacity(count, header.active_row_count)? != bytes.len()
    { return Err(SemanticTransitionError::ObservationMismatch); }
    let layouts_end = size_of::<TensorLayoutTableHeader>() + count * size_of::<SemanticTensorLayout>();
    let layouts = bytes[size_of::<TensorLayoutTableHeader>()..layouts_end].chunks_exact(size_of::<SemanticTensorLayout>())
        // SAFETY: each chunk is a complete integer-only C record.
        .map(|chunk| unsafe { chunk.as_ptr().cast::<SemanticTensorLayout>().read_unaligned() }).collect();
    let rows = bytes[layouts_end..].chunks_exact(size_of::<SemanticActiveRow>())
        // SAFETY: each chunk is a complete integer-only C record.
        .map(|chunk| unsafe { chunk.as_ptr().cast::<SemanticActiveRow>().read_unaligned() }).collect();
    Ok((header, layouts, SemanticActiveRows { computed: header.active_computed == 1, rows }))
}
...
impl PublicationMaterialRange {
    fn into_feedback_record(self) -> Result<SemanticFeedbackRecordMaterial, SemanticTransitionError> {
        if !matches!(self.range.role, 15..=17) || self.range.generation != 1
            || self.range.length_bytes != self.bytes.len() as u64 || self.bytes.is_empty()
            || self.bytes.len() > self.capacity || self.range.logical_begin != 0
            || (self.range.role != 15 && self.range.logical_end != 0)
            || (self.range.role == 15 && (self.bytes.len() % size_of::<RawFeedbackRecord>() != 0
                || self.range.logical_end > (self.bytes.len() / size_of::<RawFeedbackRecord>()) as u64))
        ...
        Ok(SemanticFeedbackRecordMaterial {
            ...
            index: self.range.index, generation: self.range.generation, capacity_bytes: self.capacity as u64,
            logical_begin: self.range.logical_begin, logical_end: self.range.logical_end,
            identity: self.range.digest, bytes: self.bytes,
        })
    }
    ...
    fn record_digest(&self, bytes: &[u8]) -> Identity256 {
        ...
        prefix[..5].copy_from_slice(&[0x786c6f6772616e31, self.range.role,
            self.range.index, self.range.logical_begin, self.range.logical_end]);
        let mut hash = Sha256::new();
        for word in prefix { hash.update(word.to_le_bytes()); }
        hash.update(bytes);
        Identity256::from_bytes(hash.finalize().into())
    }
    ...
    fn attempt(&self) -> Result<AttemptReceipt, SemanticTransitionError> {
        if self.range.role != 33 || self.bytes.len() != size_of::<AttemptReceipt>() {
            return Err(publication_input_error("replay evidence requires the whole native attempt receipt"));
        }
    ...
    fn logical_record_digest(&self) -> Result<Identity256, SemanticTransitionError> {
        ...
        match self.range.role {
            14 | 33 => {
                ...
                } else { (size_of::<AttemptReceipt>(), 1, 6, 39) };
                if bytes.len() != extent {
                    return Err(publication_input_error("replay record has another native ABI extent"));
                }
            ...
            30 => {
                if bytes.len() < size_of::<IntentQueueHeader>() {
                    return Err(publication_input_error("replay intent queue omits its native header"));
                }
                // SAFETY: the checked prefix is an all-integer, padding-free ABI.
                let header = unsafe { std::ptr::read_unaligned(bytes.as_ptr().cast::<IntentQueueHeader>()) };
                if header.abi != 1 || header.count > header.capacity
                    || header.count.checked_mul(size_of::<IntentEntry>() as u64)
                        .and_then(|n| n.checked_add(size_of::<IntentQueueHeader>() as u64)) != Some(bytes.len() as u64) {
                    return Err(publication_input_error("replay intent queue differs from its exact native extent"));
                }
                for entry in bytes[size_of::<IntentQueueHeader>()..].chunks_exact_mut(size_of::<IntentEntry>()) {
                    entry[30 * 8..35 * 8].fill(0);
                }
            }
            27 | 28 | 31 | 32 => {}
            _ => return Err(publication_input_error("record is not a native replay comparison record")),
        }
        Ok(self.record_digest(&bytes))
    }

    fn encode_into(&self, bytes: &mut Vec<u8>) -> Result<(), SemanticTransitionError> {
        material_bytes(bytes, &publication_abi_bytes(&[self.range])).map_err(SemanticTransitionError::Semantic)?;
    ...
    fn decode_from(reader: &mut SemanticMaterialReader<'_>) -> Result<Self, SemanticTransitionError> {
        ...
        let capacity = usize::try_from(reader.u64().map_err(SemanticTransitionError::Semantic)?)
            .map_err(|_| publication_input_error("publication material capacity exceeds host extent"))?;
        let bytes = reader.bytes().map_err(SemanticTransitionError::Semantic)?.to_vec();
        Ok(Self { range, capacity, bytes })
    }
}
...
fn publication_fold_digest(digest: Identity256, role: u64, index: u64, content: Identity256) -> Identity256 {
    let mut hash = Sha256::new();
...
fn publication_action_receipts_digest(codebook: &PublicationMaterialRange, receipts: &[SemanticTransitionReceipt])
    -> Result<Identity256, SemanticTransitionError> {
    ...
    if codebook.range.role != 48 || bytes.len() < 25 * 8 || bytes.len() % 8 != 0 {
        return Err(publication_input_error("replay action codebook has another native extent"));
    }
    let word = |index: usize| u64::from_le_bytes(bytes[index * 8..(index + 1) * 8].try_into().unwrap());
    let (offset, length) = (word(8), word(9));
    if offset % 8 != 0 || offset > bytes.len() as u64 || length > bytes.len() as u64 - offset
        || (offset + length).div_ceil(8) != (bytes.len() / 8) as u64 {
        return Err(publication_input_error("replay action codebook omits its exact statement payload"));
    }
    let mut hash = Sha256::new();
    ...
    for (index, bytes) in bytes.chunks_exact(8).enumerate() {
        hash.update(if matches!(index, 14..=16 | 21..=24) { &[0; 8] } else { bytes });
    }
...
fn publication_semantic_receipts_digest(receipts: &[[u64; 42]; 7]) -> Identity256 {
    ...
    for receipt in receipts {
        for (index, &word) in receipt.iter().enumerate() {
            hash.update(if matches!(index, 3..=12 | 34..=41) { 0 } else { word }.to_le_bytes());
        }
    }
    Identity256::from_bytes(hash.finalize().into())
}
...
fn decode_publication_identity(reader: &mut SemanticMaterialReader<'_>)
    -> Result<SemanticPublishedIdentity, SemanticTransitionError> {
    let instance = Identity256::from_bytes(reader.take(32).map_err(SemanticTransitionError::Semantic)?.try_into().unwrap());
    let word = reader.u64().map_err(SemanticTransitionError::Semantic)?;
    let logical_digest = Identity256::from_bytes(reader.take(32).map_err(SemanticTransitionError::Semantic)?.try_into().unwrap());
    let state_digest = Identity256::from_bytes(reader.take(32).map_err(SemanticTransitionError::Semantic)?.try_into().unwrap());
    Ok(SemanticPublishedIdentity { instance, word, logical_digest, state_digest })
}
...
impl PublicationReplayProvenance {
    ...
    fn decode(bytes: &[u8]) -> Result<Self, SemanticTransitionError> {
        ...
        if reader.take(REPLAY_PROVENANCE_MAGIC.len()).map_err(SemanticTransitionError::Semantic)? != REPLAY_PROVENANCE_MAGIC
            || reader.u32().map_err(SemanticTransitionError::Semantic)? != 1
            || reader.take(32).map_err(SemanticTransitionError::Semantic)? != publication_material_runtime() {
            return Err(publication_input_error("replay provenance requires its exact pinned native runtime"));
        }
        ...
        Ok(Self { predecessor, authority, decision })
    }

    fn validate(&self, predecessor: &PublicationMaterial) -> Result<(), SemanticTransitionError> {
        let header = predecessor.bank.header;
        let original = SemanticPublishedIdentity { instance: header.instance, word: header.publication_word,
            logical_digest: header.logical_digest, state_digest: header.state_digest };
        if self.predecessor != original {
            return Err(publication_input_error("replay provenance names another original native predecessor"));
        }
        for (item, role) in [(&self.authority, 38), (&self.decision, 39)] {
            let saved = predecessor.ranges.iter().find(|item| item.range.role == role && item.range.index == 0)
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            if (item.range.role, item.range.index) != (role, 0) || item.range.generation != 1
                || item.range.length_bytes != item.bytes.len() as u64 || item.capacity != saved.capacity
                || item.capacity == 0 || item.bytes.is_empty() || item.bytes.len() > item.capacity
                || item.range.logical_begin > item.range.logical_end
                || item.original_record_digest() != item.range.digest {
                return Err(publication_input_error("replay provenance changes an originally sealed authority record"));
            }
            if role == 38 && (item.bytes != saved.bytes
                || publication_abi_bytes(&[item.range]) != publication_abi_bytes(&[saved.range])) {
...
impl PublicationReplayEvidence {
    fn range(&self, role: u64) -> Result<&PublicationMaterialRange, SemanticTransitionError> {
        self.ranges.iter().find(|item| item.range.role == role && item.range.index == 0)
    ...
    fn state_digest(&self) -> Result<Identity256, SemanticTransitionError> {
        ...
        for item in &self.ranges {
            digest = publication_fold_digest(digest, item.range.role, item.range.index, item.logical_record_digest()?);
        }
        Ok(digest)
    }
    ...
    fn encode(&self) -> Result<Vec<u8>, SemanticTransitionError> {
        ...
        material_u32(&mut bytes, u32::try_from(self.ranges.len()).map_err(|_| SemanticTransitionError::GenerationExhausted)?);
        for item in &self.ranges { item.encode_into(&mut bytes)?; }
        Ok(bytes)
    }

    fn decode(bytes: &[u8]) -> Result<Self, SemanticTransitionError> {
        let mut reader = SemanticMaterialReader::new(bytes);
        if reader.take(REPLAY_EVIDENCE_MAGIC.len()).map_err(SemanticTransitionError::Semantic)? != REPLAY_EVIDENCE_MAGIC
            || reader.u32().map_err(SemanticTransitionError::Semantic)? != 1
            || reader.take(32).map_err(SemanticTransitionError::Semantic)? != publication_material_runtime() {
            return Err(publication_input_error("replay evidence requires its exact pinned native runtime"));
        }
        let successor = decode_publication_identity(&mut reader)?;
        if reader.u32().map_err(SemanticTransitionError::Semantic)? != REPLAY_EVIDENCE_ROLES.len() as u32 {
            return Err(publication_input_error("replay evidence requires all six native publication records"));
        }
        let mut ranges = Vec::with_capacity(REPLAY_EVIDENCE_ROLES.len());
        for _ in REPLAY_EVIDENCE_ROLES { ranges.push(PublicationMaterialRange::decode_from(&mut reader)?); }
        reader.finish().map_err(SemanticTransitionError::Semantic)?;
        Ok(Self { successor, ranges })
    }

    fn validate(&self, predecessor: &PublicationMaterial) -> Result<SemanticTransitionKind, SemanticTransitionError> {
        let base = predecessor.bank.header;
        let successor_word = (base.publication_word >> 1).checked_add(1)
            .filter(|&epoch| epoch <= u64::MAX >> 1).map(|epoch| (epoch << 1) | ((base.publication_word & 1) ^ 1))
            .ok_or(SemanticTransitionError::GenerationExhausted)?;
        if self.successor.instance != base.instance || self.successor.word != successor_word
            || self.ranges.len() != REPLAY_EVIDENCE_ROLES.len() {
            return Err(publication_input_error("replay evidence is not the original predecessor's immediate successor"));
        }
        for (item, role) in self.ranges.iter().zip(REPLAY_EVIDENCE_ROLES) {
            let previous = predecessor.ranges.iter().find(|item| item.range.role == role && item.range.index == 0)
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            if (item.range.role, item.range.index) != (role, 0) || item.range.generation != 1
                || item.range.length_bytes != item.bytes.len() as u64 || item.capacity != previous.capacity
                || item.capacity == 0 || item.bytes.len() > item.capacity
                || item.range.logical_begin > item.range.logical_end
                || item.original_record_digest() != item.range.digest {
                return Err(publication_input_error("replay evidence changes or omits an originally sealed publication record"));
            }
        }
        let attempt_range = self.range(33)?;
        let attempt = attempt_range.attempt()?;
        let previous_attempt = predecessor.ranges.iter().find(|item| item.range.role == 33 && item.range.index == 0)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        if attempt.abi != 1 || attempt.instance != self.successor.instance
            || attempt.base_word != base.publication_word || attempt.next_word != self.successor.word
            ...
            return Err(publication_input_error("replay evidence differs from its whole native attempt and state links"));
        }
        let no_draw = attempt.action_receipts_digest == Identity256::from_bytes(Sha256::digest([]).into());
        match (base.terminal, no_draw) {
            ...
            _ => Err(publication_input_error("replay attempt cannot follow this predecessor's native terminal state")),
        }
    }

    fn validate_observed(&self, predecessor: &PublicationMaterial, bank: &PublicationBank,
        coverage: &PublicationMaterialRange, codebook: &PublicationMaterialRange)
        -> Result<(), SemanticTransitionError> {
        let kind = self.validate(predecessor)?;
        let base = predecessor.bank.header;
        let header = bank.header;
        let draws = if kind == SemanticTransitionKind::Proposal { COMPONENT_COUNT } else { 0 };
        let proposal = base.proposal.checked_add(u64::from(draws != 0))
            .ok_or(SemanticTransitionError::GenerationExhausted)?;
        if header.abi != 1 || header.instance != self.successor.instance
            || header.publication_word != self.successor.word || header.base_word != base.publication_word
            || header.sealed_epoch != self.successor.word >> 1
            || header.logical_digest != self.successor.logical_digest || header.state_digest != self.successor.state_digest
            || header.proposal != proposal || bank.state.proposal != base.proposal
            || bank.state.next_proposal != proposal || bank.state.blocks != draws as u64 || bank.state.status != 0
            ...
            return Err(publication_input_error("replay evidence does not describe the actual acquired native transition"));
        }
        let attempt = self.range(33)?.attempt()?;
        if coverage.range.role != 14 || coverage.bytes.len() != size_of::<CompletionCoverage>()
            || coverage.original_record_digest() != coverage.range.digest
            || codebook.original_record_digest() != codebook.range.digest {
            return Err(publication_input_error("replay evidence sources differ from their actual native seals"));
        }
        // SAFETY: the exact coverage extent was checked and all ABI fields are integers.
        let completion = unsafe { std::ptr::read_unaligned(coverage.bytes.as_ptr().cast::<CompletionCoverage>()) };
        let coverage_digest = coverage.logical_record_digest()?;
        if completion.abi != 1 || completion.instance != header.instance
            || completion.base_word != base.publication_word || completion.receipt_digest != coverage_digest
            || attempt.coverage_digest != coverage_digest
            || attempt.action_receipts_digest != publication_action_receipts_digest(codebook, &bank.receipts[..draws])?
            || attempt.semantic_receipts_digest != publication_semantic_receipts_digest(&bank.state.semantic_receipts) {
            return Err(publication_input_error("whole native attempt differs from actual action, semantic or completion receipts"));
        }
        Ok(())
    }
}
...
impl SemanticReplayMaterial {
    ...
    pub fn decode(predecessor_bytes: &[u8], evidence_bytes: &[u8],
        expected_predecessor_logical: Identity256, expected_successor_logical: Identity256)
        -> Result<Self, SemanticTransitionError> {
        ...
        if predecessor.bank.header.logical_digest != expected_predecessor_logical
            ...
            return Err(publication_input_error("replay native materials differ from the execution's logical identities"));
        }
        let kind = evidence.validate(&predecessor)?;
        Ok(Self { predecessor, evidence, kind })
    }

    pub fn predecessor_identity(&self) -> SemanticPublishedIdentity {
        let header = self.predecessor.bank.header;
        SemanticPublishedIdentity { instance: header.instance, word: header.publication_word,
            logical_digest: header.logical_digest, state_digest: header.state_digest }
    }

    pub fn successor_identity(&self) -> SemanticPublishedIdentity { self.evidence.successor }

    pub fn transition_kind(&self) -> SemanticTransitionKind { self.kind }
...
fn relocate_publication_codebooks(saved: &[u8], actual: &[u64]) -> Result<Vec<u8>, SemanticTransitionError> {
    if actual.len() < 25 || saved.len() != actual.len().checked_mul(8).ok_or(SemanticTransitionError::GenerationExhausted)? {
        return Err(publication_input_error("restored codebook has another actual extent"));
    }
    for (index, (saved, &actual)) in saved.chunks_exact(8).zip(actual).enumerate() {
        if !matches!(index, 14..=16 | 21..=24) && u64::from_le_bytes(saved.try_into().unwrap()) != actual {
            return Err(publication_input_error("restored codebook changes its admitted logical content or original base"));
        }
    }
...
fn runtime_contract_bytes(contract: PublicationContract, counts: &[u64; 55],
    terminals: &[u64], layouts: &BTreeMap<(u64, u64), SemanticTensorLayout>,
    capacities: &[(u64, u64, usize)], model_mode: &[u8], model_memory: &ModelMemoryGeometry) -> Result<Vec<u8>, SemanticTransitionError> {
    if model_mode.is_empty() || model_mode.len() > 16 * 1024 * 1024 {
        return Err(publication_input_error("runtime contract requires the complete bounded model numerical mode"));
    }
    let mut bytes = RUNTIME_CONTRACT_MAGIC.to_vec();
    material_u32(&mut bytes, 1);
    bytes.extend_from_slice(&publication_material_runtime());
    material_bytes(&mut bytes, model_mode).map_err(SemanticTransitionError::Semantic)?;
    for field in [contract.abi, contract.range_capacity, contract.prefix_capacity,
        contract.window_capacity, contract.feedback_capacity, contract.max_position,
        contract.pad_token, contract.terminal_token_count, contract.final_intent_payload_bytes,
        contract.model_generation, contract.policy_generation, contract.authority_generation,
        contract.role_count] { material_u64(&mut bytes, field); }
    let model = contract.model_contract_layout;
    for field in [model.schema_begin, model.schema_bytes, model.schema_digest_offset,
        model.generation_offset, model.numerical_digest_offset, model.identity_offset] {
        material_u64(&mut bytes, field);
    }
    for identity in [contract.task_identity, contract.topology_identity, contract.table_identity] {
        bytes.extend_from_slice(identity.as_bytes());
    }
    for &count in counts { material_u64(&mut bytes, count); }
    for &token in terminals { material_u64(&mut bytes, token); }
    ...
    for &(role, index, capacity) in capacities {
        for field in [role, index, capacity as u64] { material_u64(&mut bytes, field); }
    }
    model_memory.encode_into(&mut bytes)?;
    Ok(bytes)
}

fn verified_runtime_model_mode<'a>(bytes: &'a [u8], contract: PublicationContract,
    counts: &[u64; 55], terminals: &[u64], layouts: &BTreeMap<(u64, u64), SemanticTensorLayout>,
    capacities: &[(u64, u64, usize)], table: &PublicationMaterialRange,
    model_memory: &ModelMemoryGeometry) -> Result<&'a [u8], SemanticTransitionError> {
    ...
    if (table.range.role, table.range.index, table.range.generation) != (55, 0, 1)
        || table.range.length_bytes != table.bytes.len() as u64 || table.bytes.len() > table.capacity
    ...
    if actual_layouts != layouts.values().copied().collect::<Vec<_>>() {
        return Err(publication_input_error("publication material layout differs from its sealed table"));
    }
    let mut reader = SemanticMaterialReader::new(bytes);
    if reader.take(RUNTIME_CONTRACT_MAGIC.len()).map_err(SemanticTransitionError::Semantic)? != RUNTIME_CONTRACT_MAGIC
        || reader.u32().map_err(SemanticTransitionError::Semantic)? != 1
        || reader.take(32).map_err(SemanticTransitionError::Semantic)? != publication_material_runtime() {
        return Err(publication_input_error("unsupported native runtime contract format or identity"));
    }
    ...
    if runtime_contract_bytes(contract, counts, terminals, layouts, capacities, mode, model_memory)? != bytes {
        return Err(publication_input_error("runtime contract differs from actual cold publication state"));
    }
    Ok(mode)
}

fn initial_runtime_contract_record(contract: PublicationContract, counts: &[u64; 55],
    terminals: &[u64], layouts: &BTreeMap<(u64, u64), SemanticTensorLayout>,
    mut capacities: Vec<(u64, u64, usize)>, model_mode: &[u8], model_memory: &ModelMemoryGeometry)
    -> Result<SemanticStateRecord, SemanticTransitionError> {
    if capacities.iter().any(|&(role, _, _)| role == 43) || counts[42] != 1 {
        return Err(publication_input_error("runtime contract is generated exactly once by the native owner"));
    }
    ...
    if capacities.len() != validate_publication_counts(counts)?
        ...
        return Err(publication_input_error("runtime capacity roster differs from the actual publication directory"));
    }
    // Only the value of a fixed-width capacity cell changes between passes.
    let capacity = runtime_contract_bytes(contract, counts, terminals, layouts, &capacities, model_mode, model_memory)?.len();
    capacities.iter_mut().find(|entry| entry.0 == 43).unwrap().2 = capacity;
    let bytes = runtime_contract_bytes(contract, counts, terminals, layouts, &capacities, model_mode, model_memory)?;
    debug_assert_eq!(bytes.len(), capacity);
    Ok(SemanticStateRecord { role: SemanticStateRole::RuntimeContract, index: 0, capacity_bytes: capacity, bytes })
}
...
unsafe fn publication_material_record<T: Copy>(reader: &mut SemanticMaterialReader<'_>)
    -> Result<T, SemanticTransitionError> {
    let bytes = reader.bytes().map_err(SemanticTransitionError::Semantic)?;
    if bytes.len() != size_of::<T>() {
        return Err(publication_input_error("publication material record has another ABI extent"));
    }
...
impl PublicationMaterial {
    fn model_numerical_mode(&self) -> Result<&[u8], SemanticTransitionError> {
        let record = self.ranges.iter().find(|item| item.range.role == 43 && item.range.index == 0)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        let table = self.ranges.iter().find(|item| item.range.role == 55 && item.range.index == 0)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        let capacities = self.ranges.iter().map(|item| (item.range.role, item.range.index, item.capacity)).collect::<Vec<_>>();
        verified_runtime_model_mode(&record.bytes, self.contract, &self.role_counts,
            &self.terminals, &self.layouts, &capacities, table, &self.model_memory)
    }
    ...
    fn encode(&self) -> Result<Vec<u8>, SemanticTransitionError> {
        ...
        material_u32(&mut bytes, 1);
        bytes.extend_from_slice(&publication_material_runtime());
        material_bytes(&mut bytes, &self.graph.encode().map_err(SemanticTransitionError::Semantic)?)
            .map_err(SemanticTransitionError::Semantic)?;
        for record in [publication_abi_bytes(&[self.bank]), publication_abi_bytes(&[self.contract]),
            publication_abi_bytes(&self.role_counts), publication_abi_bytes(&self.terminals)] {
            material_bytes(&mut bytes, &record).map_err(SemanticTransitionError::Semantic)?;
        }
        material_u32(&mut bytes, u32::try_from(self.layouts.len()).map_err(|_| SemanticTransitionError::GenerationExhausted)?);
        for layout in self.layouts.values() {
            material_bytes(&mut bytes, &publication_abi_bytes(&[*layout])).map_err(SemanticTransitionError::Semantic)?;
        }
        ...
        for range in &self.ranges {
            if matches!(range.range.role, 18..=25) {
                ...
                material_bytes(&mut bytes, &publication_abi_bytes(&[range.range])).map_err(SemanticTransitionError::Semantic)?;
                material_u64(&mut bytes, range.capacity as u64);
            } else { range.encode_into(&mut bytes)?; }
        }
        Ok(bytes)
    }
    ...
    fn decode(bytes: &[u8]) -> Result<Self, SemanticTransitionError> {
        let mut reader = SemanticMaterialReader::new(bytes);
        if reader.take(26).map_err(SemanticTransitionError::Semantic)? != b"XLOG-PUBLICATION-MATERIAL\0"
            || reader.u32().map_err(SemanticTransitionError::Semantic)? != 1
            || reader.take(32).map_err(SemanticTransitionError::Semantic)? != publication_material_runtime() {
            return Err(publication_input_error("publication material requires its exact pinned native runtime"));
        }
        ...
        let graph = SemanticRootMaterial::decode(graph_bytes, SemanticAdmissionLimits {
            max_records: limit, max_terms: limit, max_references: limit, max_utf8_bytes: graph_bytes.len(),
        }).map_err(SemanticTransitionError::Semantic)?;
        ...
        if terminal_bytes.len() % 8 != 0 {
            return Err(publication_input_error("publication terminal material is not integral"));
        }
        let terminals = terminal_bytes.chunks_exact(8).map(|item| u64::from_le_bytes(item.try_into().unwrap())).collect();
        let count = reader.count(4 + size_of::<SemanticTensorLayout>()).map_err(SemanticTransitionError::Semantic)?;
        let mut layouts = BTreeMap::new();
        let mut previous = None;
        for _ in 0..count {
            // SAFETY: the tensor-layout ABI contains only integer fields.
            let layout = unsafe { publication_material_record::<SemanticTensorLayout>(&mut reader)? };
            let key = (layout.role, layout.index);
            if previous.is_some_and(|previous| previous >= key) {
                return Err(publication_input_error("publication material tensor layouts are not in canonical order"));
            }
            previous = Some(key);
        ...
        for &extent in &model_memory.allocation_bytes {
            ...
            if allocation.len() as u64 != extent {
                return Err(publication_input_error("publication material truncates a complete model backing allocation"));
            }
            model_allocations.push(allocation.to_vec());
        }
        let count = reader.count(12 + size_of::<PublicationRange>()).map_err(SemanticTransitionError::Semantic)?;
        let mut ranges = Vec::with_capacity(count);
        for _ in 0..count {
            // SAFETY: the range ABI contains only integer/digest fields.
            let range = unsafe { publication_material_record::<PublicationRange>(&mut reader)? };
            let capacity = usize::try_from(reader.u64().map_err(SemanticTransitionError::Semantic)?)
                .map_err(|_| publication_input_error("publication material range capacity overflows"))?;
            let payload = if matches!(range.role, 18..=25) {
                let (allocation, offset) = model_memory.location(range.role, range.index)?;
                let end = usize::try_from(range.length_bytes).ok().and_then(|bytes| offset.checked_add(bytes))
                    .ok_or(SemanticTransitionError::ObservationMismatch)?;
                model_allocations[allocation].get(offset..end)
                    .ok_or(SemanticTransitionError::ObservationMismatch)?
            } else { reader.bytes().map_err(SemanticTransitionError::Semantic)? };
            ranges.push(PublicationMaterialRange { range, capacity, bytes: payload.to_vec() });
        }
        reader.finish().map_err(SemanticTransitionError::Semantic)?;
        let material = Self { bank, contract, role_counts, terminals, layouts, model_memory, model_allocations, ranges, graph };
        material.validate()?;
        for range in &material.ranges {
            if !is_tensor_role(range.range.role) && range.original_record_digest() != range.range.digest {
                return Err(publication_input_error("publication material changes an originally sealed native record"));
            }
        }
        if material.original_descriptor_digest() != material.bank.header.descriptor_digest {
            return Err(publication_input_error("publication material differs from its original descriptor seal"));
        }
        Ok(material)
    }
    ...
    fn original_descriptor_digest(&self) -> Identity256 {
        ...
        let mut digest = Identity256::from_bytes(Sha256::digest(publication_abi_bytes(&[bank])).into());
        for material in &self.ranges {
            let range = material.range;
            let content = Identity256::from_bytes(Sha256::digest(publication_abi_bytes(&[range])).into());
    ...
    fn validate(&self) -> Result<(), SemanticTransitionError> {
        ...
        let model = self.ranges.iter().find(|item| (item.range.role, item.range.index) == (44, 0))
            .ok_or_else(|| publication_input_error("publication material lacks its complete model contract"))?;
        self.contract.model_contract_layout.validate(model.bytes.len() as u64)?;
        if self.bank.header.model_geometry_digest != self.model_memory.digest(&self.layouts)? {
            return Err(publication_input_error("publication model geometry differs from its original sealed map"));
        }
        if self.model_allocations.len() != self.model_memory.allocation_bytes.len()
            || self.model_allocations.iter().zip(&self.model_memory.allocation_bytes)
                .any(|(bytes, &extent)| bytes.len() as u64 != extent) {
            return Err(publication_input_error("publication material lacks complete model backing bytes"));
        }
        let mut model_slots = BTreeMap::new();
        let mut slot_allocations = BTreeMap::new();
        let backing_digests = self.model_allocations.iter()
            .map(|bytes| Identity256::from_bytes(Sha256::digest(bytes).into())).collect::<Vec<_>>();
        for item in self.ranges.iter().filter(|item| matches!(item.range.role, 18..=25)) {
            let (allocation, offset) = self.model_memory.location(item.range.role, item.range.index)?;
            let bytes = &self.model_allocations[allocation];
            let end = offset.checked_add(item.bytes.len()).ok_or(SemanticTransitionError::ObservationMismatch)?;
            if item.range.offset_bytes != offset as u64 || item.capacity != bytes.len() - offset
                || bytes.get(offset..end) != Some(item.bytes.as_slice())
                || item.range.backing_digest != backing_digests[allocation]
                || model_slots.insert(allocation, item.range.storage_slot).is_some_and(|slot| slot != item.range.storage_slot)
                || slot_allocations.insert(item.range.storage_slot, allocation).is_some_and(|prior| prior != allocation) {
                return Err(publication_input_error("publication material changes model backing or view relationships"));
            }
        }
        if self.ranges.iter().any(|item| !matches!(item.range.role, 18..=25)
            && (slot_allocations.contains_key(&item.range.storage_slot)
                || item.range.backing_digest != Identity256::default())) {
            return Err(publication_input_error("model backing aliases native control or cache storage"));
        }
        let header = self.bank.header;
        let contract = self.contract;
        if !cfg!(target_endian = "little") || header.abi != 1 || contract.abi != 1
            || header.sealed_epoch != header.publication_word >> 1
            || contract.terminal_tokens != 0 || contract.role_counts != 0 || contract.semantic_owner != 0
            || contract.window_capacity != 32 || contract.role_count != 55
            || contract.terminal_token_count != self.terminals.len() as u64
            || contract.model_generation == 0 || header.model_generation < contract.model_generation
            ...
            || header.range_count != self.ranges.len() as u64 || contract.range_capacity != header.range_count {
            return Err(publication_input_error("publication material header or graph differs from its complete owned state"));
        }
        let mut ranges = self.ranges.iter();
        for (ordinal, &count) in self.role_counts.iter().enumerate() {
            let role = ordinal as u64 + 1;
            for index in 0..count {
                let item = ranges.next().ok_or(SemanticTransitionError::ObservationMismatch)?;
                if (item.range.role, item.range.index) != (role, index)
                    || item.range.generation != 1 || item.range.length_bytes != item.bytes.len() as u64
                    || (item.capacity == 0 && !self.layouts.get(&(role, index)).is_some_and(empty_model_tensor_layout))
                    || item.bytes.len() > item.capacity
                    || item.range.logical_begin > item.range.logical_end {
                    return Err(publication_input_error("publication material omits, reorders or changes a reachable range"));
                }
                if is_tensor_role(role) {
                    let layout = self.layouts.get(&(role, index)).ok_or_else(|| publication_input_error("publication material omits a tensor layout"))?;
                    if tensor_layout_bytes(layout)? > item.capacity
                        || (empty_model_tensor_layout(layout) && (item.range.logical_begin != 0 || item.range.logical_end != 0)) {
                        return Err(publication_input_error("publication material tensor exceeds its actual capacity"));
                    }
                } else if self.layouts.contains_key(&(role, index)) {
                    return Err(publication_input_error("publication material gives a tensor layout to a native record"));
                }
            }
        }
        if self.layouts.len() != self.ranges.iter().filter(|item| is_tensor_role(item.range.role)).count() {
            return Err(publication_input_error("publication material has an undeclared tensor layout"));
        }
...
fn allocate_publication<T: DeviceRepr>(provider: &CudaKernelProvider, count: usize) -> Result<TrackedCudaSlice<T>, SemanticTransitionError> {
    provider.memory().alloc::<T>(count).map_err(|error| runtime_error("publication allocation", error))
}

fn upload_publication<T: DeviceRepr>(provider: &CudaKernelProvider, values: &[T], destination: &TrackedCudaSlice<T>) -> Result<(), SemanticTransitionError> {
    let mut destination = destination.view();
    provider.htod_launch_metadata_sync_copy_into(values, &mut destination)
        .map_err(|error| runtime_error("publication cold metadata upload", error))
}

impl PublicationStorage {
    #[allow(clippy::too_many_arguments)]
    fn enqueue_content_guard(&self, domain: &ResidentExecutionDomain, poisoned: &mut bool,
        reader: DeviceMemoryView<PublicationLease>, execute: &CudaFunction,
        role: u64, index: u64, layout: SemanticTensorLayout) -> Result<(), SemanticTransitionError> {
        let mut recorder = domain.new_strict_recorder();
        recorder.read(&reader); recorder.read(&self.control); recorder.read(&self.storage); recorder.read(&self.contract);
        for bank in &self.banks { recorder.read(bank); }
        for directory in &self.directories { recorder.read(directory); }
        for allocation in &self.allocations { recorder.read(allocation); }
        let arguments = (self.control.device_ptr_value(), role, index, layout,
            u64::from(is_tensor_role(role)), *reader.device_ptr());
        enqueue_recorded(domain, poisoned, recorder, |enqueue| {
            ...
            unsafe { execute.clone().launch_in(enqueue, LaunchConfig { grid_dim: (1, 1, 1),
                block_dim: (1, 1, 1), shared_mem_bytes: 0 }, arguments) }
    ...
    fn allocate(
        ...
        if model_payloads.len() != model_memory.allocation_bytes.len() {
            return Err(publication_input_error("model backing upload roster differs from its complete memory map"));
        }
        ...
        for plan in plans {
            if (plan.capacity == 0 && !layouts.get(&(plan.role, plan.index)).is_some_and(empty_model_tensor_layout))
                || plan.length > plan.capacity
                || (layouts.get(&(plan.role, plan.index)).is_some_and(empty_model_tensor_layout)
                    && (plan.logical_begin != 0 || plan.logical_end != 0)) {
                return Err(publication_input_error("owned publication allocation has invalid capacity"));
            }
            let mut slot = 0;
            ...
            for (bank, directory) in bank_templates.iter_mut().enumerate() {
                if let Some((allocation, offset)) = model_view {
                    ...
                    if plan.capacity != allocations[slot].len() - offset {
                        return Err(publication_input_error("model range capacity differs from its actual backing view"));
                    }
                ...
                directory.push(PublicationRange {
                    role: plan.role, index: plan.index, storage_slot: slot as u64, generation: 1,
                    offset_bytes: model_view.map_or(0, |(_, offset)| offset as u64), length_bytes: plan.length as u64,
                    logical_begin: plan.logical_begin, logical_end: plan.logical_end,
                    digest: Identity256::default(),
                    backing_digest: Identity256::default(),
                });
            }
        }
        let pending_layouts = layouts.values().filter(|layout| continuation_role(layout.role))
            .map(|layout| continuation_capacity_layout(*layout)).collect::<Result<Vec<_>, _>>()?;
        ...
        for plan in plans.iter().filter(|plan| continuation_role(plan.role)) {
            let capacity = if plan.role == 1 { 32 * size_of::<SemanticTextSlot>() }
                else if plan.role == 39 { plan.capacity }
                else if plan.role == 55 { tensor_table_capacity(pending_layouts.len(), 32 + contract.feedback_capacity)? }
                else {
                    tensor_layout_bytes(&continuation_capacity_layout(layouts[&(plan.role, plan.index)])?)?
                };
            if capacity == 0 { return Err(publication_input_error("continuation destination has no actual capacity")); }
            ...
            let length = if plan.role == 55 {
                ...
                pending_table.len()
            } else if matches!(plan.role, 1 | 39) { 0 } else { capacity };
        ...
        Ok((Self {
            ...
            banks: [allocate_publication(provider, 1)?, allocate_publication(provider, 1)?],
            directories: [allocate_publication(provider, plans.len())?, allocate_publication(provider, plans.len())?],
            storage: allocate_publication(provider, allocations.len())?,
            contract: allocate_publication(provider, 1)?, role_counts, terminals,
            continuation: allocate_publication(provider, 1)?,
            continuation_directory: allocate_publication(provider, continuation_templates.len())?,
            allocations, model_memory, model_slots, bank_templates, continuation_templates, layouts, contract_value, instance,
        }, uploads))
    }

    fn record(&self, recorder: &mut LaunchRecorder) {
        recorder.read_write(&self.control);
        for bank in &self.banks { recorder.read_write(bank); }
        for directory in &self.directories { recorder.read_write(directory); }
        ...
        for allocation in &self.allocations { recorder.read_write(allocation); }
    }
}
...
fn prepare_continuation_payloads(storage: &PublicationStorage, original: &[PreparedSemanticTensor],
    authority_decisions: &[u8])
    -> Result<(Vec<(usize, PublicationPayload)>, BTreeMap<(u64, u64), SemanticTensorLayout>), SemanticTransitionError> {
    let mut pending_layouts = BTreeMap::new();
    let mut inputs = BTreeMap::new();
    for tensor in original {
        let key = (tensor.layout.role, tensor.layout.index);
        let model = storage.layouts.get(&key).ok_or_else(|| publication_input_error("continuation tensor has no cold model owner"))?;
        if !continuation_role(key.0) || !is_tensor_role(key.0) || inputs.contains_key(&key) {
            return Err(publication_input_error("continuation tensor type or role differs from the model"));
        }
        let destination = continuation_tensor_layout(&tensor.layout, tensor.logical_begin, tensor.logical_end, model)?;
    ...
    for range in &storage.continuation_templates {
        ...
        if matches!(range.role, 1 | 55) { continue; }
        let payload = if range.role == 39 {
            PublicationPayload::Metadata(authority_decisions.to_vec())
        } else {
            PublicationPayload::Tensor(inputs.remove(&key)
                .ok_or_else(|| publication_input_error("continuation omits an actual required forward tensor"))?)
        };
        let length = match &payload {
            ...
            PublicationPayload::Uncomputed => unreachable!("continuation accepts original computed tensors only"),
        };
        if length > storage.allocations[range.storage_slot as usize].len() {
            return Err(publication_input_error("actual continuation exceeds its owned fixed destination capacity"));
        }
        uploads.push((range.storage_slot as usize, payload));
    }
    if !inputs.is_empty() { return Err(publication_input_error("continuation contains an undeclared tensor owner")); }
...
impl PreparedContinuation {
    ...
    fn prepare(provider: &CudaKernelProvider, storage: Arc<PublicationStorage>,
        reader: DeviceMemoryView<PublicationLease>, text_binding: Arc<TextBindingStorage>,
        inputs: ContinuationInputs, uploads: &[(usize, PublicationPayload)],
        layouts: &BTreeMap<(u64, u64), SemanticTensorLayout>) -> Result<Self, SemanticTransitionError> {
        let mut sources = Vec::new();
        let mut copies = Vec::new();
        for (slot, payload) in uploads {
            let PublicationPayload::Tensor(tensor) = payload else { continue; };
            let destination = storage.allocations.get(*slot).ok_or(SemanticTransitionError::ObservationMismatch)?;
            if let Some(source) = &tensor.source {
                let layout = layouts.get(&(tensor.layout.role, tensor.layout.index))
                    .ok_or(SemanticTransitionError::ObservationMismatch)?;
                for (source_offset, destination_offset, bytes) in tensor_copy_plan(&tensor.layout, layout)? {
                    if source_offset.checked_add(bytes as u64).is_none_or(|end| end > source.len() as u64)
                        || destination_offset.checked_add(bytes as u64).is_none_or(|end| end > destination.len() as u64) {
                        return Err(SemanticTransitionError::ObservationMismatch);
                    }
                    let source = source.device_ptr().checked_add(source_offset)
                        .ok_or(SemanticTransitionError::ObservationMismatch)?;
                    let destination = destination.device_ptr_value().checked_add(destination_offset)
                        .ok_or(SemanticTransitionError::ObservationMismatch)?;
        ...
        let execute = provider.device().inner().get_func("xlog_semantic_transition", "semantic_publication_prepare_continuation")
            .ok_or_else(|| runtime_error("kernel lookup", "continuation preparation unavailable"))?;
        Ok(Self { storage, reader, text_binding, sources, copies, inputs, execute })
    }
    ...
    fn enqueue(&self, domain: &ResidentExecutionDomain, poisoned: &mut bool)
        -> Result<(), SemanticTransitionError> {
        let mut recorder = domain.new_strict_recorder();
        ...
        for tensor in &self.sources {
            if let Some(source) = &tensor.source { recorder.read(source); }
        }
        let arguments = (self.storage.control.device_ptr_value(), *self.reader.device_ptr(), self.inputs);
        enqueue_recorded(domain, poisoned, recorder, |enqueue| {
            for &(source, destination, bytes) in &self.copies {
                ...
                unsafe { sys::cuMemcpyDtoDAsync_v2(destination, source, bytes, enqueue.stream().cu_stream()) }
            ...
            unsafe { self.execute.clone().launch_in(enqueue, LaunchConfig { grid_dim: (1, 1, 1),
                block_dim: (1, 1, 1), shared_mem_bytes: 0 }, arguments) }
                .map_err(|error| XlogError::Kernel(error.to_string()))
        })
    }
...
fn step_input_overlap(data: u64, bytes: usize, base: u64, capacity: usize)
    -> Result<bool, SemanticTransitionError> {
    let end = data.checked_add(bytes as u64).ok_or(SemanticTransitionError::ObservationMismatch)?;
    let limit = base.checked_add(capacity as u64).ok_or(SemanticTransitionError::ObservationMismatch)?;
    Ok(if bytes == 0 { data >= base && data < limit } else { data < limit && base < end })
}

fn plan_step_inputs(banks: &[Vec<PublicationRange>; 2],
    layouts: &BTreeMap<(u64, u64), SemanticTensorLayout>, prefix_capacity: u64,
    allocation_bytes: &[usize]) -> Result<Vec<StepInputPlan>, SemanticTransitionError> {
    let mut expected = BTreeSet::from([(1, 0), (3, 0), (15, 0), (16, 0), (17, 0), (44, 0)]);
    expected.extend(banks[0].iter().filter(|range| matches!(range.role, 16 | 17))
        .map(|range| (range.role, range.index)));
    for (&key, layout) in layouts.iter().filter(|(key, _)| matches!(key.0, 4..=13 | 18..=25)) {
        let model = matches!(key.0, 18..=25);
        let bytes = tensor_layout_bytes(layout)?;
        if key != (layout.role, layout.index) || (!model && (canonical_tensor_layout(*layout)? != *layout || bytes == 0))
            || (model && layout.logical_axis != u64::MAX) {
            return Err(publication_input_error("step input must retain its original tensor layout"));
        }
        expected.insert(key);
    }
    for role in (4..=13).chain(15..=25) {
        for (index, key) in expected.iter().filter(|key| key.0 == role).enumerate() {
            if key.1 != index as u64 { return Err(publication_input_error("step input tensor indexes must be contiguous")); }
        }
    }
    for bank in banks {
        let actual = bank.iter().filter(|range| matches!(range.role, 1 | 3..=13 | 15..=25 | 44))
            .map(|range| (range.role, range.index)).collect::<Vec<_>>();
        if actual.len() != expected.len() || actual.iter().copied().collect::<BTreeSet<_>>() != expected {
            return Err(publication_input_error("step input banks must have the exact original input roster"));
        }
    }
    let mut plan = Vec::with_capacity(expected.len());
    let mut spans = Vec::new();
    for (role, index) in expected {
        let ranges = banks.each_ref().map(|bank| *bank.iter().find(|range|
            (range.role, range.index) == (role, index)).expect("validated input roster"));
        let layout = if role == 1 { committed_prefix_layout(&ranges[0], prefix_capacity)? }
            else if matches!(role, 3 | 15..=17 | 44) { SemanticTensorLayout { role, index, scalar_type: 1, element_bytes: 1,
                rank: 1, logical_axis: u64::MAX, dimensions: [ranges[0].length_bytes, 0, 0, 0], strides_bytes: [1, 0, 0, 0] } }
            else { layouts[&(role, index)] };
        let bytes = tensor_layout_bytes(&layout)?;
        if (bytes == 0 && !matches!(role, 18..=25)) || (role == 3 && ranges.iter().any(|range| range.length_bytes != 32)) {
            return Err(publication_input_error("step input has no complete fixed physical capacity"));
        }
        if matches!(role, 15..=17) && ranges.iter().any(|range| range.logical_begin != 0
            || if role == 15 { range.length_bytes % size_of::<RawFeedbackRecord>() as u64 != 0
                || range.logical_end > range.length_bytes / size_of::<RawFeedbackRecord>() as u64 }
            else { range.logical_end != 0 }) {
            return Err(publication_input_error("raw feedback input differs from its original record extent"));
        }
        let mut resolved = Vec::with_capacity(2);
        for (bank, range) in ranges.iter().enumerate() {
            if role == 1 { committed_prefix_layout(range, prefix_capacity)?; }
            let slot = usize::try_from(range.storage_slot).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
            let capacity = *allocation_bytes.get(slot).ok_or(SemanticTransitionError::ObservationMismatch)?;
            let span = publication_export_span(range, &layout, capacity)?;
            if span.len() != bytes { return Err(publication_input_error("step input differs from its full typed capacity")); }
            spans.push((bank, role, index, slot, span.clone()));
            resolved.push((slot, span, capacity));
        }
        if matches!(role, 1 | 4 | 5) && resolved[0] != resolved[1] {
            return Err(publication_input_error("prefix and key/value step inputs must share their original capacity allocation"));
        }
        if matches!(role, 15 | 18..=25 | 44) && (resolved[0].0 == resolved[1].0 || resolved[0].1 != resolved[1].1
        ...
        plan.push(StepInputPlan { role, index, layout, storage_slot: resolved[0].0,
            allocation_bytes: resolved[0].2, span: resolved[0].1.clone() });
    }
    for (ordinal, (bank, role, index, slot, span)) in spans.iter().enumerate() {
        for (other_bank, other_role, other_index, other_slot, other_span) in &spans[..ordinal] {
            let same_shared = bank != other_bank && role == other_role && index == other_index && matches!(*role, 1 | 4 | 5 | 16 | 17);
            let same_model = bank == other_bank && matches!(*role, 18..=25) && matches!(*other_role, 18..=25);
            if slot == other_slot && span.start < other_span.end && other_span.start < span.end && !same_shared && !same_model {
                return Err(publication_input_error("step input caches and distinct tensor ranges must be isolated"));
            }
        }
    }
    Ok(plan)
...
impl PreparedStepInputs {
    fn plan(storage: &PublicationStorage) -> Result<Vec<StepInputPlan>, SemanticTransitionError> {
        plan_step_inputs(&storage.bank_templates, &storage.layouts, storage.contract_value.prefix_capacity,
            &storage.allocations.iter().map(TrackedCudaSlice::len).collect::<Vec<_>>())
    }

    fn allocation_bytes(plans: &[StepInputPlan]) -> Result<usize, SemanticTransitionError> {
        let fixed = size_of::<PublicationHeader>() + 32 * size_of::<SourceSlot>() + 8 * size_of::<u64>();
        let bytes = plans.len().checked_mul(size_of::<PublicationRange>() + size_of::<PublicationStepInput>())
            .and_then(|n| n.checked_add(fixed)).ok_or(SemanticTransitionError::GenerationExhausted)?;
        let mut model_roots = BTreeSet::new();
        plans.iter().filter(|input| !matches!(input.role, 1 | 4 | 5)).try_fold(bytes, |sum, input| {
            let extent = if matches!(input.role, 18..=25) {
                if !model_roots.insert(input.storage_slot) { return Ok(sum); }
                input.allocation_bytes
            } else { input.span.len() };
            sum.checked_add(extent).ok_or(SemanticTransitionError::GenerationExhausted)
        })
    }

    fn allocate(provider: &CudaKernelProvider, storage: Arc<PublicationStorage>,
        reader: DeviceMemoryView<PublicationLease>) -> Result<Self, SemanticTransitionError> {
        let plans = Self::plan(&storage)?;
        let mut reservation = provider.memory().reserve_bytes(Self::allocation_bytes(&plans)? as u64)
    ...
    fn allocate_reserved(provider: &CudaKernelProvider, storage: Arc<PublicationStorage>,
        reader: DeviceMemoryView<PublicationLease>, plans: Vec<StepInputPlan>,
        reservation: &mut GpuMemoryReservation) -> Result<Self, SemanticTransitionError> {
        let contract = storage.contract_value;
        if contract.abi != 1 || contract.model_generation == 0 || contract.window_capacity != 32 || reader.len() != 1 {
            return Err(publication_input_error("fixed step inputs require the original admitted model contract and device reader"));
        }
        let execute = provider.device().inner().get_func("xlog_semantic_transition", "semantic_publication_step_inputs")
            .ok_or_else(|| runtime_error("kernel lookup", "publication step input producer unavailable"))?;
        let guard = provider.device().inner().get_func("xlog_semantic_transition", "semantic_publication_step_input_guard")
            .ok_or_else(|| runtime_error("kernel lookup", "publication step input guard unavailable"))?;
        ...
        for input in &plans {
            let view = if matches!(input.role, 1 | 4 | 5) {
                storage.allocations[input.storage_slot].view().slice(input.span.clone())
            } else if matches!(input.role, 18..=25) {
                let ordinal = if let Some(&ordinal) = model_roots.get(&input.storage_slot) { ordinal } else {
                    let ordinal = private.len();
                    private.push(reservation.alloc::<u8>(input.allocation_bytes)
                        .map_err(|error| runtime_error("step model backing allocation", error))?);
                    model_roots.insert(input.storage_slot, ordinal);
                    ordinal
                };
                private[ordinal].view().slice(input.span.clone())
            } else {
                private.push(reservation.alloc::<u8>(input.span.len())
                    .map_err(|error| runtime_error("step private input allocation", error))?);
                private.last().expect("retained private input allocation").view()
            };
            let backing = if matches!(input.role, 18..=25) {
                view.allocation_view().ok_or(SemanticTransitionError::ObservationMismatch)?
            } else { view.clone() };
            binding_values.push(PublicationStepInput { role: input.role, index: input.index,
                destination: *view.device_ptr(), capacity_bytes: view.len() as u64,
                backing: *backing.device_ptr(), backing_bytes: backing.len() as u64,
                layout: if matches!(input.role, 3 | 15..=17 | 44) { SemanticTensorLayout::default() } else { input.layout } });
            views.insert((input.role, input.index), view);
        }
        let owner = Self { header: reservation.alloc(1).map_err(|error| runtime_error("step header allocation", error))?,
            source: reservation.alloc(32).map_err(|error| runtime_error("step source allocation", error))?,
            ranges: reservation.alloc(plans.len()).map_err(|error| runtime_error("step range allocation", error))?,
            metadata_digests: reservation.alloc(8).map_err(|error| runtime_error("step digest allocation", error))?,
            bindings: reservation.alloc(plans.len()).map_err(|error| runtime_error("step binding allocation", error))?,
            storage, reader, plans, views, private,
            binding_values, execute, guard };
    ...
    fn validate_output_regions(&self) -> Result<(), SemanticTransitionError> {
        let mut output = vec![
            (self.header.device_ptr_value(), size_of::<PublicationHeader>()),
            (self.source.device_ptr_value(), 32 * size_of::<SourceSlot>()),
            (self.ranges.device_ptr_value(), self.ranges.len() * size_of::<PublicationRange>()),
            (self.metadata_digests.device_ptr_value(), 8 * size_of::<u64>()),
            (self.bindings.device_ptr_value(), self.bindings.len() * size_of::<PublicationStepInput>()),
        ];
        output.extend(self.private.iter().map(|allocation| (allocation.device_ptr_value(), allocation.len())));
        let mut original = self.storage.allocations.iter().map(|allocation| (allocation.device_ptr_value(), allocation.len())).collect::<Vec<_>>();
        original.extend(self.storage.banks.iter().map(|bank| (bank.device_ptr_value(), size_of::<PublicationBank>())));
        original.extend(self.storage.directories.iter().map(|directory| (directory.device_ptr_value(), directory.len() * size_of::<PublicationRange>())));
        original.extend([
            (*self.reader.device_ptr(), size_of::<PublicationLease>()),
            (self.storage.control.device_ptr_value(), size_of::<PublicationControl>()),
            (self.storage.contract.device_ptr_value(), size_of::<PublicationContract>()),
            (self.storage.storage.device_ptr_value(), self.storage.storage.len() * size_of::<PublicationStorageEntry>()),
            (self.storage.role_counts.device_ptr_value(), self.storage.role_counts.len() * size_of::<PublicationRoleCount>()),
            (self.storage.terminals.device_ptr_value(), self.storage.terminals.len() * size_of::<u64>()),
        ]);
        for (ordinal, &(begin, bytes)) in output.iter().enumerate() {
            if bytes == 0 { continue; }
            let end = begin.checked_add(bytes as u64).filter(|&end| begin != 0 && end > begin)
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            for &(other, other_bytes) in original.iter().chain(&output[..ordinal]) {
                let other_end = other.checked_add(other_bytes as u64).ok_or(SemanticTransitionError::ObservationMismatch)?;
                if begin < other_end && other < end {
                    return Err(publication_input_error("fixed step outputs overlap original storage or another output"));
                }
            }
        }
        Ok(())
    ...
    fn enqueue(&self, domain: &ResidentExecutionDomain, poisoned: &mut bool) -> Result<(), SemanticTransitionError> {
        let mut recorder = domain.new_strict_recorder();
        recorder.read(&self.reader); recorder.read(&self.storage.control); recorder.read(&self.storage.storage);
        recorder.read(&self.storage.contract); recorder.read(&self.storage.role_counts); recorder.read(&self.storage.terminals);
        for bank in &self.storage.banks { recorder.read(bank); }
        for directory in &self.storage.directories { recorder.read(directory); }
        for allocation in &self.storage.allocations { recorder.read(allocation); }
        recorder.read(&self.bindings); recorder.write(&self.header); recorder.write(&self.source);
        recorder.write(&self.ranges); recorder.write(&self.metadata_digests);
        for allocation in &self.private { recorder.write(allocation); }
        let arguments = (self.storage.control.device_ptr_value(), *self.reader.device_ptr(), self.bindings.device_ptr_value(),
            self.bindings.len() as u64, self.header.device_ptr_value(), self.source.device_ptr_value(),
            self.ranges.device_ptr_value(), self.metadata_digests.device_ptr_value());
        enqueue_recorded(domain, poisoned, recorder, |enqueue| {
    ...
    fn verify(&self, domain: &ResidentExecutionDomain, poisoned: &mut bool) -> Result<(), SemanticTransitionError> {
        let mut recorder = domain.new_strict_recorder();
        recorder.read(&self.header); recorder.read(&self.source); recorder.read(&self.ranges);
        recorder.read(&self.bindings); recorder.read(&self.metadata_digests);
        for view in self.views.values() { recorder.read(view); }
        for allocation in &self.private { recorder.read(allocation); }
        let arguments = (self.header.device_ptr_value(), self.source.device_ptr_value(), self.bindings.device_ptr_value(),
            self.bindings.len() as u64, self.ranges.device_ptr_value(), self.metadata_digests.device_ptr_value());
        enqueue_recorded(domain, poisoned, recorder, |enqueue| {
            ...
            unsafe { self.guard.clone().launch_in(enqueue, LaunchConfig { grid_dim: (1, 1, 1),
                block_dim: (1, 1, 1), shared_mem_bytes: 0 }, arguments) }
                .map_err(|error| XlogError::Kernel(error.to_string()))
        })
    }

    fn field_view(&self, field: PublicationBankField) -> Result<DeviceMemoryView<u8>, SemanticTransitionError> {
        ...
        unsafe { match field {
            PublicationBankField::Source => self.source.view().cast::<u8>().ok_or(SemanticTransitionError::ObservationMismatch),
            _ => self.header.view().cast::<u8>().map(|view| view.slice(span))
                .ok_or(SemanticTransitionError::ObservationMismatch),
        } }
    }

    fn owns_tensor(&self, tensor: &PreparedSemanticTensor, directory: &[PublicationRange])
        -> Result<bool, SemanticTransitionError> {
        let bytes = tensor.source.as_ref().map_or(0, DeviceMemoryView::len);
        for field in [PublicationBankField::Source, PublicationBankField::PrefixExtent, PublicationBankField::RingHead, PublicationBankField::Terminal] {
            let view = self.field_view(field)?;
            let start = *view.device_ptr();
            if !step_input_overlap(tensor.data, bytes, start, view.len())? { continue; }
            let (_, shape, strides) = field.layout();
            let mut layout = SemanticTensorLayout { role: 0, index: tensor.layout.index, scalar_type: 3,
                element_bytes: 8, rank: shape.len() as u64, logical_axis: u64::MAX, ..SemanticTensorLayout::default() };
            for (axis, dimension) in shape.into_iter().enumerate() { layout.dimensions[axis] = dimension as u64; }
            for (axis, stride) in strides.into_iter().enumerate() { layout.strides_bytes[axis] = stride as u64 * 8; }
            if tensor.data != start || bytes != view.len() || tensor.layout != layout
            ...
            if !step_input_allocation_matches(tensor, &view) {
                return Err(publication_input_error("native source and scalar allocation origin does not match its producer"));
            }
            return Ok(true);
        }
        for (pointer, capacity) in [
            ...
            (self.ranges.device_ptr_value(), self.ranges.len() * size_of::<PublicationRange>()),
            (self.bindings.device_ptr_value(), self.bindings.len() * size_of::<PublicationStepInput>()),
            (self.metadata_digests.device_ptr_value(), self.metadata_digests.len() * size_of::<u64>()),
        ] {
        ...
        if let Some(input) = self.plans.iter().find(|input| (input.role, input.index) == (tensor.layout.role, tensor.layout.index)) {
            let view = &self.views[&(input.role, input.index)];
            let range = directory.iter().find(|range| (range.role, range.index) == (input.role, input.index))
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            ...
            if step_input_alias_matches(input, range, *view.device_ptr(), view.len(), tensor_content_identity(tensor))
        ...
        for input in &self.plans {
            ...
            if !step_input_overlap(tensor.data, bytes, start, view.len())?
                && !(input.role == 1 && bytes == 0 && tensor.data == start) { continue; }
            let range = directory.iter().find(|range| (range.role, range.index) == (input.role, input.index))
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            if input.role == 1 && step_input_alias_matches(input, range, start, view.len(), tensor_content_identity(tensor))
                && step_input_allocation_matches(tensor, view) {
                return Ok(true);
            }
        ...
        for allocation in &self.private {
            let retained_origin = tensor.native_allocation.as_ref().is_some_and(|origin|
                allocation.view().allocation_provenance().is_some_and(|expected| origin.same_allocation(&expected)));
            if retained_origin || step_input_overlap(tensor.data, bytes, allocation.device_ptr_value(), allocation.len())? {
                return Err(publication_input_error("native step backing padding cannot become a separate content operand"));
            }
        }
        Ok(false)
    }
...
fn step_input_allocation_matches(tensor: &PreparedSemanticTensor, view: &DeviceMemoryView<u8>) -> bool {
    match &tensor.native_allocation {
        Some(origin) => view.allocation_provenance().is_some_and(|expected| origin.same_allocation(&expected)),
...
fn step_input_alias_matches(input: &StepInputPlan, range: &PublicationRange, data: u64, capacity: usize,
    actual: (SemanticTensorLayout, u64, u64, u64, usize)) -> bool {
    if actual.3 != data { return false; }
    let typed_capacity = actual.4 == capacity && actual.0 == input.layout
    ...
    let raw_prefix = input.role == 1 && input.index == 0 && range.role == 1 && range.index == 0
        && actual.0 == publication_record_layout(range) && (actual.1, actual.2) == (0, 0)
...
impl StepContentStorage {
    fn new() -> Self {
        Self { identity: None, content_witnesses: Arc::new(()), aliases: Arc::new(()),
            consumer_streams: BTreeSet::new(), inputs: None, feedback: Vec::new(), content: Vec::new(), adjoints: Vec::new(), prepared: None }
    }
}
...
impl SemanticPreparedSegmentCapture {
    ...
    where P: FnOnce(u64) -> Result<(), E>, E: fmt::Display,
        F: FnOnce(&crate::cuda_graph::ConditionalCudaGraphBody)
    ...
    pub fn instantiate(self) -> Result<SemanticPreparedExecutable, SemanticTransitionError> {
        let graph = self.builder.instantiate().map_err(|error| runtime_error("bounded segment instantiation", error))?;
        Ok(SemanticPreparedExecutable { scope: self.scope, graph })
    }
}
...
impl PreparedSegmentState {
    fn new(issuer: Arc<()>, tokens: Vec<u64>, transitions: impl ExactSizeIterator<Item = SemanticTransitionKind> + Clone)
        -> Result<Self, SemanticTransitionError> {
        if tokens.is_empty() || tokens.iter().any(|&token| token == 0)
            || tokens.iter().copied().collect::<BTreeSet<_>>().len() != tokens.len()
            || tokens.len() != transitions.len() || transitions.clone().any(|kind| kind == SemanticTransitionKind::Drain) {
        ...
        owned.try_reserve_exact(transitions.len()).map_err(|error| runtime_error("prepared schedule reservation", error))?;
        owned.extend(transitions);
        Ok(Self { issuer, scope: Arc::new(()), tokens, transitions: owned, next: 0, active: false, finished: false,
            capturing: false, submitted: false, completed: false })
    }

    fn handles(&self) -> Result<Vec<SemanticPreparedStep>, SemanticTransitionError> {
        let mut handles = Vec::new();
        handles.try_reserve_exact(self.tokens.len()).map_err(|error| runtime_error("prepared handle reservation", error))?;
        handles.extend(self.tokens.iter().map(|&token| SemanticPreparedStep {
            issuer: Arc::clone(&self.issuer), scope: Arc::clone(&self.scope), token,
        }));
        Ok(handles)
    }

    fn check(&self, step: &SemanticPreparedStep, issuer: &Arc<()>, recording: bool)
        -> Result<(), SemanticTransitionError> {
        self.check_retained(step, issuer)?;
        if self.finished
            || (recording && (!self.active || self.tokens.get(self.next) != Some(&step.token))) {
            return Err(publication_input_error("prepared step differs from its owning Session or active construction scope"));
        }
        Ok(())
    }

    fn check_retained(&self, step: &SemanticPreparedStep, issuer: &Arc<()>)
        -> Result<(), SemanticTransitionError> {
        if !Arc::ptr_eq(issuer, &self.issuer) || !Arc::ptr_eq(&step.issuer, issuer)
            || !Arc::ptr_eq(&step.scope, &self.scope) || !self.tokens.contains(&step.token) {
            return Err(publication_input_error("retained prepared step differs from its original Session and construction scope"));
        }
        Ok(())
    }

    fn requested_kind(&self, step: &SemanticPreparedStep, issuer: &Arc<()>) -> Result<SemanticTransitionKind, SemanticTransitionError> {
        self.check_retained(step, issuer)?;
        let index = self.tokens.iter().position(|&token| token == step.token)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        Ok(self.transitions[index])
    }

    fn enter(&mut self, step: &SemanticPreparedStep, issuer: &Arc<()>) -> Result<SemanticTransitionKind, SemanticTransitionError> {
        self.check(step, issuer, false)?;
        if self.active || self.tokens.get(self.next) != Some(&step.token) {
            return Err(publication_input_error("prepared steps must record once in their original bounded order"));
        }
        self.active = true;
        Ok(self.transitions[self.next])
    }

    fn leave(&mut self, step: &SemanticPreparedStep, issuer: &Arc<()>) -> Result<(), SemanticTransitionError> {
    ...
    fn finish(&mut self) -> Result<(), SemanticTransitionError> {
        if self.finished || self.active || self.next != self.tokens.len() {
            return Err(publication_input_error("prepared segment has unrecorded or unfinished steps"));
        }
        self.finished = true;
        Ok(())
    }
    ...
    fn submit(&mut self) -> Result<(), SemanticTransitionError> {
        if !self.finished || self.active || self.next != self.tokens.len() || self.submitted || self.completed {
            return Err(publication_input_error("prepared segment submission requires one complete unused construction"));
        }
        self.submitted = true;
        Ok(())
    }
...
impl PreparedModelWork {
    fn descriptor(&self) -> ModelWorkInput {
        ModelWorkInput { events: self.device.device_ptr_value(),
            count: self.recording.events().len() as u64,
            bound: self.recording.frozen_bound().expect("original work frozen before enqueue") }
    }
}
...
fn model_content_copy_plan(directory: &[PublicationRange], tensor_ranges: &[usize],
    allocation_bytes: &[usize]) -> Result<ModelContentCopyPlan, SemanticTransitionError> {
    let mut contracts = directory.iter().enumerate().filter(|(_, range)| range.role == 44);
    let (contract_index, contract) = contracts.next().ok_or(SemanticTransitionError::ObservationMismatch)?;
    let mut seen = BTreeSet::new();
    if contracts.next().is_some() || contract.index != 0 || contract.generation != 1
        || contract.length_bytes == 0 || tensor_ranges.is_empty()
        || tensor_ranges.iter().any(|&index| !seen.insert(index)
            || directory.get(index).is_none_or(|range| !matches!(range.role, 18..=20))) {
        return Err(SemanticTransitionError::ObservationMismatch);
    }
    let contract_storage_slot = usize::try_from(contract.storage_slot)
        .map_err(|_| SemanticTransitionError::ObservationMismatch)?;
    let allocation = *allocation_bytes.get(contract_storage_slot)
        .ok_or(SemanticTransitionError::ObservationMismatch)?;
    let begin = usize::try_from(contract.offset_bytes).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
    let bytes = usize::try_from(contract.length_bytes).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
    let end = begin.checked_add(bytes).filter(|&end| end <= allocation)
        .ok_or(SemanticTransitionError::ObservationMismatch)?;
    let directory_offsets = tensor_ranges.iter().copied().chain(std::iter::once(contract_index))
        .map(|index| index.checked_mul(size_of::<PublicationRange>())
            .ok_or(SemanticTransitionError::ObservationMismatch))
        .collect::<Result<Vec<_>, _>>()?;
    directory_offsets.len().checked_mul(size_of::<PublicationRange>())
        .ok_or(SemanticTransitionError::ObservationMismatch)?;
    Ok(ModelContentCopyPlan { directory_offsets, contract_storage_slot, contract_span: begin..end })
}

impl ModelContentSeals {
    fn allocate(provider: &CudaKernelProvider, plan: &ModelContentCopyPlan)
        -> Result<Self, SemanticTransitionError> {
        let guard = provider.device().inner().get_func("xlog_semantic_transition", "semantic_retained_model_contract_guard")
            .ok_or_else(|| runtime_error("kernel lookup", "retained model contract guard unavailable"))?;
        Ok(Self { ranges: allocate_publication(provider, plan.directory_offsets.len())?,
            contract: allocate_publication(provider, plan.contract_span.len())?, guard, prepared: None })
    }

    fn snapshot_prepared(&self, domain: &ResidentExecutionDomain, poisoned: &mut bool)
        -> Result<(), SemanticTransitionError> {
        let source = self.prepared.as_ref().ok_or(SemanticTransitionError::ObservationMismatch)?;
        let mut recorder = domain.new_strict_recorder();
        source.storage.record(&mut recorder); recorder.read(&source.reader); recorder.read(&source.roster);
        recorder.write(&self.ranges); recorder.write(&self.contract);
        let arguments = (source.storage.control.device_ptr_value(), *source.reader.device_ptr(),
            source.roster.device_ptr_value(), (source.roster.len() / 2) as u64,
            self.ranges.device_ptr_value(), self.contract.device_ptr_value(), self.contract.len() as u64);
        enqueue_recorded(domain, poisoned, recorder, |enqueue| {
            ...
            unsafe { source.execute.clone().launch_in(enqueue, LaunchConfig { grid_dim: (1, 1, 1),
                block_dim: (1, 1, 1), shared_mem_bytes: 0 }, arguments) }
                .map_err(|error| XlogError::Kernel(error.to_string()))
        })
    }

    fn snapshot(&self, domain: &ResidentExecutionDomain, poisoned: &mut bool,
        directory: &TrackedCudaSlice<PublicationRange>, source: &TrackedCudaSlice<u8>,
        plan: &ModelContentCopyPlan) -> Result<(), SemanticTransitionError> {
        let row_bytes = size_of::<PublicationRange>();
        let directory_bytes = directory.len().checked_mul(row_bytes)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        if self.ranges.len() != plan.directory_offsets.len() || self.contract.len() != plan.contract_span.len()
            || plan.contract_span.end > source.len() || plan.directory_offsets.iter().any(|&offset|
                offset.checked_add(row_bytes).is_none_or(|end| end > directory_bytes)) {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
        let mut recorder = domain.new_strict_recorder();
        recorder.read(directory); recorder.read(source);
        recorder.write(&self.ranges); recorder.write(&self.contract);
        enqueue_recorded(domain, poisoned, recorder, |enqueue| {
            ...
            unsafe {
                for (index, &offset) in plan.directory_offsets.iter().enumerate() {
                    sys::cuMemcpyDtoDAsync_v2(self.ranges.device_ptr_value() + (index * row_bytes) as u64,
                        directory.device_ptr_value() + offset as u64, row_bytes, enqueue.stream().cu_stream())
                        .result().map_err(|e| XlogError::Kernel(format!("original model seal snapshot: {e}")))?;
                }
                sys::cuMemcpyDtoDAsync_v2(self.contract.device_ptr_value(),
                    source.device_ptr_value() + plan.contract_span.start as u64,
                    self.contract.len(), enqueue.stream().cu_stream())
                    .result().map_err(|e| XlogError::Kernel(format!("original model contract snapshot: {e}")))?;
            }
            Ok::<(), XlogError>(())
        })
    }

    fn verify_contract(&self, domain: &ResidentExecutionDomain, poisoned: &mut bool)
        -> Result<(), SemanticTransitionError> {
        let mut recorder = domain.new_strict_recorder();
        recorder.read(&self.ranges); recorder.read(&self.contract);
        let arguments = (self.contract.device_ptr_value(), self.contract.len() as u64,
            self.ranges.device_ptr_value() + ((self.ranges.len() - 1) * size_of::<PublicationRange>()) as u64);
        ...
        enqueue_recorded(domain, poisoned, recorder, |enqueue| {
            ...
            unsafe { execute.launch_in(enqueue, LaunchConfig { grid_dim: (1, 1, 1),
                block_dim: (1, 1, 1), shared_mem_bytes: 0 }, arguments) }
...
fn tensor_content_identity(tensor: &PreparedSemanticTensor) -> (SemanticTensorLayout, u64, u64, u64, usize) {
    (tensor.layout, tensor.logical_begin, tensor.logical_end, tensor.data,
        tensor.source.as_ref().map_or(0, DeviceMemoryView::len))
}

fn same_tensor_content_owner(expected: &PreparedSemanticTensor, actual: &PreparedSemanticTensor) -> bool {
...
impl StepContentStorage {
    fn feedback_origin_digest(&self, tensor: &PreparedSemanticTensor)
        ...
        let end = tensor.data.checked_add(bytes).ok_or(SemanticTransitionError::ObservationMismatch)?;
        for feedback in &self.feedback {
            for (index, original) in feedback.content.tensors.iter().enumerate() {
                let source = original.source.as_ref().expect("native feedback output owner");
                let original_end = original.data.checked_add(source.len() as u64)
                    .ok_or(SemanticTransitionError::ObservationMismatch)?;
                if bytes == 0 || tensor.data >= original_end || original.data >= end { continue; }
                ...
                return Ok(Some(CapturedTensorDigest::Tensor {
                    cells: Arc::clone(cells), offset: *offset, producer_sealed: true,
                }));
            }
        }
        Ok(None)
...
impl TensorContentBuffers {
    fn enqueue(&self, domain: &ResidentExecutionDomain, poisoned: &mut bool,
        execute: &CudaFunction, verify: bool) -> Result<(), SemanticTransitionError> {
        ...
        for (ordinal, tensor) in self.tensors.iter().enumerate() {
            ...
            let (pointer, bytes) = tensor.source.as_ref().map_or((0, 0), |source| (*source.device_ptr(), source.len()));
            let range = tensor_content_range(&tensor.layout, tensor.logical_begin, tensor.logical_end, bytes)?;
            let mut recorder = domain.new_strict_recorder();
            if let Some(source) = &tensor.source { recorder.read(source); }
            let (expected, verify) = match &self.seals {
                TensorContentSeals::Captured(digests) => {
                    let CapturedTensorDigest::Tensor { cells, offset, producer_sealed } = &digests[ordinal] else {
                        return Err(SemanticTransitionError::ObservationMismatch);
                    };
                    let verify = verify || *producer_sealed;
                    if verify { recorder.read(cells.as_ref()); }
                    else { recorder.write(cells.as_ref()); }
                    if offset.checked_add(4).is_none_or(|end| end > cells.len()) {
                        return Err(SemanticTransitionError::ObservationMismatch);
                    }
                    (cells.device_ptr_value() + (*offset * size_of::<u64>()) as u64, verify)
                }
                TensorContentSeals::Model(model) => {
                    recorder.read(&model.ranges);
                    (model.ranges.device_ptr_value() + (ordinal * size_of::<PublicationRange>()
                        + std::mem::offset_of!(PublicationRange, digest)) as u64, true)
                }
            };
            let arguments = (pointer, bytes as u64, range, tensor.layout, expected, u64::from(verify));
...
fn model_content_ranges(
    ...
    if directory.iter().filter(|range| range.role == 44).count() != 1
        || !directory.iter().any(|range| range.role == 44 && range.index == 0
            && range.generation == 1 && range.length_bytes != 0)
        || !directory.iter().any(|range| range.role == 18)
    {
        return Err(publication_input_error("model content requires the complete original roster and sealed model contract"));
    }
    let expected: BTreeSet<_> = directory.iter().filter(|range| matches!(range.role, 18..=20))
        .map(|range| (range.role, range.index)).collect();
    let mut seen = BTreeSet::new();
    let mut ranges = Vec::new();
    for (actual, begin, end, bytes) in tensors {
        let key = (actual.role, actual.index);
        if !expected.contains(&key) || !seen.insert(key) {
            return Err(publication_input_error("model content must name each original model tensor exactly once"));
        }
        tensor_content_range(&actual, begin, end, bytes)?;
        let (ordinal, range) = directory.iter().enumerate().find(|(_, range)|
            (range.role, range.index) == key).ok_or(SemanticTransitionError::ObservationMismatch)?;
        let mut original = *layouts.get(&key).ok_or(SemanticTransitionError::ObservationMismatch)?;
        if original.logical_axis != u64::MAX {
            let axis = usize::try_from(original.logical_axis).ok().filter(|&axis| axis < 4)
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            original.dimensions[axis] = range.logical_end.checked_sub(range.logical_begin)
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
        }
        ...
        if original != actual || range.generation != 1
            || (range.logical_begin, range.logical_end) != (begin, end)
        {
            return Err(publication_input_error("live model tensor differs from the original logical type, shape or interval"));
        }
        ranges.push(ordinal);
    }
    if seen != expected {
        return Err(publication_input_error("model content omits an original model tensor"));
    }
    Ok(ranges)
}

fn tensor_content_range(layout: &SemanticTensorLayout, begin: u64, end: u64, bytes: usize)
    -> Result<PublicationRange, SemanticTransitionError> {
    if (layout.role != 0 && SemanticStateRole::from_code(layout.role).is_none()) || end < begin
        || tensor_layout_bytes(layout)? != bytes
        || if layout.logical_axis == u64::MAX { begin != 0 || end != 0 }
            else { end - begin != layout.dimensions[layout.logical_axis as usize] }
    {
        return Err(publication_input_error("content tensor must retain its exact type, shape, interval and byte extent"));
    }
    Ok(PublicationRange { role: layout.role, index: layout.index,
        length_bytes: bytes as u64, logical_begin: begin, logical_end: end,
        ..PublicationRange::default() })
}

fn validate_content_coordinates(coordinates: impl Iterator<Item = (u64, u64)>)
    -> Result<(), SemanticTransitionError> {
    ...
    for (ordinal, (role, index)) in coordinates.enumerate() {
        if (role == 0 && index != ordinal as u64)
            ...
            return Err(publication_input_error("content coordinates require unique state roles or exact private capture positions"));
        }
    }
    Ok(())
}
...
fn committed_prefix_layout(range: &PublicationRange, prefix_capacity: u64)
    -> Result<SemanticTensorLayout, SemanticTransitionError> {
    let row_bytes = size_of::<SemanticTextSlot>() as u64;
    if range.role != SemanticStateRole::PrefixSource as u64 || range.index != 0
        || range.logical_begin != 0 || range.logical_end > prefix_capacity
    ...
    let layout = SemanticTensorLayout {
        role: SemanticStateRole::PrefixSource as u64, index: 0,
        scalar_type: 3, element_bytes: 8, rank: 2, logical_axis: 0,
        dimensions: [prefix_capacity, 8, 0, 0], strides_bytes: [row_bytes, 8, 0, 0],
    };
    tensor_layout_bytes(&layout)?;
    Ok(layout)
}
...
fn publication_export_span(range: &PublicationRange, layout: &SemanticTensorLayout,
    allocation_bytes: usize) -> Result<std::ops::Range<usize>, SemanticTransitionError> {
    let bytes = tensor_layout_bytes(layout)?;
    let begin = usize::try_from(range.offset_bytes).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
    let sealed_bytes = usize::try_from(range.length_bytes).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
    let capacity = matches!(range.role, 1 | 4 | 5 | 51..=54);
    if range.generation != 1 || (range.role, range.index) != (layout.role, layout.index)
        || range.offset_bytes % layout.element_bytes != 0
        || begin.checked_add(sealed_bytes).is_none_or(|end| end > allocation_bytes)
        || range.logical_end < range.logical_begin
        || (layout.logical_axis != u64::MAX && (range.logical_begin != 0
            || range.logical_end > layout.dimensions[layout.logical_axis as usize]))
        || (!capacity && bytes > sealed_bytes)
    { return Err(SemanticTransitionError::ObservationMismatch); }
    let end = begin.checked_add(bytes.max(sealed_bytes))
        .filter(|&end| end <= allocation_bytes).ok_or(SemanticTransitionError::ObservationMismatch)?;
    Ok(begin..end)
}

fn publication_record_layout(range: &PublicationRange) -> SemanticTensorLayout {
    SemanticTensorLayout { role: range.role, index: range.index,
        element_bytes: 1, scalar_type: 1, rank: 1, logical_axis: u64::MAX,
        dimensions: [range.length_bytes, 0, 0, 0], strides_bytes: [1, 0, 0, 0] }
}

impl SemanticPublishedLease {
    fn model_context_header(&self) -> Result<PublicationHeader, SemanticTransitionError> {
        let header = self.header;
        if header.abi != 1 || header.instance != self.identity.instance
...
impl SemanticFeedbackSchema {
    pub fn new(statements: [&[u8]; 2]) -> Result<Self, SemanticTransitionError> {
        if statements.iter().any(|statement| statement.is_empty()) {
            return Err(publication_input_error("native feedback requires two nonempty typed statements"));
        }
        let statement_bytes = statements.iter().map(|statement| statement.len()).max().unwrap();
        let feature_width = statement_bytes.checked_mul(9).and_then(|n| n.checked_add(4))
        ...
        Ok(Self { identity: hash.finalize().into(), statement_bytes, feature_width })
    }
}
...
pub const SEMANTIC_FEEDBACK_SUPPORT_FIELDS: [&str; 12] = [
    "original_statement", "original_support", "support_slot", "support_generation",
    "version_slot", "version_generation", "insertion_ordinal", "polarity",
    "support_digest_0", "support_digest_1", "support_digest_2", "support_digest_3",
];
...
impl SemanticFeedback {
    ...
    pub fn into_dlpack(self) -> [DlpackManagedTensor; 6] {
        [self.features, self.device_validity, self.query_receipts,
            self.original_statements, self.support_offsets, self.supports]
    }
}
...
impl FeedbackAllocationPlan {
    fn new(schema: &SemanticFeedbackSchema,
        slots: u64, support_capacity: u64) -> Result<Self, SemanticTransitionError> {
        let slots = usize::try_from(slots)
            .ok().filter(|&n| n >= 2 && i64::try_from(n).is_ok())
            .ok_or_else(|| publication_input_error("feedback slot capacity is not representable"))?;
        let support_capacity = usize::try_from(support_capacity)
            .ok().filter(|&n| n != 0 && i64::try_from(n).is_ok())
            .ok_or_else(|| publication_input_error("feedback support capacity is not representable"))?;
        let cells = slots.checked_mul(schema.feature_width)
            .ok_or_else(|| publication_input_error("feedback feature extent overflows"))?;
        let query_cells = slots.checked_mul(42)
            .ok_or_else(|| publication_input_error("feedback receipt extent overflows"))?;
        let support_cells = support_capacity.checked_mul(SEMANTIC_FEEDBACK_SUPPORT_FIELDS.len())
            .ok_or_else(|| publication_input_error("feedback support extent overflows"))?;
        let offset_cells = slots.checked_add(1)
            .ok_or_else(|| publication_input_error("feedback offset extent overflows"))?;
        let service_bytes = query_cells.checked_add(slots).and_then(|n| n.checked_add(offset_cells))
            .and_then(|n| n.checked_add(support_cells)).and_then(|n| n.checked_mul(8))
            .ok_or_else(|| publication_input_error("feedback service extent overflows"))?;
        let bytes = cells.checked_mul(size_of::<f32>())
            .and_then(|n| slots.checked_mul(9).and_then(|metadata| n.checked_add(metadata)))
        ...
        Ok(Self { slots, support_capacity, cells, query_cells, support_cells, offset_cells, bytes })
    }
}
...
impl FeedbackBuffers {
    ...
    fn allocate(provider: &CudaKernelProvider, schema: SemanticFeedbackSchema,
        slots: u64, support_capacity: u64) -> Result<Self, SemanticTransitionError> {
        let plan = FeedbackAllocationPlan::new(&schema, slots, support_capacity)?;
        let mut reservation = provider.memory().reserve_bytes(plan.bytes as u64)
            .map_err(|error| runtime_error("feedback reservation", error))?;
        Self::allocate_reserved(provider, schema, plan, &mut reservation)
    }

    fn allocate_reserved(provider: &CudaKernelProvider, schema: SemanticFeedbackSchema,
        plan: FeedbackAllocationPlan, reservation: &mut GpuMemoryReservation) -> Result<Self, SemanticTransitionError> {
        let FeedbackAllocationPlan { slots, support_capacity, cells, query_cells, support_cells, offset_cells, .. } = plan;
        let encode = provider.device().inner()
            .get_func("xlog_semantic_transition", "semantic_feedback_encode")
            .ok_or_else(|| runtime_error("kernel lookup", "semantic feedback kernel unavailable"))?;
        let project = provider.device().inner()
            .get_func("xlog_semantic_transition", "semantic_feedback_project")
            .ok_or_else(|| runtime_error("kernel lookup", "semantic feedback projection unavailable"))?;
        let witness = provider.device().inner()
        ...
        let mut buffers = Self {
            schema, slots, support_capacity, encode, project, witness,
            content: TensorContentBuffers { tensors: Vec::new(),
                seals: TensorContentSeals::Captured(Vec::new()), verification_inputs: Vec::new() },
            features: reservation.alloc::<f32>(cells).map_err(|error| runtime_error("feedback features allocation", error))?,
            validity: reservation.alloc::<u8>(slots).map_err(|error| runtime_error("feedback validity allocation", error))?,
            status: reservation.alloc::<u64>(slots).map_err(|error| runtime_error("feedback status allocation", error))?,
            query_receipts: reservation.alloc::<u64>(query_cells).map_err(|error| runtime_error("feedback receipt allocation", error))?,
            original_statements: reservation.alloc::<u64>(slots).map_err(|error| runtime_error("feedback origin allocation", error))?,
            support_offsets: reservation.alloc::<u64>(offset_cells).map_err(|error| runtime_error("feedback offset allocation", error))?,
            supports: reservation.alloc::<u64>(support_cells).map_err(|error| runtime_error("feedback support allocation", error))?,
        };
        ...
        let features = unsafe { buffers.features.view().cast::<u8>() }
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        let originals = [features, buffers.validity.view(), words(buffers.query_receipts.view())?,
            words(buffers.original_statements.view())?, words(buffers.support_offsets.view())?,
            words(buffers.supports.view())?];
        let shapes = [(6, 4, 2, [slots as u64, buffers.schema.feature_width as u64, 0, 0]),
            (8, 1, 1, [slots as u64, 0, 0, 0]), (3, 8, 2, [slots as u64, 42, 0, 0]),
            (3, 8, 1, [slots as u64, 0, 0, 0]), (3, 8, 1, [offset_cells as u64, 0, 0, 0]),
            (3, 8, 2, [support_capacity as u64, 12, 0, 0])];
        let mut digests = Vec::with_capacity(originals.len());
        for (index, (source, (scalar_type, element_bytes, rank, dimensions))) in originals.into_iter().zip(shapes).enumerate() {
            let layout = canonical_tensor_layout(SemanticTensorLayout {
                role: 0, index: index as u64, scalar_type, element_bytes, rank, dimensions,
                logical_axis: u64::MAX, strides_bytes: [0; 4],
            })?;
            tensor_content_range(&layout, 0, 0, source.len())?;
            buffers.content.tensors.push(PreparedSemanticTensor {
                layout, logical_begin: 0, logical_end: 0, data: *source.device_ptr(),
                source: Some(source), native_allocation: None, _empty_owner: None,
            });
            digests.push(CapturedTensorDigest::Tensor {
                cells: Arc::new(reservation.alloc::<u64>(4)
    ...
    #[allow(clippy::too_many_arguments)]
    fn enqueue(&self, domain: &ResidentExecutionDomain, poisoned: &mut bool,
        graph: &SemanticHypergraph, storage: &PublicationStorage,
        task: &TrackedCudaSlice<u64>, lease: &TrackedCudaSlice<PublicationLease>,
        descriptor: Descriptor) -> Result<(), SemanticTransitionError> {
        let mut recorder = domain.new_strict_recorder();
        recorder.read(lease);
        ...
        for directory in &storage.directories { recorder.read(directory); }
        for allocation in &storage.allocations { recorder.read(allocation); }
        recorder.write(&self.features);
        recorder.write(&self.validity);
        recorder.write(&self.status);
        let arguments = (storage.control.device_ptr_value(), lease.device_ptr_value(),
            self.schema.statement_bytes as u64, self.slots as u64,
            self.features.device_ptr_value(), self.validity.device_ptr_value(), self.status.device_ptr_value());
        enqueue_recorded(domain, poisoned, recorder, |stream| {
            ...
            unsafe { self.encode.clone().launch_in(stream, LaunchConfig {
                grid_dim: (self.slots.min(65535) as u32, 1, 1),
                block_dim: (256, 1, 1), shared_mem_bytes: 0 }, arguments) }
                .map_err(|error| XlogError::Kernel(error.to_string()))
        })?;
        let mut recorder = domain.new_strict_recorder();
        ...
        let arguments = (descriptor, lease.device_ptr_value(), self.slots as u64,
            self.query_receipts.device_ptr_value(), self.original_statements.device_ptr_value(),
            self.support_offsets.device_ptr_value(), self.supports.device_ptr_value(),
            self.support_capacity as u64, self.status.device_ptr_value());
        enqueue_recorded(domain, poisoned, recorder, |stream| {
            ...
            unsafe { self.project.clone().launch_in(stream, LaunchConfig {
                grid_dim: (1, 1, 1), block_dim: (1, 1, 1), shared_mem_bytes: 0 }, arguments) }
                .map_err(|error| XlogError::Kernel(error.to_string()))
        })?;
...
#[allow(clippy::too_many_arguments)]
fn export_owned_allocation(view: DeviceMemoryView<u8>, shape: Vec<i64>, strides: Vec<i64>,
    dtype: (u8, u8), device_id: i32, alias_guard: Arc<()>, publication: Arc<PublicationStorage>,
    continuation: Option<Arc<TextBindingStorage>>) -> DlpackManagedTensor {
    let mut export = Box::new(PublishedTensorExport {
        managed: crate::dlpack::DLManagedTensor { dl_tensor: crate::dlpack::DLTensor {
            data: *view.device_ptr() as usize as *mut std::ffi::c_void,
            device: crate::dlpack::DLDevice { device_type: 2, device_id },
            ndim: shape.len() as i32,
            dtype: crate::dlpack::DLDataType { code: dtype.0, bits: dtype.1, lanes: 1 },
            shape: std::ptr::null_mut(), strides: std::ptr::null_mut(), byte_offset: 0,
        }, manager_ctx: std::ptr::null_mut(), deleter: Some(delete_published_tensor) },
        shape, strides, _view: view, _alias_guard: alias_guard, _publication: publication,
...
fn dlpack_consumer_stream(stream: u64) -> Result<sys::CUstream, SemanticTransitionError> {
    if matches!(stream, 0 | 2) { return Err(publication_input_error("DLPack consumer requires legacy default stream 1 or an explicit live stream; zero and per-thread default are unsupported")); }
    Ok(if stream == 1 { std::ptr::null_mut() } else { stream as usize as sys::CUstream })
}
...
fn publication_abi_bytes<T: DeviceRepr>(records: &[T]) -> Vec<u8> {
    ...
    unsafe { std::slice::from_raw_parts(records.as_ptr().cast(), std::mem::size_of_val(records)) }.to_vec()
}
...
fn initial_intent_records(
    ...
    let entries_bytes = usize::try_from(entry_capacity).ok()
        .and_then(|count| count.checked_mul(size_of::<IntentEntry>()))
        .and_then(|bytes| bytes.checked_add(size_of::<IntentQueueHeader>()))
        .ok_or_else(|| publication_input_error("intent entry capacity overflows its native allocation"))?;
    let required_payload = u64::try_from(effect.len()).ok()
        .and_then(|bytes| bytes.checked_add(final_payload_bytes));
    if effect.is_empty() || entry_capacity == 0 || final_payload_bytes == 0
        || required_payload.is_none_or(|bytes| bytes > payload_capacity as u64)
    {
        return Err(publication_input_error("initial intent requires its actual effect and reserved final-output capacity"));
    }
    let header = IntentQueueHeader {
        abi: 1, count: 0, capacity: entry_capacity,
        payload_used_bytes: effect.len() as u64, payload_capacity_bytes: payload_capacity as u64,
        effect_offset_bytes: 0, effect_length_bytes: effect.len() as u64,
        chain_head: Identity256::default(),
    };
    Ok([
        SemanticStateRecord { role: SemanticStateRole::IntentEntries, index: 0,
            bytes: publication_abi_bytes(&[header]), capacity_bytes: entries_bytes },
        SemanticStateRecord { role: SemanticStateRole::IntentPayload, index: 0,
            bytes: effect.to_vec(), capacity_bytes: payload_capacity },
    ])
}
...
fn continuation_role(role: u64) -> bool { matches!(role, 1 | 4..=13 | 39 | 51..=55) }

fn validate_parent_records(parent: &SemanticParentBinding, tensors: &[PreparedSemanticTensor]) -> Result<(), SemanticTransitionError> {
    validate_publication_counts(&parent.role_counts)?;
    validate_text_parent(&parent.source, parent.prefix.len() as u64,
        parent.prefix_capacity, parent.feedback_capacity, parent.max_position,
        parent.ring_head, parent.provenance_records)?;
    if parent.feedback_capacity < 2 || parent.rng.stream_serial >= 1 << 56
        ...
        || parent.terminal_tokens.iter().any(|&token| token >= TEXT_CARDINALITY as u64)
        || parent.terminal_tokens.iter().copied().collect::<BTreeSet<_>>().len() != parent.terminal_tokens.len()
    { return Err(publication_input_error("parent generations, feedback extent, or token contract mismatch")); }
    for (position, slot) in parent.prefix.iter().enumerate() {
        if slot.kind != 1 || slot.valid != 1 || slot.committed != 1 || slot.logical_position != position as u64
            || slot.token >= TEXT_CARDINALITY as u64 || !matches!(slot.provenance, 1 | 2)
            || slot.provenance_record >= parent.provenance_records || slot.recomputed > 1
        { return Err(publication_input_error("committed prefix source is not the exact ordered valid prefix")); }
    }
    let mut keys = BTreeSet::new();
    for record in &parent.records {
        let role = record.role as u64;
        if is_tensor_role(role) || matches!(role, 1 | 3 | 14..=16 | 33 | 43 | 47 | 48 | 55)
            || record.index >= parent.role_counts[role as usize - 1]
            || (record.bytes.is_empty() && role != 2) || record.bytes.len() > record.capacity_bytes
            || !keys.insert((role, record.index))
        { return Err(publication_input_error("duplicate, generated, tensor, empty, or out-of-capacity control record")); }
        if role == 2 && (parent.provenance_records.checked_mul(size_of::<TokenProvenance>() as u64) != Some(record.bytes.len() as u64)
            || (parent.provenance_records + 32).checked_mul(size_of::<TokenProvenance>() as u64).is_none_or(|n| n > record.capacity_bytes as u64))
        { return Err(publication_input_error("token provenance records or append capacity differ from the actual source ledger")); }
        if role == 44 { parent.model_contract_layout.validate(record.bytes.len() as u64)?; }
        if role == 30 {
            if record.bytes.len() < size_of::<IntentQueueHeader>() { return Err(publication_input_error("intent queue has no complete owner header")); }
            // SAFETY: complete checked header bytes, integral padding-free ABI.
            let header = unsafe { record.bytes.as_ptr().cast::<IntentQueueHeader>().read_unaligned() };
            if header.abi != 1 || header.count > header.capacity
                || header.count.checked_mul(size_of::<IntentEntry>() as u64).and_then(|n| n.checked_add(size_of::<IntentQueueHeader>() as u64)) != Some(record.bytes.len() as u64)
                || header.capacity.checked_mul(size_of::<IntentEntry>() as u64).and_then(|n| n.checked_add(size_of::<IntentQueueHeader>() as u64)) != Some(record.capacity_bytes as u64)
                || header.payload_used_bytes > header.payload_capacity_bytes
                || header.effect_length_bytes == 0
                || header.effect_offset_bytes.checked_add(header.effect_length_bytes).is_none_or(|n| n > header.payload_used_bytes)
            { return Err(publication_input_error("intent queue count, capacity, or actual effect payload is inconsistent")); }
            let payload = parent.records.iter().find(|record| record.role == SemanticStateRole::IntentPayload)
                .ok_or_else(|| publication_input_error("intent queue payload owner is absent"))?;
            if payload.bytes.len() as u64 != header.payload_used_bytes || payload.capacity_bytes as u64 != header.payload_capacity_bytes {
                return Err(publication_input_error("intent queue does not resolve to its actual payload allocation"));
            }
        }
    }
    for tensor in tensors {
        let layout = &tensor.layout;
        if !is_tensor_role(layout.role) || layout.role >= 51
        ...
        if matches!(layout.role, 4 | 5) {
            if tensor.logical_begin != 0 || tensor.logical_end != parent.prefix.len() as u64
                || layout.dimensions[2] != parent.prefix.len() as u64 {
                return Err(publication_input_error("initial attention tensor does not cover exactly the acquired prefix"));
            }
            prefix_capacity_layout(*layout, parent.prefix_capacity)?;
        } else if layout.logical_axis != u64::MAX || (matches!(layout.role, 18..=25) && (tensor.logical_begin != 0 || tensor.logical_end != 0)) {
            return Err(publication_input_error("non-positional model tensors require a non-sliced exact layout"));
        }
        if matches!(layout.role, 8..=13) && (layout.scalar_type != 6 || layout.element_bytes != 4) {
            return Err(publication_input_error("value and edit accumulators require original FP32 storage"));
        }
    }
    for layout in &parent.active_layouts {
        if !matches!(layout.role, 51..=54) || layout.index >= parent.role_counts[layout.role as usize - 1]
            || !keys.insert((layout.role, layout.index)) || tensor_layout_bytes(layout)? == 0 {
            return Err(publication_input_error("active output capacity layout is absent, duplicate, or outside the cold model roster"));
        }
        validate_active_capacity(layout, 32u64.checked_add(parent.feedback_capacity)
            .ok_or_else(|| publication_input_error("active row capacity overflow"))?)?;
    }
    for role in 1..=55u64 {
        if matches!(role, 1 | 3 | 14..=16 | 33 | 43 | 47 | 48 | 55) { continue; }
        for index in 0..parent.role_counts[role as usize - 1] {
            if !keys.contains(&(role, index)) { return Err(publication_input_error("publication role lacks its actual cold owner")); }
        }
    }
    Ok(())
}
...
fn validate_text_parent(
    ...
    for slot in slots {
        if slot.kind > 2 || (if slot.kind == 2 { slot.token != MASK_TOKEN as u64 } else { slot.token >= TEXT_CARDINALITY as u64 }) {
            return Err(invalid("text token or row kind is outside its cold contract"));
        }
        if slot.kind == 0 {
        ...
        {
            return Err(invalid("active source slot has inconsistent position or provenance"));
        }
        if slot.kind == 1 {
            if !matches!(slot.provenance, 1 | 2) || slot.provenance_record >= provenance_records {
                return Err(invalid("filled source slot requires source or generated provenance"));
            }
            filled.insert(slot.logical_position);
        } else if slot.provenance != 0 || slot.provenance_record != 0 || slot.recomputed != 0 {
            return Err(invalid("mask source slot cannot carry filled-token coverage"));
        }
    }
    let mut structural_end = prefix_extent;
    while filled.contains(&structural_end) {
        if structural_end == prefix_capacity {
            return Err(invalid("contiguous source completion exceeds prefix capacity"));
        }
        structural_end += 1;
    }
    Ok(structural_end)
...
impl SemanticPolicyGradients {
    ...
    pub fn into_dlpack(self) -> Result<[DlpackManagedTensor; 2], SemanticTransitionError> {
        ...
        let export = |values: TrackedCudaSlice<f32>, layout: SemanticTensorLayout| {
            ...
            let shape = layout.dimensions[..rank].iter().map(|&value| value as i64).collect();
            let strides = layout.strides_bytes[..rank].iter()
                .map(|&value| (value / layout.element_bytes) as i64).collect();
            export_owned_allocation(values.into_bytes().view(), shape, strides, (2, 32),
                self.provider.device().ordinal() as i32, Arc::clone(&self.step_aliases),
                Arc::clone(&self.publication), Some(Arc::clone(&self.continuation)))
        };
        Ok([export(self.parameters, layouts[2]), export(self.text_logits, layouts[0])])
    }
}

#[cfg(feature = "semantic-policy")]
fn policy_producer_layouts(support_cells: usize, parameter_cells: usize)
    -> Result<[SemanticTensorLayout; 3], SemanticTransitionError> {
    let layout = |index, scalar_type, element_bytes, rank, dimensions| {
        canonical_tensor_layout(SemanticTensorLayout {
            role: 0, index, scalar_type, element_bytes, rank, dimensions,
            logical_axis: u64::MAX, strides_bytes: [0; 4],
        })
    };
    Ok([
...
#[cfg(feature = "semantic-policy")]
fn policy_source_views(originals: &[PreparedSemanticTensor]) -> Result<PolicyProducerViews, SemanticTransitionError> {
    if originals.len() != 3 { return Err(publication_input_error("policy requires its complete original producer roster")); }
    let source = |index: usize| originals[index].source.clone()
    ...
    let text_logits = unsafe { source(0)?.cast::<f32>() }
        .ok_or_else(|| publication_input_error("original text logits are not aligned F32 storage"))?;
    // SAFETY: the same typed original admission holds for model parameters.
    let parameters = unsafe { source(2)?.cast::<f32>() }
        .ok_or_else(|| publication_input_error("original policy parameters are not aligned F32 storage"))?;
    Ok(PolicyProducerViews { text_logits, product_support: source(1)?, parameters })
}

#[cfg(feature = "semantic-policy")]
fn enqueue_policy_snapshots(domain: &ResidentExecutionDomain, poisoned: &mut bool,
    buffers: &PolicyBuffers, support: &TrackedCudaSlice<u8>, original: &PolicyProducerViews)
    -> Result<(), SemanticTransitionError> {
    if original.text_logits.len() != buffers.text_logits.len()
        || original.product_support.len() != support.len() || original.parameters.len() != buffers.parameters.len() {
        return Err(publication_input_error("original policy producers differ from their fixed native capacities"));
    }
    let mut recorder = domain.new_strict_recorder();
    ...
    enqueue_recorded(domain, poisoned, recorder, |enqueue| {
        for (destination, source, bytes) in [
            (buffers.text_logits.device_ptr_value(), *original.text_logits.device_ptr(), original.text_logits.len() * 4),
            (support.device_ptr_value(), *original.product_support.device_ptr(), original.product_support.len()),
            (buffers.parameters.device_ptr_value(), *original.parameters.device_ptr(), original.parameters.len() * 4),
        ] {
...
macro_rules! content_kernel_parameter {
    ($ty:ty) => {
        ...
        impl crate::cuda_compat::IntoKernelParamStorage for $ty {
            type Storage = Self;
            fn into_kernel_param_storage(self) -> Self { self }
        }
    };
}
...
mod task_state_contract {
    ...
    fn text_null_requires_the_complete_singleton_receipt() {
        let mut receipt = SemanticTransitionReceipt {
            kind: COMPONENT_KIND_TEXT as u32, choice: TEXT_NULL as u32,
            legal_count: 1, active_count: 1, p: [1, 0, 0], q: [1, 0, 0],
            factor_denominator: [1, 0, 0], cdf_end: 1 << 63, mass: 1 << 63,
            ..SemanticTransitionReceipt::default()
        };
        assert!(canonical_text_null(&receipt));
        let exact = receipt;
        receipt.choice = 0; assert!(!canonical_text_null(&receipt));
        receipt = exact; receipt.p[1] = 1; assert!(!canonical_text_null(&receipt));
        receipt = exact; receipt.cdf_start = 1; assert!(!canonical_text_null(&receipt));
        receipt = exact; receipt.active_count = 2; assert!(!canonical_text_null(&receipt));
        receipt = exact; receipt.mass -= 1; assert!(!canonical_text_null(&receipt));
    }
    ...
    fn ordinary_refusal_requires_original_coordinates_and_completed_cleanup() {
        let rng = SemanticRngBinding { model_generation: 3, family_id: 4, stream_serial: 5, proposal: 6 };
        let binding = SemanticActionCatalogue::current().binding();
        let original = DeviceState {
            model_generation: rng.model_generation, family_id: u32::from(rng.family_id),
            stream_serial: rng.stream_serial, proposal: 6, next_proposal: 6,
            catalogue_generation: binding.generation, catalogue_digest: binding.digest,
            binding_digest: binding.digest, status: 5, ..DeviceState::default()
        };
        assert_eq!(refused_terminal(&original, rng, binding, binding.digest).unwrap(),
        ...
        state.status = 2;
        state.blocks = 50;
        assert_eq!(refused_terminal(&state, rng, binding, binding.digest).unwrap(),
            SemanticTransitionRefusal::InvalidFinalSupport { completed_draws: 50 });
        state.status = 11;
        assert!(refused_terminal(&state, rng, binding, binding.digest).is_err());
        state.execution_work.overflow = 1;
        assert_eq!(refused_terminal(&state, rng, binding, binding.digest).unwrap(),
            SemanticTransitionRefusal::WorkCounterOverflow { completed_draws: 50 });
    ...
    fn refused_seal_cleanup_distinguishes_aliases_from_retired_private_roots() {
        ...
        assert!(refused_root_cleanup(&state, 17, 2, 3).is_err(), "base aliases are never retired");
        ...
        assert!(refused_root_cleanup(&state, 17, 2, 3).is_err(), "no seal exists in the other lane");
    }
}
...
fn refused_terminal(state: &DeviceState, rng: SemanticRngBinding,
    catalogue: SemanticCatalogueBinding, binding_digest: Identity256,
) -> Result<SemanticTransitionRefusal, SemanticTransitionError> {
    if state.model_generation != rng.model_generation || state.family_id != u32::from(rng.family_id)
        || state.stream_serial != rng.stream_serial || state.proposal != u64::from(rng.proposal)
        || state.next_proposal != state.proposal || state.blocks >= COMPONENT_COUNT as u64
        || state.catalogue_generation != catalogue.generation || state.catalogue_digest != catalogue.digest
        || state.binding_digest != binding_digest || state.importance_weight != 0.0
        || state.retired_roots_mask & !3 != 0 || state.cleanup_receipts.iter().any(|receipt| receipt[0] != 0) {
        return Err(SemanticTransitionError::ObservationMismatch);
    }
    match state.status {
        2 => Ok(SemanticTransitionRefusal::InvalidFinalSupport { completed_draws: state.blocks }),
        5 => Ok(SemanticTransitionRefusal::NonFinitePolicyInput { completed_draws: state.blocks }),
        11 if state.execution_work.overflow == 1 =>
            Ok(SemanticTransitionRefusal::WorkCounterOverflow { completed_draws: state.blocks }),
        _ => Err(SemanticTransitionError::ObservationMismatch),
    }
}

fn refused_root_cleanup(state: &DeviceState, owner: u64, base_slot: u64, base_generation: u64)
    -> Result<(), SemanticTransitionError> {
    ...
    for lane in 0..2 {
        ...
        if sealed[0] != 0 || sealed[33] != 1 {
            if consumed || cleanup.iter().any(|&word| word != 0) { return Err(mismatch()); }
            continue;
        }
        if !consumed || sealed[39] != owner { return Err(mismatch()); }
        match sealed[1] {
            // An unchanged seal only aliases the protected base, so there is
            // deliberately no retirement receipt for it.
            2 if sealed[34] == base_slot && sealed[35] == base_generation
                && cleanup.iter().all(|&word| word == 0) => {}
            1 if cleanup[0] == 0 && cleanup[1] == 0 && cleanup[39] == owner
                && cleanup[3] == sealed[34]
...
pub enum SemanticPreparedStepOutcome {
    Skipped { step: SemanticPreparedStep, status: u64 },
    Completed { step: SemanticPreparedStep, invocation: SemanticRngBinding,
        transition: SemanticTransitionKind, outcome: SemanticTransitionOutcome },
}

const _: () = assert!(size_of::<PreparedStepResult>() == size_of::<PublicationHeader>() + 32);

fn prepared_step_allocation_bytes(input_bytes: usize, feedback_bytes: usize, policy_bytes: usize)
    -> Result<u64, SemanticTransitionError> {
    [size_of::<PublicationLease>(), size_of::<PreparedStepResult>(), input_bytes, feedback_bytes, policy_bytes]
        .into_iter().try_fold(0usize, |sum, bytes| sum.checked_add(bytes)
            .ok_or(SemanticTransitionError::GenerationExhausted))
        .and_then(|bytes| u64::try_from(bytes).map_err(|_| SemanticTransitionError::GenerationExhausted))
}

fn prepared_segment_allocation_bytes(step_bytes: u64, transition_bound: usize)
    -> Result<u64, SemanticTransitionError> {
    if transition_bound == 0 { return Err(publication_input_error("a prepared segment requires a positive transition bound")); }
    u64::try_from(transition_bound).ok().and_then(|bound| step_bytes.checked_mul(bound))
        .ok_or(SemanticTransitionError::GenerationExhausted)
}

#[cfg(feature = "semantic-policy")]
fn policy_buffer_bytes(layout: &SemanticPolicyLayout) -> Result<usize, SemanticTransitionError> {
    [layout.parameter_cells, 128, layout.score_cells(), layout.recurrent_cells(), 32 * TEXT_CARDINALITY]
        .into_iter().try_fold(0usize, |sum, cells| sum.checked_add(cells)
            .ok_or(SemanticTransitionError::GenerationExhausted))?
        .checked_mul(size_of::<f32>()).ok_or(SemanticTransitionError::GenerationExhausted)
}

fn validate_rebinding_ownership(prepared_segment: Option<&PreparedSegmentState>,
    unreconciled_acquisition: bool) -> Result<(), SemanticTransitionError> {
    if prepared_segment.is_some() {
        return Err(publication_input_error("a prepared segment still owns its executable and native resources"));
    }
    if unreconciled_acquisition {
        return Err(publication_input_error("an unreconciled acquisition still owns its native step"));
    }
    Ok(())
}

fn validate_prepared_completion(lease: &PublicationLease, parent: &PublicationHeader,
    result: &PreparedStepResult, instance: Identity256) -> Result<SemanticTransitionKind, SemanticTransitionError> {
    if lease.abi != 1 || lease.status != 0 || lease.active != 0 || lease.instance != instance
        || lease.bank > 1 || lease.bank != lease.word & 1 || lease.epoch != lease.word >> 1
        || parent.abi != 1 || parent.instance != instance || parent.publication_word != lease.word
        || parent.sealed_epoch != lease.epoch || result.abi != 1 || result.header.abi != 1
        || result.header.instance != instance || result.header.publication_word != result.word
        || result.header.sealed_epoch != result.word >> 1 || result.advanced>1
        || result.advanced!=u64::from(result.word!=lease.word && result.refusal==0) {
        return Err(SemanticTransitionError::ObservationMismatch);
    }
    if result.word != lease.word {
        let epoch = lease.epoch.checked_add(1).filter(|&epoch| epoch <= u64::MAX >> 1)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        if result.word != (epoch << 1) | ((lease.word ^ 1) & 1) || result.header.base_word != lease.word {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
    }
...
impl SemanticTransitionSession {
    ...
    pub fn launch_prepared_segment(&mut self, authority_decisions: Vec<u8>) -> Result<(), SemanticTransitionError> {
        self.ensure_quiescent()?;
        let storage=Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let build=self.prepared_segment.as_ref().ok_or(SemanticTransitionError::NotCaptured)?;
        if !build.finished || build.submitted || self.captured.is_none() || authority_decisions.is_empty() {
            return Err(publication_input_error("prepared submission requires one unused graph and fresh canonical authority bytes"));
        }
        for token in &build.tokens {
            let prepared=self.steps.get(token).and_then(|step| step.prepared.as_ref())
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            let continuation=prepared.continuation.as_ref().ok_or(SemanticTransitionError::NotBound)?;
            if !prepared.transition_recorded || continuation.inputs.authority_bytes != authority_decisions.len() as u64 {
                return Err(publication_input_error("fresh authority snapshot differs from the captured continuation byte extent"));
            }
        }
        let authority=storage.continuation_templates.iter().find(|range| range.role==39 && range.index==0)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        let slot=usize::try_from(authority.storage_slot).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
        let destination=storage.allocations.get(slot).ok_or(SemanticTransitionError::ObservationMismatch)?;
        if authority.offset_bytes != 0 || authority_decisions.len()>destination.len() {
            return Err(publication_input_error("fresh authority snapshot exceeds its original fixed allocation"));
        }
        let recorder=self.prepared_segment_recorder()?;
        self.prepared_segment.as_mut().expect("checked segment").submit()?;
        self.publication_uploads=vec![(slot,PublicationPayload::Metadata(authority_decisions))];
        let result=(|| {
            let PublicationPayload::Metadata(bytes)=&self.publication_uploads[0].1 else { unreachable!("owned authority metadata") };
            let mut destination=storage.allocations[slot].view().slice(..bytes.len());
            self.provider.htod_launch_metadata_sync_copy_into(bytes,&mut destination)
                .map_err(|error| runtime_error("prepared fresh authority upload",error))?;
            let graph=self.captured.as_ref().expect("checked original captured graph");
            enqueue_recorded(&self.domain,&mut self.poisoned,recorder,|enqueue| graph.launch_in(enqueue))
        })();
        if result.is_err() { self.poisoned=true;return result; }
        self.pending=true;
        Ok(())
    }
    ...
    pub fn complete_prepared_segment(&mut self) -> Result<Vec<SemanticPreparedStepOutcome>,SemanticTransitionError> {
        if self.is_poisoned() { return Err(SemanticTransitionError::Poisoned); }
        if !self.pending || !self.prepared_segment.as_ref().is_some_and(|build| build.submitted && !build.completed) {
        ...
        if result.is_err() { self.poisoned=true; }
            ...
            self.continuation_base=None;self.admitted_transition=None;self.text_binding=None;
        }
        result
    }

    fn complete_prepared_segment_inner(&mut self) -> Result<Vec<SemanticPreparedStepOutcome>,SemanticTransitionError> {
        ...
        wait_on_stream(&self.stream,&mut self.poisoned,&mut self.stream_waits,
            "prepared graph terminal wait",CudaStream::synchronize)?;
        self.prepared_segment.as_mut().expect("submitted segment").completed=true;
        let handles=self.prepared_segment.as_ref().expect("submitted segment").handles()?;
        let storage=Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let mut outcomes=Vec::new();
        outcomes.try_reserve_exact(handles.len()).map_err(|error| runtime_error("prepared outcome reservation",error))?;
        let mut previous=None;
        for step in handles {
            let prepared=self.steps[&step.token].prepared.as_ref().ok_or(SemanticTransitionError::ObservationMismatch)?;
            let reader=prepared.reader.view();let result_view=prepared.result.view();
            let lease=self.publication_read(reader)?[0];
            let result=self.publication_read(result_view)?[0];
            if result.abi==0 {
                if lease.active!=0 || lease.abi!=0 || lease.transition_kind!=0 || !matches!(lease.status,1..=5|7) {
                    return Err(SemanticTransitionError::ObservationMismatch);
                }
                outcomes.push(SemanticPreparedStepOutcome::Skipped { step,status:lease.status });
                continue;
            }
            let owner=&self.steps[&step.token];
            let prepared=owner.prepared.as_ref().expect("original prepared owner");
            let inputs=Arc::clone(owner.inputs.as_ref().ok_or(SemanticTransitionError::ObservationMismatch)?);
            let state_view=prepared.state.view();
            let receipts=prepared.receipts.as_ref().ok_or(SemanticTransitionError::ObservationMismatch)?.view();
            let parent=self.publication_read(inputs.header.view())?[0];
            if previous.is_some_and(|header| header!=parent) { return Err(SemanticTransitionError::ObservationMismatch); }
            let transition=validate_prepared_completion(&lease,&parent,&result,storage.instance)?;
            let state=self.publication_read(state_view)?[0];
            let work = self.steps[&step.token].prepared.as_ref().expect("original prepared owner").model_work.as_ref()
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            if !state.execution_work.validate_model(&work.recording, state.status == 11) {
                return Err(SemanticTransitionError::ObservationMismatch);
            }
            let components=if transition==SemanticTransitionKind::Proposal { self.publication_read(receipts)? } else { Vec::new() };
            let invocation=parent.rng_binding()?;
            let outcome=self.reconcile_prepared_transition(&parent,&result,state,components,invocation,transition)?;
            previous=Some(result.header);
            #[cfg(feature="semantic-policy")]
            if transition==SemanticTransitionKind::Proposal {
                self.take_prepared_policy_tape(&step,invocation,match &outcome {
                    SemanticTransitionOutcome::Refused(refusal)=>Some(*refusal),_=>None })?;
            }
            self.steps.get_mut(&step.token).expect("retained original step").prepared.as_mut()
                .expect("retained original prepared owner").observed=true;
            outcomes.push(SemanticPreparedStepOutcome::Completed { step,invocation,transition,outcome });
        }
    ...
    fn reconcile_prepared_transition(&mut self,parent:&PublicationHeader,result:&PreparedStepResult,
        state:DeviceState,components:Vec<SemanticTransitionReceipt>,invocation:SemanticRngBinding,
        transition:SemanticTransitionKind) -> Result<SemanticTransitionOutcome,SemanticTransitionError> {
        if result.refusal!=0 || state.status==10 {
            return Err(SemanticTransitionError::PublicationRefused { status:result.refusal });
        }
        if matches!(state.status,2|5|11) {
            let refusal=refused_terminal(&state,invocation,SemanticActionCatalogue::current().binding(),self.binding().digest)?;
            ...
            refused_root_cleanup(&state,self.graph.transition_arena()[1],u64::from(root.slot()),root.generation())?;
            self.root=root;self.base_snapshot=snapshot;self.next_proposal=parent.proposal;
        ...
        let next=parent.proposal.checked_add(u64::from(transition==SemanticTransitionKind::Proposal))
            .ok_or(SemanticTransitionError::GenerationExhausted)?;
        if state.status!=0 || result.word==parent.publication_word || result.header.proposal!=next {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
        let winner=if transition==SemanticTransitionKind::Proposal { state.task_evaluation.winner } else { 0 };
        let index=match winner { 0=>0,1=>3,2=>6,_=>return Err(SemanticTransitionError::ObservationMismatch) };
        if winner>0 && state.retired_roots_mask & (1<<(winner-1))!=0 {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
        let root=self.observe_publication_root(&state,&result.header,index)?;
        self.finish_observed_transition(state,components,invocation,Some((result.header,root,transition)),true)
    }
}
...
mod prepared_completion_tests {
    ...
    fn prepared_allocation_plan_counts_original_banks_before_expanding_bound() {
        ...
        let expected_feedback=2*schema.feature_width*size_of::<f32>()+2*9+
            (2*42+2+3+3*SEMANTIC_FEEDBACK_SUPPORT_FIELDS.len())*size_of::<u64>()+6*4*size_of::<u64>();
        assert_eq!(feedback.bytes,expected_feedback);
        let input_bytes=PreparedStepInputs::allocation_bytes(&[]).unwrap();
        assert_eq!(input_bytes,size_of::<PublicationHeader>()+32*size_of::<SourceSlot>()+8*size_of::<u64>());
        let plans=[StepInputPlan { role:1,index:0,layout:SemanticTensorLayout::default(),storage_slot:0,allocation_bytes:1024,span:0..1024 },
            StepInputPlan { role:2,index:0,layout:SemanticTensorLayout::default(),storage_slot:1,allocation_bytes:32,span:0..32 }];
        assert_eq!(PreparedStepInputs::allocation_bytes(&plans).unwrap(),
            input_bytes+2*(size_of::<PublicationRange>()+size_of::<PublicationStepInput>())+32);
        assert!(PreparedStepInputs::allocation_bytes(&[StepInputPlan { span:0..usize::MAX,..plans[1].clone() }]).is_err());
        let step_bytes=prepared_step_allocation_bytes(input_bytes,feedback.bytes,0).unwrap();
        assert_eq!(step_bytes,(input_bytes+feedback.bytes+size_of::<PublicationLease>()+size_of::<PreparedStepResult>()) as u64);
        assert_eq!(prepared_segment_allocation_bytes(step_bytes,3).unwrap(),3*step_bytes);
    ...
    fn prepared_scope_blocks_ordinary_rebinding_until_retirement() {
        let issuer=Arc::new(());
        let mut build=PreparedSegmentState::new(Arc::clone(&issuer),vec![1],std::iter::once(SemanticTransitionKind::Proposal)).unwrap();
        ...
        build.enter(&step,&issuer).unwrap();build.leave(&step,&issuer).unwrap();build.finish().unwrap();
        build.submit().unwrap();build.completed=true;
        assert!(validate_rebinding_ownership(Some(&build),false).is_err());
    }
    ...
    fn completion_uses_original_parent_and_requires_device_release() {
        let parent=PublicationHeader { abi:1,publication_word:2,sealed_epoch:1,proposal:37,
            model_generation:9,family_id:3,stream_serial:91,..PublicationHeader::default() };
        let mut lease=PublicationLease { abi:1,word:2,epoch:1,transition_kind:1,..PublicationLease::default() };
        let mut result=PreparedStepResult { abi:1,word:5,advanced:1,header:PublicationHeader {
            abi:1,base_word:2,publication_word:5,sealed_epoch:2,proposal:38,..parent },..PreparedStepResult::default() };
        assert_eq!(validate_prepared_completion(&lease,&parent,&result,parent.instance).unwrap(),SemanticTransitionKind::Proposal);
        assert_eq!(parent.rng_binding().unwrap().proposal,37);
        lease.active=1;assert!(validate_prepared_completion(&lease,&parent,&result,parent.instance).is_err());
        lease.active=0;result.word=8;assert!(validate_prepared_completion(&lease,&parent,&result,parent.instance).is_err());
        result.word=5;lease.transition_kind=0;assert!(validate_prepared_completion(&lease,&parent,&result,parent.instance).is_err());
    }
}
    ...
    fn drop(&mut self) {
        ...
        if !self.readers.is_empty() || !self.steps.is_empty() {
            if let Some(publication) = self.publication.take() {
                ...
                let owners = UnreleasedPublicationOwners { _graph: graph, _publication: publication,
                    _readers: std::mem::take(&mut self.readers), _events: std::mem::take(&mut self.release_events),
                    _steps: std::mem::take(&mut self.steps), _text_binding: self.text_binding.take(),
                ...
                let _ = UNRELEASED_PUBLICATIONS.try_with(|quarantine| quarantine.borrow_mut().push(owners));
                return;
            }
        }
...
impl SemanticTransitionSession {
    ...
    pub fn prepare_segment_steps(&mut self, transitions: impl ExactSizeIterator<Item = SemanticTransitionKind> + Clone)
        -> Result<Vec<SemanticPreparedStep>, SemanticTransitionError> {
        self.ensure_rebindable()?;
        if transitions.len() == 0 || transitions.clone().any(|kind| kind == SemanticTransitionKind::Drain) {
            return Err(publication_input_error("a prepared segment requires frozen proposal or recompute steps"));
        }
        if self.prepared_segment.is_some() || !self.readers.is_empty() || self.captured.is_some()
            || self.admitted_transition.is_some() || self.text_binding.is_some() {
        ...
        let bound = u64::try_from(transition_bound).map_err(|_| SemanticTransitionError::GenerationExhausted)?;
        let end = self.next_reader.checked_add(bound).ok_or(SemanticTransitionError::GenerationExhausted)?;
        let schema = SemanticFeedbackSchema::new(self.feedback_statement_bytes()?)?;
        let descriptor = self.descriptor();
        let input_plans = PreparedStepInputs::plan(&storage)?;
        let feedback_plan = FeedbackAllocationPlan::new(&schema,
            storage.contract_value.feedback_capacity, descriptor.arena[4])?;
        ...
        let policy_bytes = policy_buffer_bytes(&self.policy_layout()?)?
            ...
            .ok_or(SemanticTransitionError::GenerationExhausted)?;
        let step_bytes = prepared_step_allocation_bytes(PreparedStepInputs::allocation_bytes(&input_plans)?,
            feedback_plan.bytes, policy_bytes)?;
        ...
        let mut reservation = self.provider.memory().reserve_bytes(bytes)
            .map_err(|error| runtime_error("prepared segment reservation", error))?;
        let mut tokens = Vec::new();
        tokens.try_reserve_exact(transition_bound).map_err(|error| runtime_error("prepared token reservation", error))?;
        tokens.extend((self.next_reader..end).map(|token| token + 1));
        let build = PreparedSegmentState::new(Arc::clone(&self.publication_issuer), tokens, transitions)?;
        let handles = build.handles()?;
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
            .map_err(|error| runtime_error("segment preparation stream admission", error))?;
        let kernel = |name| self.provider.device().inner().get_func("xlog_semantic_transition", name)
        ...
        let result = (|| {
            for handle in &handles {
                let reader = reservation.alloc(1).map_err(|error| runtime_error("prepared reader allocation", error))?;
                let result = reservation.alloc(1).map_err(|error| runtime_error("prepared result allocation", error))?;
                let inputs = Arc::new(PreparedStepInputs::allocate_reserved(&self.provider, Arc::clone(&storage),
                    reader.view(), input_plans.clone(), &mut reservation)?);
                let feedback = FeedbackBuffers::allocate_reserved(&self.provider, schema.clone(), feedback_plan, &mut reservation)?;
                #[cfg(feature = "semantic-policy")]
                let (policy_buffers, support, receipts, state) = self.prepare_step_policy_storage(&mut reservation)?;
                let mut step = StepContentStorage::new();
                step.inputs = Some(Arc::clone(&inputs));
                step.feedback.push(feedback);
                step.prepared = Some(PreparedStepStorage { reader, digests: None, next_digest: 0, model_work: None,
                    admit: admit.clone(), release: release.clone(), witness: witness.clone(), content_guard: content_guard.clone(), inputs_recorded: false,
                    continuation: None, transition_recorded: false, observed: false,
                    result, result_kernel: result_kernel.clone(),
                    #[cfg(feature = "semantic-policy")]
                ...
                let prepared = self.steps[&handle.token].prepared.as_ref().expect("installed fixed step");
                // Only immutable catalogue identity is initialized on the host;
                // actual invocation coordinates come from device admission.
                #[cfg(feature = "semantic-policy")]
                upload_publication(&self.provider, &[DeviceState {
                    catalogue_generation: self.binding().generation,
                    catalogue_digest: Identity256::from_bytes(CATALOGUE_DIGEST),
                    binding_digest: self.binding().digest,
                    ..DeviceState::default()
                }], &prepared.state)?;
                upload_publication(&self.provider, &[PublicationLease::default()], &prepared.reader)?;
                upload_publication(&self.provider, &[PreparedStepResult::default()], &prepared.result)?;
                inputs.initialize(&self.provider)?;
            }
            if reservation.remaining_bytes() != 0 { return Err(SemanticTransitionError::ObservationMismatch); }
            Ok(handles)
        })();
        if result.is_err() { self.poisoned = true; }
        result
    ...
    fn checked_prepared_step(&self, step: &SemanticPreparedStep, recording: bool)
        -> Result<&StepContentStorage, SemanticTransitionError> {
        self.ensure_quiescent()?;
        let build = self.prepared_segment.as_ref().ok_or_else(|| publication_input_error("Session has no prepared segment"))?;
        if recording { build.check(step, &self.publication_issuer, true)?; }
        else { build.check_retained(step, &self.publication_issuer)?; }
        self.steps.get(&step.token).filter(|owner| owner.prepared.is_some() && owner.identity.is_none())
            .ok_or_else(|| publication_input_error("prepared step has no original native storage owner"))
    }
    ...
    pub fn prepared_transition_kind(&self, step: &SemanticPreparedStep) -> Result<SemanticTransitionKind, SemanticTransitionError> {
        ...
        let build = self.prepared_segment.as_ref().expect("checked prepared scope");
        build.check(step, &self.publication_issuer, false)?;
        build.requested_kind(step, &self.publication_issuer)
    }

    fn check_prepared_cold(&self, step: &SemanticPreparedStep) -> Result<(), SemanticTransitionError> {
        self.checked_prepared_step(step, false)?;
        if self.prepared_segment.as_ref().expect("checked build").capturing {
            return Err(publication_input_error("fixed resources must be prepared before capture begins"));
        }
        Ok(())
    }
    ...
    pub fn reserve_tensor_content(&mut self, step: &SemanticPreparedStep, tensor_capacity: usize)
        -> Result<(), SemanticTransitionError> {
        self.check_prepared_cold(step)?;
        if tensor_capacity == 0 { return Err(publication_input_error("content reservation requires a positive original tensor capacity")); }
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
            .map_err(|error| runtime_error("content reservation stream admission", error))?;
        if self.steps[&step.token].prepared.as_ref().expect("prepared owner").digests.is_some() {
            return Err(publication_input_error("original content capacity is reserved once per fixed step"));
        }
        let cells = tensor_capacity.checked_mul(4).ok_or(SemanticTransitionError::GenerationExhausted)?;
        let digest = Arc::new(allocate_publication(&self.provider, cells)?);
        self.steps.get_mut(&step.token).expect("checked fixed step").prepared.as_mut()
            .expect("checked prepared owner").digests = Some(digest);
        Ok(())
    }
    ...
    pub fn reserve_model_work(&mut self, step: &SemanticPreparedStep, event_capacity: usize)
        -> Result<(), SemanticTransitionError> {
        self.check_prepared_cold(step)?;
        if self.steps[&step.token].prepared.as_ref().expect("prepared owner").model_work.is_some() {
            return Err(publication_input_error("original model work capacity is reserved once per step"));
        }
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
            .map_err(|error| runtime_error("model work reservation stream admission", error))?;
        let bytes = event_capacity.checked_mul(size_of::<ModelWorkEvent>())
            .ok_or(SemanticTransitionError::GenerationExhausted)?;
        ...
        let device = reservation.alloc(event_capacity)
            .map_err(|error| runtime_error("original model work allocation", error))?;
        self.steps.get_mut(&step.token).expect("checked original step").prepared.as_mut()
            .expect("prepared owner").model_work = Some(PreparedModelWork { recording, device });
        Ok(())
    }
    ...
    pub fn record_model_work(&mut self, step: &SemanticPreparedStep, kind: ModelWorkKind,
        dimensions: &[u64]) -> Result<(), SemanticTransitionError> {
        self.check_prepared_content_stream(step, self.stream.cu_stream() as u64)?;
        let result = (|| {
            let event = ModelWorkEvent::operation(kind, dimensions).map_err(publication_input_error)?;
            self.steps.get_mut(&step.token).expect("checked original step").prepared.as_mut()
                .expect("prepared owner").model_work.as_mut()
                .ok_or_else(|| publication_input_error("model work requires its original cold reservation"))?
    ...
    pub fn record_saved_tensor_work(&mut self, step: &SemanticPreparedStep,
        witness: &SemanticTensorContentWitness) -> Result<(), SemanticTransitionError> {
        self.check_prepared_content_stream(step, self.stream.cu_stream() as u64)?;
        let result = (|| {
            let owner = self.steps.get_mut(&step.token).expect("checked original step");
            if !Arc::ptr_eq(&witness.issuer, &self.publication_issuer)
                || witness.reader_token != step.token || !Arc::ptr_eq(&witness._witness, &owner.content_witnesses) {
                return Err(publication_input_error("saved work requires this original step and tensor witness"));
            }
            let content = owner.content.get(witness.index)
                .ok_or_else(|| publication_input_error("saved work has no retained original tensor occurrence"))?;
            let recording = &mut owner.prepared.as_mut().expect("prepared owner").model_work.as_mut()
                .ok_or_else(|| publication_input_error("saved work requires its original cold reservation"))?.recording;
            for (index, tensor) in content.tensors.iter().enumerate() {
                let layout = &tensor.layout;
                let dimensions = layout.dimensions.get(..layout.rank as usize)
                    .ok_or(SemanticTransitionError::ObservationMismatch)?;
                let event = ModelWorkEvent::saved(witness.index as u64, index as u64, dimensions, layout.element_bytes)
    ...
    fn export_prepared_view(&mut self, step: &SemanticPreparedStep, view: DeviceMemoryView<u8>,
        shape: Vec<i64>, strides: Vec<i64>, dtype: (u8, u8), consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
        self.check_prepared_cold(step)?;
        let guard = Arc::clone(&self.steps[&step.token].aliases);
        self.steps.get_mut(&step.token).expect("checked fixed step").consumer_streams.insert(consumer_stream);
        self.export_owned_view(view, shape, strides, dtype, guard, consumer_stream)
    }

    pub fn prepared_source(&mut self, step: &SemanticPreparedStep, consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
        self.check_prepared_cold(step)?;
        let (_, shape, strides) = PublicationBankField::Source.layout();
        let view = self.steps[&step.token].inputs.as_ref().expect("fixed input owner").field_view(PublicationBankField::Source)?;
        self.export_prepared_view(step, view, shape, strides, (1, 64), consumer_stream)
    }

    pub fn prepared_prefix(&mut self, step: &SemanticPreparedStep, consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
        self.prepared_tensor(step, SemanticStateRole::PrefixSource, 0, consumer_stream)
    }

    pub fn prepared_record(&mut self, step: &SemanticPreparedStep, role: SemanticStateRole, index: u64,
        consumer_stream: u64) -> Result<DlpackManagedTensor, SemanticTransitionError> {
        self.check_prepared_cold(step)?;
        if is_tensor_role(role as u64) { return Err(publication_input_error("tensor roles require their original model layout")); }
        if role == SemanticStateRole::PrefixSource {
            return Err(publication_input_error("prepared prefixes require the fixed typed capacity and original device extent"));
        }
        let storage = self.publication.as_ref().expect("prepared publication");
        let key = (role as u64, index);
        let inputs = self.steps[&step.token].inputs.as_ref().expect("fixed inputs");
        let view = if let Some(view) = inputs.views.get(&key) { view.clone() } else {
            let original = storage.bank_templates[0].iter().find(|range| (range.role, range.index) == key)
                .ok_or_else(|| publication_input_error("record has no original fixed declaration"))?;
            let other = storage.bank_templates[1].iter().find(|range| (range.role, range.index) == key)
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            if publication_mutable_role(role as u64) || original.storage_slot != other.storage_slot
                || original.offset_bytes != other.offset_bytes || original.length_bytes != other.length_bytes {
                return Err(publication_input_error("bank-varying records require a private prepared input producer"));
            }
            let begin = original.offset_bytes as usize;
            let end = begin.checked_add(original.length_bytes as usize).ok_or(SemanticTransitionError::ObservationMismatch)?;
            storage.allocations[original.storage_slot as usize].view().slice(begin..end)
        };
        let bytes = i64::try_from(view.len()).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
        self.export_prepared_view(step, view, vec![bytes], vec![1], (1, 8), consumer_stream)
    }

    pub fn prepared_range_keys(&self, step: &SemanticPreparedStep)
        -> Result<Vec<(SemanticStateRole, u64)>, SemanticTransitionError> {
        self.checked_prepared_step(step, false)?;
        self.publication.as_ref().expect("prepared publication").bank_templates[0].iter()
            .map(|range| SemanticStateRole::from_code(range.role).map(|role| (role, range.index))
                .ok_or(SemanticTransitionError::ObservationMismatch)).collect()
    }

    /// Export the fixed original capacity; live extents remain device inputs.
    pub fn prepared_tensor(&mut self, step: &SemanticPreparedStep, role: SemanticStateRole, index: u64,
        consumer_stream: u64) -> Result<DlpackManagedTensor, SemanticTransitionError> {
        let (view, layout) = self.prepared_tensor_view(step, role, index)?;
        let shape = layout.dimensions[..layout.rank as usize].iter().map(|&value| i64::try_from(value)
            .map_err(|_| SemanticTransitionError::ObservationMismatch)).collect::<Result<Vec<_>, _>>()?;
        let strides = layout.strides_bytes[..layout.rank as usize].iter().map(|&value| i64::try_from(value / layout.element_bytes)
            .map_err(|_| SemanticTransitionError::ObservationMismatch)).collect::<Result<Vec<_>, _>>()?;
        let dtype = match layout.scalar_type { 1 => (1, 8), 2 => (1, 32), 3 => (1, 64), 4 => (2, 16),
            5 => (4, 16), 6 => (2, 32), 7 => (0, 64), 8 => (6, 8), _ => return Err(SemanticTransitionError::ObservationMismatch) };
    ...
    pub fn prepared_tensor_allocation(&mut self, step: &SemanticPreparedStep,
        role: SemanticStateRole, index: u64, consumer_stream: u64)
        ...
        let provenance = view.allocation_provenance()
            .ok_or_else(|| publication_input_error("prepared tensor has no complete native allocation owner"))?;
        let allocation = view.allocation_view().ok_or(SemanticTransitionError::ObservationMismatch)?;
        let bytes = i64::try_from(allocation.len()).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
        let tensor = self.export_prepared_view(step, allocation, vec![bytes], vec![1], (1, 8), consumer_stream)?;
        Ok((provenance, tensor))
    }

    fn prepared_tensor_view(&self, step: &SemanticPreparedStep, role: SemanticStateRole, index: u64)
        -> Result<(DeviceMemoryView<u8>, SemanticTensorLayout), SemanticTransitionError> {
        self.check_prepared_cold(step)?;
        let inputs = self.steps[&step.token].inputs.as_ref().expect("fixed input owner");
        let plan = inputs.plans.iter().find(|plan| (plan.role, plan.index) == (role as u64, index))
            .ok_or_else(|| publication_input_error("prepared tensor has no fixed original input owner"))?;
        Ok((inputs.views[&(role as u64, index)].clone(), plan.layout))
    }

    pub fn prepared_terminal(&mut self, step: &SemanticPreparedStep, consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
        self.prepared_bank_field(step, PublicationBankField::Terminal, consumer_stream)
    }

    pub fn prepared_prefix_extent(&mut self, step: &SemanticPreparedStep, consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
        self.prepared_bank_field(step, PublicationBankField::PrefixExtent, consumer_stream)
    }

    pub fn prepared_ring_head(&mut self, step: &SemanticPreparedStep, consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
        self.prepared_bank_field(step, PublicationBankField::RingHead, consumer_stream)
    }

    fn prepared_bank_field(&mut self, step: &SemanticPreparedStep, field: PublicationBankField, consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
        self.check_prepared_cold(step)?;
        let inputs = self.steps[&step.token].inputs.as_ref().expect("prepared input owner");
        let (_, shape, strides) = field.layout();
        self.export_prepared_view(step, inputs.field_view(field)?, shape, strides, (1, 64), consumer_stream)
    }

    /// Immutable admitted model geometry; no future publication values occur.
    pub fn prepared_model_geometry(&self, step: &SemanticPreparedStep)
        -> Result<(Identity256, Identity256, u64, u64, u64, u64), SemanticTransitionError> {
        self.checked_prepared_step(step, false)?;
        let contract = self.publication.as_ref().expect("prepared publication").contract_value;
        Ok((contract.topology_identity, contract.table_identity, contract.model_generation,
            contract.prefix_capacity, contract.feedback_capacity, contract.max_position))
    }
    ...
    pub fn prepared_model_binding(&mut self, step: &SemanticPreparedStep,
        consumer_streams: &[u64])
        -> Result<(SemanticPublishedIdentity, Option<SemanticPublishedIdentity>, u64,
            Identity256, Identity256), SemanticTransitionError> {
        let owner = self.checked_prepared_step(step, false)?;
        let prepared = owner.prepared.as_ref().expect("checked prepared owner");
        if !self.prepared_segment.as_ref().expect("checked prepared scope").completed
            || !prepared.observed {
            return Err(publication_input_error("model input observation requires this step's known completed execution"));
        }
        let inputs = Arc::clone(owner.inputs.as_ref().ok_or(SemanticTransitionError::ObservationMismatch)?);
        let reader = prepared.reader.view();
        let result = prepared.result.view();
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
            .map_err(|error| runtime_error("completed model observation stream admission", error))?;
        ...
        if validate_prepared_completion(&reader, &parent, &result, inputs.storage.instance).is_err()
            || result.refusal != 0 || (result.advanced == 0 && result.header != parent) {
            self.poisoned = true;
            return Err(SemanticTransitionError::ObservationMismatch);
        }
        let identity = |header: PublicationHeader| SemanticPublishedIdentity {
            instance: header.instance, word: header.publication_word,
            logical_digest: header.logical_digest, state_digest: header.state_digest,
        };
        Ok((identity(parent), (result.advanced == 1).then(|| identity(result.header)),
            parent.model_generation, parent.model_geometry_digest, parent.model_numerical_digest))
    }

    pub fn prepared_feedback(&mut self, step: &SemanticPreparedStep, consumer_stream: u64)
        ...
        let layouts = feedback.content.tensors.iter().map(|tensor| (tensor.source.as_ref().expect("feedback owner").clone(), tensor.layout))
            .collect::<Vec<_>>();
        let mut outputs = Vec::with_capacity(6);
        for (view, layout) in layouts {
            let shape = layout.dimensions[..layout.rank as usize].iter().map(|&value| value as i64).collect();
            let strides = layout.strides_bytes[..layout.rank as usize].iter().map(|&value| (value / layout.element_bytes) as i64).collect();
            let dtype = match layout.scalar_type { 3 => (1, 64), 6 => (2, 32), 8 => (6, 8), _ => return Err(SemanticTransitionError::ObservationMismatch) };
            outputs.push(self.export_prepared_view(step, view, shape, strides, dtype, consumer_stream)?);
        }
        Ok((schema, outputs.try_into().map_err(|_| SemanticTransitionError::ObservationMismatch)?))
    }
    ...
    pub fn prepared_feedback_records(&mut self, step: &SemanticPreparedStep, consumer_streams: &[u64])
        -> Result<(SemanticPublishedIdentity, Option<SemanticPublishedIdentity>, Vec<SemanticFeedbackRecordMaterial>), SemanticTransitionError> {
        let binding = self.prepared_model_binding(step, consumer_streams)?;
        let inputs = Arc::clone(self.checked_prepared_step(step, false)?.inputs.as_ref()
            .ok_or(SemanticTransitionError::ObservationMismatch)?);
        let ranges = self.publication_read(inputs.ranges.view())?;
        let mut records = Vec::new();
        for range in ranges.into_iter().filter(|range| matches!(range.role, 15..=17)) {
            let original = inputs.storage.allocations.get(usize::try_from(range.storage_slot)
                .map_err(|_| SemanticTransitionError::ObservationMismatch)?)
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            let capacity = usize::try_from(range.offset_bytes).ok().and_then(|begin| original.len().checked_sub(begin))
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            let view = inputs.views.get(&(range.role, range.index)).ok_or(SemanticTransitionError::ObservationMismatch)?;
            let bytes = self.publication_read(view.clone())?;
            match (PublicationMaterialRange { range, capacity, bytes }).into_feedback_record() {
    ...
    pub fn begin_prepared_segment(&mut self, resources: Vec<Arc<dyn Send + Sync>>)
        ...
        if build.capturing || build.finished { return Err(publication_input_error("prepared segment construction already began")); }
        ...
        let builder = crate::cuda_graph::ConditionalCudaGraphSequenceBuilder::new_retaining(&self.stream, resources)
            .map_err(|error| runtime_error("bounded segment capture", error))?;
        let scope = Arc::clone(&build.scope);
        self.prepared_segment.as_mut().expect("checked build").capturing = true;
        self.graph.enter_transition();
        Ok((SemanticPreparedSegmentCapture { builder, scope }, Arc::clone(&self.stream)))
    }

    fn check_prepared_content_stream(&self, step: &SemanticPreparedStep, consumer_stream: u64)
        -> Result<(), SemanticTransitionError> {
        self.checked_prepared_step(step, true)?;
        if dlpack_consumer_stream(consumer_stream)? != self.stream.cu_stream() {
            return Err(publication_input_error("recorded content must use its original native capture stream"));
        }
        if !self.steps[&step.token].prepared.as_ref().expect("prepared owner").inputs_recorded {
            return Err(publication_input_error("original step inputs must precede recorded content"));
        }
        Ok(())
    }
    ...
    pub fn bind_prepared_model_content(&mut self, step: &SemanticPreparedStep,
        tensors: Vec<SemanticTensorInput>, consumer_stream: u64)
        ...
        let indices = model_content_ranges(directory, &storage.layouts,
            original.iter().map(|tensor| (tensor.layout, tensor.logical_begin, tensor.logical_end,
                tensor.source.as_ref().map_or(0, DeviceMemoryView::len))))?;
        let plan = model_content_copy_plan(directory, &indices,
            &storage.allocations.iter().map(TrackedCudaSlice::len).collect::<Vec<_>>())?;
        let mut seals = ModelContentSeals::allocate(&self.provider, &plan)?;
        let values = original.iter().flat_map(|tensor| [tensor.layout.role, tensor.layout.index]).collect::<Vec<_>>();
        let roster = allocate_publication(&self.provider, values.len())?;
        let execute = self.provider.device().inner().get_func("xlog_semantic_transition", "semantic_publication_model_seals")
            .ok_or_else(|| runtime_error("kernel lookup", "original model seal snapshot unavailable"))?;
        let reader = self.steps[&step.token].prepared.as_ref().expect("prepared lease").reader.view();
        seals.prepared = Some(PreparedModelContentSeals { storage, reader, roster, execute });
        ...
        owner.content.push(TensorContentBuffers { tensors: original, seals: TensorContentSeals::Model(seals),
            verification_inputs: Vec::new() });
        let handle = SemanticTensorContentWitness { issuer: Arc::clone(&self.publication_issuer), reader_token: step.token,
            index, _witness: Arc::clone(&owner.content_witnesses) };
        let TensorContentSeals::Model(seals) = &owner.content[index].seals else { unreachable!("installed original model seals") };
        if let Err(error) = upload_publication(&self.provider, &values, &seals.prepared.as_ref().expect("installed original roster").roster) {
            self.poisoned = true; return Err(error);
        }
        Ok(handle)
    }
    ...
    pub fn capture_prepared_tensor_content(&mut self, step: &SemanticPreparedStep,
        tensors: Vec<SemanticTensorInput>, consumer_stream: u64)
        -> Result<SemanticTensorContentWitness, SemanticTransitionError> {
        self.check_prepared_content_stream(step, consumer_stream)?;
        if tensors.is_empty() { return Err(publication_input_error("original content requires an actual tensor roster")); }
        let original = prepare_semantic_tensors(&self.provider, tensors)?;
        ...
        let owner = self.steps.get_mut(&step.token).expect("checked fixed step");
        let index = owner.content.len();
        owner.content.push(TensorContentBuffers { tensors: original,
            seals: TensorContentSeals::Captured(Vec::new()), verification_inputs: Vec::new() });
        let result = (|| {
            let owner = &self.steps[&step.token];
            let original = &owner.content[index].tensors;
            validate_content_coordinates(original.iter().map(|tensor| (tensor.layout.role, tensor.layout.index)))?;
            ...
            for tensor in original {
                ...
                if let Some(native) = native { digests.push(native); continue; }
                for (&token, other) in &self.steps {
                    if token == step.token { continue; }
                    let other_input = match &other.inputs {
                        Some(inputs) => inputs.owns_tensor(tensor, &inputs.storage.bank_templates[0])?,
                        None => false,
                    };
                    if other.feedback_origin_digest(tensor)?.is_some()
                        || other_input {
                        return Err(publication_input_error("original content belongs to another native prepared step"));
                    }
                }
                ...
                tensor_content_range(&tensor.layout, tensor.logical_begin, tensor.logical_end,
                    tensor.source.as_ref().map_or(0, DeviceMemoryView::len))?;
                let cells = prepared.digests.as_ref().filter(|cells| next < cells.len() / 4).ok_or_else(||
                    publication_input_error("original tensor roster exceeds its cold native content reservation"))?;
                let offset = next * 4;
                next += 1;
                digests.push(CapturedTensorDigest::Tensor { cells: Arc::clone(cells), offset, producer_sealed: false });
            }
            let owner = self.steps.get_mut(&step.token).expect("checked fixed step");
            owner.prepared.as_mut().expect("prepared content owner").next_digest = next;
    ...
    fn checked_prepared_content_witness(&self, step: &SemanticPreparedStep, witness: &SemanticTensorContentWitness)
        -> Result<(), SemanticTransitionError> {
        let owner = self.checked_prepared_step(step, false)?;
        if witness.reader_token != step.token || !Arc::ptr_eq(&witness.issuer, &self.publication_issuer)
            || !Arc::ptr_eq(&witness._witness, &owner.content_witnesses) || witness.index >= owner.content.len() {
            return Err(publication_input_error("original content witness belongs to another native prepared step"));
        }
        Ok(())
    }

    pub fn record_prepared_tensor_content(&mut self, step: &SemanticPreparedStep,
        witness: &SemanticTensorContentWitness, verify: bool) -> Result<(), SemanticTransitionError> {
        self.checked_prepared_step(step, true)?;
        self.checked_prepared_content_witness(step, witness)?;
        let owner = &self.steps[&step.token];
        let execute = &owner.prepared.as_ref().expect("prepared content owner").witness;
    ...
    pub fn bind_prepared_continuation(&mut self, step: &SemanticPreparedStep,
        continuation: SemanticContinuationInput, witness: &SemanticTensorContentWitness, consumer_stream: u64)
        -> Result<(), SemanticTransitionError> {
        self.check_prepared_content_stream(step, consumer_stream)?;
        self.checked_prepared_content_witness(step, witness)?;
        if self.steps[&step.token].prepared.as_ref().expect("prepared owner").continuation.is_some() {
            return Err(publication_input_error("original prepared continuation binds once"));
        }
        if !matches!(self.steps[&step.token].content[witness.index].seals, TensorContentSeals::Captured(_)) {
            return Err(publication_input_error("continuation requires its original transient producer witness"));
        }
        let storage = Arc::clone(self.publication.as_ref().expect("prepared publication"));
        let tensor_count = continuation.tensors.len();
        let capacity = storage.contract_value.feedback_capacity.checked_add(32)
            .ok_or(SemanticTransitionError::GenerationExhausted)?;
        let service_layouts = continuation_service_layouts(tensor_count, capacity)?;
        let SemanticContinuationInput { mut tensors, authority_decisions, text_rows, text_row_count,
            selected_text, active_rows, active_row_count, numerical_admissibility } = continuation;
        if authority_decisions.is_empty() { return Err(publication_input_error("continuation requires its original authority snapshot geometry")); }
        tensors.extend([text_rows, text_row_count, selected_text, active_rows, active_row_count, numerical_admissibility]
            .into_iter().zip(service_layouts).map(|(tensor, layout)| SemanticTensorInput {
                tensor, layout, logical_begin: 0, logical_end: 0, native_allocation: None,
            }));
        self.verify_prepared_tensor_content(step, witness, tensors, consumer_stream)?;
        let owner = &self.steps[&step.token];
        let original = &owner.content[witness.index].tensors;
        let services: [PreparedSemanticTensor; 6] = original[tensor_count..].to_vec().try_into()
            .map_err(|_| publication_input_error("continuation witness has the wrong original service roster"))?;
        let binding = Arc::new(TextBindingStorage { inputs: services, _witness: witness.clone(), consumer_stream,
            parent: TextBindingParent::Prepared(Arc::clone(owner.inputs.as_ref().expect("fixed inputs"))) });
        let (uploads, layouts) = prepare_continuation_payloads(&storage, &original[..tensor_count], &authority_decisions)?;
        let continuation = PreparedContinuation::prepare(&self.provider, storage,
            owner.prepared.as_ref().expect("prepared lease").reader.view(), Arc::clone(&binding),
            binding.continuation_inputs(self.prepared_segment.as_ref().expect("prepared scope")
                .requested_kind(step, &self.publication_issuer)?, authority_decisions.len() as u64),
            &uploads, &layouts)?;
        self.steps.get_mut(&step.token).expect("checked prepared step").prepared.as_mut().expect("prepared owner")
            .continuation = Some(continuation);
        Ok(())
    }

    pub fn verify_prepared_tensor_content(&mut self, step: &SemanticPreparedStep,
        witness: &SemanticTensorContentWitness, tensors: Vec<SemanticTensorInput>, consumer_stream: u64)
        -> Result<(), SemanticTransitionError> {
        self.checked_prepared_content_witness(step, witness)?;
        let build = self.prepared_segment.as_ref().expect("checked prepared scope");
        let recorded = !build.finished;
        if recorded { self.check_prepared_content_stream(step, consumer_stream)?; }
        else if !build.completed || !self.steps[&step.token].prepared.as_ref().expect("prepared owner").observed {
            return Err(publication_input_error("retained content requires an actually completed prepared invocation"));
        }
        let actual = prepare_semantic_tensors(&self.provider, tensors)?;
        let content = &mut self.steps.get_mut(&step.token).expect("checked fixed step").content[witness.index];
        ...
        if content.tensors.len() != actual.len() || content.tensors.iter().zip(actual)
        ...
        if recorded { self.record_prepared_tensor_content(step, witness, true) }
        else { self.enqueue_content_witness(witness, consumer_stream, true) }
    }

    pub fn record_prepared_step_admission(&mut self, step: &SemanticPreparedStep, conditional_handle: u64)
        -> Result<(), SemanticTransitionError> {
        self.checked_prepared_step(step, false)?;
        if conditional_handle == 0 || !self.prepared_segment.as_ref().expect("checked build").capturing {
            return Err(publication_input_error("step admission requires the actual conditional capture handle"));
        }
        let requested_kind = self.prepared_segment.as_mut().expect("checked build").enter(step, &self.publication_issuer)?;
        let storage = self.publication.as_ref().expect("prepared publication");
        let prepared = self.steps[&step.token].prepared.as_ref().expect("prepared lease owner");
        let build = self.prepared_segment.as_ref().expect("checked prepared scope");
        let previous = build.next.checked_sub(1).map(|index|
            &self.steps[&build.tokens[index]].prepared.as_ref().expect("previous prepared step").result);
        let mut recorder = self.domain.new_strict_recorder();
        storage.record(&mut recorder); recorder.write(&prepared.reader);
        if let Some(previous) = previous { recorder.read(previous); }
        let arguments = (storage.control.device_ptr_value(), prepared.reader.device_ptr_value(), requested_kind.code(), conditional_handle,
        ...
        enqueue_recorded(&self.domain, &mut self.poisoned, recorder, |enqueue| {
            ...
            unsafe { prepared.admit.clone().launch_in(enqueue, LaunchConfig { grid_dim: (1, 1, 1),
                block_dim: (1, 1, 1), shared_mem_bytes: 0 }, arguments) }
                .map_err(|error| XlogError::Kernel(error.to_string()))
        })
    }
    ...
    pub fn record_prepared_step_inputs(&mut self, step: &SemanticPreparedStep) -> Result<(), SemanticTransitionError> {
        self.checked_prepared_step(step, true)?;
        if self.steps[&step.token].prepared.as_ref().expect("prepared owner").inputs_recorded {
            return Err(publication_input_error("prepared input producer records exactly once"));
        }
        ...
        let owner = &self.steps[&step.token];
        let descriptor = self.descriptor();
        owner.inputs.as_ref().expect("fixed input owner").enqueue(&self.domain, &mut self.poisoned)?;
        owner.feedback[0].enqueue(&self.domain, &mut self.poisoned, &self.graph,
            self.publication.as_ref().expect("prepared publication"),
            &self.task.as_ref().ok_or(SemanticTransitionError::InvalidTaskBank)?.1,
            &owner.prepared.as_ref().expect("prepared lease").reader, descriptor)?;
        for content in &owner.content {
            if let TensorContentSeals::Model(seals) = &content.seals {
                ...
                content.enqueue(&self.domain, &mut self.poisoned,
                    &owner.prepared.as_ref().expect("prepared witness producer").witness, true)?;
            }
        }
        self.steps.get_mut(&step.token).expect("checked step").prepared.as_mut().expect("prepared owner").inputs_recorded = true;
        Ok(())
    }

    pub fn guard_prepared_content(&mut self, step: &SemanticPreparedStep,
        keys: &[(SemanticStateRole, u64)], consumer_stream: u64) -> Result<(), SemanticTransitionError> {
        self.checked_prepared_step(step, true)?;
        if dlpack_consumer_stream(consumer_stream)? != self.stream.cu_stream() || keys.is_empty() {
            return Err(publication_input_error("prepared content guard requires original ranges on the native capture stream"));
        }
        let storage = self.publication.as_ref().expect("prepared publication");
        let owner = &self.steps[&step.token];
        ...
        for &(role, index) in keys {
            if !distinct.insert((role as u64, index))
                || !storage.bank_templates[0].iter().any(|range| (range.role, range.index) == (role as u64, index)) {
                return Err(publication_input_error("prepared guard names duplicate or undeclared original ranges"));
            }
            let layout = if is_tensor_role(role as u64) { *storage.layouts.get(&(role as u64, index))
                .ok_or(SemanticTransitionError::ObservationMismatch)? } else { SemanticTensorLayout::default() };
            storage.enqueue_content_guard(&self.domain, &mut self.poisoned, prepared.reader.view(),
                &prepared.content_guard, role as u64, index, layout)?;
        }
        if prepared.inputs_recorded {
            owner.inputs.as_ref().expect("fixed input owner").verify(&self.domain, &mut self.poisoned)?;
            owner.feedback[0].content.enqueue(&self.domain, &mut self.poisoned, &prepared.witness, true)?;
        }
        Ok(())
    }
    ...
    pub fn record_prepared_step_release(&mut self, step: &SemanticPreparedStep) -> Result<(), SemanticTransitionError> {
        self.checked_prepared_step(step, true)?;
        let storage = self.publication.as_ref().expect("prepared publication");
        let prepared = self.steps[&step.token].prepared.as_ref().expect("prepared lease");
        if !prepared.transition_recorded {
            return Err(publication_input_error("release must follow this prepared step's complete native transition"));
        }
        let mut recorder = self.domain.new_strict_recorder();
        storage.record(&mut recorder); recorder.read_write(&prepared.reader); recorder.write(&prepared.result);
        let arguments = (storage.control.device_ptr_value(), prepared.reader.device_ptr_value());
        enqueue_recorded(&self.domain, &mut self.poisoned, recorder, |enqueue| {
            // SAFETY: save the actual successor or refusal while the exact
            // acquired parent and current bank still belong to this step.
            unsafe { prepared.result_kernel.clone().launch_in(enqueue, LaunchConfig { grid_dim: (1, 1, 1),
                block_dim: (1, 1, 1), shared_mem_bytes: 0 },
                (storage.control.device_ptr_value(), prepared.reader.device_ptr_value(), prepared.result.device_ptr_value())) }
                .map_err(|error| XlogError::Kernel(error.to_string()))?;
            ...
            unsafe { prepared.release.clone().launch_in(enqueue, LaunchConfig { grid_dim: (1, 1, 1),
                block_dim: (1, 1, 1), shared_mem_bytes: 0 }, arguments) }
                .map_err(|error| XlogError::Kernel(error.to_string()))
        })?;
        self.prepared_segment.as_mut().expect("checked build").leave(step, &self.publication_issuer)
    }

    pub fn finish_prepared_segment(&mut self, executable: &mut Option<SemanticPreparedExecutable>)
        -> Result<(), SemanticTransitionError> {
        self.ensure_quiescent()?;
        let build = self.prepared_segment.as_mut().ok_or(SemanticTransitionError::NotBound)?;
        if self.captured.is_some() || executable.as_ref().is_none_or(|executable| !Arc::ptr_eq(&executable.scope, &build.scope)) {
        ...
        for token in &build.tokens {
            let work = self.steps[token].prepared.as_ref().expect("original prepared owner").model_work.as_ref()
                .ok_or_else(|| publication_input_error("prepared body has no frozen original model work"))?;
            if work.recording.frozen_bound().is_none() {
                return Err(publication_input_error("original model work was not frozen by its transition"));
            }
            let mut destination = work.device.view().slice(..work.recording.events().len());
            self.provider.htod_launch_metadata_sync_copy_into(work.recording.events(), &mut destination)
        ...
        self.captured = Some(executable.take().expect("checked authentic executable").graph);
        Ok(())
    }
    ...
    pub fn take_prepared_executable_for_retirement(&mut self)
        -> Result<Option<CapturedCudaGraph>, SemanticTransitionError> {
        if self.prepared_segment.as_ref().is_none_or(|build| !build.completed) {
            return Err(publication_input_error("prepared executable retirement requires known actual completion"));
        }
    ...
    pub fn take_prepared_resources_for_retirement(&mut self)
        -> Result<Vec<Arc<dyn Send + Sync>>, SemanticTransitionError> {
        let build = self.prepared_segment.as_ref().ok_or(SemanticTransitionError::NotBound)?;
        if !build.completed || self.captured.is_some() || build.tokens.iter().any(|token| self.steps.contains_key(token)) {
            return Err(publication_input_error("original prepared resources still own an executable or retained step"));
        }
    ...
    fn prepared_segment_recorder(&self) -> Result<LaunchRecorder, SemanticTransitionError> {
        ...
        self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?.record(&mut recorder);
        for token in &build.tokens {
            let owner = self.steps.get(token).ok_or(SemanticTransitionError::ObservationMismatch)?;
            let prepared = owner.prepared.as_ref().ok_or(SemanticTransitionError::ObservationMismatch)?;
            recorder.read_write(&prepared.reader); recorder.write(&prepared.result);
            if let Some(work) = &prepared.model_work { recorder.read(&work.device); }
            if let Some(digests) = &prepared.digests { recorder.read_write(digests.as_ref()); }
            let inputs = owner.inputs.as_ref().ok_or(SemanticTransitionError::ObservationMismatch)?;
            recorder.read(&inputs.bindings); recorder.write(&inputs.header); recorder.write(&inputs.source);
            recorder.write(&inputs.ranges); recorder.write(&inputs.metadata_digests);
            for allocation in &inputs.private { recorder.write(allocation); }
            for view in inputs.views.values() { recorder.read_write(view); }
            for feedback in &owner.feedback { recorder.write(&feedback.status); }
            for content in owner.content.iter().chain(owner.feedback.iter().map(|feedback| &feedback.content)) {
                for tensor in content.tensors.iter().chain(&content.verification_inputs) {
                    if let Some(source) = &tensor.source { recorder.read_write(source); }
                }
                match &content.seals {
                    TensorContentSeals::Captured(digests) => for digest in digests {
                        if let CapturedTensorDigest::Tensor { cells, .. } = digest { recorder.read_write(cells.as_ref()); }
                    },
                    TensorContentSeals::Model(model) => {
                        recorder.read_write(&model.ranges); recorder.read_write(&model.contract);
                        if let Some(source) = &model.prepared { recorder.read(&source.roster); recorder.read(&source.reader); }
                    }
                }
            }
            if let Some(continuation) = &prepared.continuation {
                continuation.text_binding.record(&mut recorder);
                for tensor in &continuation.sources { if let Some(source) = &tensor.source { recorder.read(source); } }
            }
            #[cfg(feature = "semantic-policy")]
            {
                recorder.write(&prepared.state);
                recorder.write(prepared.support.as_ref().ok_or(SemanticTransitionError::ObservationMismatch)?);
                recorder.write(prepared.receipts.as_ref().ok_or(SemanticTransitionError::ObservationMismatch)?);
                if let Some(policy) = &prepared.policy {
                    for buffer in [&policy.parameters, &policy.hidden, &policy.scores, &policy.recurrent, &policy.text_logits] {
                        recorder.read_write(buffer);
                    }
                    policy.text_binding.record(&mut recorder);
                } else {
                    recorder.read(&prepared.policy_buffers.as_ref().ok_or(SemanticTransitionError::NotBound)?.text_logits);
                }
    ...
    pub fn bind_parent(&mut self, mut parent: SemanticParentBinding) -> Result<SemanticPublishedIdentity, SemanticTransitionError> {
        ...
        let model_memory = prepare_model_memory(&allocations,
            std::mem::take(&mut parent.model_memory.storages),
            std::mem::take(&mut parent.model_memory.views), &tensors)?;
        self.ensure_rebindable()?;
        if self.publication.is_some() { return Err(publication_input_error("this Session already owns a canonical publication instance")); }
        let task_identity = self.task_evaluation_identity().ok_or(SemanticTransitionError::NotBound)?;
        if parent.authority_generation != self.task_epoch {
            return Err(publication_input_error("parent authority generation differs from the retained task import"));
        }
        if parent.recovered_instance.is_some() || parent.records.iter().any(|record|
            matches!(record.role, SemanticStateRole::IntentEntries | SemanticStateRole::IntentPayload))
        {
        ...
        parent.records.extend(initial_intent_records(&parent.intent_effect, parent.intent_entry_capacity,
            parent.intent_payload_capacity_bytes, parent.final_intent_payload_bytes)?);
        validate_parent_records(&parent, &tensors)?;
        let mut layouts = BTreeMap::new();
        let mut plans = Vec::new();
        for tensor in tensors {
            let layout = if matches!(tensor.layout.role, 18..=25) { tensor.layout }
                else if matches!(tensor.layout.role, 4 | 5) { prefix_capacity_layout(tensor.layout, parent.prefix_capacity)? }
                else { canonical_tensor_layout(tensor.layout)? };
            let bytes = tensor_layout_bytes(&layout)?;
            if bytes == 0 && !empty_model_tensor_layout(&layout) {
                return Err(publication_input_error("non-active tensor destination has no owned extent"));
            }
            ...
            plans.push(PublicationAllocationPlan {
                role: layout.role, index: layout.index, capacity, length: bytes,
                logical_begin: tensor.logical_begin, logical_end: tensor.logical_end,
                payload: if matches!(layout.role, 18..=25) { PublicationPayload::Uncomputed }
                    else { PublicationPayload::Tensor(tensor) },
            });
        }
        for layout in &parent.active_layouts {
            ...
            let bytes = tensor_layout_bytes(&layout)?;
            layouts.insert((layout.role, layout.index), layout);
            plans.push(PublicationAllocationPlan { role: layout.role, index: layout.index,
                capacity: bytes, length: 0, logical_begin: 0, logical_end: 0,
                payload: PublicationPayload::Uncomputed });
        }
        for record in &parent.records {
            plans.push(PublicationAllocationPlan { role: record.role as u64, index: record.index,
                capacity: record.capacity_bytes, length: record.bytes.len(), logical_begin: 0,
                logical_end: if record.role == SemanticStateRole::TokenProvenanceRecords { parent.provenance_records } else { 0 },
        ...
        let prefix_capacity = usize::try_from(parent.prefix_capacity).ok().and_then(|n| n.checked_mul(size_of::<SemanticTextSlot>()))
            .ok_or_else(|| publication_input_error("prefix source capacity overflow"))?;
        let feedback_capacity = usize::try_from(parent.feedback_capacity).ok().and_then(|n| n.checked_mul(size_of::<RawFeedbackRecord>()))
            .ok_or_else(|| publication_input_error("feedback capacity overflow"))?;
        for (role, index, bytes, capacity, end) in [
            (1, 0, publication_abi_bytes(&parent.prefix), prefix_capacity, parent.prefix.len() as u64),
            // Output storage only; native initialization fills the actual
            // source digest before this bank can be sealed or acquired.
            (3, 0, vec![0; 32], 32, 0),
            (14, 0, vec![0; size_of::<CompletionCoverage>()], size_of::<CompletionCoverage>(), 0),
            ...
            (33, 0, vec![0; size_of::<AttemptReceipt>()], size_of::<AttemptReceipt>(), 0),
            (47, 0, include_bytes!("semantic_action_catalogue_v1.def").to_vec(), include_bytes!("semantic_action_catalogue_v1.def").len(), 0),
            (48, 0, publication_abi_bytes(&self.codebooks.words), self.codebooks.words.len() * 8, 0),
            (55, 0, tensor_table_bytes(&layout_rows, false, &[])?, tensor_table_capacity(layout_rows.len(), 32 + parent.feedback_capacity)?, 0),
        ] {
            plans.push(PublicationAllocationPlan { role, index, capacity, length: bytes.len(),
                logical_begin: 0, logical_end: end, payload: PublicationPayload::Metadata(bytes) });
        }
        let mut instance = [0u8; 32];
        getrandom::fill(&mut instance).map_err(|error| runtime_error("publication instance entropy", error))?;
        let instance = Identity256::from_bytes(instance);
        let contract = PublicationContract {
            abi: 1, range_capacity: plans.len() as u64 + 1, prefix_capacity: parent.prefix_capacity,
            window_capacity: 32, feedback_capacity: parent.feedback_capacity, max_position: parent.max_position,
            pad_token: parent.pad_token, terminal_token_count: parent.terminal_tokens.len() as u64,
            final_intent_payload_bytes: parent.final_intent_payload_bytes,
            model_generation: parent.model_generation, policy_generation: parent.policy_generation,
            authority_generation: parent.authority_generation, semantic_owner: self.graph.transition_arena()[1], task_identity,
            topology_identity: parent.topology_identity, table_identity: parent.table_identity,
            model_contract_layout: parent.model_contract_layout,
            role_count: 55, ..PublicationContract::default()
        };
        let runtime = initial_runtime_contract_record(contract, &parent.role_counts, &parent.terminal_tokens,
            &layouts, plans.iter().map(|plan| (plan.role, plan.index, plan.capacity)).collect(),
            &parent.model_numerical_mode, &model_memory)?;
        plans.push(PublicationAllocationPlan { role: runtime.role as u64, index: runtime.index,
            capacity: runtime.capacity_bytes, length: runtime.bytes.len(), logical_begin: 0, logical_end: 0,
        ...
        let model_payloads = allocations.into_iter().map(PublicationPayload::Tensor).collect::<Vec<_>>();
        let (storage, uploads) = PublicationStorage::allocate(&self.provider, &plans, layouts,
            model_memory, &model_payloads, contract, &parent.terminal_tokens, instance)?;
        ...
        let header = PublicationHeader {
            instance, ring_head: parent.ring_head, prefix_extent: parent.prefix.len() as u64,
            semantic_owner: self.graph.transition_arena()[1], semantic_slot: u64::from(self.root.slot()),
            semantic_generation: self.root.generation(), semantic_digest: Identity256::from_bytes(*self.base_snapshot.digest().as_bytes()),
            semantic_extents: [u64::from(extents.statements()), u64::from(extents.supports()), u64::from(extents.versions())],
            model_generation: parent.model_generation, neural_generation: parent.neural_generation,
            cache_generation: parent.cache_generation, authority_generation: parent.authority_generation,
            fuel: parent.fuel, proposal: u64::from(parent.rng.proposal), stream_serial: parent.rng.stream_serial,
            family_id: u64::from(parent.rng.family_id), training_cursor: parent.training_cursor,
            training_rng: parent.training_rng, range_count: contract.range_capacity, model_geometry_digest,
            ..PublicationHeader::default()
        };
        let state = DeviceState { model_generation: parent.rng.model_generation,
            family_id: u32::from(parent.rng.family_id), stream_serial: parent.rng.stream_serial,
            next_proposal: u64::from(parent.rng.proposal), catalogue_generation: self.binding().generation,
            catalogue_digest: Identity256::from_bytes(CATALOGUE_DIGEST), binding_digest: self.binding().digest,
            ..DeviceState::default() };
        let bank = PublicationBank { header, source: parent.source, state,
            receipts: [SemanticTransitionReceipt::default(); COMPONENT_COUNT] };
        let result = self.initialize_publication(bank, &parent.terminal_tokens, &parent.role_counts, parent.rng, false);
        if result.is_err() { self.poisoned = true; }
        result
    }
    ...
    pub fn restore_state_material(&mut self, bytes: &[u8]) -> Result<SemanticPublishedIdentity, SemanticTransitionError> {
        ...
        if self.publication.is_some() || self.captured.is_some() || self.task.is_none() {
            return Err(publication_input_error("state restoration requires a fresh uncaptured task-bound Session"));
        }
        ...
        if header.authority_generation == 0 {
            return Err(publication_input_error("restored RNG or task generation exceeds its native domain"));
        }
        ...
        for (index, statement) in self.feedback_statement_bytes()?.iter().enumerate() {
            let saved = material.ranges.iter().find(|item| item.range.role == 16 && item.range.index == index as u64)
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            if saved.bytes != *statement {
                return Err(publication_input_error("restored feedback statement differs from the actual admitted task"));
            }
        }
        let result = (|| {
            let root = self.graph.restore_root(&material.graph).map_err(SemanticTransitionError::Semantic)?;
            let snapshot = self.graph.snapshot(crate::SemanticView::Root(root)).map_err(SemanticTransitionError::Semantic)?;
            let admission = self.graph.admission().ok_or(SemanticTransitionError::NotBound)?;
            let codebooks = ActionCodebooks::derive(admission, self.graph.transition_arena()[1])?;
            if codebooks.input_cells != self.codebooks.input_cells || codebooks.words.len() != self.device_codebooks.len() {
                return Err(publication_input_error("restored action catalogue changes its allocated shape"));
            }
            let (task, device) = self.task.as_mut().ok_or(SemanticTransitionError::NotBound)?;
            task.admission_identity = admission.identity();
            if task.identity() != material.contract.task_identity {
                return Err(publication_input_error("restored native task differs from its original complete admission"));
            }
            let words = task.words(self.graph.transition_arena()[1]);
            self.provider.htod_sync_copy_into_tracked(&words, device)
                .map_err(|error| runtime_error("restored task owner upload", error))?;
            let codebook = material.ranges.iter_mut().find(|item| item.range.role == 48)
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            ...
            getrandom::fill(&mut instance).map_err(|error| runtime_error("restored instance entropy", error))?;
            let instance = Identity256::from_bytes(instance);
            if instance == header.instance { return Err(publication_input_error("restoration must create a fresh instance")); }
            let mut contract = material.contract;
            contract.semantic_owner = self.graph.transition_arena()[1];
            let plans: Vec<_> = material.ranges.into_iter().map(|item| PublicationAllocationPlan {
                role: item.range.role, index: item.range.index, capacity: item.capacity,
                length: item.bytes.len(), logical_begin: item.range.logical_begin, logical_end: item.range.logical_end,
                payload: if matches!(item.range.role, 18..=25) { PublicationPayload::Uncomputed }
                    else { PublicationPayload::Metadata(item.bytes) },
            }).collect();
            let model_payloads = material.model_allocations.into_iter().map(PublicationPayload::Metadata).collect::<Vec<_>>();
            let (storage, uploads) = PublicationStorage::allocate(&self.provider, &plans, material.layouts,
                material.model_memory, &model_payloads, contract, &material.terminals, instance)?;
            ...
            self.publication = Some(Arc::new(storage));
            self.publication_uploads = uploads;
            let restored = self.initialize_publication(material.bank, &material.terminals, &material.role_counts, rng, true)?;
            if restored.logical_digest != header.logical_digest || restored.state_digest != header.state_digest {
                return Err(SemanticTransitionError::ObservationMismatch);
            }
            Ok(restored)
        })();
    ...
    fn initialize_publication(&mut self, bank: PublicationBank, terminal_tokens: &[u64], role_counts: &[u64; 55],
        rng: SemanticRngBinding, restored: bool) -> Result<SemanticPublishedIdentity, SemanticTransitionError> {
        ...
        enqueue_recorded(&self.domain, &mut self.poisoned, recorder, |enqueue| {
            for allocation in &storage.allocations {
                ...
                unsafe { sys::cuMemsetD8Async(allocation.device_ptr_value(), 0, allocation.len(), enqueue.stream().cu_stream()) }.result().map_err(|error| XlogError::Kernel(error.to_string()))?;
            }
            for (slot, payload) in &self.publication_uploads {
                if let PublicationPayload::Tensor(tensor) = payload {
                    if let Some(source) = &tensor.source {
                        let destination = &storage.allocations[*slot];
                        let layout = if tensor.layout.role == 0 { &tensor.layout }
                            else { &storage.layouts[&(tensor.layout.role, tensor.layout.index)] };
                        for (source_offset, destination_offset, bytes) in tensor_copy_plan(&tensor.layout, layout)
                            .map_err(|error| XlogError::Kernel(error.to_string()))? {
                            unsafe { sys::cuMemcpyDtoDAsync_v2(destination.device_ptr_value() + destination_offset,
                                *source.device_ptr() + source_offset, bytes, enqueue.stream().cu_stream()) }.result().map_err(|error| XlogError::Kernel(error.to_string()))?;
                        }
                    }
                }
            }
        ...
        wait_on_stream(&self.stream, &mut self.poisoned, &mut self.stream_waits, "cold publication tensor snapshot", CudaStream::synchronize)?;
        for (slot, payload) in &self.publication_uploads {
        ...
        for bank in 0..2 { upload_publication(&self.provider, &storage.bank_templates[bank], &storage.directories[bank])?; }
        let entries: Vec<_> = storage.allocations.iter().map(|allocation| PublicationStorageEntry {
            pointer: allocation.device_ptr_value(), bytes: allocation.len() as u64, generation: 1,
        }).collect();
        ...
        let counts: Vec<_> = role_counts.iter().enumerate().map(|(index, &count)| PublicationRoleCount { role: index as u64 + 1, count }).collect();
        upload_publication(&self.provider, &counts, &storage.role_counts)?;
        upload_publication(&self.provider, &[PendingContinuation {
            abi: 1, ranges: storage.continuation_directory.device_ptr_value(),
            range_count: storage.continuation_templates.len() as u64,
            ..PendingContinuation::default()
        }], &storage.continuation)?;
        upload_publication(&self.provider, &storage.continuation_templates, &storage.continuation_directory)?;
        upload_publication(&self.provider, &[PublicationControl {
            abi: 1, instance: storage.instance,
            banks: storage.banks.each_ref().map(|bank| bank.device_ptr_value()),
            directories: storage.directories.each_ref().map(|directory| directory.device_ptr_value()),
            storage: storage.storage.device_ptr_value(), storage_count: storage.allocations.len() as u64,
            contract: storage.contract.device_ptr_value(), continuation: storage.continuation.device_ptr_value(),
            ..PublicationControl::default()
        }], &storage.control)?;
        self.upload_rng_state(self.binding(), rng)?;
        self.publication_command(if restored { 4 } else { 1 }, None)?;
        let control = self.publication_read(storage.control.view())?[0];
        if control.refusal != 0 { return Err(SemanticTransitionError::PublicationRefused { status: control.refusal }); }
        let header = self.read_publication_header(0)?;
        if header.abi != 1 || header.instance != storage.instance || header.publication_word != 0 || control.word != 0
        ...
        Ok(SemanticPublishedIdentity { instance: header.instance, word: header.publication_word,
            logical_digest: header.logical_digest, state_digest: header.state_digest })
    }

    fn publication_command(&mut self, operation: u64, token: Option<u64>) -> Result<(), SemanticTransitionError> {
        let storage = self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?;
        let mut descriptor = self.descriptor();
        descriptor.publication = PublicationCommand { control: storage.control.device_ptr_value(), operation,
            lease: token.map_or(0, |token| self.readers[&token].device.device_ptr_value()) };
        let mut recorder = self.kernel_recorder();
        if let Some(token) = token { recorder.read_write(&self.readers[&token].device); }
        ...
        enqueue_recorded(&self.domain, &mut self.poisoned, recorder, |stream| {
            ...
            unsafe { execute.launch_in(stream, LaunchConfig { grid_dim: (1, 1, 1), block_dim: (256, 1, 1), shared_mem_bytes: 0 }, (descriptor,)) }
                .map_err(|error| XlogError::Kernel(error.to_string()))
        })?;
        wait_on_stream(&self.stream, &mut self.poisoned, &mut self.stream_waits, "publication command completion", CudaStream::synchronize)
    }

    fn publication_read<T: DeviceRepr + Copy>(&mut self, source: DeviceMemoryView<T>) -> Result<Vec<T>, SemanticTransitionError> {
        let bytes = source.len().checked_mul(size_of::<T>()).ok_or_else(|| publication_input_error("publication metadata read overflow"))?;
        ...
        enqueue_recorded(&self.domain, &mut self.poisoned, recorder, |stream| {
            ...
            unsafe { pinned.enqueue(stream.stream(), |destination| {
                sys::cuMemcpyDtoHAsync_v2(destination.cast(), *source.device_ptr(), bytes, stream.stream().cu_stream()).result()
            }) }.map_err(|error| XlogError::Kernel(error.to_string()))
        })?;
        wait_on_stream(&self.stream, &mut self.poisoned, &mut self.stream_waits, "publication metadata read completion", CudaStream::synchronize)?;
        ...
        self.provider.record_final_observation_transfer(bytes as u64);
        Ok(result)
    }

    fn read_publication_header(&mut self, bank: usize) -> Result<PublicationHeader, SemanticTransitionError> {
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        ...
        let view = unsafe { storage.banks[bank].view().cast::<u8>().expect("byte-aligned bank ABI")
            .slice(..size_of::<PublicationHeader>()).cast::<PublicationHeader>().expect("aligned fixed header ABI") };
        Ok(self.publication_read(view)?[0])
    }

    pub fn acquire(&mut self) -> Result<SemanticPublishedLease, SemanticTransitionError> {
        if self.pending {
            let proposal = u32::try_from(self.next_proposal).map_err(|_| SemanticTransitionError::GenerationExhausted)?;
        ...
        self.ensure_quiescent()?;
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let token = self.next_reader.checked_add(1).ok_or(SemanticTransitionError::GenerationExhausted)?;
        self.next_reader = token;
        let device = allocate_publication(&self.provider, 1)?;
        self.steps.insert(token, StepContentStorage::new());
        self.readers.insert(token, PublishedReader { device, aliases: Arc::new(()), consumer_streams: BTreeSet::new() });
        // Lease storage is retained before upload or Acquire can enqueue.
        if let Err(error) = upload_publication(&self.provider, &[PublicationLease::default()], &self.readers[&token].device) {
            self.poisoned = true; return Err(error);
        }
        ...
        if lease.status != 0 {
            ...
            return Err(SemanticTransitionError::PublicationRefused { status: lease.status });
        }
        if lease.abi != 1 || lease.active != 1 || lease.instance != storage.instance
            || lease.bank > 1 || lease.bank != (lease.word & 1) || lease.epoch != lease.word >> 1 {
            self.poisoned = true; return Err(SemanticTransitionError::ObservationMismatch);
        }
        ...
        let source_view = unsafe { storage.banks[lease.bank as usize].view().cast::<u8>().expect("byte-aligned bank ABI")
            .slice(size_of::<PublicationHeader>()..size_of::<PublicationHeader>() + 32 * size_of::<SemanticTextSlot>())
            .cast::<SemanticTextSlot>().expect("aligned fixed source ABI") };
        let source: [SemanticTextSlot; 32] = self.publication_read(source_view)?.try_into().map_err(|_| SemanticTransitionError::ObservationMismatch)?;
        if header.abi != 1 || header.instance != lease.instance || header.publication_word != lease.word
            || header.sealed_epoch != lease.epoch || header.range_count as usize != directory.len() {
            self.poisoned = true; return Err(SemanticTransitionError::ObservationMismatch);
        }
        let identity = SemanticPublishedIdentity { instance: lease.instance, word: lease.word,
            logical_digest: header.logical_digest, state_digest: header.state_digest };
        self.steps.get_mut(&token).expect("installed acquisition step").identity = Some(identity);
        Ok(SemanticPublishedLease { issuer: Arc::clone(&self.publication_issuer), token, identity,
            header, source, directory, active: true })
    }

    fn checked_reader(&self, lease: &SemanticPublishedLease) -> Result<&PublishedReader, SemanticTransitionError> {
        self.ensure_quiescent()?;
        if !lease.active || !Arc::ptr_eq(&lease.issuer, &self.publication_issuer) {
            return Err(publication_input_error("lease is released or belongs to a different Session"));
        }
        self.readers.get(&lease.token).ok_or_else(|| publication_input_error("lease has no live native reader owner"))
    }

    fn checked_step(&self, lease: &SemanticPublishedLease) -> Result<&StepContentStorage, SemanticTransitionError> {
        self.ensure_quiescent()?;
        if !Arc::ptr_eq(&lease.issuer, &self.publication_issuer) {
            return Err(publication_input_error("retained step belongs to a different Session"));
        }
        let step = self.steps.get(&lease.token)
            .ok_or_else(|| publication_input_error("original step has been fully released"))?;
        if step.identity != Some(lease.identity) || lease.active != self.readers.contains_key(&lease.token) {
            return Err(publication_input_error("retained step differs from its original acquired identity"));
        }
        Ok(step)
    }
    ...
    pub fn retained_step_identity(&self, lease: &SemanticPublishedLease)
        -> Result<SemanticPublishedIdentity, SemanticTransitionError> {
    ...
    pub fn published_identity(&self, lease: &SemanticPublishedLease) -> Result<SemanticPublishedIdentity, SemanticTransitionError> {
        self.checked_reader(lease)?; Ok(lease.identity)
    }
    ...
    pub fn published_range_keys(&self, lease: &SemanticPublishedLease)
        -> Result<Vec<(SemanticStateRole, u64)>, SemanticTransitionError> {
        self.checked_reader(lease)?;
        lease.directory.iter().map(|range| SemanticStateRole::from_code(range.role)
            .map(|role| (role, range.index)).ok_or(SemanticTransitionError::ObservationMismatch)).collect()
    }
...
fn order_content_inputs(&mut self, lease: &SemanticPublishedLease, consumer_stream: u64)
        -> Result<(), SemanticTransitionError> {
        self.readers.get_mut(&lease.token).expect("validated publication reader")
            .consumer_streams.insert(consumer_stream);
        self.order_step_content_inputs(lease.token, consumer_stream)
    }

    fn order_step_content_inputs(&mut self, reader_token: u64, consumer_stream: u64)
        -> Result<(), SemanticTransitionError> {
        let consumer = dlpack_consumer_stream(consumer_stream)?;
        self.stream.context().bind_to_thread().map_err(|e| runtime_error("content context binding", e))?;
        self.steps.get_mut(&reader_token).expect("validated content step")
            .consumer_streams.insert(consumer_stream);
        for source in [std::ptr::null_mut(), consumer] {
            let event = self.stream.context().new_event(None).map_err(|e| runtime_error("content producer event", e))?;
            ...
            self.stream.wait(event).map_err(|e| runtime_error("content producer event join", e))?;
        }
        Ok(())
    }

    fn order_content_consumers(&mut self, consumer_stream: u64) -> Result<(), SemanticTransitionError> {
        let event = self.stream.context().new_event(None).map_err(|e| runtime_error("content guard event", e))?;
        self.release_events.push(event);
        let event = self.release_events.last().expect("retained content guard event");
        ...
        unsafe {
            ...
            sys::cuStreamWaitEvent(dlpack_consumer_stream(consumer_stream)?, event.cu_event(), 0)
                .result().map_err(|e| runtime_error("content guard consumer join", e))?;
        }
        Ok(())
    }
    ...
    pub fn guard_published_content(&mut self, lease: &SemanticPublishedLease,
        keys: &[(SemanticStateRole, u64)], consumer_stream: u64) -> Result<(), SemanticTransitionError> {
        self.checked_reader(lease)?;
        dlpack_consumer_stream(consumer_stream)?;
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let inputs = if keys.iter().any(|(role, _)| matches!(*role as u64, 1 | 3..=13)) {
        ...
        if keys.is_empty() { return Err(publication_input_error("content guard requires actual publication ranges")); }
        for &(role, index) in keys {
            let key = (role as u64, index);
            if !distinct.insert(key) { return Err(publication_input_error("content guard has duplicate publication ranges")); }
            let (range_index, &range) = lease.directory.iter().enumerate().find(|(_, range)| (range.role, range.index) == key)
                .ok_or_else(|| publication_input_error("content guard names an absent publication range"))?;
            if range_index >= directory.len() { return Err(SemanticTransitionError::ObservationMismatch); }
            let allocation = storage.allocations.get(range.storage_slot as usize).ok_or(SemanticTransitionError::ObservationMismatch)?;
            if range.generation != 1 || range.offset_bytes.checked_add(range.length_bytes)
                .is_none_or(|end| end > allocation.len() as u64) {
                return Err(SemanticTransitionError::ObservationMismatch);
            }
            let layout = if is_tensor_role(role as u64) {
                *storage.layouts.get(&key).ok_or(SemanticTransitionError::ObservationMismatch)?
            } else { SemanticTensorLayout::default() };
        ...
        let execute = self.provider.device().inner().get_func("xlog_semantic_transition", "semantic_publication_content_guard")
            .ok_or_else(|| runtime_error("kernel lookup", "publication content guard unavailable"))?;
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
        ...
        let result = (|| {
            ...
            if let Some(inputs) = &inputs {
                ...
                self.steps.get_mut(&lease.token).expect("validated input step").consumer_streams.insert(consumer_stream);
                inputs.verify(&self.domain, &mut self.poisoned)?;
            }
            for (role, index, layout) in ranges {
                let reader = self.readers.get(&lease.token).expect("validated native reader");
                storage.enqueue_content_guard(&self.domain, &mut self.poisoned, reader.device.view(),
                    &execute, role, index, layout)?;
            }
    ...
    pub fn capture_tensor_content(&mut self, lease: &SemanticPublishedLease,
        tensors: Vec<SemanticTensorInput>, consumer_stream: u64)
        -> Result<SemanticTensorContentWitness, SemanticTransitionError> {
    ...
    pub fn bind_model_content(&mut self, lease: &SemanticPublishedLease,
        tensors: Vec<SemanticTensorInput>, consumer_stream: u64)
        -> Result<SemanticTensorContentWitness, SemanticTransitionError> {
        self.bind_tensor_content(lease, tensors, consumer_stream, true)
    }

    fn bind_tensor_content(&mut self, lease: &SemanticPublishedLease,
        tensors: Vec<SemanticTensorInput>, consumer_stream: u64, model: bool)
        -> Result<SemanticTensorContentWitness, SemanticTransitionError> {
        let empty = tensors.is_empty();
        let mut handoff = RetainedTensorAdmission { inputs: Some(tensors),
            stream: Arc::clone(self.provider.device().inner().stream()), ready: false };
        self.checked_reader(lease)?;
        dlpack_consumer_stream(consumer_stream)?;
        if empty { return Err(publication_input_error("content witness requires an actual tensor roster")); }
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
            .map_err(|e| runtime_error("content binding stream admission", e))?;
        if !self.provider.device().inner().stream().context().has_async_alloc() {
            return Err(publication_input_error("device content capture requires stream-ordered CUDA allocation support"));
        }
        ...
        self.steps.get_mut(&lease.token).expect("validated content step")
            .consumer_streams.insert(consumer_stream);
        let tensors = prepare_semantic_tensors(&self.provider,
            handoff.inputs.take().expect("retained original content inputs"))?;
        let reader = self.steps.get_mut(&lease.token).expect("validated content step");
        let index = reader.content.len();
        reader.content.push(TensorContentBuffers { tensors,
            seals: TensorContentSeals::Captured(Vec::new()), verification_inputs: Vec::new() });
        let reader = &self.steps[&lease.token];
        let content = &reader.content[index];
        validate_content_coordinates(content.tensors.iter().map(|tensor| (tensor.layout.role, tensor.layout.index)))?;
        ...
        for tensor in &content.tensors {
            ...
            if !model {
                ...
                if let Some(inputs) = &reader.inputs {
                    if inputs.owns_tensor(tensor, &lease.directory)? { original = Some(Arc::clone(inputs)); }
                }
                if original.is_none() {
                    for (&token, owner) in &self.steps {
                        if token == lease.token { continue; }
                        if let Some(inputs) = &owner.inputs {
                            if inputs.owns_tensor(tensor, &lease.directory)? {
                                return Err(publication_input_error("native step content belongs to another acquired reader"));
                            }
                        }
                    }
                }
        ...
        let model_plan = if model {
            let storage = self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?;
            let ranges = model_content_ranges(&lease.directory, &storage.layouts,
                content.tensors.iter().map(|tensor| (tensor.layout, tensor.logical_begin,
                    tensor.logical_end, tensor.source.as_ref().map_or(0, DeviceMemoryView::len))))?;
            Some(model_content_copy_plan(&lease.directory, &ranges,
                &storage.allocations.iter().map(TrackedCudaSlice::len).collect::<Vec<_>>())?)
        } else { None };
                ...
                for (&token, owner) in &self.steps {
                    if let Some(digest) = owner.feedback_origin_digest(tensor)? {
                        if token != lease.token {
                            return Err(publication_input_error("native feedback content belongs to another acquired reader"));
                        }
                        origin = Some(digest);
                        break;
                    }
        ...
        let handle = SemanticTensorContentWitness { issuer: Arc::clone(&self.publication_issuer),
            reader_token: lease.token, index, _witness: Arc::clone(&reader.content_witnesses) };
        self.steps.get_mut(&lease.token).expect("validated content step").content[index].seals = seals;
        if let Some(plan) = model_plan {
            ...
            self.guard_published_content(lease, &[(SemanticStateRole::ModelContract, 0)], consumer_stream)?;
            let storage = self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?;
            let TensorContentSeals::Model(model) = &self.steps[&lease.token].content[index].seals else {
                return Err(SemanticTransitionError::ObservationMismatch);
            };
            model.snapshot(&self.domain, &mut self.poisoned,
                &storage.directories[(lease.identity.word & 1) as usize],
                &storage.allocations[plan.contract_storage_slot], &plan)?;
        }
    ...
    pub fn verify_tensor_content(&mut self, lease: &SemanticPublishedLease,
        witness: &SemanticTensorContentWitness, tensors: Vec<SemanticTensorInput>, consumer_stream: u64)
        -> Result<(), SemanticTransitionError> {
        let mut handoff = RetainedTensorAdmission { inputs: Some(tensors),
            stream: Arc::clone(self.provider.device().inner().stream()), ready: false };
        ...
        dlpack_consumer_stream(consumer_stream)?;
        self.steps.get_mut(&lease.token).expect("validated witness step")
            .consumer_streams.insert(consumer_stream);
        let tensors = prepare_semantic_tensors(&self.provider,
            handoff.inputs.take().expect("retained original verification inputs"))?;
        let content = &mut self.steps.get_mut(&lease.token).expect("validated witness step").content[witness.index];
        ...
        if actual.len() != content.tensors.len() || content.tensors.iter().zip(actual).any(|(expected, actual)| {
    ...
    fn checked_content_witness(&self, lease: &SemanticPublishedLease, witness: &SemanticTensorContentWitness)
        -> Result<(), SemanticTransitionError> {
        let reader = self.checked_step(lease)?;
        if !Arc::ptr_eq(&self.publication_issuer, &witness.issuer) || witness.reader_token != lease.token
            || !Arc::ptr_eq(&reader.content_witnesses, &witness._witness) || witness.index >= reader.content.len() {
            return Err(publication_input_error("content witness belongs to another parent or Session"));
        }
        Ok(())
    }

fn enqueue_tensor_content(&mut self, lease: &SemanticPublishedLease, witness: &SemanticTensorContentWitness,
        consumer_stream: u64, verify: bool) -> Result<(), SemanticTransitionError> {
        self.checked_content_witness(lease, witness)?;
    ...
    fn guard_continuation_content(&mut self, binding: &TextBindingStorage) -> Result<(), SemanticTransitionError> {
        let witness = &binding._witness;
        let reader = self.steps.get(&witness.reader_token)
            .ok_or_else(|| publication_input_error("original continuation step has been released"))?;
        let original_parent = match &binding.parent {
            TextBindingParent::Published { .. } => reader.identity.is_some(),
            TextBindingParent::Prepared(inputs) => reader.inputs.as_ref().is_some_and(|original| Arc::ptr_eq(original, inputs))
                && reader.prepared.as_ref().is_some_and(|prepared| prepared.observed)
                && self.prepared_segment.as_ref().is_some_and(|build| build.completed),
        };
        if !Arc::ptr_eq(&self.publication_issuer, &witness.issuer)
            || !original_parent
            || !Arc::ptr_eq(&reader.content_witnesses, &witness._witness)
            || reader.content.get(witness.index).is_none_or(|content| !matches!(content.seals, TensorContentSeals::Captured(_))) {
            return Err(publication_input_error("continuation no longer owns its original captured content"));
        }
        self.enqueue_content_witness(witness, binding.consumer_stream, true)
    }

    fn enqueue_content_witness(&mut self, witness: &SemanticTensorContentWitness,
        consumer_stream: u64, verify: bool) -> Result<(), SemanticTransitionError> {
        let execute = self.provider.device().inner().get_func("xlog_semantic_transition", "semantic_tensor_content_witness")
            .ok_or_else(|| runtime_error("kernel lookup", "tensor content witness unavailable"))?;
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
    ...
    fn ensure_step_inputs(&mut self, lease: &SemanticPublishedLease) -> Result<Arc<PreparedStepInputs>, SemanticTransitionError> {
        self.checked_reader(lease)?;
        self.checked_step(lease)?;
        lease.model_context_header()?;
        if let Some(inputs) = &self.steps[&lease.token].inputs { return Ok(Arc::clone(inputs)); }
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
            .map_err(|error| runtime_error("step input preparation stream admission", error))?;
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let inputs = Arc::new(PreparedStepInputs::allocate(&self.provider, storage, self.readers[&lease.token].device.view())?);
        ...
        self.steps.get_mut(&lease.token).expect("validated input step").inputs = Some(Arc::clone(&inputs));
        let result = inputs.initialize(&self.provider).and_then(|()| inputs.enqueue(&self.domain, &mut self.poisoned));
    ...
    pub fn published_source(&mut self, lease: &SemanticPublishedLease, consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
    ...
    pub fn published_terminal(&mut self, lease: &SemanticPublishedLease, consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
    ...
    pub fn published_model_context(&mut self, lease: &SemanticPublishedLease, consumer_stream: u64)
        -> Result<SemanticModelContext, SemanticTransitionError> {
        dlpack_consumer_stream(consumer_stream)?;
        self.checked_reader(lease)?;
        let header = lease.model_context_header()?;
        let contract = self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?.contract_value;
        let prefix_range = lease.directory.iter().find(|range|
            range.role == SemanticStateRole::PrefixIdentity as u64 && range.index == 0)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        if prefix_range.length_bytes != 32 { return Err(SemanticTransitionError::ObservationMismatch); }
        let prefix_identity = self.published_record(lease, SemanticStateRole::PrefixIdentity, 0, consumer_stream)?;
        let prefix_extent = self.export_publication_bank_field(lease, PublicationBankField::PrefixExtent, consumer_stream)?;
        let ring_head = self.export_publication_bank_field(lease, PublicationBankField::RingHead, consumer_stream)?;
        let source = self.published_source(lease, consumer_stream)?;
        Ok(SemanticModelContext {
            parent: lease.identity, descriptor_digest: header.descriptor_digest,
            numerical_state: (header.model_geometry_digest, header.model_numerical_digest),
            semantic_root: (header.semantic_owner, header.semantic_slot, header.semantic_generation,
                header.semantic_digest, header.semantic_extents),
            prefix: (prefix_identity, prefix_extent), ring_head, source,
            generations: (header.model_generation, header.neural_bank, header.neural_generation,
                header.cache_generation, header.authority_generation),
            topology_identity: contract.topology_identity, table_identity: contract.table_identity,
            capacities: (contract.prefix_capacity, contract.feedback_capacity, contract.max_position),
        })
    }
    ...
    pub fn published_prefix(&mut self, lease: &SemanticPublishedLease, consumer_stream: u64)
        -> Result<(SemanticPublishedIdentity, DlpackManagedTensor), SemanticTransitionError> {
        self.checked_reader(lease)?;
        let range = *lease.directory.iter().find(|range|
            range.role == SemanticStateRole::PrefixSource as u64 && range.index == 0)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        if range.logical_end != lease.header.prefix_extent { return Err(SemanticTransitionError::ObservationMismatch); }
        let capacity = self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?.contract_value.prefix_capacity;
    ...
    pub fn continuation_rng(&self, lease: &SemanticPublishedLease) -> Result<SemanticRngBinding, SemanticTransitionError> {
        ...
        if self.continuation_base != Some(lease.identity.word)
            || self.admitted_transition.is_none_or(|(token, word, _)| token != lease.token || word != lease.identity.word) {
            return Err(publication_input_error("execution requires this exact acquired reader's admitted continuation"));
        }
    ...
    pub fn admit_external_activation(&mut self, lease: &SemanticPublishedLease) -> Result<(), SemanticTransitionError> {
        self.checked_reader(lease)?;
        if lease.header.terminal != 2 { return Err(publication_input_error("external activation requires a genuinely published FINAL parent")); }
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let range = lease.directory.iter().find(|range| range.role == 30).ok_or(SemanticTransitionError::ObservationMismatch)?;
        let payload = lease.directory.iter().find(|range| range.role == 31).ok_or(SemanticTransitionError::ObservationMismatch)?;
        let owner = storage.allocations.get(range.storage_slot as usize).ok_or(SemanticTransitionError::ObservationMismatch)?;
        if range.offset_bytes != 0 || range.length_bytes < size_of::<IntentQueueHeader>() as u64 || range.length_bytes > owner.len() as u64 {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
        ...
        let header_view = unsafe { owner.view().slice(..size_of::<IntentQueueHeader>()).cast::<IntentQueueHeader>() }
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        let header = self.publication_read(header_view)?[0];
        if header.abi != 1 || header.count == 0 || header.count > header.capacity
            || header.count.checked_mul(size_of::<IntentEntry>() as u64).and_then(|n| n.checked_add(size_of::<IntentQueueHeader>() as u64)) != Some(range.length_bytes)
            || header.payload_used_bytes != payload.length_bytes || header.payload_used_bytes > header.payload_capacity_bytes
            || header.effect_length_bytes == 0 || header.effect_offset_bytes.checked_add(header.effect_length_bytes).is_none_or(|n| n > header.payload_used_bytes)
        { return Err(publication_input_error("FINAL bank has no complete native terminal intent queue")); }
        let begin = size_of::<IntentQueueHeader>() + (header.count as usize - 1) * size_of::<IntentEntry>();
        // SAFETY: exact final entry extent follows from the checked queue count.
        let entry_view = unsafe { owner.view().slice(begin..begin + size_of::<IntentEntry>()).cast::<IntentEntry>() }
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        let entry = self.publication_read(entry_view)?[0];
        if entry.audit_instance != lease.identity.instance || entry.audit_epoch != lease.identity.word >> 1
            || entry.result_logical != lease.identity.logical_digest || entry.chain != header.chain_head
            || entry.payload_len < 8 || entry.payload_offset.checked_add(entry.payload_len).is_none_or(|n| n > header.payload_used_bytes)
            || entry.stable_identity == Identity256::default() || entry.effect_digest == Identity256::default()
        { return Err(publication_input_error("terminal intent is not the actual final effect of this acquired publication")); }
        Ok(())
    }
    ...
    pub fn published_tensor_allocation(&mut self, lease: &SemanticPublishedLease,
        role: SemanticStateRole, index: u64, consumer_stream: u64)
        -> Result<(DeviceAllocationProvenance, DlpackManagedTensor), SemanticTransitionError> {
        dlpack_consumer_stream(consumer_stream)?;
        let (range, layout) = self.published_tensor_layout(lease, role, index)?;
        ...
        let provenance = view.allocation_provenance()
            .ok_or_else(|| publication_input_error("published tensor has no complete native allocation owner"))?;
        let allocation = view.allocation_view().ok_or(SemanticTransitionError::ObservationMismatch)?;
        let bytes = i64::try_from(allocation.len()).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
        let tensor = self.export_publication_tensor_view(lease, range.role, allocation,
            vec![bytes], vec![1], (1, 8), consumer_stream)?;
        Ok((provenance, tensor))
    }

    fn published_tensor_layout(&self, lease: &SemanticPublishedLease,
        role: SemanticStateRole, index: u64) -> Result<(PublicationRange, SemanticTensorLayout), SemanticTransitionError> {
        self.checked_reader(lease)?;
        let storage = self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?;
        let layout = *storage.layouts.get(&(role as u64, index))
            .ok_or_else(|| publication_input_error("acquired role has no model tensor layout"))?;
        let range = *lease.directory.iter().find(|range| range.role == role as u64 && range.index == index)
    ...
    pub fn published_active_rows(&mut self, lease: &SemanticPublishedLease) -> Result<SemanticActiveRows, SemanticTransitionError> {
        ...
        if layouts != storage.layouts.values().copied().collect::<Vec<_>>()
            ...
            || lease.directory.iter().filter(|range| matches!(range.role, 51..=54)).any(|range|
                range.logical_begin != 0 || range.logical_end != active.rows.len() as u64
    ...
    pub fn published_model_numerical_mode(&mut self, lease: &SemanticPublishedLease)
        -> Result<Vec<u8>, SemanticTransitionError> {
        self.ensure_quiescent()?;
        let streams = self.checked_reader(lease)?.consumer_streams.iter()
            .chain(&self.checked_step(lease)?.consumer_streams).copied().collect::<BTreeSet<_>>()
        ...
        let table = self.read_published_material_range(lease, SemanticStateRole::TensorLayout as u64, 0)?;
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        ...
        for (index, count) in counts.iter().enumerate() {
            if count.role != index as u64 + 1 { return Err(SemanticTransitionError::ObservationMismatch); }
            role_counts[index] = count.count;
        }
        let terminals = self.publication_read(storage.terminals.view())?;
        let capacities = lease.directory.iter().map(|range| {
            let allocation = usize::try_from(range.storage_slot).ok().and_then(|slot| storage.allocations.get(slot))
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            let capacity = usize::try_from(range.offset_bytes).ok().and_then(|offset| allocation.len().checked_sub(offset))
                .ok_or(SemanticTransitionError::ObservationMismatch)?;
            Ok((range.role, range.index, capacity))
        }).collect::<Result<Vec<_>, SemanticTransitionError>>()?;
        Ok(verified_runtime_model_mode(&record.bytes, contract, &role_counts, &terminals,
            &storage.layouts, &capacities, &table, &storage.model_memory)?.to_vec())
    }
    ...
    pub fn published_state_material(&mut self, lease: &SemanticPublishedLease) -> Result<Vec<u8>, SemanticTransitionError> {
        ...
        let graph = self.graph.export_transition_root_parts(bank.header.semantic_owner,
            bank.header.semantic_slot, bank.header.semantic_generation).map_err(SemanticTransitionError::Semantic)?;
        let counts = self.publication_read(storage.role_counts.view())?;
        let mut role_counts = [0; 55];
        for (index, count) in counts.iter().enumerate() {
            if count.role != index as u64 + 1 { return Err(SemanticTransitionError::ObservationMismatch); }
        ...
        let terminals = if storage.terminals.len() == 0 { Vec::new() }
            else { self.publication_read(storage.terminals.view())? };
        ...
        for slots in &storage.model_slots {
            ...
            model_allocations.push(if allocation.len() == 0 { Vec::new() }
                else { self.publication_read(allocation.view())? });
        }
        for range in &lease.directory {
            if matches!(range.role, 18..=25) {
                let (allocation, offset) = storage.model_memory.location(range.role, range.index)?;
                let end = usize::try_from(range.length_bytes).ok().and_then(|bytes| offset.checked_add(bytes))
                    .ok_or(SemanticTransitionError::ObservationMismatch)?;
                let bytes = model_allocations[allocation].get(offset..end)
                    .ok_or(SemanticTransitionError::ObservationMismatch)?.to_vec();
                ranges.push(PublicationMaterialRange { range: *range,
                    capacity: model_allocations[allocation].len() - offset, bytes });
            } else { ranges.push(self.read_published_material_range(lease, range.role, range.index)?); }
        }
        ...
        PublicationMaterial { bank, contract, role_counts, terminals, layouts: storage.layouts.clone(),
            model_memory: storage.model_memory.clone(), model_allocations, ranges, graph }.encode()
    }
    ...
    pub fn published_replay_evidence(&mut self, predecessor: &SemanticPublishedLease, successor: &SemanticPublishedLease)
        -> Result<Vec<u8>, SemanticTransitionError> {
        self.guard_replay_publications(predecessor, successor)?;
        let predecessor = PublicationMaterial::decode(&self.published_state_material(predecessor)?)?;
    ...
    pub fn published_replay_provenance(&mut self, predecessor: &SemanticPublishedLease, successor: &SemanticPublishedLease)
        -> Result<Vec<u8>, SemanticTransitionError> {
        self.guard_replay_publications(predecessor, successor)?;
        let identity = predecessor.identity;
        let predecessor = PublicationMaterial::decode(&self.published_state_material(predecessor)?)?;
        self.read_replay_evidence(&predecessor, successor)?;
        let authority = predecessor.ranges.iter().find(|item| item.range.role == 38 && item.range.index == 0)
            .ok_or(SemanticTransitionError::ObservationMismatch)?.clone();
        let decision = self.read_published_material_range(successor, 39, 0)?;
        let provenance = PublicationReplayProvenance { predecessor: identity, authority, decision };
    ...
    pub fn restore_replay_material(&mut self, material: &SemanticReplayMaterial)
        -> Result<SemanticPublishedIdentity, SemanticTransitionError> {
    ...
    pub fn verify_replay_publication(&mut self, material: &SemanticReplayMaterial,
        predecessor: &SemanticPublishedLease, successor: &SemanticPublishedLease)
        -> Result<(), SemanticTransitionError> {
        self.guard_replay_publications(predecessor, successor)?;
        ...
        let actual_predecessor = PublicationMaterial::decode(&self.published_state_material(predecessor)?)?;
        let header = actual_predecessor.bank.header;
        let expected = material.predecessor_identity();
        if header.instance == expected.instance || header.recovered_instance != expected.instance
            || header.publication_word != 0 || header.logical_digest != expected.logical_digest
    ...
    fn guard_replay_publications(&mut self, predecessor: &SemanticPublishedLease, successor: &SemanticPublishedLease)
        -> Result<(), SemanticTransitionError> {
    ...
    fn read_replay_evidence(&mut self, predecessor: &PublicationMaterial, successor: &SemanticPublishedLease)
        -> Result<PublicationReplayEvidence, SemanticTransitionError> {
        let bank = self.read_published_bank(successor)?;
        let mut ranges = Vec::with_capacity(REPLAY_EVIDENCE_ROLES.len());
        for role in REPLAY_EVIDENCE_ROLES { ranges.push(self.read_published_material_range(successor, role, 0)?); }
        let evidence = PublicationReplayEvidence { successor: successor.identity, ranges };
    ...
    fn read_published_bank(&mut self, lease: &SemanticPublishedLease) -> Result<PublicationBank, SemanticTransitionError> {
        self.checked_reader(lease)?;
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let bank = self.publication_read(storage.banks[(lease.identity.word & 1) as usize].view())?[0];
    ...
    fn read_published_material_range(&mut self, lease: &SemanticPublishedLease, role: u64, index: u64)
        -> Result<PublicationMaterialRange, SemanticTransitionError> {
        self.checked_reader(lease)?;
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let range = lease.directory.iter().find(|range| (range.role, range.index) == (role, index))
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        let owner = storage.allocations.get(usize::try_from(range.storage_slot)
            .map_err(|_| SemanticTransitionError::ObservationMismatch)?).ok_or(SemanticTransitionError::ObservationMismatch)?;
        let begin = usize::try_from(range.offset_bytes).map_err(|_| SemanticTransitionError::ObservationMismatch)?;
        let end = usize::try_from(range.length_bytes).ok().and_then(|n| begin.checked_add(n))
            .filter(|&end| end <= owner.len()).ok_or(SemanticTransitionError::ObservationMismatch)?;
        if range.generation != 1 { return Err(SemanticTransitionError::ObservationMismatch); }
        let bytes = if begin == end { Vec::new() } else { self.publication_read(owner.view().slice(begin..end))? };
        Ok(PublicationMaterialRange { range: *range, capacity: owner.len() - begin, bytes })
    }

    fn read_published_record_bytes(&mut self, lease: &SemanticPublishedLease, role: SemanticStateRole) -> Result<Vec<u8>, SemanticTransitionError> {
        Ok(self.read_published_material_range(lease, role as u64, 0)?.bytes)
    }
    ...
    pub fn published_record(&mut self, lease: &SemanticPublishedLease, role: SemanticStateRole, index: u64, consumer_stream: u64) -> Result<DlpackManagedTensor, SemanticTransitionError> {
        self.checked_reader(lease)?;
        if is_tensor_role(role as u64) { return Err(publication_input_error("tensor roles require their exact model layout export")); }
        let range = *lease.directory.iter().find(|range| range.role == role as u64 && range.index == index)
            .ok_or_else(|| publication_input_error("acquired record range is absent"))?;
        self.export_publication_range(lease, range, publication_record_layout(&range), consumer_stream)
    }
    ...
    pub fn published_feedback(&mut self, lease: &SemanticPublishedLease, consumer_stream: u64)
        -> Result<SemanticFeedback, SemanticTransitionError> {
        self.checked_reader(lease)?;
        dlpack_consumer_stream(consumer_stream)?;
        if self.admitted_transition.is_none_or(|(token, word, _)| token != lease.token || word != lease.identity.word) {
            return Err(publication_input_error("feedback encoding requires this acquired parent's admitted segment"));
        }
        ...
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let descriptor = self.descriptor();
        let buffers = FeedbackBuffers::allocate(&self.provider, schema,
            storage.contract_value.feedback_capacity, descriptor.arena[4])?;
        ...
        let reader = self.steps.get_mut(&lease.token).expect("validated feedback step");
        let output_index = reader.feedback.len();
        reader.feedback.push(buffers);
        let result = (|| {
            let reader = self.readers.get(&lease.token).expect("validated native reader");
            let buffers = &self.steps[&lease.token].feedback[output_index];
            buffers.enqueue(&self.domain, &mut self.poisoned, &self.graph, &storage,
                &self.task.as_ref().ok_or(SemanticTransitionError::InvalidTaskBank)?.1,
            ...
            let features = self.export_step_view(lease, features,
                vec![slots as i64, schema.feature_width as i64], vec![schema.feature_width as i64, 1], (2, 32), consumer_stream)?;
            let device_validity = self.export_step_view(lease, device_validity,
                vec![slots as i64], vec![1], (6, 8), consumer_stream)?;
            let mut export_words = |view: DeviceMemoryView<u64>, shape: Vec<i64>, strides: Vec<i64>| {
                // SAFETY: u64 has no padding; the complete tracked owner stays
                // attached to each byte view and acquired-reader export.
                let bytes = unsafe { view.cast::<u8>() }.ok_or(SemanticTransitionError::ObservationMismatch)?;
            ...
            let supports = export_words(support_rows, vec![support_capacity as i64, 12], vec![12, 1])?;
            Ok(SemanticFeedback { parent: lease.identity, schema,
                features, device_validity,
                query_receipts, original_statements, support_offsets, supports })
        })();
        if result.is_err() {
            self.poisoned = true;
        }
    ...
    fn publication_tensor_view(&mut self, lease: &SemanticPublishedLease,
        range: PublicationRange, layout: SemanticTensorLayout) -> Result<DeviceMemoryView<u8>, SemanticTransitionError> {
        self.checked_reader(lease)?;
        let view = if matches!(range.role, 1 | 3..=13 | 18..=25) {
            let inputs = self.ensure_step_inputs(lease)?;
            let view = inputs.views.get(&(range.role, range.index)).ok_or(SemanticTransitionError::ObservationMismatch)?;
            let local = PublicationRange { offset_bytes: 0, ..range };
            view.slice(publication_export_span(&local, &layout, view.len())?)
        } else {
            let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
            let allocation = storage.allocations.get(range.storage_slot as usize).ok_or(SemanticTransitionError::ObservationMismatch)?;
            allocation.view().slice(publication_export_span(&range, &layout, allocation.len())?)
        };
        Ok(view)
    }

    fn export_publication_range(&mut self, lease: &SemanticPublishedLease, range: PublicationRange, layout: SemanticTensorLayout, consumer_stream: u64) -> Result<DlpackManagedTensor, SemanticTransitionError> {
        dlpack_consumer_stream(consumer_stream)?;
        let view = self.publication_tensor_view(lease, range, layout)?;
        let shape = layout.dimensions[..layout.rank as usize].iter().map(|&n| i64::try_from(n)
            .map_err(|_| publication_input_error("DLPack shape exceeds signed extent"))).collect::<Result<Vec<_>, _>>()?;
        let strides = layout.strides_bytes[..layout.rank as usize].iter().map(|&n| i64::try_from(n / layout.element_bytes)
            .map_err(|_| publication_input_error("DLPack stride exceeds signed extent"))).collect::<Result<Vec<_>, _>>()?;
        let dtype = match layout.scalar_type {
            1 => (1, 8), 2 => (1, 32), 3 => (1, 64), 4 => (2, 16), 5 => (4, 16), 6 => (2, 32),
            7 => (0, 64), 8 => (6, 8),
            _ => return Err(SemanticTransitionError::ObservationMismatch),
        };
        self.export_publication_tensor_view(lease, range.role, view, shape, strides, dtype, consumer_stream)
    }

    #[allow(clippy::too_many_arguments)]
    fn export_publication_tensor_view(&mut self, lease: &SemanticPublishedLease, role: u64,
        view: DeviceMemoryView<u8>, shape: Vec<i64>, strides: Vec<i64>, dtype: (u8, u8),
        consumer_stream: u64) -> Result<DlpackManagedTensor, SemanticTransitionError> {
    ...
    fn export_publication_bank_field(&mut self, lease: &SemanticPublishedLease,
        field: PublicationBankField, consumer_stream: u64) -> Result<DlpackManagedTensor, SemanticTransitionError> {
        dlpack_consumer_stream(consumer_stream)?;
        let inputs = self.ensure_step_inputs(lease)?;
        let (_, shape, strides) = field.layout();
        self.export_step_view(lease, inputs.field_view(field)?, shape, strides, (1, 64), consumer_stream)
    }

fn export_reader_view(&mut self, lease: &SemanticPublishedLease, view: DeviceMemoryView<u8>,
        shape: Vec<i64>, strides: Vec<i64>, dtype: (u8, u8), consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
        dlpack_consumer_stream(consumer_stream)?;
        let guard = Arc::clone(&self.checked_reader(lease)?.aliases);
        self.readers.get_mut(&lease.token).expect("validated native reader")
            .consumer_streams.insert(consumer_stream);
        self.export_owned_view(view, shape, strides, dtype, guard, consumer_stream)
    }

    fn export_step_view(&mut self, lease: &SemanticPublishedLease, view: DeviceMemoryView<u8>,
        shape: Vec<i64>, strides: Vec<i64>, dtype: (u8, u8), consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
        dlpack_consumer_stream(consumer_stream)?;
        let guard = Arc::clone(&self.checked_step(lease)?.aliases);
        self.steps.get_mut(&lease.token).expect("validated output step")
            .consumer_streams.insert(consumer_stream);
        self.export_owned_view(view, shape, strides, dtype, guard, consumer_stream)
    }

    #[allow(clippy::too_many_arguments)]
    fn export_owned_view(&mut self, view: DeviceMemoryView<u8>, shape: Vec<i64>, strides: Vec<i64>,
        dtype: (u8, u8), guard: Arc<()>, consumer_stream: u64)
        -> Result<DlpackManagedTensor, SemanticTransitionError> {
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
        ...
        Ok(export_owned_allocation(view, shape, strides, dtype, self.provider.device().ordinal() as i32,
            guard, storage, None))
    }
    ...
    fn complete_step_consumers(&mut self, lease: &SemanticPublishedLease, consumer_streams: &[u64]) -> Result<(), SemanticTransitionError> {
        self.checked_step(lease)?;
        self.complete_step_consumers_by_token(lease.token, consumer_streams)
    }

    fn complete_step_consumers_by_token(&mut self, token: u64, consumer_streams: &[u64]) -> Result<(), SemanticTransitionError> {
        let step = self.steps.get(&token).ok_or_else(|| publication_input_error("original step has been fully released"))?;
        let mut streams = consumer_streams.iter().copied().collect::<BTreeSet<_>>();
        if !step.consumer_streams.is_subset(&streams)
            || self.readers.get(&token).is_some_and(|reader| !reader.consumer_streams.is_subset(&streams)) {
            return Err(publication_input_error("release omits a consumer stream that received a published alias"));
        }
        for &stream in &streams { dlpack_consumer_stream(stream)?; }
        ...
        if !step.content.is_empty() { streams.insert(1); }
        let result = (|| {
            self.stream.context().bind_to_thread().map_err(|error| runtime_error("release context binding", error))?;
            for stream in streams {
                let event = self.stream.context().new_event(None).map_err(|error| runtime_error("consumer completion event", error))?;
                self.release_events.push(event);
                ...
                unsafe { cudarc::driver::result::event::record(event.cu_event(), dlpack_consumer_stream(stream)?) }
                    .map_err(|error| runtime_error("consumer final event record", error))?;
                self.stream.wait(event).map_err(|error| runtime_error("consumer final event join", error))?;
            }
            wait_on_stream(&self.stream, &mut self.poisoned, &mut self.stream_waits, "published reader consumer completion", CudaStream::synchronize)?;
            Ok(())
        })();
        if result.is_err() {
            self.poisoned = true;
    ...
    fn quiesce_step_content(&mut self, lease: &SemanticPublishedLease, consumer_streams: &[u64]) -> Result<(), SemanticTransitionError> {
        let step = self.checked_step(lease)?;
        if Arc::strong_count(&step.content_witnesses) != 1 {
            return Err(publication_input_error("content witnesses still retain this step"));
        }
        if step.content.is_empty() && (Arc::strong_count(&step.aliases) != 1
            || self.readers.get(&lease.token).is_some_and(|reader| Arc::strong_count(&reader.aliases) != 1)) {
            return Err(publication_input_error("published tensor aliases still retain this reader"));
        }
        ...
        let step = self.steps.get_mut(&lease.token).expect("completed content step");
        step.content.clear();
        if Arc::strong_count(&step.aliases) != 1 {
            return Err(publication_input_error("output aliases still retain this step"));
        }
        if self.readers.get(&lease.token).is_some_and(|reader| Arc::strong_count(&reader.aliases) != 1) {
            return Err(publication_input_error("published tensor aliases still retain this reader"));
        }
        Ok(())
    }
    ...
    pub fn release_published_reader(&mut self, lease: &mut SemanticPublishedLease, consumer_streams: &[u64])
        -> Result<(), SemanticTransitionError> {
        ...
        if Arc::strong_count(&reader.aliases) != 1
            || self.admitted_transition.is_some_and(|(token, _, _)| token == lease.token) {
            return Err(publication_input_error("publication reader still owns aliases or an admitted transition"));
        }
        self.complete_step_consumers(lease, consumer_streams)?;
        self.retire_published_reader(lease)
    }

    fn retire_published_reader(&mut self, lease: &mut SemanticPublishedLease) -> Result<(), SemanticTransitionError> {
        let reader = self.checked_reader(lease)?;
        if Arc::strong_count(&reader.aliases) != 1
            || (self.admitted_transition.is_some_and(|(token, _, _)| token == lease.token)
                && (self.continuation_base.is_some() || self.text_binding.is_some())) {
            return Err(publication_input_error("publication reader still owns aliases or an unfinished transition"));
        }
        let result = (|| {
            self.publication_command(3, Some(lease.token))?;
            let result = self.publication_read(self.readers[&lease.token].device.view())?[0];
            if result.status != 0 || result.active != 0 || result.instance != lease.identity.instance || result.word != lease.identity.word {
                return Err(SemanticTransitionError::PublicationRefused { status: result.status });
            }
            Ok(())
        })();
        if result.is_err() { self.poisoned = true; return result; }
        ...
        if self.admitted_transition.is_some_and(|(token, _, _)| token == lease.token) {
    ...
    pub fn release(&mut self, lease: &mut SemanticPublishedLease, consumer_streams: &[u64]) -> Result<(), SemanticTransitionError> {
        ...
        self.quiesce_step_content(lease, consumer_streams)?;
        if lease.active { self.retire_published_reader(lease)?; }
    ...
    pub fn release_prepared_step(&mut self, step: &SemanticPreparedStep, consumer_streams: &[u64])
        -> Result<(), SemanticTransitionError> {
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
        ...
        let owner = self.steps.get_mut(&step.token).expect("completed prepared step");
        ...
        if Arc::strong_count(&owner.content_witnesses) != 1 {
            return Err(publication_input_error("content witnesses still retain this prepared step"));
        }
        owner.content.clear();
        if Arc::strong_count(&owner.aliases) != 1 {
            return Err(publication_input_error("output aliases still retain this prepared step"));
        }
    ...
    pub fn quiesce_prepared_step(&mut self, step: &SemanticPreparedStep, consumer_streams: &[u64])
        -> Result<(), SemanticTransitionError> {
        self.checked_prepared_step(step, false)?;
        if !self.prepared_segment.as_ref().expect("checked prepared scope").completed {
            return Err(publication_input_error("prepared step cannot retire before actual segment completion"));
        }
        #[cfg(feature = "semantic-policy")]
        if self.policy_tapes.iter().any(|tape| tape.policy.text_binding._witness.reader_token == step.token) {
    ...
    pub fn admit_transition(&mut self, lease: &SemanticPublishedLease, kind: SemanticTransitionKind) -> Result<(), SemanticTransitionError> {
        ...
        if self.admitted_transition.is_some() || self.continuation_base.is_some() {
            return Err(publication_input_error("an admitted transition already owns the pending invocation"));
        }
        let control = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        ...
        if current.instance != lease.identity.instance || current.word != lease.identity.word {
            return Err(publication_input_error("transition requires the current acquired parent, not an older retained reader"));
        }
        if lease.header.terminal == 2 { return Err(publication_input_error("final publication admits reads, not another transition")); }
        let reserve = if kind == SemanticTransitionKind::Proposal { 2 } else { 1 };
        if lease.header.fuel < reserve { return Err(publication_input_error("acquired publication has insufficient transition fuel")); }
    ...
    pub fn bind_continuation(&mut self, lease: &SemanticPublishedLease, continuation: SemanticContinuationInput,
        witness: &SemanticTensorContentWitness, consumer_stream: u64) -> Result<(), SemanticTransitionError> {
        let mut handoff = RetainedTensorAdmission { inputs: Some(vec![continuation]),
            stream: Arc::clone(self.provider.device().inner().stream()), ready: false };
        let _ordinary = crate::cuda_graph::reserve_uncaptured_stream(&self.stream)
        ...
        self.checked_content_witness(lease, witness)?;
        dlpack_consumer_stream(consumer_stream)?;
        let (_, base_word, kind) = self.admitted_transition.filter(|(token, word, _)| *token == lease.token && *word == lease.identity.word)
            .ok_or_else(|| publication_input_error("continuation has no admitted invocation for this exact acquired reader"))?;
        if self.continuation_base.is_some() {
            return Err(publication_input_error("an unconsumed continuation already owns the pending bank"));
        }
        if !matches!(self.steps[&lease.token].content[witness.index].seals, TensorContentSeals::Captured(_)) {
            return Err(publication_input_error("continuation requires its original transient producer witness"));
        }
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let continuation = handoff.inputs.as_ref().expect("retained continuation").first().expect("one continuation");
        let tensor_count = continuation.tensors.len();
        let active_capacity = storage.contract_value.feedback_capacity.checked_add(32)
        ...
        if continuation.authority_decisions.is_empty() {
            return Err(publication_input_error("continuation lacks its actual fresh authority decision snapshot"));
        }
        let continuation = handoff.inputs.take().expect("retained continuation").pop().expect("one continuation");
        let SemanticContinuationInput { mut tensors, authority_decisions, text_rows, text_row_count,
            selected_text, active_rows, active_row_count, numerical_admissibility } = continuation;
        tensors.extend([text_rows, text_row_count, selected_text, active_rows, active_row_count, numerical_admissibility]
            .into_iter().zip(service_layouts).map(|(tensor, layout)| SemanticTensorInput {
                layout, logical_begin: 0, logical_end: 0, tensor, native_allocation: None,
            }));
        ...
        let services: [PreparedSemanticTensor; 6] = original[tensor_count..].to_vec().try_into()
            .map_err(|_| publication_input_error("continuation witness has the wrong original service roster"))?;
        let text_binding = Arc::new(TextBindingStorage { inputs: services, _witness: witness.clone(), consumer_stream,
            parent: TextBindingParent::Published { bank: (lease.identity.word & 1) as usize, header: lease.header } });
        let authority_bytes = authority_decisions.len() as u64;
        let (uploads, pending_layouts) = prepare_continuation_payloads(&storage, &original[..tensor_count], &authority_decisions)?;
        let prepared = PreparedContinuation::prepare(&self.provider, Arc::clone(&storage),
            self.checked_reader(lease)?.device.view(), Arc::clone(&text_binding),
            text_binding.continuation_inputs(kind, authority_bytes), &uploads, &pending_layouts)?;
        ...
        let result = (|| {
            for (slot, payload) in &self.publication_uploads {
                if let PublicationPayload::Metadata(bytes) = payload { if !bytes.is_empty() {
                    let mut destination = storage.allocations[*slot].view().slice(..bytes.len());
                    self.provider.htod_launch_metadata_sync_copy_into(bytes, &mut destination)
                        .map_err(|error| runtime_error("continuation fresh authority upload", error))?;
                } }
            }
    ...
    pub fn policy_support_layout(&self) -> Result<(Vec<std::ops::Range<usize>>, usize), SemanticTransitionError> {
        self.ensure_quiescent()?;
    ...
    pub fn task_observation_roots(
        ...
        if self.publication.is_some() || self.captured.is_some() {
            return Err(publication_input_error("task observation roots require a cold unpublished Session"));
        }
        let admission = self.graph.admission().ok_or(SemanticTransitionError::NotBound)?;
        spec.validate_records(admission.records())?;
        if let Some(replay) = replay {
            return replay.predecessor.graph.task_observation_roots(admission, spec.statement_records)
                .map_err(SemanticTransitionError::Semantic);
        }
        let material = self.graph.export_transition_root(self.root)
            .map_err(SemanticTransitionError::Semantic)?;
        let admission = self.graph.admission().ok_or(SemanticTransitionError::NotBound)?;
        material.task_observation_roots(admission, spec.statement_records)
            .map_err(SemanticTransitionError::Semantic)
    }
    ...
    pub fn bind_task_evaluation(
        ...
        if self.publication.is_some() {
            return Err(publication_input_error("task data is immutable after its canonical parent is initialized"));
        }
        let admission = self.graph.admission().ok_or(SemanticTransitionError::NotBound)?;
        let next_epoch = self.task_epoch.checked_add(1)
            .ok_or(SemanticTransitionError::GenerationExhausted)?;
        spec.validate_records(admission.records())?;
        let observation = spec.program.observe(Arc::clone(&self.provider))
            .map_err(|error| {
                self.poisoned = true;
                error
            })?;
        ...
        enqueue_recorded(&self.domain, &mut self.poisoned, recorder, |_| Ok::<(), XlogError>(()))?;
        wait_on_stream(&self.stream, &mut self.poisoned, &mut self.stream_waits,
            "private task adoption wait", CudaStream::synchronize)?;
    ...
    pub fn task_evaluation_spec(&self) -> Result<&SemanticTaskEvaluationSpec, SemanticTransitionError> {
        self.ensure_quiescent()?;
        let (task, _) = self.task.as_ref().ok_or(SemanticTransitionError::NotBound)?;
        Ok(task.spec())
    }
    ...
    pub fn bind_policy_dlpack(
        ...
        let mut handoff = RetainedTensorAdmission { inputs: Some(vec![text_logits, product_support, parameters]),
            stream: Arc::clone(self.provider.device().inner().stream()), ready: false };
        self.ensure_rebindable()?;
        if binding != self.binding() {
            return Err(SemanticTransitionError::CatalogueMismatch);
        ...
        let inputs = handoff.inputs.take().expect("retained policy producers").into_iter().zip(layouts)
            .map(|(tensor, layout)| SemanticTensorInput {
                layout, logical_begin: 0, logical_end: 0, tensor, native_allocation: None,
            }).collect();
        let views = prepare_semantic_tensors(&self.provider, inputs)?.into_iter()
            .map(|input| input.source.ok_or_else(|| publication_input_error("policy producer storage is empty")))
            .collect::<Result<Vec<_>, _>>()?;
        let [text_logits, product_support, parameters]: [_; 3] = views.try_into()
    ...
    #[cfg(feature = "semantic-policy")]
    fn prepare_step_policy_storage(&self, reservation: &mut GpuMemoryReservation) -> Result<(PolicyBuffers, TrackedCudaSlice<u8>,
        TrackedCudaSlice<SemanticTransitionReceipt>, TrackedCudaSlice<DeviceState>), SemanticTransitionError> {
        let buffers = self.allocate_policy_buffers_reserved(reservation)?;
        let support = reservation.alloc(self.codebooks.input_cells).map_err(|error| runtime_error("prepared support allocation", error))?;
        let receipts = reservation.alloc(COMPONENT_COUNT).map_err(|error| runtime_error("prepared receipt allocation", error))?;
        let state = reservation.alloc(1).map_err(|error| runtime_error("prepared state allocation", error))?;
    ...
    #[cfg(feature = "semantic-policy")]
    pub fn bind_prepared_policy(&mut self, step: &SemanticPreparedStep, binding: SemanticCatalogueBinding,
        text_logits: DlpackManagedTensor, product_support: DlpackManagedTensor, parameters: DlpackManagedTensor,
        witness: &SemanticTensorContentWitness, consumer_stream: u64) -> Result<(), SemanticTransitionError> {
        self.check_prepared_content_stream(step, consumer_stream)?;
        if self.prepared_transition_kind(step)? != SemanticTransitionKind::Proposal {
            return Err(publication_input_error("a recompute step does not bind a learned policy"));
        }
        if binding != self.binding() { return Err(SemanticTransitionError::CatalogueMismatch); }
        let layouts = policy_producer_layouts(self.codebooks.input_cells, self.policy_layout()?.parameter_cells)?;
        let tensors = [text_logits, product_support, parameters].into_iter().zip(layouts)
            .map(|(tensor, layout)| SemanticTensorInput { layout, logical_begin: 0, logical_end: 0, tensor, native_allocation: None }).collect();
        self.verify_prepared_tensor_content(step, witness, tensors, consumer_stream)?;
        let sources = self.steps[&step.token].content[witness.index].tensors.clone();
        let owner = self.steps.get_mut(&step.token).expect("checked original step");
        ...
        if prepared.policy.is_some() || !prepared.policy_sources.is_empty() {
            return Err(publication_input_error("prepared policy already owns its original invocation"));
        }
        let text_binding = Arc::clone(&prepared.continuation.as_ref()
            .ok_or_else(|| publication_input_error("prepared policy requires its original model continuation"))?.text_binding);
        if text_binding._witness.reader_token != step.token {
            return Err(publication_input_error("prepared policy and continuation belong to different original steps"));
        }
        let buffers = prepared.policy_buffers.take()
            .ok_or_else(|| publication_input_error("prepared policy has no unused cold allocation"))?;
        prepared.policy_sources = sources;
        prepared.policy_witness = Some(witness.clone());
        prepared.policy = Some(PolicyStorage { buffers, text_binding, replacement: None, tape_live: false });
        Ok(())
    }
    ...
    #[cfg(feature = "semantic-policy")]
    pub fn enqueue_prepared_transition(&mut self, step: &SemanticPreparedStep) -> Result<(), SemanticTransitionError> {
        self.check_prepared_content_stream(step, self.stream.cu_stream() as u64)?;
        let freeze = self.steps.get_mut(&step.token).expect("checked original step").prepared.as_mut()
            .expect("prepared owner").model_work.as_mut()
            .ok_or_else(|| publication_input_error("prepared transition requires original model work"))
            .and_then(|work| work.recording.freeze().map_err(publication_input_error));
        if let Err(error) = freeze { self.poisoned = true; return Err(error); }
        let kind = self.prepared_transition_kind(step)?;
        let owner = &self.steps[&step.token];
        let prepared = owner.prepared.as_ref().expect("prepared transition owner");
        if prepared.transition_recorded || prepared.continuation.is_none()
        ...
        let model_witnesses = owner.content.iter().enumerate().filter_map(|(index, content)|
            matches!(content.seals, TensorContentSeals::Model(_)).then(|| SemanticTensorContentWitness {
                issuer: Arc::clone(&self.publication_issuer), reader_token: step.token, index,
                _witness: Arc::clone(&owner.content_witnesses),
            })).collect::<Vec<_>>();
        if model_witnesses.is_empty() {
            return Err(publication_input_error("prepared transition has no original admitted model content"));
        }
        let policy_witness = if kind == SemanticTransitionKind::Proposal {
            Some(prepared.policy_witness.clone()
                .ok_or_else(|| publication_input_error("prepared transition has no original policy content"))?)
        } else { None };
        let continuation_witness = prepared.continuation.as_ref().expect("checked continuation").text_binding._witness.clone();
        for witness in &model_witnesses { self.record_prepared_tensor_content(step, witness, true)?; }
        if let Some(witness) = &policy_witness { self.record_prepared_tensor_content(step, witness, true)?; }
        self.record_prepared_tensor_content(step, &continuation_witness, true)?;
        let result = (|| {
            let prepared = self.steps[&step.token].prepared.as_ref().expect("retained original step");
            if let Some(policy) = &prepared.policy {
                let sources = policy_source_views(&prepared.policy_sources)?;
                enqueue_policy_snapshots(&self.domain, &mut self.poisoned, &policy.buffers,
                    prepared.support.as_ref().expect("original support bank"), &sources)?;
            }
            prepared.continuation.as_ref().expect("checked original continuation")
            ...
            enqueue_recorded(&self.domain, &mut self.poisoned, recorder, |enqueue| {
                ...
                unsafe { execute.launch_in(enqueue, LaunchConfig { grid_dim: (1, 1, 1),
                    block_dim: (256, 1, 1), shared_mem_bytes: 0 }, (descriptor,)) }
                    .map_err(|error| XlogError::Kernel(error.to_string()))
            })
        })();
        if result.is_err() { self.poisoned = true; return result; }
        let prepared = self.steps.get_mut(&step.token).expect("retained original step").prepared.as_mut()
    ...
    #[cfg(feature = "semantic-policy")]
    fn take_prepared_policy_tape(&mut self, step: &SemanticPreparedStep, invocation: SemanticRngBinding,
        refusal: Option<SemanticTransitionRefusal>) -> Result<(), SemanticTransitionError> {
        if !Arc::ptr_eq(&step.issuer, &self.publication_issuer)
            || self.policy_tapes.iter().any(|tape| tape.invocation == invocation) {
            return Err(publication_input_error("observed prepared policy differs from its original Session or invocation"));
        }
        let binding = self.binding();
        let prepared = self.steps.get_mut(&step.token).and_then(|owner| owner.prepared.as_mut())
            .ok_or_else(|| publication_input_error("observed prepared policy has no retained original step"))?;
        if !prepared.transition_recorded || prepared.policy.is_none() || prepared.support.is_none() || prepared.receipts.is_none() {
            return Err(publication_input_error("observed prepared policy was not recorded or has already retired"));
        }
        ...
        prepared.policy.as_mut().expect("checked original policy").tape_live = true;
        self.policy_tapes.push(PolicyTape {
            invocation, refusal, binding,
            ...
            components: self.device_components.view(), codebooks: self.device_codebooks.view(),
        });
        Ok(())
    }
    ...
    fn bind_policy_views(
        ...
        let layout = self.policy_layout()?;
        let text_binding = Arc::clone(self.text_binding.as_ref().filter(|_| self.continuation_base.is_some())
            .ok_or_else(|| publication_input_error("policy inputs require an admitted original model continuation"))?);
        if rng != text_binding.published_parent()?.1.rng_binding()? {
            return Err(publication_input_error("policy invocation differs from its actual acquired parent"));
        }
        let text_cells = 32 * TEXT_CARDINALITY;
        let support_cells = self.codebooks.input_cells;
        if self.admitted_transition.is_none_or(|(_, _, kind)| kind != SemanticTransitionKind::Proposal)
            || self.policy_tapes.iter().any(|tape| tape.invocation == rng) {
            return Err(publication_input_error("policy binding requires a fresh admitted proposal invocation"));
        }
        if rng.stream_serial >= (1u64 << 56)
        ...
        enqueue_policy_snapshots(&self.domain, &mut self.poisoned, &policy.buffers, &self.support,
            &PolicyProducerViews { text_logits, product_support, parameters })?;
    ...
    pub fn backward_policy_dlpack(
        &mut self, invocation: SemanticRngBinding, score_cotangents: DlpackManagedTensor,
        consumer_stream: u64,
    ) -> Result<SemanticPolicyGradients, SemanticTransitionError> {
        let mut handoff = RetainedTensorAdmission { inputs: Some(vec![score_cotangents]),
            stream: Arc::clone(self.provider.device().inner().stream()), ready: false };
        dlpack_consumer_stream(consumer_stream)?;
        let pointer = handoff.inputs.as_ref().expect("retained cotangent producer")[0].as_ptr();
        if pointer.is_null() { return Err(publication_input_error("selected-score cotangent managed owner is null")); }
        ...
        let (data, bytes) = unsafe { policy_score_cotangent_metadata(&(*pointer).dl_tensor,
            self.provider.device().ordinal() as i32) }?;
        handoff.ready = true;
        let tensor = handoff.inputs.take().expect("retained cotangent producer").pop().expect("one cotangent");
        ...
        let bytes = unsafe { CudaColumn::dlpack(data, bytes, Arc::clone(&handoff.stream), tensor) }.device_view();
        ...
        let values = unsafe { bytes.cast::<f64>() }
            .ok_or_else(|| publication_input_error("selected-score cotangent span is not aligned F64 storage"))?;
        self.backward_policy_view(invocation, values, consumer_stream)
    }

    #[cfg(feature = "semantic-policy")]
    fn checked_prepared_policy_tape(&self, step: &SemanticPreparedStep, invocation: SemanticRngBinding)
        -> Result<usize, SemanticTransitionError> {
        let owner = self.checked_prepared_step(step, false)?;
        if !self.prepared_segment.as_ref().is_some_and(|build| build.completed)
            || !owner.prepared.as_ref().is_some_and(|prepared| prepared.observed) {
            return Err(publication_input_error("prepared policy requires its actual completed native outcome"));
        }
        self.policy_tapes.iter().position(|tape| tape.invocation == invocation
            && tape.policy.text_binding._witness.reader_token == step.token)
            .ok_or_else(|| publication_input_error("prepared policy has no unconsumed original tape for this step"))
    }
    ...
    #[cfg(feature = "semantic-policy")]
    pub fn require_prepared_policy_invocation(&self, step: &SemanticPreparedStep,
        invocation: SemanticRngBinding) -> Result<(), SemanticTransitionError> {
        self.checked_prepared_policy_tape(step, invocation).map(|_| ())
    }

    /// Differentiate the original tape of this actually completed native step.
    #[cfg(feature = "semantic-policy")]
    pub fn backward_prepared_policy_dlpack(&mut self, step: &SemanticPreparedStep, invocation: SemanticRngBinding,
        score_cotangents: DlpackManagedTensor, consumer_stream: u64)
        -> Result<SemanticPolicyGradients, SemanticTransitionError> {
    ...
    #[cfg(feature = "semantic-policy")]
    pub fn finish_prepared_policy_invocation(&mut self, step: &SemanticPreparedStep, invocation: SemanticRngBinding,
    ...
    #[cfg(feature = "semantic-policy")]
    pub fn finish_policy_invocation(&mut self, lease: &SemanticPublishedLease,
        invocation: SemanticRngBinding, consumer_stream: u64) -> Result<(), SemanticTransitionError> {
        self.ensure_quiescent()?;
        self.checked_step(lease)?;
        dlpack_consumer_stream(consumer_stream)?;
        let index = self.policy_tapes.iter().position(|tape| tape.invocation == invocation)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
    ...
    #[cfg(feature = "semantic-policy")]
    fn finish_policy_tape(&mut self, index: usize, consumer_stream: u64) -> Result<(), SemanticTransitionError> {
        self.ensure_quiescent()?;
        dlpack_consumer_stream(consumer_stream)?;
        let original = Arc::clone(&self.policy_tapes[index].policy.text_binding);
        let result = (|| {
    ...
    fn backward_policy_view(
        &mut self, invocation: SemanticRngBinding, score_cotangents: DeviceMemoryView<f64>, consumer_stream: u64,
    ) -> Result<SemanticPolicyGradients, SemanticTransitionError> {
        self.ensure_quiescent()?;
        dlpack_consumer_stream(consumer_stream)?;
        let tape_index = self.policy_tapes.iter().position(|tape| tape.invocation == invocation)
            .ok_or(SemanticTransitionError::ObservationMismatch)?;
        ...
        self.guard_continuation_content(&original)?;
        if let Err(error) = self.order_step_content_inputs(original._witness.reader_token, consumer_stream) {
            self.poisoned = true;
            return Err(error);
        }
        let publication = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        ...
        let io = TransitionKernelIo {
            logits: &policy.text_logits, support: &tape.support, receipts: &tape.receipts,
            state: self.steps[&original._witness.reader_token].prepared.as_ref()
                .map_or(&self.state, |prepared| &prepared.state),
            policy: Some(policy), text: Some(&policy.text_binding), lease: None,
            model_work: None,
        };
        ...
        self.steps.get_mut(&original._witness.reader_token).expect("retained original step")
            .adjoints.extend(output_views);
        enqueue_recorded(&self.domain, &mut self.poisoned, recorder, |enqueue| {
        ...
        Ok(SemanticPolicyGradients {
            ...
            step_aliases, publication, continuation: original, support_cells,
        })
    }
    ...
    fn upload_cold_codebooks(&mut self) -> Result<(), SemanticTransitionError> {
        let upload = (|| {
            self.provider.htod_sync_copy_into_tracked(&self.codebooks.words, &mut self.device_codebooks)?;
            self.provider.htod_sync_copy_into_tracked(&self.codebooks.components, &mut self.device_components)
        })();
    ...
    fn upload_rng_state(
        ...
        let upload = self.provider.htod_sync_copy_into_tracked(&[state], &mut self.state);
        upload.map_err(|e| {
            self.poisoned = true;
    ...
    pub fn capture(&mut self) -> Result<(), SemanticTransitionError> {
        self.ensure_rebindable()?;
        if self.publication.is_some() && (self.continuation_base.is_none()
            || self.admitted_transition.is_none_or(|(_, word, _)| Some(word) != self.continuation_base)) {
            return Err(publication_input_error("publication capture requires the admitted acquired-parent continuation"));
        }
        if self.rng.is_none() {
        ...
        self.poisoned = true;
        let captured = CapturedCudaGraph::capture_on_stream_retaining(&self.stream, Vec::new(), || {
    ...
    fn retain_policy_tape(&mut self, refusal: Option<SemanticTransitionRefusal>) {
        ...
        let replacement = policy.replacement.take().expect("proposal reserved its next working banks");
        let invocation = self.rng.take().expect("observed policy retains its original invocation");
        self.captured = None;
        self.policy_tapes.push(PolicyTape {
            invocation, refusal, binding: self.binding(), policy,
    ...
    fn observe_terminal(
        ...
        if state.status == 11 {
            return Err(SemanticTransitionError::WorkCounterOverflow { completed_draws: state.blocks });
        }
        match state.status {
            7 => return Err(SemanticTransitionError::InvalidTaskBank),
            8 => return Err(SemanticTransitionError::InvalidTaskBase),
        ...
        let rng = self.rng.ok_or(SemanticTransitionError::ObservationMismatch)?;
    ...
    fn finish_observed_transition(&mut self, state: DeviceState, components: Vec<SemanticTransitionReceipt>,
        rng: SemanticRngBinding,
        published: Option<(PublicationHeader, (SemanticRootHandle, SemanticRootSnapshot), SemanticTransitionKind)>,
        policy_text: bool) -> Result<SemanticTransitionOutcome, SemanticTransitionError> {
        ...
        if let Some((header, (root, snapshot), kind)) = published {
            if kind != SemanticTransitionKind::Proposal {
                if state.status != 0 || state.blocks != 0 || state.proposal != proposal
                    || state.next_proposal != proposal || state.importance_weight != 1.0 {
                    return Err(SemanticTransitionError::ObservationMismatch);
                }
                self.root = root;
                self.base_snapshot = snapshot;
                self.next_proposal = header.proposal;
                return Ok(SemanticTransitionOutcome::Published(SemanticTransitionObservation {
                    components: Vec::new(), root_handle: root, root: snapshot,
                    next_proposal: header.proposal,
                    lanes: std::array::from_fn(|_| SemanticTransitionLane {
                        root: None, task_refusal: None, edits: [Ok(None), Ok(None)],
                        work: SemanticTransitionWork::default(),
                    }), importance_weight: state.importance_weight, task_evaluation: None,
                }));
            }
        }
        ...
        for (row, component) in components.iter().zip(&self.codebooks.components) {
            let inactive_text = policy_text && component.kind == COMPONENT_KIND_TEXT as u32 && canonical_text_null(row);
            if row.ordinal != component.ordinal
                ...
                || if inactive_text { !canonical_text_null(row) } else { row.choice >= component.cardinality }
        ...
        let task_evaluation = if let Some((binding, _)) = &self.task {
            ...
            if task.query_count < 2 || task.query_count > 6 || task.query_count % 2 != 0
                || task.winner > 2 || task.facts[task.winner as usize].eligible != 1
        ...
        let lanes = std::array::from_fn(|i| {
            ...
            let root = (task_refusal.is_none() && state.retired_roots_mask & (1 << i) == 0).then(|| self
                .graph
        ...
        Ok(SemanticTransitionOutcome::Published(SemanticTransitionObservation {
            components,
    ...
    fn reconcile_refusal(&mut self, state: &DeviceState, expected_proposal: u32)
        -> Result<SemanticTransitionRefusal, SemanticTransitionError> {
        let rng = self.rng.ok_or(SemanticTransitionError::ObservationMismatch)?;
        if rng.proposal != expected_proposal || u64::from(expected_proposal) != self.next_proposal {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
        let refusal = refused_terminal(state, rng, SemanticActionCatalogue::current().binding(), self.binding().digest)?;
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let (token, base_word, _) = self.admitted_transition.ok_or(SemanticTransitionError::ObservationMismatch)?;
        let original = Arc::clone(self.text_binding.as_ref().ok_or(SemanticTransitionError::ObservationMismatch)?);
        let reader = self.readers.get(&token).ok_or(SemanticTransitionError::ObservationMismatch)?;
        let step = self.steps.get(&token).ok_or(SemanticTransitionError::ObservationMismatch)?;
        if self.continuation_base != Some(base_word) || original._witness.reader_token != token
            ...
            || step.content.get(original._witness.index)
        ...
        let control = self.publication_read(storage.control.view())?[0];
        if lease.abi != 1 || lease.status != 0 || lease.active != 1
            || lease.instance != storage.instance || lease.word != base_word
            || lease.bank != base_word & 1 || lease.epoch != base_word >> 1
            || control.reader_counts[lease.bank as usize] == 0
            || control.abi != 1 || control.instance != storage.instance || control.word != base_word
            || control.reader_gate != 0 || control.refusal != 0 {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
        let (bank, parent_header) = original.published_parent()?;
        let header = self.read_publication_header(bank)?;
        if header != parent_header || header.abi != 1 || header.instance != storage.instance
            || header.publication_word != base_word || header.sealed_epoch != base_word >> 1
            || header.proposal != state.proposal || header.model_generation != u64::from(state.model_generation)
            || header.family_id != u64::from(state.family_id) || header.stream_serial != state.stream_serial {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
    ...
    fn observe_publication_root(&mut self, state: &DeviceState, header: &PublicationHeader, index: usize)
        -> Result<(SemanticRootHandle, SemanticRootSnapshot), SemanticTransitionError> {
        let (root, snapshot) = self.graph.observe_transition_root(state.semantic_receipts[index])
            .map_err(SemanticTransitionError::Semantic)?;
        let extents = snapshot.extents();
        if header.semantic_owner != self.graph.transition_arena()[1] || header.semantic_slot != u64::from(root.slot())
            || header.semantic_generation != root.generation() || header.semantic_digest.as_bytes() != snapshot.digest().as_bytes()
            || header.semantic_extents != [u64::from(extents.statements()), u64::from(extents.supports()), u64::from(extents.versions())] {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
        Ok((root, snapshot))
    }

    fn reconcile_publication(&mut self, state: &DeviceState, expected_proposal: u32)
        -> Result<(PublicationHeader, (SemanticRootHandle, SemanticRootSnapshot), SemanticTransitionKind), SemanticTransitionError> {
        let storage = Arc::clone(self.publication.as_ref().ok_or(SemanticTransitionError::NotBound)?);
        let (_, base_word, kind) = self.admitted_transition.ok_or(SemanticTransitionError::ObservationMismatch)?;
        let control = self.publication_read(storage.control.view())?[0];
        if control.refusal != 0 || state.status == 10 {
            return Err(SemanticTransitionError::PublicationRefused { status: control.refusal });
        }
        let epoch = (base_word >> 1).checked_add(1).filter(|&epoch| epoch <= u64::MAX >> 1)
            .ok_or(SemanticTransitionError::GenerationExhausted)?;
        let expected_word = (epoch << 1) | ((base_word ^ 1) & 1);
        if state.status != 0 || control.abi != 1 || control.instance != storage.instance
            || control.word != expected_word || self.continuation_base != Some(base_word)
            || u64::from(expected_proposal) != self.next_proposal {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
        let header = self.read_publication_header((expected_word & 1) as usize)?;
        let next_proposal = self.next_proposal.checked_add(u64::from(kind == SemanticTransitionKind::Proposal))
            .ok_or(SemanticTransitionError::GenerationExhausted)?;
        if header.abi != 1 || header.instance != control.instance || header.base_word != base_word
            || header.publication_word != expected_word || header.sealed_epoch != epoch || header.proposal != next_proposal {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
        let winner = if kind == SemanticTransitionKind::Proposal { state.task_evaluation.winner } else { 0 };
        let index = match winner { 0 => 0, 1 => 3, 2 => 6, _ => return Err(SemanticTransitionError::ObservationMismatch) };
        if winner > 0 && state.retired_roots_mask & (1 << (winner - 1)) != 0 {
            return Err(SemanticTransitionError::ObservationMismatch);
        }
    ...
    fn ensure_rebindable(&self) -> Result<(), SemanticTransitionError> {
        self.ensure_quiescent()?;
        validate_rebinding_ownership(self.prepared_segment.as_ref(),
    ...
    fn transition_kernel_io(&self) -> TransitionKernelIo<'_> {
        ...
        TransitionKernelIo {
            logits, support: &self.support, receipts: &self.receipts, state: &self.state, text,
            model_work: None,
            lease: self.admitted_transition.map(|(token, _, _)| &self.readers[&token].device),
            #[cfg(feature = "semantic-policy")]
    ...
    #[cfg(feature = "semantic-policy")]
    fn prepared_kernel_io(&self, step: &SemanticPreparedStep) -> Result<TransitionKernelIo<'_>, SemanticTransitionError> {
        let prepared = self.steps.get(&step.token).and_then(|owner| owner.prepared.as_ref())
            .ok_or_else(|| publication_input_error("prepared transition has no original storage"))?;
        let policy = prepared.policy.as_ref();
        let text = &prepared.continuation.as_ref()
            .ok_or_else(|| publication_input_error("prepared transition has no original continuation"))?.text_binding;
        let logits = match policy {
            Some(policy) => &policy.text_logits,
            None => &prepared.policy_buffers.as_ref()
                .ok_or_else(|| publication_input_error("prepared transition has no retained cold buffers"))?.text_logits,
        };
        Ok(TransitionKernelIo {
            ...
            logits,
            support: prepared.support.as_ref().ok_or_else(|| publication_input_error("prepared support has retired"))?,
            receipts: prepared.receipts.as_ref().ok_or_else(|| publication_input_error("prepared receipts have retired"))?,
            state: &prepared.state, text: Some(text), lease: Some(&prepared.reader), policy,
            model_work: prepared.model_work.as_ref(),
        })
    }

    fn descriptor(&self) -> Descriptor { self.descriptor_with(&self.transition_kernel_io()) }

    fn descriptor_with(&self, io: &TransitionKernelIo<'_>) -> Descriptor {
        #[cfg(not(feature = "semantic-policy"))]
        let policy = PolicyDescriptor::default();
        #[cfg(feature = "semantic-policy")]
        let policy = io.policy.map_or(PolicyDescriptor::default(), Self::retained_policy_descriptor);
        Descriptor {
            logits: 0,
            support: 0,
            ...
            task: self.task.as_ref().map_or(0, |(_, device)| device.device_ptr_value()),
            publication: PublicationCommand {
                control: self.publication.as_ref().map_or(0, |storage| storage.control.device_ptr_value()),
                lease: 0,
                operation: 0,
            },
            ...
            logits: io.logits.device_ptr_value(), support: io.support.device_ptr_value(),
            receipts: io.receipts.device_ptr_value(), state: io.state.device_ptr_value(),
            lease: io.lease.map_or(0, TrackedCudaSlice::device_ptr_value),
            text: io.text.map_or(TextBinding::default(), TextBindingStorage::descriptor), policy,
            model_work: io.model_work.map_or(ModelWorkInput::default(), PreparedModelWork::descriptor),
        })
    }
    ...
    fn retained_policy_descriptor(policy: &PolicyStorage) -> PolicyDescriptor {
            let parameter =
                |offset: usize| policy.parameters.device_ptr_value() + (offset * 4) as u64;
            PolicyDescriptor {
    ...
    fn validate_ranges_with(&self, io: &TransitionKernelIo<'_>) -> Result<(), SemanticTransitionError> {
        ...
        let ranges = {
            ...
            if let Some(policy) = io.policy {
                for buffer in [
                    ...
                    if !buffer.is_empty() { ranges.push((buffer.device_ptr_value(), (buffer.len() * 4) as u64)); }
                }
            }
            ranges
        };
        ...
        if let Some(text) = io.text {
            for input in &text.inputs {
                if let Some(source) = &input.source { ranges.push((input.data, source.len() as u64)); }
            }
        }
        disjoint_ranges(&ranges)
    }

    fn kernel_recorder(&self) -> LaunchRecorder { self.kernel_recorder_with(&self.transition_kernel_io()) }

    fn kernel_recorder_with(&self, io: &TransitionKernelIo<'_>) -> LaunchRecorder {
        let mut recorder = self.domain.new_strict_recorder();
        recorder.read(io.logits);
        recorder.read(io.support);
        if let Some(work) = io.model_work { recorder.read(&work.device); }
        recorder.write(&self.scratch);
        ...
        if let Some(publication) = &self.publication { publication.record(&mut recorder); }
        if let Some(lease) = io.lease { recorder.read(lease); }
        if let Some(text) = io.text { text.record(&mut recorder); }
        recorder
    }
}
...
mod tests {
    ...
    fn raw_prefix_export_retains_the_original_input_seal() {
        let range = PublicationRange { role: 1, index: 0, generation: 1, length_bytes: 64,
            logical_begin: 0, logical_end: 1, ..PublicationRange::default() };
        let input = StepInputPlan { role: 1, index: 0, layout: committed_prefix_layout(&range, 4).unwrap(),
            storage_slot: 0, allocation_bytes: 256, span: 0..256 };
        let raw = publication_record_layout(&range);
        let exported = publication_export_span(&range, &raw, 256).unwrap();
        assert_eq!(exported, 0..64);
        assert!(step_input_alias_matches(&input, &range, 4096, 256, (raw, 0, 0, 4096, exported.len())));
        assert!(!step_input_alias_matches(&input, &range, 4096, 256, (raw, 0, 0, 4097, exported.len())));
        assert!(!step_input_alias_matches(&input, &range, 4096, 256, (raw, 0, 1, 4096, exported.len())));
        assert!(!step_input_alias_matches(&input, &range, 4096, 256, (raw, 0, 0, 4096, 128)));
        assert!(step_input_alias_matches(&input, &range, 4096, 256, (input.layout, 0, 1, 4096, 256)));
        let empty = PublicationRange { length_bytes: 0, logical_end: 0, ..range };
        let raw_empty = publication_record_layout(&empty);
        assert_eq!(publication_export_span(&empty, &raw_empty, 256).unwrap(), 0..0);
        assert!(step_input_alias_matches(&input, &empty, 4096, 256, (raw_empty, 0, 0, 4096, 0)));
    }
    ...
    fn transition_descriptors_preserve_each_original_policy_and_outcome() {
        ...
        let shared = Descriptor {
            logits: 0, support: 0, scratch: 8, receipts: 0, state: 0,
            components: 16, codebooks: 24, arena: [32; 7],
            policy: PolicyDescriptor::default(), backward: PolicyBackward::default(),
            task: 40, publication: PublicationCommand { control: 48, lease: 0, operation: 0 },
            text: TextBinding::default(),
            model_work: ModelWorkInput::default(),
        };
        let original = |base| TransitionKernelPointers {
            logits: base, support: base + 8, receipts: base + 16, state: base + 24,
            lease: base + 32, text: TextBinding { rows: base + 40, count: base + 48, selected: base + 56 },
            policy: PolicyDescriptor { z: base + 64, recurrent: base + 72, ..PolicyDescriptor::default() },
            model_work: ModelWorkInput { events: base + 80, count: base + 88, bound: base + 96 },
        };
        ...
        for (descriptor, base) in [(first, 1024), (second, 2048)] {
            assert_eq!((descriptor.logits, descriptor.support, descriptor.receipts, descriptor.state),
                (base, base + 8, base + 16, base + 24));
            assert_eq!((descriptor.policy.z, descriptor.policy.recurrent, descriptor.publication.lease),
                (base + 64, base + 72, base + 32));
            assert_eq!((descriptor.text.rows, descriptor.text.count, descriptor.text.selected),
                (base + 40, base + 48, base + 56));
            assert_eq!((descriptor.model_work.events, descriptor.model_work.count, descriptor.model_work.bound),
                (base + 80, base + 88, base + 96));
            assert_eq!((descriptor.scratch, descriptor.components, descriptor.codebooks, descriptor.task),
                (8, 16, 24, 40));
    ...
    fn prepared_schedule_preserves_each_requested_mode_until_recording() {
        let issuer = Arc::new(());
        let schedule = [SemanticTransitionKind::Proposal, SemanticTransitionKind::Recompute,
            SemanticTransitionKind::Proposal];
        let mut build = PreparedSegmentState::new(Arc::clone(&issuer), vec![4, 5, 6], schedule.into_iter()).unwrap();
        ...
        assert!(PreparedSegmentState::new(Arc::clone(&issuer), vec![9], schedule.into_iter()).is_err());
        assert!(PreparedSegmentState::new(issuer, vec![9], std::iter::once(SemanticTransitionKind::Drain)).is_err());
    }

    #[test]
    fn prepared_step_scope_rejects_foreign_owners_and_out_of_order_recording() {
        let issuer = Arc::new(());
        let mut build = PreparedSegmentState::new(Arc::clone(&issuer), vec![4, 5],
            [SemanticTransitionKind::Proposal, SemanticTransitionKind::Proposal].into_iter()).unwrap();
        let steps = build.handles().unwrap();
    ...
    fn publication_role_roster_preserves_every_required_owner() {
        ...
        for code in 1..=55 { assert_eq!(SemanticStateRole::from_code(code).unwrap() as u64, code); }
    }

    #[test]
    fn prefix_snapshot_copy_respects_owned_capacity_strides() {
        let source = SemanticTensorLayout {
            role: 4, index: 0, element_bytes: 2, scalar_type: 4, rank: 4,
            logical_axis: 2, dimensions: [1, 2, 3, 4], strides_bytes: [48, 24, 8, 2],
        };
        ...
        assert_eq!(tensor_copy_plan(&source, &destination).unwrap(), [(0, 0, 24), (24, 64, 24)]);
    ...
    fn policy_score_cotangent_admission_requires_exact_fp64_metadata() {
        ...
        let mut tensor = DLTensor {
            ...
            device: DLDevice { device_type: K_DLCUDA, device_id: 0 },
            ndim: 1, dtype: DLDataType { code: K_DLFLOAT, bits: 64, lanes: 1 },
        ...
        assert_eq!(admit(&tensor).unwrap(), (4096, COMPONENT_COUNT * size_of::<f64>()));
        tensor.strides = stride.as_mut_ptr();
        tensor.byte_offset = 8;
        assert_eq!(admit(&tensor).unwrap(), (4104, COMPONENT_COUNT * size_of::<f64>()));
        tensor.byte_offset = 0;
        tensor.dtype.code = K_DLINT;
        assert!(admit(&tensor).is_err(), "Int64 cells must not be bitcast as FP64 cotangents");
        ...
        stride[0] = 2;
        assert!(matches!(admit(&tensor), Err(SemanticTransitionError::InvalidInput { .. })));
        stride[0] = 1;
        tensor.strides = stride.as_mut_ptr();
        tensor.ndim = 2;
        assert!(admit(&tensor).is_err());
    ...
    fn policy_inference_finish_retires_published_and_refused_original_owners() {
        for completion in [PolicyFixtureCompletion::Inference, PolicyFixtureCompletion::NumericalRefusal] {
    ...
    #[cfg(feature = "semantic-policy")]
    #[derive(Clone, Copy)]
    enum PolicyFixtureCompletion { Backward, Inference, NumericalRefusal }
    ...
    fn policy_dlpack_binding_fixture(completion: PolicyFixtureCompletion) {
        ...
        let domain = provider
            ...
            .unwrap();
        let mut session = super::text_parent_tests::publication_test_session(Arc::clone(&provider), &domain);
        session.bind_parent(super::text_parent_tests::publication_test_parent(&provider)).unwrap();
        let mut parent = session.acquire().unwrap();
        session.admit_transition(&parent, SemanticTransitionKind::Proposal).unwrap();
        let (continuation, witness) =
            super::text_parent_tests::publication_test_continuation_with_numerical_admissibility(
                &mut session, &parent, &provider,
                u8::from(!matches!(completion, PolicyFixtureCompletion::NumericalRefusal)));
        session.bind_continuation(&parent, continuation, &witness, 1).unwrap();
        ...
        let logits = crate::dlpack::export_slice_managed_tensor(
            Arc::new(logits.into_bytes()), provider.device().ordinal() as i32,
            crate::dlpack::DLDataType { code: 2, bits: 32, lanes: 1 },
            32, TEXT_CARDINALITY,
        ).unwrap();
        ...
        session
            ...
            .unwrap();
        assert_eq!(session.host_io_stats(), before_policy_binding,
        ...
        assert_eq!(
            read(session.policy.as_ref().unwrap().text_logits.view().slice(0..1)),
            2.0
        );
        assert_eq!(
            read(session.policy.as_ref().unwrap().parameters.view().slice(0..1)),
            3.0
        );
        assert_eq!(
            provider
        ...
        assert_eq!(session.device_components.device_ptr_value(), original_components);
        assert_eq!(session.device_codebooks.device_ptr_value(), original_codebooks);
        assert_eq!(*session.policy_tapes[0].components.device_ptr(), original_components);
        assert_eq!(*session.policy_tapes[0].codebooks.device_ptr(), original_codebooks);
        ...
        assert_eq!(session.retained_step_identity(&parent).unwrap(), parent_identity);
        assert!(session.published_identity(&parent).is_err());
        if !matches!(completion, PolicyFixtureCompletion::Backward) {
            let mut foreign_session = super::text_parent_tests::publication_test_session(
                Arc::clone(&provider), &domain);
            foreign_session.bind_parent(super::text_parent_tests::publication_test_parent(&provider)).unwrap();
            ...
            for invalid_stream in [0, 2] {
                assert!(session.finish_policy_invocation(&parent, rng, invalid_stream).is_err());
                assert_eq!(session.policy_tapes.len(), 1);
                assert_eq!(session.policy_tapes[0].invocation, rng);
                assert_eq!(Arc::strong_count(&session.steps[&parent.token].content_witnesses), witness_owners);
            }
            assert!(session.finish_policy_invocation(&foreign_parent, rng, 1).is_err());
            assert_eq!(session.policy_tapes.len(), 1);
            assert_eq!(session.policy_tapes[0].invocation, rng);
            assert_eq!(Arc::strong_count(&session.steps[&parent.token].content_witnesses), witness_owners);
            assert_eq!(session.retained_step_identity(&parent).unwrap(), parent_identity);
            ...
            assert_eq!(session.host_io_stats(), before_final_use,
                "inference final use must use original device guards and stream events only");
            let retained_witness_owners = Arc::strong_count(&session.steps[&parent.token].content_witnesses);
            assert!(retained_witness_owners > 1 && !parent.active);
            assert!(session.finish_policy_invocation(&parent, rng, 1).is_err());
            assert_eq!(Arc::strong_count(&session.steps[&parent.token].content_witnesses), retained_witness_owners);
            assert_eq!(session.retained_step_identity(&parent).unwrap(), parent_identity);
            drop(witness);
            drop((original_logit, original_support, original_parameter));
            drop(outcome);
            assert_eq!(Arc::strong_count(&session.steps[&parent.token].content_witnesses), 1);
        ...
        provider.htod_sync_copy_into_tracked(&vec![1.0f64; COMPONENT_COUNT], &mut coefficients).unwrap();
        let buffer = provider.buffer_from_columns(vec![CudaColumn::Owned(coefficients.into_bytes())],
            COMPONENT_COUNT as u64, Schema::new(vec![("cotangent".into(), ScalarType::F64)])).unwrap();
        ...
        assert_eq!(session.host_io_stats(), before_backward,
        ...
        unsafe {
            ...
            assert!(!parameter_tensor.shape.is_null() && !text_tensor.shape.is_null() && !text_tensor.strides.is_null());
            assert_eq!(std::slice::from_raw_parts(parameter_tensor.shape, 1), [layout.parameter_cells as i64]);
            assert_eq!(std::slice::from_raw_parts(text_tensor.shape, 2), [32, TEXT_CARDINALITY as i64]);
            assert_eq!(std::slice::from_raw_parts(text_tensor.strides, 2), [TEXT_CARDINALITY as i64, 1]);
            assert_eq!(parameter_tensor.data as usize as u64, parameter_pointer);
            assert_eq!(text_tensor.data as usize as u64, text_pointer);
            assert_eq!((text_tensor.dtype.code, text_tensor.dtype.bits, text_tensor.dtype.lanes), (2, 32, 1));
            assert_eq!(text_tensor.device.device_id, provider.device().ordinal() as i32);
        }
        assert!(session.steps[&parent.token].consumer_streams.contains(&1));
        assert!(session.release(&mut parent, &[1]).is_err(), "live gradient owners retain their original parent");
        drop(parameters); drop(text);
        drop(witness);
        drop((original_logit, original_support, original_parameter));
        drop(outcome);
        session.release(&mut parent, &[1]).unwrap();
...
mod text_parent_tests {
    ...
    fn model_content_snapshot_copies_original_seals_and_exact_contract_interval() {
        let directory = [
            PublicationRange { role: 18, generation: 1, length_bytes: 4,
                ..PublicationRange::default() },
            PublicationRange { role: 44, generation: 1, storage_slot: 2,
                offset_bytes: 7, length_bytes: 11, ..PublicationRange::default() },
            PublicationRange { role: 20, generation: 1, length_bytes: 4,
                ..PublicationRange::default() },
        ];
        let plan = model_content_copy_plan(&directory, &[2, 0], &[4, 4, 19]).unwrap();
        assert_eq!(plan.directory_offsets, [2 * size_of::<PublicationRange>(), 0,
        ...
        assert_eq!(changed_plan.directory_offsets, plan.directory_offsets,
    ...
    fn model_content_snapshot_refuses_incomplete_or_unowned_ranges() {
        let directory = [
            PublicationRange { role: 18, generation: 1, length_bytes: 4,
                ..PublicationRange::default() },
            PublicationRange { role: 44, generation: 1, storage_slot: 1,
                offset_bytes: 3, length_bytes: 5, ..PublicationRange::default() },
        ];
        ...
        changed = directory; changed[1].length_bytes = 0;
        assert!(plan(&changed, &[0], &[4, 8]).is_err());
        changed = directory; changed[1].generation = 2;
        assert!(plan(&changed, &[0], &[4, 8]).is_err());
        changed = directory; changed[1].index = 1;
        assert!(plan(&changed, &[0], &[4, 8]).is_err());
    ...
    fn model_content_binding_requires_the_complete_original_roster() {
        let layout = |role, index| SemanticTensorLayout { role, index, scalar_type: 6,
            element_bytes: 4, rank: 2, logical_axis: u64::MAX,
            dimensions: [2, 3, 0, 0], strides_bytes: [12, 4, 0, 0] };
        let layouts: BTreeMap<_, _> = [(18, 0), (18, 1), (19, 0), (20, 0)]
            .into_iter().map(|key| (key, layout(key.0, key.1))).collect();
        let mut directory: Vec<_> = layouts.values().map(|layout| PublicationRange {
            role: layout.role, index: layout.index, generation: 1, length_bytes: 24,
            ..PublicationRange::default()
        }).collect();
        directory.push(PublicationRange { role: 44, generation: 1, length_bytes: 8,
            ..PublicationRange::default() });
        ...
        assert!(bind(&[]).is_err());
        let mut duplicate = original.clone(); duplicate[3] = original[2];
        assert!(bind(&duplicate).is_err());
        let mut transient = original.clone(); transient[0].0.role = 0;
        assert!(bind(&transient).is_err());
        let mut extra = original.clone(); extra.push((layout(18, 2), 0, 0, 24));
        assert!(bind(&extra).is_err());
        let mut reordered = original.clone(); reordered.reverse();
        assert_eq!(bind(&reordered).unwrap(), vec![3, 2, 1, 0]);
        let without_adapter: Vec<_> = directory.iter().copied().filter(|row| row.role != 19).collect();
        let absent_adapter: Vec<_> = original.iter().copied().filter(|row| row.0.role != 19).collect();
        assert!(model_content_ranges(&without_adapter, &layouts,
            absent_adapter.iter().copied()).is_ok());
        assert!(model_content_ranges(&without_adapter, &layouts,
            original.iter().copied()).is_err());
        assert!(model_content_ranges(&directory[..4], &layouts,
            original.iter().copied()).is_err());
    }

    #[test]
    fn model_content_binding_accepts_logical_equality_not_changed_shape_or_interval() {
        let expected = SemanticTensorLayout { role: 18, index: 0, scalar_type: 6,
            element_bytes: 4, rank: 2, logical_axis: 0,
            dimensions: [2, 3, 0, 0], strides_bytes: [12, 4, 0, 0] };
        let layouts = BTreeMap::from([((18, 0), expected)]);
        let directory = [PublicationRange { role: 18, generation: 1, length_bytes: 24,
            logical_begin: 5, logical_end: 7, ..PublicationRange::default() },
            PublicationRange { role: 44, generation: 1, length_bytes: 8, ..PublicationRange::default() }];
        let bind = |layout, begin, end, bytes| model_content_ranges(&directory, &layouts,
        ...
        actual = expected; actual.dimensions = [3, 2, 0, 0]; actual.strides_bytes = [8, 4, 0, 0];
        assert!(bind(actual, 5, 8, 24).is_err());
        actual = expected; actual.scalar_type = 2;
        assert!(bind(actual, 5, 7, 24).is_err());
    }
    ...
    fn tensor_content_range_accepts_private_operands_without_publication_roles() {
        for (scalar_type, element_bytes, dtype) in [
            (7, 8, crate::dlpack::DLDataType { code: 0, bits: 64, lanes: 1 }),
            (8, 1, crate::dlpack::DLDataType { code: 6, bits: 8, lanes: 1 }),
            (6, 4, crate::dlpack::DLDataType { code: 2, bits: 32, lanes: 1 }),
        ] {
            let mut layout = SemanticTensorLayout { role: 0, index: 0, scalar_type,
                element_bytes, rank: 2, logical_axis: u64::MAX, dimensions: [2, 3, 0, 0],
                strides_bytes: [element_bytes, 2 * element_bytes, 0, 0] };
            let metadata = TensorMetadata { device_type: 2, device_id: 0, dtype,
                shape: &[2, 3], strides: Some(&[1, 2]), data: 4096, byte_offset: 0 };
            let bytes = 6 * element_bytes as usize;
            assert_eq!(validate_tensor_metadata(&layout, &metadata, 0).unwrap(), (4096, bytes));
            ...
            layout.role = 0; layout.dimensions[0] = 0;
            assert_eq!(tensor_content_range(&layout, 0, 0, 0).unwrap().length_bytes, 0);
        }
    }
    ...
    fn content_coordinates_keep_private_occurrences_in_original_roster_order() {
        ...
        for coordinates in [vec![(0, 1)], vec![(0, 0), (0, 0)],
            vec![(0, 0), (0, 2)], vec![(4, 0), (4, 0)], vec![(56, 0)]] {
    ...
    fn tensor_content_range_preserves_signed_tokens_and_boolean_masks() {
        for (scalar_type, element_bytes, dtype) in [
            ...
            let layout = SemanticTensorLayout { role: 1, index: 0, scalar_type,
                element_bytes, rank: 2, logical_axis: 1, dimensions: [1, 3, 0, 0],
                strides_bytes: [3 * element_bytes, element_bytes, 0, 0] };
            let metadata = TensorMetadata { device_type: 2, device_id: 0, dtype,
                shape: &[1, 3], strides: Some(&[3, 1]), data: 4096, byte_offset: 0 };
            assert_eq!(validate_tensor_metadata(&layout, &metadata, 0).unwrap(), (4096, 3 * element_bytes as usize));
            let range = tensor_content_range(&layout, 11, 14, 3 * element_bytes as usize).unwrap();
            assert_eq!((range.role, range.index, range.logical_begin, range.logical_end), (1, 0, 11, 14));
            assert!(tensor_content_range(&layout, 11, 13, 3 * element_bytes as usize).is_err());
            let mut wrong = metadata; wrong.dtype.code = 1;
    ...
    fn tensor_content_range_checks_transposed_empty_and_exact_byte_extents() {
        let mut layout = SemanticTensorLayout { role: 4, index: 2, scalar_type: 6,
            element_bytes: 4, rank: 2, logical_axis: u64::MAX,
            dimensions: [2, 3, 0, 0], strides_bytes: [4, 8, 0, 0] };
        assert_eq!(tensor_content_range(&layout, 0, 0, 24).unwrap().length_bytes, 24);
        ...
        layout.logical_axis = 0; layout.dimensions[0] = 0;
        assert_eq!(tensor_content_range(&layout, 9, 9, 0).unwrap().length_bytes, 0);
        assert!(tensor_content_range(&layout, 9, 8, 0).is_err());
    }

    fn publication_test_tensor(provider: &CudaKernelProvider, role: u64, value: f32) -> SemanticTensorInput {
        let bytes = value.to_le_bytes();
        let owner = allocate_publication::<u8>(provider, bytes.len()).unwrap();
        upload_publication(provider, &bytes, &owner).unwrap();
        let tensor = crate::dlpack::export_slice_managed_tensor(Arc::new(owner), provider.device().ordinal() as i32,
            crate::dlpack::DLDataType { code: 2, bits: 32, lanes: 1 }, 1, 1).unwrap();
        SemanticTensorInput { layout: SemanticTensorLayout { role, index: 0, scalar_type: 6,
            element_bytes: 4, rank: 2, logical_axis: u64::MAX,
            dimensions: [1, 1, 0, 0], strides_bytes: [4, 4, 0, 0] },
            logical_begin: 0, logical_end: 0, tensor, native_allocation: None }
    }

    pub(super) fn publication_test_session(provider: Arc<CudaKernelProvider>, domain: &ResidentExecutionDomain) -> SemanticTransitionSession {
        ...
        let mut graph = provider.allocate_semantic_hypergraph(domain,
            crate::SemanticHypergraphCapacities::try_new(4, 8, 8, 8).unwrap()).unwrap();
        let records = SemanticAdmissionRecords {
            predicates: vec![SemanticPredicateRecord { predicate: RelId(1), role: SemanticRecordRole::Statement,
                schema: Schema::new(vec![("left".into(), ScalarType::U32), ("right".into(), ScalarType::U32)])
                    .with_sort_labels(vec!["bit".into(), "bit".into()]).unwrap() }],
            records: [[1, 1], [0, 1]].into_iter().map(|values| SemanticTypedRecord {
                predicate: RelId(1), arguments: values.map(SemanticArgument::U32).to_vec(), qualifiers: vec![],
            }).collect(), supports: vec![],
        };
        graph.admit_records(graph.empty_root(), records, SemanticAdmissionLimits {
            max_records: 8, max_terms: 32, max_references: 8, max_utf8_bytes: 256,
        }).unwrap();
        let mut session = SemanticTransitionSession::from_hypergraph(graph).unwrap();
        session.bind_task_evaluation(task_binding_tests::task_spec([0, 1], vec![])).unwrap();
        session
    }

    fn publication_test_provider() -> (Arc<CudaKernelProvider>, ResidentExecutionDomain) {
        let provider = Arc::new(crate::CudaProviderBuilder::new(0, xlog_core::MemoryBudget::with_limit(512 * 1024 * 1024))
    ...
    pub(super) fn publication_test_parent(provider: &CudaKernelProvider) -> SemanticParentBinding {
        let mut counts = [1u64; 55]; counts[15] = 2;
        for role in [4, 5, 6, 7, 19, 20, 21, 22, 23, 24, 25, 51, 52, 53, 54] { counts[role - 1] = 0; }
        let mut records = Vec::new();
        let mut model_contract_layout = SemanticModelContractLayout::default();
        for role in 1..=55 {
            if is_tensor_role(role) || matches!(role, 1..=3 | 14..=16 | 30 | 31 | 33 | 43 | 47 | 48 | 55) { continue; }
            let mut bytes = format!("application record {role}").into_bytes();
            if role == 44 {
                model_contract_layout = SemanticModelContractLayout { schema_begin: 104,
                    schema_bytes: bytes.len() as u64, schema_digest_offset: 0, generation_offset: 32,
                    numerical_digest_offset: 40, identity_offset: 72 };
                let mut record = vec![0; 104]; record.extend_from_slice(&bytes); bytes = record;
            }
            records.push(SemanticStateRecord { role: SemanticStateRole::from_code(role).unwrap(), index: 0,
                capacity_bytes: 128, bytes });
        }
        let slot = |token, logical_position, committed| SemanticTextSlot { token, logical_position,
            kind: 1, provenance: 1, valid: 1, committed, recomputed: 1, provenance_record: 0 };
        let mut source = [SemanticTextSlot::default(); 32]; source[7] = slot(8, 1, 0);
        ...
        let tensor = crate::dlpack::export_slice_managed_tensor(Arc::clone(&model_owner), provider.device().ordinal() as i32,
            crate::dlpack::DLDataType { code: 2, bits: 32, lanes: 1 }, 1, 1).unwrap();
        let backing = crate::dlpack::export_slice_managed_tensor(Arc::clone(&model_owner), provider.device().ordinal() as i32,
            crate::dlpack::DLDataType { code: 1, bits: 8, lanes: 1 }, 1, 4).unwrap();
        ...
        let model_layout = SemanticTensorLayout { role: 18, index: 0, scalar_type: 6,
            element_bytes: 4, rank: 2, logical_axis: u64::MAX,
            dimensions: [1, 1, 0, 0], strides_bytes: [4, 4, 0, 0] };
        let mut tensors = (8..=13).map(|role| publication_test_tensor(provider, role, role as f32)).collect::<Vec<_>>();
        tensors.push(SemanticTensorInput { layout: model_layout, logical_begin: 0, logical_end: 0, tensor, native_allocation: None });
        let mut parent = SemanticParentBinding { recovered_instance: None, source, prefix: vec![slot(7, 0, 1)],
            ring_head: 7, provenance_records: 0, prefix_capacity: 64, feedback_capacity: 2, max_position: 262144,
            pad_token: 0, terminal_tokens: vec![100], final_intent_payload_bytes: 8,
            intent_effect: b"record result".to_vec(), intent_entry_capacity: 4, intent_payload_capacity_bytes: 256,
            model_generation: 1, policy_generation: 1, neural_generation: 1, cache_generation: 1,
            model_numerical_mode: b"native test numerical mode".to_vec(),
            model_contract_layout,
            authority_generation: 1, training_cursor: 0, training_rng: [1, 2, 3, 4], fuel: 8,
            rng: SemanticRngBinding { model_generation: 1, stream_serial: 5, family_id: 2, proposal: 0 },
            topology_identity: Identity256::from_bytes([4; 32]), table_identity: Identity256::from_bytes([5; 32]),
            role_counts: counts, records, tensors,
            model_memory: SemanticModelMemory {
                allocations: vec![SemanticTensorInput { layout: SemanticTensorLayout { role: 0, index: 0,
                    scalar_type: 1, element_bytes: 1, rank: 1, logical_axis: u64::MAX,
                    dimensions: [4, 0, 0, 0], strides_bytes: [1, 0, 0, 0] },
                    logical_begin: 0, logical_end: 0, tensor: backing, native_allocation: None }],
                storages: vec![SemanticModelStorage { allocation: 0, byte_offset: 0, span_bytes: 4 }],
                views: vec![SemanticModelView { role: 18, index: 0, storage: 0, byte_offset: 0 }],
            },
            active_layouts: vec![],
        };
        parent.bind_initial_sources(&[SemanticObservedSource { identity: "observed-input".into(), tokens: vec![7, 8],
            origin: b"complete observed fixture input".to_vec() }],
            &(0..2).map(|position| SemanticSourceMapping { source: "observed-input".into(), offset: position,
                logical_position: position }).collect::<Vec<_>>(), b"current fixture authority", 64).unwrap();
        parent
    }

    fn publication_test_tensor_pair(provider: &CudaKernelProvider, layout: SemanticTensorLayout,
        bytes: &[u8]) -> (SemanticTensorInput, SemanticTensorInput) {
        use xlog_core::{ScalarType, Schema};
        let owner = allocate_publication::<u8>(provider, bytes.len()).unwrap();
        upload_publication(provider, bytes, &owner).unwrap();
        let scalar = match layout.scalar_type { 3 => ScalarType::U64, 6 => ScalarType::F32,
            8 => ScalarType::Bool, _ => panic!("fixture tensor has an unsupported scalar type") };
        let (original, captured) = if layout.rank == 1 {
            let buffer = provider.buffer_from_columns(vec![crate::CudaColumn::Owned(owner)], layout.dimensions[0],
                Schema::new(vec![("value".into(), scalar)])).unwrap();
            ...
            let dtype = match scalar {
                ScalarType::U64 => crate::dlpack::DLDataType { code: 1, bits: 64, lanes: 1 },
                ScalarType::F32 => crate::dlpack::DLDataType { code: 2, bits: 32, lanes: 1 },
                _ => unreachable!("fixture matrices use floating point or unsigned integers"),
            };
            let owner = Arc::new(owner);
            let export = || crate::dlpack::export_slice_managed_tensor(Arc::clone(&owner),
                provider.device().ordinal() as i32, dtype, layout.dimensions[0] as usize,
                layout.dimensions[1] as usize).unwrap();
            (export(), export())
        };
        let input = |tensor| SemanticTensorInput { layout, logical_begin: 0, logical_end: 0, tensor, native_allocation: None };
        (input(original), input(captured))
    }

    pub(super) fn publication_test_continuation(session: &mut SemanticTransitionSession,
        old: &SemanticPublishedLease, provider: &CudaKernelProvider)
    ...
    pub(super) fn publication_test_continuation_with_numerical_admissibility(
        session: &mut SemanticTransitionSession, old: &SemanticPublishedLease,
        provider: &CudaKernelProvider, numerical_admissibility: u8)
        -> (SemanticContinuationInput, SemanticTensorContentWitness) {
        assert!(numerical_admissibility <= 1);
        let contract = session.publication.as_ref().unwrap().contract_value;
        let feedback = session.read_published_record_bytes(old, SemanticStateRole::RawFeedback).unwrap();
        assert_eq!(feedback.len(), contract.feedback_capacity as usize * size_of::<RawFeedbackRecord>());
        let mut filled: Vec<_> = old.source.iter().enumerate().filter(|(_, slot)| slot.valid == 1 && slot.kind == 1).collect();
        let mut masks: Vec<_> = old.source.iter().enumerate().filter(|(_, slot)| slot.valid == 1 && slot.kind == 2).collect();
        ...
        for (row, &(slot, source)) in masks.iter().enumerate() {
            text_rows[row * 2] = slot as u64; text_rows[row * 2 + 1] = source.logical_position;
        }
        ...
        for (slot, record) in feedback.chunks_exact(size_of::<RawFeedbackRecord>()).enumerate() {
            let valid = u64::from_ne_bytes(record[..8].try_into().unwrap());
            assert!(valid <= 1);
            if valid == 1 {
                append((filled.len() + slot) as u64, 32 + slot as u64,
                    contract.prefix_capacity + 32 + slot as u64, 3);
            }
        }
        for (offset, &(slot, source)) in masks.iter().enumerate() {
            append(filled.len() as u64 + contract.feedback_capacity + offset as u64,
                slot as u64, source.logical_position, 2);
        }
        let mut tensors = Vec::new();
        let mut capture = Vec::new();
        for role in 8..=13 {
            let layout = SemanticTensorLayout { role, index: 0, scalar_type: 6, element_bytes: 4,
                rank: 2, logical_axis: u64::MAX, dimensions: [1, 1, 0, 0], strides_bytes: [4, 4, 0, 0] };
            let (original, witnessed) = publication_test_tensor_pair(provider, layout, &(role as f32 + 1.0).to_ne_bytes());
            tensors.push(original); capture.push(witnessed);
        }
        let service_bytes = [publication_abi_bytes(&text_rows), publication_abi_bytes(&[masks.len() as u64]),
            vec![0u8; 32], publication_abi_bytes(&active_rows), publication_abi_bytes(&[active_count]), vec![numerical_admissibility]];
        ...
        for (layout, bytes) in layouts.into_iter().zip(service_bytes) {
            ...
            services.push(original.tensor); capture.push(witnessed);
        }
        ...
        (SemanticContinuationInput { tensors, authority_decisions: b"current fixture continuation".to_vec(),
            text_rows, text_row_count, selected_text, active_rows, active_row_count, numerical_admissibility }, witness)
    }

    fn publication_test_recompute(session: &mut SemanticTransitionSession,
        old: &SemanticPublishedLease, provider: &crate::CudaKernelProvider) {
        session.admit_transition(old, SemanticTransitionKind::Recompute).unwrap();
        let (continuation, witness) = publication_test_continuation(session, old, provider);
        session.bind_continuation(old, continuation, &witness, 1).unwrap();
        drop(witness);
        session.capture().unwrap(); session.launch().unwrap(); session.observe(0).unwrap();
    }
    ...
    fn publication_material_native_restore_preserves_acquired_prefix_feedback_and_history() {
        ...
        session.bind_parent(publication_test_parent(&provider)).unwrap();
        let mut old = session.acquire().unwrap();
        let old_prefix = session.read_published_record_bytes(&old, SemanticStateRole::PrefixSource).unwrap();
        ...
        assert_eq!(session.read_published_record_bytes(&old, SemanticStateRole::PrefixSource).unwrap(), old_prefix);
        assert_eq!(session.published_state_material(&old).unwrap(), old_material);
        assert!(session.release(&mut old, &[1]).is_err());
        drop(alias); session.release(&mut old, &[1]).unwrap();
        let bytes = session.published_state_material(&current).unwrap();
        let original = PublicationMaterial::decode(&bytes).unwrap();
        session.release(&mut current, &[]).unwrap(); drop(session);
        ...
        assert_eq!(restored.published_model_numerical_mode(&lease).unwrap(), old_mode);
        let reread = PublicationMaterial::decode(&restored.published_state_material(&lease).unwrap()).unwrap();
        for role in [1, 2, 14, 15, 27, 28, 30, 31, 32, 33, 43] {
            assert_eq!(reread.ranges.iter().find(|item| item.range.role == role).unwrap().bytes,
                original.ranges.iter().find(|item| item.range.role == role).unwrap().bytes, "role {role}");
        }
        assert_eq!(reread.graph, original.graph);
        restored.release(&mut lease, &[]).unwrap(); drop(restored);
        provider.memory().reap_pending_deallocations().unwrap();
    }
    ...
    fn publication_material_runtime_reader_rejects_completed_layout_alias_writes() {
        ...
        let original = session.read_published_record_bytes(&lease, SemanticStateRole::TensorLayout).unwrap();
        let alias = session.published_record(&lease, SemanticStateRole::TensorLayout, 0, 1).unwrap();
        ...
        for offset in [size_of::<TensorLayoutTableHeader>() + 2 * size_of::<u64>(), 2 * size_of::<u64>()] {
            ...
            assert_eq!(unsafe { sys::cuMemsetD8Async(pointer + offset as u64, original[offset] ^ 1,
                1, std::ptr::null_mut()) }, sys::cudaError_enum::CUDA_SUCCESS);
            assert!(matches!(session.published_model_numerical_mode(&lease),
                Err(SemanticTransitionError::ObservationMismatch)));
            ...
            assert_eq!(unsafe { sys::cuMemsetD8Async(pointer + offset as u64, original[offset],
                1, std::ptr::null_mut()) }, sys::cudaError_enum::CUDA_SUCCESS);
            assert_eq!(session.published_model_numerical_mode(&lease).unwrap(), mode);
        }
        drop(alias);
        session.release(&mut lease, &[1]).unwrap(); drop(session);
        provider.memory().reap_pending_deallocations().unwrap();
    }
    ...
    fn replay_boundaries_reject_mutated_shared_predecessor_weights() {
        ...
        const COMPLETED: &str = "shared replay content mutation produced a fatal CUDA completion error";
        let Some(kind) = std::env::var_os(CHILD_KIND) else {
            let qualified = concat!(module_path!(), "::replay_boundaries_reject_mutated_shared_predecessor_weights");
            ...
            for kind in ["verify", "evidence", "provenance"] {
                let output = std::process::Command::new(std::env::current_exe().unwrap())
                    ...
                    .env(CHILD_KIND, kind).output().unwrap();
                assert!(output.status.success(), "{kind} child did not verify a CUDA trap:\n{}\n{}",
        ...
        original.bind_parent(publication_test_parent(&provider)).unwrap();
        ...
        assert!(original.release_events.is_empty(), "completed export events must not wait for reader release");
        let material = SemanticReplayMaterial::decode(&predecessor_bytes, &evidence,
            old.identity.logical_digest, current.identity.logical_digest).unwrap();
        ...
        let alias = replay.published_tensor(&predecessor, SemanticStateRole::SlowWeights, 0, 1).unwrap();
        let successor_alias = replay.published_tensor(&successor, SemanticStateRole::SlowWeights, 0, 1).unwrap();
        ...
        replay.verify_replay_publication(&material, &predecessor, &successor).unwrap();
        replay.published_replay_evidence(&predecessor, &successor).unwrap();
        replay.published_replay_provenance(&predecessor, &successor).unwrap();
        ...
        assert_eq!(unsafe { sys::cuMemsetD8Async(pointer, 0, 4, std::ptr::null_mut()) }, sys::cudaError_enum::CUDA_SUCCESS);
        if kind == "verify" {
            drop(alias);
            replay.quiesce_published_reader(&predecessor, &[1]).unwrap();
            assert!(replay.verify_replay_publication(&material, &predecessor, &successor).is_err());
        } else if kind == "evidence" {
            assert!(replay.published_replay_evidence(&predecessor, &successor).is_err());
        } else {
            assert!(replay.published_replay_provenance(&predecessor, &successor).is_err());
        }
        ...
        assert!(matches!(completion, sys::cudaError_enum::CUDA_ERROR_ILLEGAL_INSTRUCTION
            | sys::cudaError_enum::CUDA_ERROR_LAUNCH_FAILED), "unexpected CUDA completion: {completion:?}");
        println!("{COMPLETED}");
        std::io::Write::flush(&mut std::io::stdout()).unwrap();
        std::process::exit(0);
    }

    fn publication_test_alias(session: &mut SemanticTransitionSession, lease: &SemanticPublishedLease,
        role: SemanticStateRole, consumer_stream: u64) -> SemanticTensorInput {
        let layout = session.publication.as_ref().unwrap().layouts[&(role as u64, 0)];
        let range = *lease.directory.iter().find(|range| range.role == role as u64 && range.index == 0).unwrap();
        let (native_allocation, backing) = session.published_tensor_allocation(lease, role, 0, consumer_stream).unwrap();
        drop(backing);
        SemanticTensorInput { layout, logical_begin: range.logical_begin, logical_end: range.logical_end,
            tensor: session.published_tensor(lease, role, 0, consumer_stream).unwrap(), native_allocation: Some(native_allocation) }
    }
    ...
    fn content_test_export(owner: Arc<TrackedCudaSlice<u8>>, device_id: i32,
        dtype: crate::dlpack::DLDataType, layout: SemanticTensorLayout,
        logical_interval: (u64, u64)) -> SemanticTensorInput {
        assert_eq!(layout.rank, 2);
        let shape = [i64::try_from(layout.dimensions[0]).unwrap(), i64::try_from(layout.dimensions[1]).unwrap()];
        let strides = [i64::try_from(layout.strides_bytes[0] / layout.element_bytes).unwrap(),
            i64::try_from(layout.strides_bytes[1] / layout.element_bytes).unwrap()];
        let backing = crate::dlpack::export_slice_managed_tensor(owner, device_id, dtype,
            usize::try_from(shape[0]).unwrap(), usize::try_from(shape[1]).unwrap()).unwrap();
        ...
        let mut export = Box::new(ContentTestExport {
            managed: crate::dlpack::DLManagedTensor { dl_tensor: crate::dlpack::DLTensor {
                data: source.data, device: source.device, ndim: 2, dtype: source.dtype,
                shape: std::ptr::null_mut(), strides: std::ptr::null_mut(), byte_offset: source.byte_offset,
            }, manager_ctx: std::ptr::null_mut(), deleter: Some(delete_content_test_export) },
            _owner: backing, shape, strides,
        });
        export.managed.dl_tensor.shape = export.shape.as_mut_ptr();
        export.managed.dl_tensor.strides = export.strides.as_mut_ptr();
        let owner = Box::into_raw(export);
        ...
        SemanticTensorInput { layout, logical_begin: logical_interval.0,
            logical_end: logical_interval.1, tensor, native_allocation: None }
    }
    ...
    fn native_feedback_alias_capture_reuses_original_producer_seals() {
        ...
        session.bind_parent(publication_test_parent(&provider)).unwrap();
        let mut lease = session.acquire().unwrap();
        session.admit_transition(&lease, SemanticTransitionKind::Proposal).unwrap();
        let feedback = session.published_feedback(&lease, 1).unwrap();
        let layouts = session.steps[&lease.token].feedback[0].content.tensors.iter()
            .map(|tensor| tensor.layout).collect::<Vec<_>>();
        let inputs = feedback.into_dlpack().into_iter().zip(layouts.iter().copied())
            .map(|(tensor, layout)| SemanticTensorInput {
                tensor, layout, logical_begin: 0, logical_end: 0, native_allocation: None,
            }).collect();
        let mut other = session.acquire().unwrap();
        assert!(session.capture_tensor_content(&other, inputs, 1).is_err(),
            "a second reader must not reseal the first reader's native outputs");
        session.release(&mut other, &[1]).unwrap();
        let feedback = session.published_feedback(&lease, 1).unwrap();
        let inputs = feedback.into_dlpack().into_iter().zip(layouts)
            .map(|(tensor, layout)| SemanticTensorInput {
                tensor, layout, logical_begin: 0, logical_end: 0, native_allocation: None,
            }).collect();
        let witness = session.capture_tensor_content(&lease, inputs, 1).unwrap();
        let reader = &session.steps[&lease.token];
        let TensorContentSeals::Captured(original) = &reader.feedback[1].content.seals else { panic!("native producer seals"); };
        let TensorContentSeals::Captured(consumer) = &reader.content[witness.index].seals else { panic!("original alias seals"); };
        assert_eq!(original.len(), 6);
        for (original, consumer) in original.iter().zip(consumer) {
            let CapturedTensorDigest::Tensor { cells: original, .. } = original else { panic!("feedback producer digest"); };
            let CapturedTensorDigest::Tensor { cells: consumer, producer_sealed, .. } = consumer else { panic!("feedback alias digest"); };
            assert!(Arc::ptr_eq(original, consumer), "alias capture must not allocate a replacement digest");
            assert!(*producer_sealed, "alias capture must verify, never overwrite the producer seal");
        }
        drop(witness);
    ...
    fn retained_model_content_survives_reader_retirement_and_bank_reuse() {
        ...
        let producer = || content_test_export(Arc::clone(&live), provider.device().ordinal() as i32,
            crate::dlpack::DLDataType { code: 2, bits: 32, lanes: 1 }, layout, (0, 0));
        let witness = session.bind_model_content(&lease, vec![producer()], 1).unwrap();
        session.admit_transition(&lease, SemanticTransitionKind::Recompute).unwrap();
        let feedback = session.published_feedback(&lease, 1).unwrap();
        assert!(session.release_published_reader(&mut lease, &[1]).is_err());
        let (continuation, continuation_witness) = publication_test_continuation(&mut session, &lease, &provider);
        session.bind_continuation(&lease, continuation, &continuation_witness, 1).unwrap();
        ...
        let before = session.host_io_stats();
        session.verify_tensor_content(&lease, &witness, vec![producer()], 1).unwrap();
        assert_eq!(session.host_io_stats(), before);
        drop(witness);
        assert!(session.release(&mut lease, &[1]).is_err(), "original feedback exports retain their step");
        drop(feedback);
        session.release(&mut lease, &[1]).unwrap();
    ...
    fn model_content_binding_uses_original_seals_and_retires_each_reader() {
        ...
        for _ in 0..2 {
            ...
            let same = producer();
            let substituted = publication_test_alias(&mut session, &lease, SemanticStateRole::SlowWeights, 1);
            let before = session.host_io_stats();
            let witness = session.bind_model_content(&lease, vec![original], 1).unwrap();
            assert!(matches!(session.steps[&lease.token].content[witness.index].seals,
                TensorContentSeals::Model(_)), "model binding must not capture a replacement baseline");
            session.verify_tensor_content(&lease, &witness, vec![same], 1).unwrap();
            assert_eq!(session.host_io_stats(), before);
            assert!(session.verify_tensor_content(&lease, &witness, vec![substituted], 1).is_err(),
                "equal publication bytes cannot replace this invocation's original live storage");
            assert!(session.release(&mut lease, &[1]).is_err());
            drop(witness);
            session.release(&mut lease, &[1]).unwrap();
            assert!(session.readers.is_empty(), "the first reader is not a permanent model baseline");
        }
        drop(session);
        provider.memory().reap_pending_deallocations().unwrap();
        assert_eq!(provider.memory().allocated_bytes(), 0);
    ...
    fn publication_content_guards_preserve_zero_transfer_and_release_parent_alias_witnesses() {
        ...
        let sealed_prefix = *lease.directory.iter().find(|range| range.role == 1).unwrap();
        ...
        assert!(matches!(session.published_model_context(&lease, 2),
            Err(SemanticTransitionError::InvalidInput { .. })));
        let outside_alias = session.published_tensor(&lease, SemanticStateRole::SlowWeights, 0, 1).unwrap();
        assert_eq!(session.host_io_stats(), before_exports,
        ...
        assert_eq!(unsafe { std::slice::from_raw_parts(prefix_tensor.shape, 2) }, [64, 8]);
        assert_eq!((prefix_tensor.dtype.code, prefix_tensor.dtype.bits), (1, 64));
        let storage = session.publication.as_ref().unwrap();
        let bank = storage.banks[(lease.identity.word & 1) as usize].device_ptr_value();
        for (tensor, field) in [(&source, PublicationBankField::Source),
            ...
            assert_eq!(tensor.data as usize as u64 + tensor.byte_offset, bank + range.start as u64);
            assert_eq!((tensor.dtype.code, tensor.dtype.bits), (1, 64));
            assert_eq!(tensor.ndim as usize, shape.len());
            assert_eq!(unsafe { std::slice::from_raw_parts(tensor.shape, shape.len()) }, shape);
            assert_eq!(unsafe { std::slice::from_raw_parts(tensor.strides, strides.len()) }, strides);
        }
        let prefix_range = lease.directory.iter().find(|range| range.role == 3).unwrap();
        let identity_tensor = unsafe { &(*context.prefix.0.as_ptr()).dl_tensor };
        assert_eq!(identity_tensor.data as usize as u64 + identity_tensor.byte_offset,
            storage.allocations[prefix_range.storage_slot as usize].device_ptr_value() + prefix_range.offset_bytes);
        assert_eq!((identity_tensor.ndim, identity_tensor.dtype.code, identity_tensor.dtype.bits), (1, 1, 8));
        ...
        let after_prefix = lease.directory.iter().find(|range| range.role == 1).unwrap();
        assert_eq!((after_prefix.logical_begin, after_prefix.logical_end, after_prefix.length_bytes, after_prefix.digest),
            (sealed_prefix.logical_begin, sealed_prefix.logical_end, sealed_prefix.length_bytes, sealed_prefix.digest));
        drop(prefix);
        drop(source);
        drop(context);
        ...
        let mut captured = vec![publication_test_alias(&mut session, &lease, SemanticStateRole::SlowWeights, 1)];
        let mut verified = vec![publication_test_alias(&mut session, &lease, SemanticStateRole::SlowWeights, 1)];
        let mut owners = Vec::new();
        let cases = [
            (SemanticTensorLayout { role: 0, index: 0,
                scalar_type: 7, element_bytes: 8, rank: 2, logical_axis: u64::MAX,
                dimensions: [1, 3, 0, 0], strides_bytes: [24, 8, 0, 0] },
                crate::dlpack::DLDataType { code: crate::dlpack::K_DLINT, bits: 64, lanes: 1 },
                [i64::MIN, -7, i64::MAX].into_iter().flat_map(i64::to_le_bytes).collect::<Vec<_>>(), (0, 0)),
            (SemanticTensorLayout { role: 0, index: 0,
                scalar_type: 8, element_bytes: 1, rank: 2, logical_axis: u64::MAX,
                dimensions: [1, 3, 0, 0], strides_bytes: [3, 1, 0, 0] },
                crate::dlpack::DLDataType { code: crate::dlpack::K_DLBOOL, bits: 8, lanes: 1 },
                vec![0, 1, 1], (0, 0)),
            (SemanticTensorLayout { role: 0, index: 0,
                scalar_type: 6, element_bytes: 4, rank: 2, logical_axis: u64::MAX,
                dimensions: [2, 3, 0, 0], strides_bytes: [4, 8, 0, 0] },
                crate::dlpack::DLDataType { code: crate::dlpack::K_DLFLOAT, bits: 32, lanes: 1 },
                [1.0f32, 4.0, 2.0, 5.0, 3.0, 6.0].into_iter().flat_map(f32::to_le_bytes).collect(), (0, 0)),
            (SemanticTensorLayout { role: 0, index: 0,
                scalar_type: 6, element_bytes: 4, rank: 2, logical_axis: u64::MAX,
                dimensions: [0, 3, 0, 0], strides_bytes: [12, 4, 0, 0] },
                crate::dlpack::DLDataType { code: crate::dlpack::K_DLFLOAT, bits: 32, lanes: 1 },
                Vec::new(), (0, 0)),
        ];
        for (mut layout, dtype, bytes, interval) in cases {
            ...
            if !bytes.is_empty() { upload_publication(&provider, &bytes, &owner).unwrap(); }
            ...
            drop(admission);
            captured.push(content_test_export(Arc::clone(&owner), device, dtype, layout, interval));
            verified.push(content_test_export(Arc::clone(&owner), device, dtype, layout, interval));
            owners.push(owner);
        }
        let before = session.host_io_stats();
        session.guard_published_content(&lease,
            &[(SemanticStateRole::PrefixSource, 0), (SemanticStateRole::SlowWeights, 0)], 1).unwrap();
        let witness = session.capture_tensor_content(&lease, captured, 1).unwrap();
        session.verify_tensor_content(&lease, &witness, verified, 1).unwrap();
        assert_eq!(session.host_io_stats(), before,
        ...
        assert_eq!(unsafe { sys::cuStreamSynchronize(std::ptr::null_mut()) }, sys::cudaError_enum::CUDA_SUCCESS);
        assert!(session.release(&mut lease, &[1]).is_err(), "a live witness retains its reader");
        assert!(session.quiesce_published_reader(&lease, &[1]).is_err());
        drop(witness);
        assert!(session.release(&mut lease, &[1]).is_err(), "an external alias still retains its reader");
        assert!(session.quiesce_published_reader(&lease, &[1]).is_err());
        drop(outside_alias);
        let identity = session.published_identity(&lease).unwrap();
        let publication = Arc::clone(session.publication.as_ref().unwrap());
        let readers = session.publication_read(publication.control.view()).unwrap()[0].reader_counts;
        session.quiesce_published_reader(&lease, &[1]).unwrap();
        assert!(session.release_events.is_empty(), "quiescence must retire completed consumer events");
        assert_eq!(session.published_identity(&lease).unwrap(), identity);
        assert!(lease.active);
        assert_eq!(session.publication_read(publication.control.view()).unwrap()[0].reader_counts, readers,
    ...
    fn mutated_content_stops_the_dependent_cuda_consumer() {
        ...
        let Some(kind) = std::env::var_os(CHILD_KIND) else {
            let qualified = concat!(module_path!(), "::mutated_content_stops_the_dependent_cuda_consumer");
            let test_name = qualified.split_once("::").unwrap().1;
        ...
        let alias = session.published_tensor(&lease, SemanticStateRole::SlowWeights, 0, 1).unwrap();
        let captured = publication_test_alias(&mut session, &lease, SemanticStateRole::SlowWeights, 1);
        let unchanged = publication_test_alias(&mut session, &lease, SemanticStateRole::SlowWeights, 1);
        let changed = publication_test_alias(&mut session, &lease, SemanticStateRole::SlowWeights, 1);
        let dependent = allocate_publication::<u8>(&provider, 4).unwrap();
        session.guard_published_content(&lease, &[(SemanticStateRole::SlowWeights, 0)], 1).unwrap();
        let witness = session.capture_tensor_content(&lease, vec![captured], 1).unwrap();
        session.verify_tensor_content(&lease, &witness, vec![unchanged], 1).unwrap();
        ...
        if let Err(error) = guarded { eprintln!("content guard handoff: {error}"); }
        ...
        let copied = unsafe { sys::cuMemcpyDtoDAsync_v2(dependent.device_ptr_value(), pointer, 4, std::ptr::null_mut()) };
        assert!(matches!(copied, sys::cudaError_enum::CUDA_SUCCESS
            | sys::cudaError_enum::CUDA_ERROR_ILLEGAL_INSTRUCTION | sys::cudaError_enum::CUDA_ERROR_LAUNCH_FAILED));
    ...
    fn aborted_session_retains_step_after_reader_retirement() {
        ...
        const COMPLETED: &str = "aborted retained step kept its original owners without a bank reader";
        if std::env::var_os(CHILD).is_none() {
            let qualified = concat!(module_path!(), "::aborted_session_retains_step_after_reader_retirement");
            let test_name = qualified.split_once("::").unwrap().1;
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                .args(["--exact", test_name, "--ignored", "--nocapture", "--test-threads=1"])
                .env(CHILD, "1").output().unwrap();
            assert!(output.status.success(), "retained-step abort child failed:\n{}\n{}",
                String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr));
            assert!(String::from_utf8_lossy(&output.stdout).contains(COMPLETED));
            return;
        }
        ...
        let before = session.host_io_stats();
        session.abort();
        assert!(matches!(session.retained_step_identity(&lease), Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.verify_tensor_content(&lease, &witness, vec![producer()], 1),
            Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.release(&mut lease, &[1]), Err(SemanticTransitionError::Poisoned)));
        assert_eq!(session.host_io_stats(), before);
        ...
        UNRELEASED_PUBLICATIONS.with(|owners| {
            ...
            assert!(matches!(step.content[witness.index].seals, TensorContentSeals::Model(_)));
            assert_eq!(step.content[witness.index].tensors[0].data, live.device_ptr_value());
        });
        println!("{COMPLETED}");
        std::io::Write::flush(&mut std::io::stdout()).unwrap();
        std::process::exit(0);
    ...
    fn aborted_session_refuses_existing_publication_and_witness_owners() {
        ...
        if std::env::var_os(CHILD).is_none() {
            let qualified = concat!(module_path!(), "::aborted_session_refuses_existing_publication_and_witness_owners");
            let test_name = qualified.split_once("::").unwrap().1;
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                .args(["--exact", test_name, "--ignored", "--nocapture", "--test-threads=1"])
                .env(CHILD, "1").output().unwrap();
            assert!(output.status.success(), "abort child did not verify unusable retained owners:\n{}\n{}",
                String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr));
            assert!(String::from_utf8_lossy(&output.stdout).contains(COMPLETED),
        ...
        session.bind_parent(publication_test_parent(&provider)).unwrap();
        let lease = session.acquire().unwrap();
        let _outside_alias = session.published_tensor(&lease, SemanticStateRole::SlowWeights, 0, 1).unwrap();
        let captured = publication_test_alias(&mut session, &lease, SemanticStateRole::SlowWeights, 1);
        let unchanged = publication_test_alias(&mut session, &lease, SemanticStateRole::SlowWeights, 1);
        let refused_capture = publication_test_alias(&mut session, &lease, SemanticStateRole::SlowWeights, 1);
        let refused_verify = publication_test_alias(&mut session, &lease, SemanticStateRole::SlowWeights, 1);
        let witness = session.capture_tensor_content(&lease, vec![captured], 1).unwrap();
        session.verify_tensor_content(&lease, &witness, vec![unchanged], 1).unwrap();
        let identity = session.published_identity(&lease).unwrap();
        ...
        session.abort();
        assert!(session.is_poisoned());
        assert_eq!(session.host_io_stats(), before, "abort must not synchronize or transfer retained content");
        assert!(session.publication.is_some(), "abort must retain live publication storage");
        assert!(session.readers.contains_key(&lease.token), "abort must retain existing reader owners");
        assert_eq!(lease.identity, identity, "an aborted Session retains its lease's historical identity");
        assert!(matches!(session.acquire(), Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.published_identity(&lease), Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.published_range_keys(&lease), Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.published_tensor(&lease, SemanticStateRole::SlowWeights, 0, 1),
            Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.published_prefix(&lease, 1), Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.guard_published_content(&lease, &[(SemanticStateRole::SlowWeights, 0)], 1),
            Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.capture_tensor_content(&lease, vec![refused_capture], 1),
            Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.verify_tensor_content(&lease, &witness, vec![refused_verify], 1),
            Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.capture(), Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.launch(), Err(SemanticTransitionError::Poisoned)));
        assert!(matches!(session.bind_task_evaluation(task_binding_tests::task_spec([0, 1], vec![])),
            Err(SemanticTransitionError::Poisoned)));
        assert!(session.is_poisoned());
        println!("{COMPLETED}");
        std::io::Write::flush(&mut std::io::stdout()).unwrap();
    ...
    fn publication_material_sample() -> PublicationMaterial {
        let mut counts = [1u64; 55]; counts[15] = 2;
        let layouts: BTreeMap<_, _> = (1..=55).filter(|&role| is_tensor_role(role)).map(|role| {
            ((role, 0), SemanticTensorLayout { role, index: 0, scalar_type: 6,
                element_bytes: 4, rank: 2, logical_axis: u64::MAX,
                dimensions: [1, 1, 0, 0], strides_bytes: [4, 4, 0, 0] })
        }).collect();
        let mut ranges = Vec::new();
        for (ordinal, &count) in counts.iter().enumerate() {
            for index in 0..count {
                let role = ordinal as u64 + 1;
                let bytes = if role == 55 { tensor_table_bytes(&layouts.values().copied().collect::<Vec<_>>(), false, &[]).unwrap() }
                    else if role == 44 { let mut bytes = vec![0; 104]; bytes.extend_from_slice(&[44, index as u8, 0x80, 0xff]); bytes }
                    else { vec![role as u8, index as u8, 0x80, 0xff] };
                ranges.push(PublicationMaterialRange {
                    range: PublicationRange { role, index, generation: 1, length_bytes: bytes.len() as u64,
                        storage_slot: ranges.len() as u64, ..PublicationRange::default() },
                    capacity: bytes.len() + 128, bytes,
                });
            }
        }
        let header = PublicationHeader { abi: 1, model_generation: 7, authority_generation: 3,
            sealed_epoch: 8, publication_word: 16, range_count: ranges.len() as u64,
            state_digest: Identity256::from_bytes([29; 32]), ..PublicationHeader::default() };
        let mut model_memory = ModelMemoryGeometry::default();
        let mut model_allocations = Vec::new();
        for range in ranges.iter_mut().filter(|range| matches!(range.range.role, 18..=25)) {
            ...
            model_memory.storages.push(SemanticModelStorage { allocation: ordinal, byte_offset: 0, span_bytes: bytes.len() as u64 });
            model_memory.views.push(SemanticModelView { role: range.range.role, index: range.range.index,
                storage: ordinal, byte_offset: 0 });
            model_allocations.push(bytes);
        }
        let mut material = PublicationMaterial {
            bank: PublicationBank { header, source: [SemanticTextSlot::default(); 32],
                state: DeviceState::default(), receipts: [SemanticTransitionReceipt::default(); COMPONENT_COUNT] },
            contract: PublicationContract { abi: 1, model_generation: 7, authority_generation: 3,
                window_capacity: 32, role_count: 55, range_capacity: ranges.len() as u64, terminal_token_count: 1,
                model_contract_layout: SemanticModelContractLayout { schema_begin: 104, schema_bytes: 4,
                    schema_digest_offset: 0, generation_offset: 32, numerical_digest_offset: 40, identity_offset: 72 },
                ..PublicationContract::default() },
            role_counts: counts, terminals: vec![3], layouts, model_memory, model_allocations, ranges,
            graph: SemanticRootMaterial { records: SemanticAdmissionRecords { predicates: vec![], records: vec![], supports: vec![] },
                symbols: vec![], insertions: vec![], digest: [0; 32], extents: [0; 3],
                admission_base_digest: [0; 32], admission_base_extents: [0; 3] },
        };
        publication_material_sample_runtime(&mut material);
        for range in &mut material.ranges {
            if !is_tensor_role(range.range.role) { range.range.digest = range.original_record_digest(); }
        }
    ...
    fn publication_material_sample_runtime(material: &mut PublicationMaterial) {
        material.bank.header.model_geometry_digest = material.model_memory.digest(&material.layouts).unwrap();
        let runtime = initial_runtime_contract_record(material.contract, &material.role_counts,
            &material.terminals, &material.layouts, material.ranges.iter().filter(|item| item.range.role != 43)
                .map(|item| (item.range.role, item.range.index, item.capacity)).collect(),
            b"codec sample numerical mode", &material.model_memory).unwrap();
        let record = material.ranges.iter_mut().find(|item| item.range.role == 43).unwrap();
    ...
    fn publication_material_codec_preserves_all_ranges_and_rejects_incomplete_content() {
        ...
        for boundary in [0, 25, 26, 29, 30, 61, 62, 100, bytes.len()/2, bytes.len()-1] {
        ...
        changed = bytes.clone(); changed[30] ^= 1;
        assert!(PublicationMaterial::decode(&changed).is_err());
        let mut incomplete = publication_material_sample(); incomplete.ranges.pop();
        assert!(incomplete.encode().is_err());
        let mut reordered = publication_material_sample(); reordered.ranges.swap(0, 1);
        assert!(reordered.encode().is_err());
        let mut address = publication_material_sample(); address.contract.terminal_tokens = 0x1000;
        assert!(address.encode().is_err());
        let mut mismatched = publication_material_sample(); mismatched.layouts.get_mut(&(4, 0)).unwrap().scalar_type = 2;
    ...
    fn publication_material_runtime_preserves_mode_and_exact_self_capacity() {
        ...
        assert_eq!(restored.model_numerical_mode().unwrap(), b"codec sample numerical mode");
        let record = restored.ranges.iter().find(|item| item.range.role == 43).unwrap();
        assert_eq!(record.capacity, record.bytes.len());
        assert_eq!(restored.encode().unwrap(), bytes);
        let capacities = material.ranges.iter().map(|item| (item.range.role, item.range.index, item.capacity)).collect::<Vec<_>>();
        ...
        let original = runtime_contract_bytes(relocated, &material.role_counts, &material.terminals,
            &material.layouts, &capacities, material.model_numerical_mode().unwrap(), &material.model_memory).unwrap();
        assert_eq!(original, record.bytes, "runtime addresses and owner handles are not portable identity");
        assert_ne!(runtime_contract_bytes(relocated, &material.role_counts, &material.terminals,
            &material.layouts, &capacities, b"different complete mode", &material.model_memory).unwrap(), original);
        assert!(runtime_contract_bytes(relocated, &material.role_counts, &material.terminals,
            &material.layouts, &capacities, b"", &material.model_memory).is_err());
        assert!(initial_runtime_contract_record(relocated, &material.role_counts, &material.terminals,
            &material.layouts, capacities, b"mode", &material.model_memory).is_err(), "caller runtime entry cannot be inserted twice");
    }
    ...
    fn publication_material_runtime_binds_logical_contract_and_terminals() {
        ...
        assert!(changed.encode().is_err(), "runtime record must bind the actual position contract");
        let mut changed = publication_material_sample();
        changed.terminals[0] += 1;
        assert!(changed.encode().is_err(), "runtime record must retain actual terminal values");
    }

    #[test]
    fn publication_material_runtime_binds_allocation_capacity() {
        let mut changed = publication_material_sample();
        changed.ranges.iter_mut().find(|range| range.range.role == 2).unwrap().capacity += 1;
        assert!(changed.encode().is_err(), "runtime record must bind actual cold capacity");
    }

    #[test]
    fn publication_material_runtime_rejects_changed_original_layout_table() {
        let material = publication_material_sample();
        assert_eq!(material.model_numerical_mode().unwrap(), b"codec sample numerical mode");
        let table_index = material.ranges.iter().position(|item| item.range.role == 55).unwrap();
        for byte in 0..material.ranges[table_index].bytes.len() {
            let mut changed = publication_material_sample();
            changed.ranges[table_index].bytes[byte] ^= 1;
            assert!(changed.model_numerical_mode().is_err(), "changed table byte {byte}");
        }
    }
    ...
    fn publication_material_runtime_rejects_resealed_layout_disagreement() {
        let mut changed = publication_material_sample();
        let table = changed.ranges.iter_mut().find(|item| item.range.role == 55).unwrap();
    ...
    fn publication_material_runtime_refuses_opaque_replacement_even_if_resealed() {
        let mut changed = publication_material_sample();
        let runtime = changed.ranges.iter_mut().find(|range| range.range.role == 43).unwrap();
        ...
        assert!(changed.encode().is_err(), "opaque caller bytes are not the native runtime contract");
    }

    #[test]
    fn capacity_prefix_export_preserves_extent_without_narrowing_shape() {
        let mut range = PublicationRange { role: 1, index: 0, generation: 1,
            length_bytes: 128, logical_begin: 0, logical_end: 2,
            ..PublicationRange::default() };
        ...
        assert_eq!((range.logical_begin, range.logical_end, range.length_bytes), (0, 2, 128));
        range.logical_end = 0;
        range.length_bytes = 0;
        assert_eq!(committed_prefix_layout(&range, 8).unwrap().dimensions, [8, 8, 0, 0]);
        assert!(committed_prefix_layout(&PublicationRange {
            logical_end: 9, length_bytes: 576, ..range
        }, 8).is_err());
    }
    ...
    fn capacity_export_bounds_keep_seal_and_allocation_limits_distinct() {
        let range = PublicationRange { role: 1, index: 0, generation: 1,
            offset_bytes: 64, length_bytes: 128, logical_begin: 0, logical_end: 2,
            ..PublicationRange::default() };
        let layout = SemanticTensorLayout { role: 1, index: 0, scalar_type: 3,
            element_bytes: 8, rank: 2, logical_axis: 0,
            dimensions: [8, 8, 0, 0], strides_bytes: [64, 8, 0, 0] };
        assert_eq!(publication_export_span(&range, &layout, 576).unwrap(), 64..576);
        assert!(publication_export_span(&range, &layout, 575).is_err());
        assert_eq!((range.length_bytes, range.logical_begin, range.logical_end), (128, 0, 2));
        for wrong in [
            PublicationRange { generation: 2, ..range },
            PublicationRange { offset_bytes: 65, ..range },
            PublicationRange { offset_bytes: u64::MAX - 7, ..range },
            PublicationRange { length_bytes: 513, ..range },
            PublicationRange { logical_end: 9, ..range },
            PublicationRange { index: 1, ..range },
        ] {
            assert!(publication_export_span(&wrong, &layout, 576).is_err());
        }
        let tensor = SemanticTensorLayout { role: 4, rank: 4, logical_axis: 2,
            dimensions: [1, 2, 8, 2], strides_bytes: [128, 64, 8, 4],
            scalar_type: 6, element_bytes: 4, ..layout };
        let tensor_range = PublicationRange { role: 4, offset_bytes: 0,
            length_bytes: 128, ..range };
        assert_eq!(publication_export_span(&tensor_range, &tensor, 128).unwrap(), 0..128);
        let model = SemanticTensorLayout { role: 18, logical_axis: u64::MAX, ..tensor };
        assert_eq!(publication_export_span(&PublicationRange { role: 18,
            logical_begin: 0, logical_end: 0, ..tensor_range }, &model, 128).unwrap(), 0..128);
        assert!(publication_export_span(&PublicationRange { role: 18, length_bytes: 16,
            logical_begin: 0, logical_end: 0, ..tensor_range }, &model, 128).is_err());
    }

    #[test]
    fn committed_prefix_export_layout_preserves_native_rows_and_empty_extent() {
        let range = PublicationRange {
            role: SemanticStateRole::PrefixSource as u64, index: 0, generation: 1,
            offset_bytes: 64, length_bytes: 128, logical_begin: 0, logical_end: 2,
            ..PublicationRange::default()
        };
        let layout = committed_prefix_layout(&range, 2).unwrap();
        assert_eq!((layout.scalar_type, layout.element_bytes, layout.rank), (3, 8, 2));
        ...
        let row = SemanticTextSlot { token: 71, logical_position: 1, kind: 1,
            provenance: 2, valid: 1, committed: 1, recomputed: 1, provenance_record: 19 };
        let words = publication_abi_bytes(&[row]).chunks_exact(8)
            .map(|bytes| u64::from_ne_bytes(bytes.try_into().unwrap())).collect::<Vec<_>>();
        assert_eq!(words, [71, 1, 1, 2, 1, 1, 1, 19]);
        let empty = committed_prefix_layout(&PublicationRange {
            length_bytes: 0, logical_end: 0, ..range
        }, 0).unwrap();
    ...
    fn committed_prefix_export_rejects_inconsistent_or_unaligned_extent() {
        let valid = PublicationRange { role: 1, index: 0, generation: 1,
            length_bytes: 128, logical_begin: 0, logical_end: 2,
            ..PublicationRange::default() };
        for wrong in [
            PublicationRange { role: 2, ..valid },
            PublicationRange { index: 1, ..valid },
            PublicationRange { offset_bytes: 1, ..valid },
            PublicationRange { logical_begin: 1, ..valid },
            PublicationRange { logical_end: 3, ..valid },
            PublicationRange { length_bytes: 64, ..valid },
            PublicationRange { length_bytes: 192, ..valid },
        ] {
    ...
    fn publication_material_decode_checks_original_descriptor_before_relocation() {
        ...
        for field in 0..4 {
            ...
            assert!(PublicationMaterial::decode(&changed.encode().unwrap()).is_err(),
    ...
    fn publication_material_decode_checks_original_record_bytes() {
        ...
        for role in [2, 14, 15, 30, 33, 48] {
            let mut changed = publication_material_sample();
            let record = changed.ranges.iter_mut().find(|item| item.range.role == role).unwrap();
            record.bytes[0] ^= 1;
            assert!(PublicationMaterial::decode(&changed.encode().unwrap()).is_err(),
        ...
        for word in [1, 5, 6, 39] {
            let mut changed = publication_material_sample();
            let record = changed.ranges.iter_mut().find(|item| item.range.role == 33).unwrap();
            ...
            changed.ranges.iter_mut().find(|item| item.range.role == 33).unwrap().bytes[word * 8] ^= 1;
            assert!(PublicationMaterial::decode(&changed.encode().unwrap()).is_err(),
    ...
    fn replay_material_sample() -> (PublicationMaterial, PublicationReplayEvidence) {
        ...
        for (role, bytes) in [
            (30, publication_abi_bytes(&[IntentQueueHeader { abi: 1, ..IntentQueueHeader::default() }])),
            (33, publication_abi_bytes(&[AttemptReceipt::default()])),
        ] {
            let item = predecessor.ranges.iter_mut().find(|item| item.range.role == role).unwrap();
        ...
        for item in &mut predecessor.ranges {
            if !is_tensor_role(item.range.role) { item.range.digest = item.original_record_digest(); }
        }
        ...
        let mut evidence = PublicationReplayEvidence {
            successor: SemanticPublishedIdentity { instance: predecessor.bank.header.instance, word: 19,
                logical_digest: Identity256::from_bytes([13; 32]), state_digest: Identity256::default() },
            ranges: predecessor.ranges.iter().filter(|item| REPLAY_EVIDENCE_ROLES.contains(&item.range.role)).cloned().collect(),
        };
        let attempt = AttemptReceipt { abi: 1, instance: evidence.successor.instance,
            base_word: predecessor.bank.header.publication_word, next_word: evidence.successor.word,
            ...
            acknowledgement_head_digest: evidence.range(32).unwrap().logical_record_digest().unwrap(),
            previous_attempt_digest: predecessor.ranges.iter().find(|item| item.range.role == 33).unwrap().logical_record_digest().unwrap(),
    ...
    fn seal_replay_sample(evidence: &mut PublicationReplayEvidence) {
        ...
        for item in &mut evidence.ranges { item.range.digest = item.original_record_digest(); }
    ...
    fn semantic_replay_material_requires_complete_linked_evidence() {
        ...
        let parse = |evidence: &[u8]| SemanticReplayMaterial::decode(&bytes, evidence,
            predecessor.bank.header.logical_digest, evidence_successor_logical());
        let decoded = parse(&evidence.encode().unwrap()).unwrap();
        assert_eq!(decoded.predecessor_identity().instance, predecessor.bank.header.instance);
        ...
        assert!(parse(&[]).is_err());
        let mut truncated = evidence.encode().unwrap(); truncated.pop();
        assert!(parse(&truncated).is_err());
        let mut trailing = evidence.encode().unwrap(); trailing.push(0);
        ...
        for change in 0..5 {
            ...
            assert!(parse(&changed.encode().unwrap()).is_err(), "accepted altered native evidence {change}");
        }
    }

    fn evidence_successor_logical() -> Identity256 { Identity256::from_bytes([13; 32]) }

    #[test]
    fn semantic_replay_provenance_preserves_original_authority_and_decision() {
        let (predecessor, evidence) = replay_material_sample();
        let replay = SemanticReplayMaterial::decode(&predecessor.encode().unwrap(), &evidence.encode().unwrap(),
            predecessor.bank.header.logical_digest, evidence.successor.logical_digest).unwrap();
        let authority = predecessor.ranges.iter().find(|item| item.range.role == 38).unwrap().clone();
        let mut decision = predecessor.ranges.iter().find(|item| item.range.role == 39).unwrap().clone();
        decision.bytes[0] ^= 1;
        decision.range.digest = decision.original_record_digest();
        let mut provenance = PublicationReplayProvenance { predecessor: replay.predecessor_identity(), authority, decision };
        let bytes = provenance.encode().unwrap();
        assert_eq!(replay.verify_provenance(&bytes).unwrap(), provenance.decision.bytes);
        ...
        assert!(replay.verify_provenance(&predecessor.encode().unwrap()).is_err());
        provenance.predecessor.word += 2;
        assert!(replay.verify_provenance(&provenance.encode().unwrap()).is_err());
        ...
        assert!(replay.verify_provenance(&provenance.encode().unwrap()).is_err());
        provenance.authority = predecessor.ranges.iter().find(|item| item.range.role == 38).unwrap().clone();
    ...
    fn semantic_replay_material_normalizes_only_native_audit_words() {
        ...
        for word in [1, 4, 5, 6, 39, 42] { attempt.bytes[word * 8] ^= 1; }
        assert_eq!(attempt.logical_record_digest().unwrap(), original);
        assert_ne!(attempt.original_record_digest(), evidence.range(33).unwrap().original_record_digest());
        attempt.bytes[7 * 8] ^= 1;
        ...
        for role in [27, 28, 31, 32] {
            ...
            item.bytes[0] ^= 1;
            assert_ne!(item.logical_record_digest().unwrap(), original, "normalized opaque role {role}");
        }
    }
    ...
    fn replay_codebook_sample() -> PublicationMaterialRange {
        ...
        let mut item = PublicationMaterialRange { range: PublicationRange { role: 48, generation: 1,
            length_bytes: bytes.len() as u64, ..PublicationRange::default() }, capacity: bytes.len(), bytes };
    ...
    fn semantic_replay_receipt_hashes_preserve_every_nonruntime_field() {
        ...
        let receipts = [SemanticTransitionReceipt { catalogue_generation: CATALOGUE_GENERATION,
            admission_binding: Identity256::from_bytes([17; 32]), choice: 5, ..SemanticTransitionReceipt::default() }];
        let expected = publication_action_receipts_digest(&codebook, &receipts).unwrap();
        for word in 0..codebook.bytes.len() / 8 {
            if matches!(word, 8 | 9) { continue; }
            let mut changed = codebook.clone(); changed.bytes[word * 8] ^= 1;
            let equal = publication_action_receipts_digest(&changed, &receipts).unwrap() == expected;
            assert_eq!(equal, matches!(word, 14..=16 | 21..=24), "codebook word {word}");
        }
        let mut changed = receipts;
        changed[0].admission_binding = Identity256::from_bytes([18; 32]);
        assert_eq!(publication_action_receipts_digest(&codebook, &changed).unwrap(), expected);
        changed[0].choice += 1;
        assert_ne!(publication_action_receipts_digest(&codebook, &changed).unwrap(), expected);
        let mut legacy = receipts; legacy[0].catalogue_generation = 0;
        ...
        assert_ne!(publication_action_receipts_digest(&codebook, &legacy).unwrap(), legacy_digest);
        assert_eq!(publication_action_receipts_digest(&codebook, &[]).unwrap(), Identity256::from_bytes(Sha256::digest([]).into()));
        let semantic = [[0u64; 42]; 7];
        let expected = publication_semantic_receipts_digest(&semantic);
        for ordinal in 0..semantic.len() {
            for word in 0..42 {
                let mut changed = semantic; changed[ordinal][word] = 1;
                assert_eq!(publication_semantic_receipts_digest(&changed) == expected,
                    matches!(word, 3..=12 | 34..=41), "semantic receipt {ordinal} word {word}");
            }
        }
        let header = IntentQueueHeader { abi: 1, count: 2, capacity: 2, ..IntentQueueHeader::default() };
        let mut bytes = publication_abi_bytes(&[header]);
        bytes.extend(publication_abi_bytes(&[IntentEntry::default(); 2]));
        let intent = PublicationMaterialRange { range: PublicationRange { role: 30, logical_end: 2,
            length_bytes: bytes.len() as u64, ..PublicationRange::default() }, capacity: bytes.len(), bytes };
        let expected = intent.logical_record_digest().unwrap();
        for ordinal in 0..2 {
            for word in 0..43 {
                let mut changed = intent.clone();
                changed.bytes[size_of::<IntentQueueHeader>() + ordinal * size_of::<IntentEntry>() + word * 8] ^= 1;
                assert_eq!(changed.logical_record_digest().unwrap() == expected,
                    matches!(word, 30..=34), "intent entry {ordinal} word {word}");
            }
        }
    }
    ...
    fn semantic_replay_observed_attempt_checks_actual_receipts_and_coverage() {
        ...
        let codebook = replay_codebook_sample();
        let mut completion = CompletionCoverage { abi: 1, instance: bank.header.instance,
            base_word: bank.header.base_word, ..CompletionCoverage::default() };
        let bytes = publication_abi_bytes(&[completion]);
        let mut coverage = PublicationMaterialRange { range: PublicationRange { role: 14, generation: 1,
            length_bytes: bytes.len() as u64, ..PublicationRange::default() }, capacity: bytes.len(), bytes };
        ...
        let item = evidence.ranges.last_mut().unwrap();
        let mut attempt = item.attempt().unwrap();
        attempt.action_receipts_digest = publication_action_receipts_digest(&codebook, &bank.receipts).unwrap();
        attempt.semantic_receipts_digest = publication_semantic_receipts_digest(&bank.state.semantic_receipts);
        attempt.coverage_digest = coverage.logical_record_digest().unwrap();
        item.bytes = publication_abi_bytes(&[attempt]);
        seal_replay_sample(&mut evidence);
        bank.header.state_digest = evidence.successor.state_digest;
        assert_eq!(evidence.validate(&predecessor).unwrap(), SemanticTransitionKind::Proposal);
        evidence.validate_observed(&predecessor, &bank, &coverage, &codebook).unwrap();

        let mut changed = bank; changed.receipts[0].choice += 1;
        assert!(evidence.validate_observed(&predecessor, &changed, &coverage, &codebook).is_err());
        changed = bank; changed.state.semantic_receipts[0][13] += 1;
        assert!(evidence.validate_observed(&predecessor, &changed, &coverage, &codebook).is_err());
        changed = bank; changed.state.semantic_receipts[0][3] += 1;
        evidence.validate_observed(&predecessor, &changed, &coverage, &codebook).unwrap();
        changed = bank; changed.state.blocks = 0;
        assert!(evidence.validate_observed(&predecessor, &changed, &coverage, &codebook).is_err());
        changed = bank; changed.header.base_word += 2;
        assert!(evidence.validate_observed(&predecessor, &changed, &coverage, &codebook).is_err());
        let mut changed_coverage = coverage.clone(); changed_coverage.bytes[6 * 8] ^= 1;
        changed_coverage.range.digest = changed_coverage.original_record_digest();
        assert!(evidence.validate_observed(&predecessor, &bank, &changed_coverage, &codebook).is_err());
        let mut changed_codebook = codebook.clone(); changed_codebook.bytes[25 * 8] ^= 1;
        changed_codebook.range.digest = changed_codebook.original_record_digest();
        assert!(evidence.validate_observed(&predecessor, &bank, &coverage, &changed_codebook).is_err());
        let item = evidence.ranges.last_mut().unwrap();
        let mut attempt = item.attempt().unwrap();
        ...
        assert!(evidence.validate_observed(&predecessor, &bank, &coverage, &codebook).is_err());
    }
    ...
    fn publication_material_codebooks_relocate_only_runtime_coordinates() {
        ...
        for word in (14..=16).chain(21..=24) { actual[word] += 500; }
        let saved = publication_abi_bytes(&original);
        assert_eq!(relocate_publication_codebooks(&saved, &actual).unwrap(), publication_abi_bytes(&actual));
        for word in (0..14).chain(17..=20).chain(25..48) {
            let mut corrupt = actual.clone(); corrupt[word] += 1;
            assert!(relocate_publication_codebooks(&saved, &corrupt).is_err(), "logical word {word}");
        }
    ...
    fn step_input_geometry_fixture() -> ([Vec<PublicationRange>; 2], BTreeMap<(u64, u64), SemanticTensorLayout>, Vec<usize>) {
        let mut layouts = BTreeMap::new();
        let mut banks = [Vec::new(), Vec::new()];
        let mut sizes = Vec::new();
        for (role, index) in std::iter::once(1).chain(3..=13).chain(15..=17).chain(std::iter::once(44))
            .flat_map(|role| (0..if role == 16 { 2 } else { 1 }).map(move |index| (role, index))) {
            let layout = if role == 1 {
                SemanticTensorLayout { role, index: 0, scalar_type: 3, element_bytes: 8,
                    rank: 2, logical_axis: 0, dimensions: [64, 8, 0, 0], strides_bytes: [64, 8, 0, 0] }
            } else {
                canonical_tensor_layout(SemanticTensorLayout { role, index: 0,
                    scalar_type: if role == 7 { 5 } else { 6 }, element_bytes: if role == 7 { 2 } else { 4 },
                    rank: 4, logical_axis: if role <= 5 { 2 } else { u64::MAX },
                    dimensions: [1, 2, 64, 3], strides_bytes: [0; 4] }).unwrap()
            };
            if (4..=13).contains(&role) { layouts.insert((role, 0), layout); }
            let capacity = if role == 3 { 32 } else if role == 44 { 108 }
                else if role == 15 { 2 * size_of::<RawFeedbackRecord>() }
                else if matches!(role, 16 | 17) { 40 + index as usize * 8 }
                else { tensor_layout_bytes(&layout).unwrap() };
            let shared = matches!(role, 1 | 4 | 5 | 16 | 17);
            let first_slot = sizes.len(); sizes.push(capacity);
            if !shared { sizes.push(capacity); }
            for (bank, directory) in banks.iter_mut().enumerate() {
                directory.push(PublicationRange { role, index, storage_slot: (first_slot + usize::from(!shared) * bank) as u64,
                    generation: 1, length_bytes: if role == 1 { 64 } else if role == 4 || role == 5 { 24 } else { capacity as u64 },
                    logical_end: u64::from(matches!(role, 1 | 4 | 5 | 15)), ..PublicationRange::default() });
            }
        }
        (banks, layouts, sizes)
    }
    ...
    fn step_input_geometry_preserves_exact_roster_and_full_shared_capacity() {
        ...
        assert_eq!(plan.iter().map(|input| (input.role, input.index)).collect::<Vec<_>>(),
            std::iter::once((1, 0)).chain((3..=13).map(|role| (role, 0)))
                .chain([(15, 0), (16, 0), (16, 1), (17, 0), (44, 0)]).collect::<Vec<_>>());
    ...
    fn step_input_geometry_retains_raw_feedback_and_every_statement_record() {
        let (banks, layouts, sizes) = step_input_geometry_fixture();
        let plan = plan_step_inputs(&banks, &layouts, 64, &sizes).unwrap();
        let records = plan.iter().filter(|input| matches!(input.role, 15..=17)).collect::<Vec<_>>();
        assert_eq!(records.iter().map(|input| (input.role, input.index, input.span.len()))
            .collect::<Vec<_>>(), vec![(15, 0, 768), (16, 0, 40), (16, 1, 48), (17, 0, 40)]);
        assert!(records.iter().all(|input| input.layout.scalar_type == 1 && input.layout.element_bytes == 1));
        ...
        for bank in &mut gapped { bank.iter_mut().find(|range| (range.role, range.index) == (16, 1)).unwrap().index = 2; }
        assert!(plan_step_inputs(&gapped, &layouts, 64, &sizes).is_err());
        let mut shared_raw = banks;
        let slot = shared_raw[0].iter().find(|range| range.role == 15).unwrap().storage_slot;
        shared_raw[1].iter_mut().find(|range| range.role == 15).unwrap().storage_slot = slot;
    ...
    fn feedback_record_material_preserves_original_seal_and_rejects_changed_content() {
        let mut original = PublicationMaterialRange {
            range: PublicationRange { role: 15, generation: 1, length_bytes: 768,
                logical_end: 1, ..PublicationRange::default() },
            capacity: 768, bytes: vec![0; 768],
        };
        ...
        assert_eq!((material.logical_begin, material.logical_end, material.capacity_bytes), (0, 1, 768));
        assert_eq!(material.bytes, original.bytes);
        let mut changed = original.clone(); changed.bytes[0] ^= 1;
        assert!(changed.into_feedback_record().is_err());
        let mut changed = original.clone(); changed.range.logical_end = 2;
        assert!(changed.into_feedback_record().is_err());
        let mut changed = original; changed.range.role = 18;
        assert!(changed.into_feedback_record().is_err());
    }
    ...
    fn step_input_geometry_rejects_missing_duplicate_and_noncanonical_coordinates() {
        let (banks, layouts, sizes) = step_input_geometry_fixture();
        let mut missing = banks.clone(); missing[1].remove(4);
        assert!(plan_step_inputs(&missing, &layouts, 64, &sizes).is_err());
        let mut duplicate = banks.clone(); let repeated = duplicate[0][4]; duplicate[0].push(repeated);
        assert!(plan_step_inputs(&duplicate, &layouts, 64, &sizes).is_err());
        let mut wrong_index = banks; wrong_index[0][4].index = 1;
    ...
    fn step_input_geometry_requires_shared_prefix_and_key_value_but_isolated_caches() {
        ...
        for role in [1, 4, 5] {
            ...
            changed[1].iter_mut().find(|range| range.role == role).unwrap().storage_slot += 1;
            assert!(plan_step_inputs(&changed, &layouts, 64, &sizes).is_err());
        }
        for role in std::iter::once(3).chain(6..=13) {
            let mut changed = banks.clone();
            let slot = changed[0].iter().find(|range| range.role == role).unwrap().storage_slot;
            changed[1].iter_mut().find(|range| range.role == role).unwrap().storage_slot = slot;
            assert!(plan_step_inputs(&changed, &layouts, 64, &sizes).is_err());
        }
    }
    ...
    fn step_input_geometry_checks_canonical_dtype_spans_and_allocation_bounds() {
        ...
        assert_eq!((bf16.layout.scalar_type, bf16.layout.element_bytes, bf16.span.len()), (5, 2, 768));
        let mut wrong = layouts.clone(); wrong.get_mut(&(7, 0)).unwrap().strides_bytes[2] += 2;
        assert!(plan_step_inputs(&banks, &wrong, 64, &sizes).is_err());
        let mut short = sizes;
        short[banks[1].iter().find(|range| range.role == 7).unwrap().storage_slot as usize] -= 1;
        assert!(plan_step_inputs(&banks, &layouts, 64, &short).is_err());
        assert!(step_input_overlap(104, 0, 100, 8).unwrap(), "empty native aliases must still preserve their origin");
    ...
    fn step_inputs_keep_private_values_after_reader_retirement_and_current_bank_reuse() {
        ...
        let mut lease = session.acquire().unwrap();
        let context = session.published_model_context(&lease, 1).unwrap();
        let cache = session.published_tensor(&lease, SemanticStateRole::from_code(8).unwrap(), 0, 1).unwrap();
        let inputs = Arc::clone(session.steps[&lease.token].inputs.as_ref().unwrap());
        let mut feedback_before = Vec::new();
        for (&(role, index), view) in inputs.views.iter().filter(|(key, _)| matches!(key.0, 15..=17)) {
            feedback_before.push(((role, index), session.publication_read(view.clone()).unwrap()));
        }
        ...
        assert!(!storage.banks.iter().any(|bank| source_address >= bank.device_ptr_value()
            && source_address < bank.device_ptr_value() + size_of::<PublicationBank>() as u64));
        assert!(!storage.allocations.iter().any(|allocation| cache_address >= allocation.device_ptr_value()
            && cache_address < allocation.device_ptr_value() + allocation.len() as u64));
        let again = session.published_source(&lease, 1).unwrap();
        assert_eq!(unsafe { (*again.as_ptr()).dl_tensor.data as u64 }, source_address);
        let source_layout = SemanticTensorLayout { role: 0, index: 0, scalar_type: 3, element_bytes: 8,
            rank: 2, logical_axis: u64::MAX, dimensions: [32, 8, 0, 0], strides_bytes: [64, 8, 0, 0] };
        let source_input = |tensor| SemanticTensorInput { layout: source_layout, logical_begin: 0, logical_end: 0, tensor, native_allocation: None };
        let original_source = source_input(session.published_source(&lease, 1).unwrap());
        let original_cache = publication_test_alias(&mut session, &lease, SemanticStateRole::from_code(8).unwrap(), 1);
        let later_source = source_input(session.published_source(&lease, 1).unwrap());
        let later_cache = publication_test_alias(&mut session, &lease, SemanticStateRole::from_code(8).unwrap(), 1);
        let witness = session.capture_tensor_content(&lease, vec![original_source, original_cache], 1).unwrap();
        let TensorContentSeals::Captured(origins) = &session.steps[&lease.token].content[witness.index].seals else {
            panic!("native input origins");
        };
        assert!(origins.iter().all(|origin| matches!(origin, CapturedTensorDigest::Publication(_))),
            "capture must retain the original native input seals");
        session.guard_published_content(&lease, &[(SemanticStateRole::from_code(8).unwrap(), 0)], 1).unwrap();
        ...
        for _ in 0..2 {
            ...
            assert_ne!(unsafe { (*current_context.source.as_ptr()).dl_tensor.data as u64 }, source_address);
            drop(current_context);
            publication_test_recompute(&mut session, &current, &provider);
            session.release(&mut current, &[1]).unwrap();
        }
        let inputs = Arc::clone(session.steps[&lease.token].inputs.as_ref().unwrap());
        assert_eq!(session.publication_read(inputs.header.view()).unwrap()[0].publication_word, lease.identity.word);
        ...
        let before = session.host_io_stats();
        session.verify_tensor_content(&lease, &witness, vec![later_source, later_cache], 1).unwrap();
        assert_eq!(session.host_io_stats(), before);
        drop(witness);
        assert!(session.release(&mut lease, &[1]).is_err());
        drop(context); drop(cache); drop(again); drop(storage); drop(inputs);
        session.release(&mut lease, &[1]).unwrap();
        drop(session);
        provider.memory().reap_pending_deallocations().unwrap();
        assert_eq!(provider.memory().allocated_bytes(), 0);
    ...
    fn model_context_joins_exact_acquired_header_prefix_and_ring() {
        ...
        let mut lease = SemanticPublishedLease {
            issuer: Arc::new(()), token: 7, active: true,
            identity: SemanticPublishedIdentity { instance: identity(1), word: 12,
                logical_digest: identity(2), state_digest: identity(3) },
            header: PublicationHeader { abi: 1, instance: identity(1), publication_word: 12,
                sealed_epoch: 6, logical_digest: identity(2), state_digest: identity(3),
                descriptor_digest: identity(4), semantic_owner: 5, semantic_slot: 6,
                semantic_generation: 7, semantic_digest: identity(8), semantic_extents: [9, 10, 11],
                prefix_extent: 2, ring_head: 17, model_generation: 12, neural_bank: 1,
                neural_generation: 13, cache_generation: 14, authority_generation: 15,
                ..PublicationHeader::default() },
            source: source_rows(), directory: Vec::new(),
        };
        let context = lease.model_context_header().unwrap();
        assert_eq!(context.descriptor_digest, identity(4));
        assert_eq!((context.semantic_owner, context.semantic_slot, context.semantic_generation,
            context.semantic_digest, context.semantic_extents), (5, 6, 7, identity(8), [9, 10, 11]));
        assert_eq!(context.ring_head, 17);
        assert_eq!(context.prefix_extent, 2);
        assert_eq!((context.model_generation, context.neural_bank, context.neural_generation,
            context.cache_generation, context.authority_generation), (12, 1, 13, 14, 15));
    ...
    fn publication_bank_views_use_original_fields_and_slot_order() {
        let header = PublicationHeader { prefix_extent: 23, ring_head: 17,
            ..PublicationHeader::default() };
        ...
        for (field, value) in [(PublicationBankField::PrefixExtent, 23u64),
    ...
    fn active_row_table_requires_independent_publication_banks() {
        assert!(publication_mutable_role(SemanticStateRole::TensorLayout as u64));
    }
    ...
    fn active_text_rows_include_the_reserved_window_before_feedback_positions() {
        for position in [63, 64, 95, 96] {
            ...
            source[7] = SourceSlot { token: MASK_TOKEN as u64, logical_position: position,
                kind: 2, valid: 1, ..SourceSlot::default() };
    ...
    fn active_table_keeps_computed_empty_distinct_from_uncomputed() {
        let layouts = [SemanticTensorLayout { role: 51, rank: 4, logical_axis: 2,
            element_bytes: 2, scalar_type: 4, dimensions: [1, 2, 35, 4],
            strides_bytes: [560, 280, 8, 2], ..SemanticTensorLayout::default() }];
        let cold = tensor_table_bytes(&layouts, false, &[]).unwrap();
        let empty = tensor_table_bytes(&layouts, true, &[]).unwrap();
        assert_ne!(cold, empty);
        assert_eq!(decode_tensor_table(&cold).unwrap().2, SemanticActiveRows { computed: false, rows: vec![] });
        assert_eq!(decode_tensor_table(&empty).unwrap().2, SemanticActiveRows { computed: true, rows: vec![] });
        ...
        let row = SemanticActiveRow { physical_row: 0, source_slot: 0, logical_position: 0, kind: 2 };
    ...
    fn active_tensor_geometry_preserves_full_physical_capacity() {
        let producer = SemanticTensorLayout { role: 51, rank: 4, logical_axis: 2,
            element_bytes: 2, scalar_type: 4, dimensions: [1, 2, 35, 4],
            strides_bytes: [560, 2, 16, 4], ..SemanticTensorLayout::default() };
        ...
        let linear = SemanticTensorLayout { role: 53, rank: 3, logical_axis: 0,
            element_bytes: 4, scalar_type: 6, dimensions: [35, 4, 2, 0],
            strides_bytes: [32, 8, 4, 0], ..SemanticTensorLayout::default() };
        assert!(validate_active_capacity(&linear, 35).is_ok());
        assert!(validate_active_capacity(&SemanticTensorLayout { logical_axis: u64::MAX, ..linear }, 35).is_err());
    }

    #[test]
    fn publication_rng_binding_uses_only_the_actual_header_coordinates() {
        let header = PublicationHeader { model_generation: 17, family_id: 29,
            stream_serial: (1u64 << 56) - 1, proposal: u64::from(u32::MAX),
            ..PublicationHeader::default() };
        assert_eq!(header.rng_binding().unwrap(), SemanticRngBinding {
            model_generation: 17, family_id: 29, stream_serial: (1u64 << 56) - 1,
            proposal: u32::MAX,
        });
        for invalid in [
            PublicationHeader { model_generation: 1u64 << 32, ..header },
            PublicationHeader { family_id: 256, ..header },
            PublicationHeader { stream_serial: 1u64 << 56, ..header },
            PublicationHeader { proposal: 1u64 << 32, ..header },
    ...
    fn cold_continuation_layouts_match_original_fixed_output_snapshots() {
        let prefix = canonical_tensor_layout(SemanticTensorLayout {
            role: 4, rank: 4, logical_axis: 2, element_bytes: 2, scalar_type: 4,
            dimensions: [1, 2, 128, 4], ..SemanticTensorLayout::default()
        }).unwrap();
        let active = canonical_tensor_layout(SemanticTensorLayout {
            role: 51, rank: 4, logical_axis: 2, element_bytes: 2, scalar_type: 4,
            dimensions: [1, 2, 35, 4], ..SemanticTensorLayout::default()
        }).unwrap();
        let state = canonical_tensor_layout(SemanticTensorLayout {
            role: 6, rank: 2, logical_axis: u64::MAX, element_bytes: 4, scalar_type: 6,
            dimensions: [4, 128, 0, 0], ..SemanticTensorLayout::default()
        }).unwrap();
        ...
        for (model, extent) in [(prefix, 32), (active, 35), (state, 0)] {
            ...
            assert_eq!(continuation_tensor_layout(&producer, 0, extent, &model).unwrap(), cold);
        ...
        assert_eq!(rows, SemanticActiveRows { computed: true, rows: vec![] });
        assert!(continuation_capacity_layout(SemanticTensorLayout { role: 3, ..state }).is_err());
        assert!(continuation_capacity_layout(SemanticTensorLayout { logical_axis: 1, ..active }).is_err());
    }
    ...
    fn continuation_services_preserve_original_device_roster_and_capacity() {
        ...
        let expected = [
            (3, 8, [32, 2, 0, 0]), (3, 8, [1, 0, 0, 0]),
            (8, 1, [32, 0, 0, 0]), (3, 8, [66, 4, 0, 0]),
            (3, 8, [1, 0, 0, 0]), (8, 1, [1, 0, 0, 0]),
        ];
        for (ordinal, (layout, (scalar_type, element_bytes, dimensions))) in
            ...
            assert_eq!((layout.scalar_type, layout.element_bytes, layout.dimensions),
    ...
    fn continuation_transport_does_not_rebase_original_suffix_or_nonpositional_outputs() {
        let suffix = canonical_tensor_layout(SemanticTensorLayout {
            role: 4, rank: 4, logical_axis: 2, element_bytes: 2, scalar_type: 4,
            dimensions: [1, 2, 32, 4], ..SemanticTensorLayout::default()
        }).unwrap();
        let cold = prefix_capacity_layout(SemanticTensorLayout { dimensions: [1, 2, 0, 4], ..suffix }, 16).unwrap();
        ...
        assert_eq!(continuation_tensor_layout(&state, 0, 0, &state).unwrap(), state);
    ...
    fn cold_intent_owner_constructs_an_empty_queue_with_the_actual_effect() {
        ...
        assert_eq!(records[0].capacity_bytes, size_of::<IntentQueueHeader>() + 2 * size_of::<IntentEntry>());
        // SAFETY: the production builder returned a complete padding-free header.
        let header = unsafe { records[0].bytes.as_ptr().cast::<IntentQueueHeader>().read_unaligned() };
        assert_eq!((header.abi, header.count, header.capacity), (1, 0, 2));
        assert_eq!(header.chain_head, Identity256::default());
        assert_eq!((header.effect_offset_bytes, header.effect_length_bytes), (0, effect.len() as u64));
    ...
    fn semantic_tensor_admission_preserves_original_type_strides_and_bounds() {
        ...
        let metadata = TensorMetadata {
            ...
            device_id: 0,
            dtype: crate::dlpack::DLDataType { code: 2, bits: 16, lanes: 1 },
            shape: &[1, 2, 3, 4],
            strides: None,
            data: 0x1000,
            byte_offset: 2,
        };
        assert_eq!(validate_tensor_metadata(&layout, &metadata, 0).unwrap(), (0x1002, 48));
        ...
        assert_eq!(validate_tensor_metadata(&transposed, &strided, 0).unwrap(), (0x1002, 48));
        ...
        assert_eq!(validate_tensor_metadata(&empty, &empty_metadata, 0).unwrap(), (0, 0));
    }

    #[test]
    fn semantic_tensor_scalar_keeps_one_cell_and_original_rank() {
        for (scalar_type, element_bytes, code) in [(1, 1, 1), (2, 4, 1), (3, 8, 1),
            (4, 2, 2), (5, 2, 4), (6, 4, 2), (7, 8, 0), (8, 1, 6)] {
            let layout = SemanticTensorLayout { role: 18, index: 0, scalar_type,
                element_bytes, rank: 0, logical_axis: u64::MAX,
                dimensions: [0; 4], strides_bytes: [0; 4] };
            let metadata = TensorMetadata { device_type: crate::dlpack::K_DLCUDA,
                device_id: 0, dtype: crate::dlpack::DLDataType {
                    code, bits: (element_bytes * 8) as u8, lanes: 1 },
                shape: &[], strides: None, data: 4096, byte_offset: element_bytes };
            assert_eq!(validate_tensor_metadata(&layout, &metadata, 0).unwrap(),
                (4096 + element_bytes, element_bytes as usize));
            assert_eq!(tensor_layout_bytes(&layout).unwrap(), element_bytes as usize);
            assert_eq!(canonical_tensor_layout(layout).unwrap(), layout);
            assert_eq!(tensor_copy_plan(&layout, &layout).unwrap(),
    ...
    fn publication_material_scalar_layout_survives_cold_codec() {
        ...
        let scalar = *layout;
        let table = material.ranges.iter_mut().find(|item| item.range.role == 55).unwrap();
        table.bytes = tensor_table_bytes(&material.layouts.values().copied().collect::<Vec<_>>(),
            false, &[]).unwrap();
        table.range.digest = table.original_record_digest();
        publication_material_sample_runtime(&mut material);
        material.bank.header.descriptor_digest = material.original_descriptor_digest();
        ...
        let original = material.ranges.iter().find(|item| item.range.role == 18).unwrap();
        let actual = restored.ranges.iter().find(|item| item.range.role == 18).unwrap();
    ...
    fn semantic_tensor_scalar_metadata_allows_null_zero_rank_arrays() {
        ...
        let mut descriptor = crate::dlpack::DLTensor {
            ...
            device: crate::dlpack::DLDevice { device_type: crate::dlpack::K_DLCUDA, device_id: 0 },
            ndim: 0, dtype: crate::dlpack::DLDataType { code: 2, bits: 32, lanes: 1 },
            shape: std::ptr::null_mut(), strides: std::ptr::null_mut(), byte_offset: 0,
        };
        let layout = SemanticTensorLayout { role: 18, index: 0, element_bytes: 4,
            scalar_type: 6, rank: 0, logical_axis: u64::MAX,
            dimensions: [0; 4], strides_bytes: [0; 4] };
        ...
        assert_eq!(validate_tensor_metadata(&layout, &actual, 0).unwrap(),
    ...
    fn publication_material_empty_model_buffer_keeps_zero_capacity() {
        for role in 18..=20 {
            ...
            let empty = *layout;
            let buffer = material.ranges.iter_mut().find(|item| item.range.role == role).unwrap();
            buffer.bytes.clear();
            buffer.capacity = 0;
            ...
            let actual = restored.ranges.iter().find(|item| item.range.role == role).unwrap();
            ...
            for (capacity, begin, end) in [(1, 0, 0), (0, 1, 1), (0, 0, 1)] {
                ...
                publication_material_sample_runtime(&mut material);
                material.bank.header.descriptor_digest = material.original_descriptor_digest();
                assert!(material.encode().is_err(), "empty buffer accepted a fictitious capacity or interval");
            }
            let buffer = material.ranges.iter_mut().find(|item| item.range.role == role).unwrap();
            buffer.capacity = 0;
            ...
            assert!(material.encode().is_err(), "nonempty model cells acquired zero capacity");
        }
    }
    ...
    fn feedback_payload_preserves_qualifiers_without_service_headers() {
        ...
        let atom = |header: u8, value: u32| {
            ...
            let end = bytes.len();
            SemanticRecordEncoding { bytes, arguments: vec![start..end] }
        };
        ...
        assert_eq!(payload, feedback_statement_payload(&records, &changed_headers, 0).unwrap());
        ...
        assert_ne!(payload, feedback_statement_payload(&records, &changed_values, 0).unwrap());
        let mut reordered = records;
        reordered.records[0].qualifiers.reverse();
        assert_ne!(payload, feedback_statement_payload(&reordered, &encoded, 0).unwrap());
    }
    ...
    fn source_rows() -> [SourceSlot; 32] {
        ...
        rows[5] = SourceSlot {
            token: 40, logical_position: 0, kind: 1, provenance: 1,
            valid: 1, committed: 0, recomputed: 0, provenance_record: 0,
        };
        rows[1] = SourceSlot {
            token: MASK_TOKEN as u64, logical_position: 1, kind: 2, provenance: 0,
            valid: 1, committed: 0, recomputed: 0, provenance_record: 0,
        };
        rows[12] = SourceSlot {
            token: 42, logical_position: 2, kind: 1, provenance: 2,
            valid: 1, committed: 0, recomputed: 0, provenance_record: 2,
        };
        rows
    }
    ...
    fn initial_source_ledger_derives_tokens_and_private_provenance_without_changing_geometry() {
        ...
        ring[7] = SourceSlot { token: 41, logical_position: 3, kind: 1, provenance: 1, valid: 1, ..SourceSlot::default() };
        ring[2] = SourceSlot { token: MASK_TOKEN as u64, logical_position: 4, kind: 2, valid: 1, ..SourceSlot::default() };
        let prefix = vec![SourceSlot { token: 40, logical_position: 0, kind: 1, provenance: 1, valid: 1, committed: 1, ..SourceSlot::default() }];
        let sources = [SemanticObservedSource { identity: "observation".into(), tokens: vec![40, 41], origin: b"complete admitted observation and manifest".to_vec() }];
        let mapping = [
            SemanticSourceMapping { source: "observation".into(), offset: 0, logical_position: 0 },
            SemanticSourceMapping { source: "observation".into(), offset: 1, logical_position: 3 },
        ];
        let (actual, committed, ledger) = initial_source_ledger(ring, prefix.clone(), &sources, &mapping, b"complete task authority", 3, 34).unwrap();
        assert_eq!((actual[7].token, actual[7].logical_position, actual[7].provenance_record), (41, 3, 1));
        ...
        let record = unsafe { ledger.bytes.as_ptr().add(size_of::<TokenProvenance>()).cast::<TokenProvenance>().read_unaligned() };
        assert_eq!((record.source_slot, record.logical_position, record.token, record.ordinal, record.authority_generation), (7, 3, 41, 1, 3));
        ...
        for field in 0..7 {
            ...
            assert!(initial_source_ledger(changed, prefix.clone(), &sources, &links, b"complete task authority", 3, 34).is_err(), "mutation {field}");
        }
    }
    ...
    fn cold_text_parent_derives_only_contiguous_acquired_filled_coverage() {
        ...
        assert_eq!(validate_text_parent(&rows, 0, 64, 2, 262144, 0, 3).unwrap(), 1);
    ...
    fn cold_mask_source_keeps_the_sentinel_without_fabricated_token_provenance() {
        let mut rows = [SourceSlot::default(); 32];
        rows[7] = SourceSlot { token: 248078, kind: 2, valid: 1, ..SourceSlot::default() };
        assert_eq!(validate_text_parent(&rows, 0, 64, 2, 262144, 0, 0).unwrap(), 0);
        rows[7].token = 0;
...
pub(crate) mod task_binding_tests {
    ...
    impl SemanticTaskProgram for ArithmeticProgram {
        fn observe(&self, _provider: Arc<CudaKernelProvider>)
            -> Result<SemanticTaskObservation, SemanticTransitionError> {
    ...
    pub(crate) fn arithmetic_observation() -> SemanticTaskObservation {
        ...
        SemanticTaskObservation {
            ...
            expected_truth: results.map(|result| if result == 1 {
                crate::SemanticTruth::True
            } else { crate::SemanticTruth::False }),
        }
    }

    pub(crate) fn task_spec(statement_records: [u32; 2], allowed_support_records: Vec<u32>)
        -> SemanticTaskEvaluationSpec {
        SemanticTaskEvaluationSpec {
            statement_records, allowed_support_records, program: Arc::new(ArithmeticProgram),
    ...
    fn task_query_validation_does_not_interpret_argument_values() {
        ...
        task_spec([0, 1], vec![0]).validate_records(&records).unwrap();
    }
    ...
    fn application_observer_retains_input_and_output_bytes() {
        ...
        assert_eq!(observation.expected_truth, [crate::SemanticTruth::True, crate::SemanticTruth::False]);
    }
    ...
    fn task_binding_identity_seals_observer_execution_and_configuration() {
        ...
        let identity = |spec, observation| TaskEvaluationBinding::bind(&admission, spec, observation)
            .unwrap().identity();
        let original = identity(task_spec([0, 1], vec![0]), arithmetic_observation());
        assert_eq!(original, identity(task_spec([0, 1], vec![0]), arithmetic_observation()),
        ...
        for changed in 0..13 {
            ...
            let field = match changed {
                0 => { observation.program_source.push(b' '); "program source" }
                1 => { observation.input_bytes.push(0); "observer inputs" }
                2 => { observation.result_bytes.push(0); "observer results" }
                3 => { observation.expected_truth[0] = crate::SemanticTruth::Both; "first expected truth" }
                4 => { observation.expected_truth[1] = crate::SemanticTruth::Neither; "second expected truth" }
                5 => { spec.scoring.correct_weight += 1; "correct weight" }
                6 => { spec.scoring.all_correct_weight += 1; "all-correct weight" }
                7 => { spec.scoring.work_weight += 1; "work weight" }
                8 => { spec.scoring.improvement_weight += 1; "improvement weight" }
                9 => { spec.scoring.refusal_weight += 1; "refusal weight" }
                10 => { spec.scoring.spent_weight += 1; "spent weight" }
                11 => { spec.admissible_truth_masks[0] = 15; "first truth mask" }
                _ => { spec.admissible_truth_masks[1] = 15; "second truth mask" }
            };
            assert_ne!(original, identity(spec, observation), "identity omitted {field}");
        }
    }
    ...
    fn task_truth_eligibility_requires_nonempty_four_valued_masks() {
        ...
        for slot in 0..2 {
            for mask in [0, 16, 128, 255] {
                ...
                let result = TaskEvaluationBinding::bind(&admission, spec, arithmetic_observation());
                assert!(matches!(result, Err(SemanticTransitionError::InvalidInput { detail })
                    if detail.contains("nonempty four-valued mask")), "slot {slot}, mask {mask}");
            }
            for mask in 1..=15 {
                let mut spec = task_spec([0, 1], vec![0]);
                spec.admissible_truth_masks[slot] = mask;
    ...
    fn action_leaf_categories_preserve_equal_original_supports() {
        ...
        for (category, meaning) in books.leaves.iter().enumerate() {
            let SemanticActionDescriptor::SupportEvent { source_record } = meaning else { panic!("wrong leaf meaning") };
            assert!(seen.insert(*source_record));
            assert_eq!(books.words[books.words[6] as usize + 18 * (category + 1) + 17], u64::from(*source_record));
        }
    ...
    fn policy_producer_geometry_keeps_full_mask_rows_and_boolean_support() {
        ...
        let metadata = TensorMetadata {
            ...
            dtype: crate::dlpack::DLDataType { code: 2, bits: 32, lanes: 1 },
            shape: &[32, TEXT_CARDINALITY as i64], strides: None,
            data: 4096, byte_offset: 4,
        };
        assert_eq!(validate_tensor_metadata(&layouts[0], &metadata, 0).unwrap(),
        ...
        let support = TensorMetadata {
            device_type: 2, device_id: 0,
            dtype: crate::dlpack::DLDataType { code: 6, bits: 8, lanes: 1 },
            shape: &[97], strides: Some(&[1]), data: 4096, byte_offset: 0,
        };
        assert_eq!(validate_tensor_metadata(&layouts[1], &support, 0).unwrap(), (4096, 97));
        let mut wrong_dtype = support; wrong_dtype.dtype.code = 1;
    ...
    fn action_codebooks_retain_typed_reconstruction_sources() {
        ...
        let admission = admit_material_records(root_material_records());
        let books = ActionCodebooks::derive(&admission, 1).unwrap();
        assert_eq!(books.words[4] - books.words[2], 6 * books.words[3],
            "operand bank lost original argument reconstruction references");
        assert_eq!(books.words[6] - books.words[4], 3 * books.words[5],
            "qualifier bank lost original bundle reconstruction references");
        for (category, meaning) in books.operands.iter().enumerate() {
            let SemanticActionDescriptor::Operand { encoded_value, .. } = meaning else { panic!("wrong operand meaning") };
            let row = books.words[2] as usize + 6 * (category + 1);
            let source = &admission.encoded_records[books.words[row + 4] as usize];
            assert_eq!(&source.bytes[source.arguments[books.words[row + 5] as usize].clone()], encoded_value);
        }
        for (category, meaning) in books.qualifiers.iter().enumerate() {
            let SemanticActionDescriptor::QualifierBundle { records } = meaning else { panic!("wrong qualifier meaning") };
            let source = books.words[books.words[4] as usize + 3 * (category + 1) + 2];
            if records.is_empty() {
                assert_eq!(source, u64::from(u32::MAX));
            } else {
                assert_eq!(&admission.records().records[source as usize].qualifiers, records);
            }
        }
    }
    ...
    pub(crate) fn verify_native_decoded_reconstruction(
        ...
        use crate::semantic_hypergraph::tests::{admit_material_records, reconstructed_material_statement, root_material_records};
        let mut records = root_material_records();
        records.supports.push(records.supports[0].clone());
        let admission = admit_material_records(records);
        let books = ActionCodebooks::derive(&admission, 1).unwrap();
        let leaf = books.leaves.iter().position(|value| matches!(value,
            SemanticActionDescriptor::SupportEvent { source_record: 3 })).unwrap() as u32 + 1;
        for (ordinal, operand) in [1, books.operands.len() as u32].into_iter().enumerate() {
            ...
            choices[2] = 1; choices[3] = operand; choices[11] = 1; choices[17] = leaf;
            let mut input = format!("{}\n", books.words.len());
            for word in &books.words { input.push_str(&format!("{word} ")); }
            for choice in choices { input.push_str(&format!("{choice} ")); }
            ...
            run(std::process::Command::new(executable).arg("--decode").arg(&source).arg(&destination));
            let words: Vec<u32> = std::fs::read_to_string(&destination).unwrap().split_whitespace()
                .map(|word| word.parse().unwrap()).collect();
            assert_eq!(words.len(), 20);
            assert_eq!(words[8], u32::MAX, "computed target invented an original occurrence");
            assert_eq!(words[19], 3, "actual decoder selected a different equal support occurrence");
            ...
            let digest: Vec<u8> = words[..8].iter().flat_map(|word| word.to_le_bytes()).collect();
            assert_eq!(digest.as_slice(), key.identity().as_bytes(), "actual decode differs from canonical restore");
            if ordinal == 0 {
                assert_eq!(key.identity(), admission.statement_key(0).unwrap().identity());
            } else {
                assert_ne!(key.identity(), admission.statement_key(0).unwrap().identity());
                assert_ne!(key.identity(), admission.statement_key(1).unwrap().identity());
            }
            assert!(reconstructed_material_statement(&admission, Some(0), &reconstruction).is_err(),
