...
use xlog_cuda::{CudaDevice, CudaKernelProvider, GpuMemoryManager, GpuDevicePool, MultiGpuMemoryManager};
...
fn main() {
    ...
    let f64_data: Vec<u8> = (0..100).map(|i| (i as f64) * 1.5).flat_map(|v| v.to_le_bytes()).collect();

    let buffer = provider.create_buffer_from_slices(&[&u32_data, &i64_data, &f64_data], schema.clone())
    ...
    let left = provider.create_buffer_from_slices(&[&left_keys, &left_vals], left_schema.clone())
        .expect("create left");
    ...
    let right = provider.create_buffer_from_slices(&[&right_keys, &right_data], right_schema.clone())
        .expect("create right");

    let join_result = provider.hash_join_v2(&left, &right, &[0], &[0], xlog_cuda::JoinType::Inner)
        .expect("Hash join failed");
    assert_eq!(join_result.num_rows(), 3, "Expected 3 matching rows (keys 2,3,4)");
    println!("✓ Inner hash join: 5 rows ⋈ 5 rows = {} matched rows", join_result.num_rows());

    // Test 3: Semi and Anti joins
    println!("\n--- Test 3: Semi/Anti Joins ---");
    let semi = provider.hash_join_v2(&left, &right, &[0], &[0], xlog_cuda::JoinType::Semi)
        .expect("Semi join failed");
    assert_eq!(semi.num_rows(), 3);
    println!("✓ Semi join: {} rows (keys in both)", semi.num_rows());

    let anti = provider.hash_join_v2(&left, &right, &[0], &[0], xlog_cuda::JoinType::Anti)
    ...
    let groups: Vec<u8> = [0u32,0,0,1,1,1,1,2,2,2].iter().flat_map(|v| v.to_le_bytes()).collect();
    let values: Vec<u8> = (1u32..=10).flat_map(|v| v.to_le_bytes()).collect();
    let group_buf = provider.create_buffer_from_slices(&[&groups, &values], group_schema.clone())
        .expect("create group buffer");

    let agg_result = provider.groupby_multi_agg(
        &group_buf,
    ...
    let dup_keys: Vec<u8> = [1u32,2,2,3,3,3,4,4,4,4].iter().flat_map(|v| v.to_le_bytes()).collect();
    let dup_buf = provider.create_buffer_from_slices(&[&dup_keys], dup_schema.clone())
    ...
    let set_a: Vec<u8> = [1u32,2,3,4,5].iter().flat_map(|v| v.to_le_bytes()).collect();
    let set_b: Vec<u8> = [4u32,5,6,7,8].iter().flat_map(|v| v.to_le_bytes()).collect();
    let buf_a = provider.create_buffer_from_slices(&[&set_a], set_schema.clone()).unwrap();
    let buf_b = provider.create_buffer_from_slices(&[&set_b], set_schema.clone()).unwrap();

    let union_result = provider.union(&buf_a, &buf_b).expect("Union failed");
    println!("✓ Union: {} ∪ {} = {} elements", buf_a.num_rows(), buf_b.num_rows(), union_result.num_rows());

    let diff_result = provider.diff(&buf_a, &buf_b).expect("Diff failed");
    assert_eq!(diff_result.num_rows(), 3, "Expected 3 elements in A-B (1,2,3)");
    ...
    let unsorted_keys: Vec<u8> = [5u32,3,1,4,2].iter().flat_map(|v| v.to_le_bytes()).collect();
    let unsorted_vals: Vec<u8> = [50u32,30,10,40,20].iter().flat_map(|v| v.to_le_bytes()).collect();
    let unsorted = provider.create_buffer_from_slices(&[&unsorted_keys, &unsorted_vals], sort_schema.clone()).unwrap();

    let sorted = provider.sort(&unsorted, &[0]).expect("Sort failed");
    let sorted_keys = provider.download_column_u32(&sorted, 0).unwrap();
    assert_eq!(sorted_keys, vec![1,2,3,4,5], "Keys should be sorted ascending");
    ...
    let arrow_vals: Vec<u8> = (0..50).map(|i| (i as f64) * 0.1).flat_map(|v| v.to_le_bytes()).collect();
    let arrow_buf = provider.create_buffer_from_slices(&[&arrow_ids, &arrow_vals], arrow_schema.clone()).unwrap();

    let record_batch = provider.to_arrow_record_batch(&arrow_buf).expect("Arrow export failed");
    let reimported = provider.from_arrow_record_batch(&record_batch).expect("Arrow import failed");
    assert_eq!(reimported.num_rows(), 50);
    println!("✓ Arrow roundtrip: export → import preserved {} rows", reimported.num_rows());
    ...
    let multi_mem = MultiGpuMemoryManager::new(Arc::new(pool), budget.clone()).expect("Multi-GPU manager");
    let _ = multi_mem.alloc_on_device::<u32>(0, 1000).expect("Multi-GPU alloc");
    ...
    let filter_buf = provider.create_buffer_from_slices(&[&filter_ids, &filter_vals], filter_schema).unwrap();

    // Filter: keep even indices (0,2,4,6,8)
    let filter_mask: Vec<u8> = (0..10).map(|i| if i % 2 == 0 { 1 } else { 0 }).collect();
    let filtered = provider.filter_by_mask(&filter_buf, &filter_mask).expect("Filter failed");
    assert_eq!(filtered.num_rows(), 5);
    println!("✓ Filter by mask: 10 rows → {} rows (even indices)", filtered.num_rows());
