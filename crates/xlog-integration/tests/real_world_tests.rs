...
/// Device-memory budget shared by the canonical provider fixture.
const TEST_BUDGET_BYTES: usize = 1024 * 1024 * 1024;

/// Single canonical provider fixture used by every test in this file.
fn create_test_executor() -> Option<(Executor, Arc<CudaKernelProvider>)> {
    let provider = Arc::new(
        CudaProviderBuilder::new(0, MemoryBudget::with_limit(TEST_BUDGET_BYTES as u64))
            .build()
            .ok()?,
    );
