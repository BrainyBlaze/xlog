...
fn run_spike_tc(fix: &Fixture, edge_buf: &CudaBuffer, domain: u32) -> (CudaBuffer, u32) {
    ...
    loop {
        ...
        let novel = provider
            .fj_delta_novel_u32_recorded(&delta, &edge_norm, &full_r, domain, StreamId::DEFAULT)
...
fn fj_delta_step_matches_cpu_oracle() {
    ...
    let novel = fix
        .provider
        .fj_delta_novel_u32_recorded(&delta_buf, &edge_norm, &r_buf, 64, StreamId::DEFAULT)
        .expect("novel step");
...
fn fj_delta_empty_inputs_yield_empty_novel() {
    ...
    let novel = fix
        .provider
        .fj_delta_novel_u32_recorded(&empty_delta, &edge_norm, &r_buf, 8, StreamId::DEFAULT)
    ...
    let novel2 = fix
        .provider
        .fj_delta_novel_u32_recorded(&delta, &edge_norm, &r_full, 8, StreamId::DEFAULT)
