...
pub fn rule_provenance(
    ...
    if let Some(generated) = generated_program {
        ...
        for rule in &generated.rules {
            ...
            out.push(rule_record(
                generated_idx,
                rule,
                RuleSourceKind::Generated,
            ));
            generated_idx += 1;
        }
    }
...
fn format_arith_expr(expr: &ArithExpr) -> String {
    match expr {
        ...
        ArithExpr::Add(left, right) => {
            format!("({} + {})", format_arith_expr(left), format_arith_expr(right))
        }
        ...
        ArithExpr::Min(left, right) => {
            format!("min({}, {})", format_arith_expr(left), format_arith_expr(right))
        }
        ArithExpr::Max(left, right) => {
            format!("max({}, {})", format_arith_expr(left), format_arith_expr(right))
        }
        ArithExpr::Pow(left, right) => {
            format!("pow({}, {})", format_arith_expr(left), format_arith_expr(right))
        }
