...
pub struct FastParseStats {
    ...
    /// Bytes of source handed to pest (the residual).
...
pub fn parse_program_with_stats(input: &str) -> Result<(Program, FastParseStats)> {
    ...
    let mut stats = FastParseStats {
        ...
        residual_bytes: scan.residual.len(),
        fell_back: false,
    };
    ...
    let merged = XlogParser::parse(Rule::program, &scan.residual)
        ...
        .and_then(|pairs| build_program_merged(pairs, facts, &scan));
    match merged {
...
/// semantically relevant: it fixes rule order and therefore the plan).
fn build_program_merged(
    pairs: ParseResult<'_>,
    facts: Vec<crate::fact_fast_path::FastFact>,
    scan: &crate::fact_fast_path::FactScan,
) -> Result<Program> {
    ...
    for pair in pairs {
        ...
        for inner in pair.into_inner() {
            ...
            let stmt_offset = scan.original_offset(inner.as_span().start());
            while let Some(fact) = facts.next_if(|fact| fact.offset < stmt_offset) {
                program.rules.push(fact.rule);
            }
...
///
/// A `not` before any operator negates the modal subformula to its right. A
/// `not` before the atom dualizes the atom-adjacent modal (`know not p` becomes
/// `not possible p`; `possible not p` becomes `not know p`). Under the same S5
/// collapse, all outer modal operators preserve the already-global inner modal
/// truth value, so the final single-level literal is determined by the
/// atom-adjacent operator, atom negation duality, and parity of `not` placements.
...
mod tests {
    ...
    fn test_nested_modal_chain_atom_adjacent_negation_dualizes() {
        // know possible not p ≡ not know p
        let lit = first_epistemic_literal("q() :- know possible not p().");
        assert_eq!(lit.op, EpistemicOp::Know);
        assert!(lit.negated);

        // possible know not p ≡ not possible p
