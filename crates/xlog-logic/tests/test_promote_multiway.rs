...
// D2 Free Join — general multiway Compiler-pipeline coverage. A
// 4-atom chain has no dedicated kernel shape; the general promoter
...
fn compile_general_chain_program_produces_generic_multiway_body() {
    ...
    match &rule.body {
        RirNode::MultiWayJoin {
            ...
            assert!(var_order.is_none(), "general promotion carries no var order");
            assert!(matches!(fallback.as_ref(), RirNode::Project { .. }));
        }
