...
/// Cache identity and authoritative event ledger for one compilation attempt.
pub(crate) struct CircuitCompilationContext<'a> {
    pub(crate) canonical_cnf_hash: Option<u64>,
    pub(crate) ledger: &'a CircuitCompilationLedger,
}
...
pub(crate) fn compile_gpu_d4_and_verify_cached_with_ledger(
    ...
    cache: &mut GpuCircuitCache,
    random_vars: &DeviceRandomVarList,
    context: CircuitCompilationContext<'_>,
) -> Result<(GpuCircuitCacheHandle, Option<CircuitCompileProfile>)> {
    let CircuitCompilationContext {
        canonical_cnf_hash,
        ledger,
    } = context;
    if config.cdcl_conflict_budget.is_some() {
