...
//! grouped by the variable-order root W, the same (W, count) row set as the
//! unfused production path (materialize 4-cycles, then groupby count) —
//! WITHOUT materializing the 4-cycle rows. Both paths are checked against a
//! host brute-force oracle.
...
/// Sorted (W, count) pairs from a 2-column (U32 key, U64 count) buffer.
...
/// Host brute-force oracle: per-W count of (X, Y, Z) 4-cycle completions.
...
/// Unfused production baseline: materialize 4-cycles, then groupby count.
...
fn run_case(
    ...
    let expected = oracle_group_counts(e1_rows, e2_rows, e3_rows, e4_rows);
    assert!(
        !expected.is_empty(),
    ...
    let baseline = baseline_group_counts(&fix, &e1, &e2, &e3, &e4);
    assert_eq!(baseline, expected, "{name}: unfused baseline vs oracle");

    let stream = fix.pool.acquire().expect("stream");
    let fused = fix
        .provider
        .wcoj_4cycle_groupby_root_count_u32_recorded(
            &e1,
            &e2,
            &e3,
            &e4,
        ...
        .expect("fused 4-cycle groupby-root count");
    let fused = download_group_counts(&fix.memory, &fused);
    assert_eq!(fused, expected, "{name}: fused vs oracle");
}

#[test]
fn cycle4_groupby_root_count_matches_oracle_small() {
    // W=1 closes 3 cycles, W=2 closes 2; W=3 has e1/e2/e3 paths but no
    // closing e4 edge back to itself.
    let e1 = sorted_unique([(1, 10), (1, 11), (2, 10), (3, 12)]);
    let e2 = sorted_unique([(10, 20), (10, 21), (11, 20), (12, 22)]);
    let e3 = sorted_unique([(20, 30), (21, 30), (22, 31)]);
    let e4 = sorted_unique([(30, 1), (30, 2), (31, 9)]);
    run_case("cycle4_small", &e1, &e2, &e3, &e4);
}

#[test]
fn cycle4_groupby_root_count_matches_oracle_skewed_hub() {
    ...
    // Heavy per-root fanout: many work units feed the same root counter.
    ...
    run_case("cycle4_skewed_hub", &e1, &e2, &e3, &e4);
}

#[test]
fn cycle4_groupby_root_count_empty_intersection_roots_are_absent() {
    // W=9 has e1 edges but no completing (X,Y,Z): it must NOT appear in
    // the fused output (group-by over the join result, not over e1).
...
fn wcoj_4cycle_groupby_root_count_measurement_fused_vs_unfused() {
    ...
    for (name, (e1_rows, e2_rows, e3_rows, e4_rows)) in &cases {
        ...
        let _ = baseline_group_counts(&fix, &e1, &e2, &e3, &e4);
        let warm = fix
            .provider
            .wcoj_4cycle_groupby_root_count_u32_recorded(
                &e1,
