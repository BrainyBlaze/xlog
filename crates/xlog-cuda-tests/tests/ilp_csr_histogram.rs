...
xlog_cuda_tests::gpu_test_skip!(
    test_csr_histogram_basic,
    |ctx: &xlog_cuda_tests::harness::TestContext| {
        ...
        let mut d_sorted = ctx
            .memory
            .alloc::<u32>(sorted_facts_host.len())
            .expect("alloc sorted_facts");
        ctx.device
            .inner()
...
xlog_cuda_tests::gpu_test_skip!(
    ...
    |ctx: &xlog_cuda_tests::harness::TestContext| {
        ...
        let d_sorted = ctx
            .memory
            .alloc::<u32>(1)
            .expect("alloc dummy sorted_facts");
...
xlog_cuda_tests::gpu_test_skip!(
    test_csr_histogram_single_element,
    |ctx: &xlog_cuda_tests::harness::TestContext| {
        ...
        assert_eq!(
            hist_host,
            vec![0, 0, 1, 0],
            "single element histogram mismatch"
        );
    }
);
