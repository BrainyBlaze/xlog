...
usage() {
  cat <<'EOF'
usage: scripts/stage_pyxlog_kernels.sh [--to <dir>] [--features <cargo-features>]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --features)
      if [[ $# -lt 2 ]]; then
        echo "--features requires a Cargo feature list" >&2
        exit 2
      fi
      features="$2"
      shift 2
...
resolve_kernel_out_dir_from_dep_info() {
  ...
  python3 - "$target_dir" "$features" <<'PY'
...
require_policy = "semantic-policy" in sys.argv[2].replace(",", " ").split()
...
build_kernels_release() {
  ...
  local build_cmd=(cargo build -p xlog-cuda --release --locked)
  if [[ -n "$features" ]]; then
