...
Where the trainable rule bodies of \Cref{subsec:trainable-bodies} learn rule \emph{weights} and per-binding confidences, differentiable ILP learns rule \emph{structure}; the two are complementary. This sparse, GPU-resident formulation avoids the cubic blowup of dense rule materialization. Unlike the trainable-body surface, this path is not evaluated in \Cref{sec:ecinduction}: it remains validated by demonstrations rather than head-to-head benchmarks, and is a beta subsystem (\Cref{sec:limits}).
