...
\begin{table}[htbp]
  \centering
  \caption{What judges what, on which path. Convergence and supplied held-out examples are preconditions, not gates: without them the pipeline returns for manual review before any row runs.}
  \label{tab:gates}
  \small
  ...
  \begin{tabularx}{\columnwidth}{@{}LLl@{}}
    ...
    \midrule
    Six always-run gates
      & the discovered rule derives every training positive and no training negative, passes a novel-fact audit against a bound on unexplained derivations, loses no fact of a protected relation, meets a held-out F1 threshold, and has typed schemas for the relations it names
      & dILP \
    \addlinespace
    Two held-out gates
      & the same rule on the supplied held-out positives and negatives
      & dILP \
    \addlinespace
    Ambiguity scan
      & which alternative candidates also explain the data; reported beside the gates, never decisive
      & dILP \
    \addlinespace
    Permutation-null fit threshold
      & whether a candidate generalizes better than shuffled labels do on its own fold
      & arbiter (\Cref{subsec:engine-mode}) \
    \bottomrule
  \end{tabularx}
\end{table}
