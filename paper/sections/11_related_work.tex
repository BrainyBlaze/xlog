...
xlog inherits ideas from a long line of logic languages, but is the first to target GPU execution of the full surface. Prolog~\cite{swiprolog} offers unification and metaprogramming on a CPU interpreter; Mercury~\cite{mercury} introduced strong typing and modes, compiling to native code. Souffl{\'e}~\cite{souffle} is a typed Datalog compiling to C++ for program analysis, and clingo~\cite{clingo} provides answer-set semantics on CPU grounders and solvers. From these xlog adopts the typed-predicate discipline, but targets device kernels, and it unifies typed Datalog, probabilistic facts, neural predicates, and epistemic operators in one language compiled entirely to the GPU.
