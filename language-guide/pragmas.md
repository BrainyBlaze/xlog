...
Pragmas are **entry-file-scoped**. The file you pass to `xlog run`, `xlog prob`, or
...
```text
warning[W0510]: `#pragma magic_sets` in imported module `corpus/base` is ignored
  = note: pragmas apply only when declared in the entry file
```

This scoping is deliberate: pragmas set program-global engine behavior — the
inference engine, the sampling seed, the rewrite strategy — and a library
module should not silently reconfigure every program that imports it. If an
imported module's pragma matters for your program, restate it in the entry
file.
...
time and must be strictly greater than zero. Both CLI flags and explicitly supplied
Python `evaluate(...)` or `evaluate_device(...)` arguments override these source values;
omitted Python arguments preserve the source pragmas.
</Note>
