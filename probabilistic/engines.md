...
The data plane runs on the GPU, while compilation and level-by-level evaluation are
host-orchestrated:
...
<Steps>
  ...
  <Step title="Decision-DNNF">
    ...
    whose worlds can be counted exactly in a single pass. XLOG smooths the circuit,
    then certifies that final smoothed circuit before caching or evaluating it. A GPU
    SAT solver (using `CDCL`, the standard conflict-driven clause-learning algorithm)
    checks logical equivalence to the formula; bounded status and error scalars are
    observed on the host, while the proof trace is checked on the GPU.
  </Step>
...
This accepted-world-view axis is available for direct and split-batch GPU execution
evidence. It covers caller-owned incremental circuits, source and parsed-program
compilation, conditioned and unconditioned evaluation, gradient evaluation, and PIR/CNF
encoding. Every entry point passes through the same evidence validation and transactional
trace accounting; batch methods apply that contract independently to each accepted world
view rather than bypassing it through a separate implementation.
...
Exceeding a structural, domain, sample, or other fixed-count capacity stops with
`CapacityExceeded`, including the name of the exhausted unit. Exceeding the configured
byte budget stops with `ResourceExhausted` before the failing allocation. Reaching a
bounded fixpoint's iteration limit without convergence stops with
`ConvergenceFailure`. These conditions are distinct so callers do not mistake a count
limit or nonconvergence for VRAM exhaustion. There is no larger host-sized execution
path.
...
**Monte Carlo — certified sampled core.** For the sampled region, the megakernel keeps
...
whether you draw `128` samples or `1024`. After the single launch and synchronization,
one pinned terminal receipt transfers authoritative convergence status, counts, and
trace metadata. A nonconverged receipt is an error even if partial counters exist.
...
per circuit level. No per-level data is read back — the circuit and its intermediate
values stay on the device. Device-result APIs retain requested result and gradient
buffers on the GPU; host-convenience APIs copy the result fields the caller requests.
The exact route also observes bounded solver status and error scalars. It therefore
does **not** offer the single-launch sampled-core guarantee of the Monte Carlo engine.
