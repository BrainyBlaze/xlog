...
<Note>
Published artifacts (GitHub releases, PyPI, crates.io) correspond to tagged
...
source](#build-from-source) or use the [local Python development
install](#local-python-development-install).
</Note>
...
Use this when you want the source-tree API or plan to modify xlog.
...
Use this when you want to call xlog from Python. Install a published `pyxlog` wheel:
...
the [local development install](#local-python-development-install) below.
...
Use this when you want the source-tree API available from Python, or when you need
`pyxlog` installed into a specific Python interpreter.
