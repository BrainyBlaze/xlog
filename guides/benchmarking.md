...
real benchmark run, and this page names the harness or the procedure that produced
them so you can rerun it. Others are **targets** — a number a benchmark is *aimed*
at, which no run backs at all. When you quote XLOG's performance, quote the
...
All three rows below carry a measured speedup. They differ in how much of the
underlying run you can inspect. The neural-symbolic cache ablation publishes its
per-run data in this repository at
`examples/neural/results/evidence/cache_ablation_20260218.json`, so you can open
...
| Result | Speedup | Fixture / setup | Reproduce |
|---|---|---|---|
| WCOJ super-hub | `10.5x` to `33.8x` | Deterministic super-hub triangle fixtures (`crates/xlog-integration/benches/wcoj_triangle_bench.rs`) | Worst-case-optimal join versus binary-join chain on skewed graphs. Run `cargo bench -p xlog-integration --bench wcoj_triangle_bench -- wcoj_triangle/superhub`, then divide the `off` cell's time by the `force` cell's time at the same width and row count (for example `u32-50K-off` over `u32-50K-force`). Needs a CUDA device. |
| Aggregate-fused WCOJ | `6.05x` / `5.37x` | Hub fixtures | count/sum/min/max-by-root fused over a triangle body. No bench target for this route ships in the repository, so time your own such query twice: once normally, and once with `XLOG_DISABLE_WCOJ_GROUPBY_FUSION=1`, which forces the materialize-then-group-by path. |
| Neural-symbolic cache ablation | `2.74x` | `01_minimal` MNIST addition, end-to-end | Circuit caching vs no caching; 95% CI `[2.29, 3.18]`, measured 2026-02-18. Committed evidence: `examples/neural/results/evidence/cache_ablation_20260218.json`. |
...
**Aggregate fusion** folds a `count`/`sum`/`min`/`max` directly into the join
...
Two different runs sit behind the neural-symbolic numbers on this page. They used
different settings, so read each number against its own run rather than mixing
them.

**The cache ablation** — the run the `2.74x` comes from — used the `01_minimal`
...
| Configuration | Mean wall time | Per-seed wall times |
|---|---|---|
| Caching on | `88.89 s` | 85.02 s, 91.77 s, 89.89 s |
| Caching off | `242.90 s` | 249.88 s, 244.94 s, 233.88 s |

The published `2.74x` is the mean of the three per-seed ratios (`2.94`, `2.67`,
...
| `first_epoch_sec` | ~75 s | Epoch 1, which pays the Decision-DNNF compile and verify; warm starts drop to ~0.26 s |
| `steady_epoch_sec_mean` | ~0.25 s | Mean of epochs 2–5, after the circuit is cached; batched evaluation |
| `per_query_ms` | ~1.0 ms | **Steady-state** cost of one query's forward and backward pass — a steady epoch divided by its 256 pairs. Not to be confused with the amortized `per_query_ms` written into the run artifacts, which divides *total* training time by queries × epochs and so carries the cold first epoch. |
...
(audited 2026-06-10). No run backs them at all. The Criterion harnesses
...
# Run one benchmark target, filtered to one Criterion group
cargo bench -p xlog-integration --bench wcoj_triangle_bench -- wcoj_triangle/superhub
```

The WCOJ benchmarks need a CUDA device. Without one they print a `Skipping …: No
CUDA device` line and exit successfully rather than failing, so check for that line
...
is the one where WCOJ has the most to gain, and it is where the `10.5x` to `33.8x`
...
| `XLOG_UNION_CHUNK_BYTES` | Chunk budget in bytes for the multiway union that merges same-head rule outputs; bounds peak device memory for many-rule heads | `1073741824` (1 GiB) |
