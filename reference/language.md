...
This reference describes XLOG's current released language surface. It has three
layers:
...
- recursion — covering determined **stratification** (layering the program so no
  predicate depends on its own negation), positive Gelfond 1991 compatibility
  recursion (Gelfond 1991 is an alternative epistemic semantics, selectable as a
  compatibility mode), stratified negated modals, and cyclic negated modals. All
  of these run through the `xlog-gpu` GPU-backed Well-Founded Semantics (WFS)
  plan — a fixpoint that assigns each atom true, false, or undefined — which
  replaces the older `xlog_prob` host-side WFS solver.

Only genuinely unbounded, unsafe, or unfounded modal cycles still fail closed:
they are rejected with a diagnostic instead of being executed.
...
The language-completeness contract is a language-surface expansion over the core runtime. New syntax is
accepted only when it can be normalized into the existing typed AST, RIR
(XLOG's internal relational intermediate representation), probabilistic IR
(PIR), optimizer, runtime, WCOJ (worst-case-optimal join), and CLI paths.
...
### Current epistemic support and boundaries

The current source accepts the epistemic literals and the `epistemic_mode`
...
- positive Gelfond 1991 compatibility recursion;
...
- modal cycles without a founded order.
...
- [`architecture/dilp-training.md`](/neural/rule-learning)
- `architecture/rfc-tensorized-ilp.md`
- [`architecture/python-bindings.md`](/reference/python)
...
The CLI explain subset reports deterministic parse, AST,
...
The CLI smoke implementation reads a multiline stdin session, preserves
...
results.
The CLI smoke implementation supports `--once`, `--explain`, and
