...
| `ResourceExhausted` | A byte-addressed GPU allocation, buffer, or workspace exceeds its memory budget; carries the operation context, required bytes, and available bytes. It is not used for row, candidate, slot, dimension, or iteration limits. |
| `CapacityExceeded` | A bounded count or representational capacity is insufficient; carries the context, required count, limit, and unit (for example candidates, rows, slots, or launch elements). |
| `ConvergenceFailure` | A bounded iterative computation reached its iteration limit without convergence; resident Monte Carlo reports the unconverged and total world counts and the configured limit. |
...
| `UnsupportedEpistemicConstruct` | An epistemic construct known to the frontend but unsupported in the given context; names the construct and the context. Common source-level constructs are described under [Epistemic rejections](#epistemic-rejections). |
| `ConstraintViolation` | An authored integrity constraint produced one or more witness rows; carries the authored source-order index, generated relation name, and witness-row count. |
...
the table below covers the source-level constructs users are most likely to encounter.
(A *modal* literal is one written with `know`
...
| `modal tuple-key cons pattern` | A modal key uses an open-ended cons pattern whose tail length is not statically fixed. Bind a fixed-arity list of scalar terms instead. |
| `modal tuple-key predicate reference` | A modal key uses a predicate reference, which has no scalar GPU key-column encoding. |
| `modal tuple-key aggregate` | A modal key contains an aggregate result instead of a fixed scalar key term. Bind the aggregate outside the modal key. |
| `nested modal tuple-key structure` | A list or compound modal key contains another structured value. Only one fixed outer structure containing scalar or symbol elements can be flattened to relation columns. |
...
After a resident Monte Carlo launch, XLOG reads a fixed three-word device summary.
If any sampled world exhausted its sparse-row capacity, evaluation returns
`CapacityExceeded`; if any world failed to converge, evaluation returns
`ConvergenceFailure`. Probabilities are never returned from a partial or
unconverged resident run.
...
| `RuntimeError` | Any `XlogError` propagated from the engine (including parse, compilation, kernel, execution, byte-memory exhaustion, count-capacity exhaustion, and convergence failure), host-output calls on a build without the `host-io` feature, and construction of fallback `RelationEvidence` when `pyxlog._native` is unavailable. |
