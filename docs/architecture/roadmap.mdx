---
title: "Roadmap"
description: "A concise capability roadmap for XLOG's implemented architecture and future engineering priorities."
---
...
## Implemented source-tree capabilities

The source tree includes the released architecture above and additional public
contracts. Release notes identify the first packaged artifact that contains each
new contract. Implemented capabilities include:

- **Native relation evidence** — ordered semantic roles and provenance records
  bind to complete facts of any positive arity and follow every relation mutation
  atomically.
...
- keep diagrams accurate as they distinguish the three reasoning
  representations (RIR, PIR, and EIR) from the shared `GpuCnf` and CDCL solver
  service;
