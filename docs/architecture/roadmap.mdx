...
New main-only work is tracked here as it lands. Currently on `main` beyond the
0.10.0 artifacts:

- **Batched multiway unions** — same-head rule outputs are merged with one
  chunked multiway union per evaluation pass instead of one union per rule
  (chunk budget overridable via `XLOG_UNION_CHUNK_BYTES`).
- **Ignored-pragma warnings** — `warning[W0510]` on stderr when a pragma appears
  in an imported module; pragmas apply only when declared in the entry file.
