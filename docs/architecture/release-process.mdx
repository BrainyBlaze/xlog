...
`CHANGELOG.md` records which release first contains a change; entries under
`[Unreleased]` describe source-tree work that has not entered a tagged artifact.
...
CUDA hardware. A targeted workflow runs native Python relation tests on the
self-hosted CUDA runner after pushes to `main`, or when a maintainer dispatches
the workflow from `main` for an explicitly reviewed full commit SHA. It never
runs pull-request head code automatically, and it is not the release gate.
Before publication a maintainer runs the validation script on a real GPU host:
...
provenance, contract, public-API, and callback suites, runs CUDA certification,
and checks the packaged kernel layout. A green GitHub-hosted CI run does not
satisfy this gate — only a passing complete GPU run does.
...
The source tree also implements chunked multiway unions for same-head rule
outputs, with a byte budget controlled by `XLOG_UNION_CHUNK_BYTES`. Imported
module pragmas produce `warning[W0510]` on stderr because pragmas apply only in
the entry file. Check the selected artifact's release notes before depending on
either behavior in a packaged build.
