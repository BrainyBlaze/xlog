...
This script requires CUDA hardware. It sets `XLOG_REQUIRE_CUDA=1`, builds and
installs the exact release wheel in an isolated environment, runs the native
relation-provenance Python suites and CUDA certification suite, and checks that
kernels are laid out correctly in the package. A green GitHub-hosted CI run does
not satisfy this gate — only a passing complete GPU run does.
...
- batched multiway unions — same-head rule outputs merged in one chunked
  multiway union per evaluation pass instead of one two-way union per rule
  (chunk budget overridable via `XLOG_UNION_CHUNK_BYTES`);
- ignored-pragma warnings — `warning[W0510]` emitted on stderr when a pragma is
  declared in an imported module; pragmas apply only when declared in the entry
  file.
