...
2. expand user-defined function calls in rule bodies after imports are merged,
   using the entry program's configured recursion limit;
...
5. desugar supported shared-variable epistemic constraints before choosing an
   execution route;
6. desugar queries and constraints when entering the deterministic compiler;
7. apply eligible magic-set rewrites (see [Magic Sets](#magic-sets));
8. validate deterministic negation safety (see [Deterministic Negation](#deterministic-negation));
9. stratify dependencies — sort the rules into layers so that negation is
   evaluated only after the relations it reads are fully computed;
10. lower to RIR — xlog's internal typed intermediate representation — and continue
