...
The deterministic engine used by `xlog run` and inspected by `xlog explain`
processes accepted source in this order. The probabilistic engines have separate
compilation pipelines and do not enter this deterministic pre-compilation sequence.
