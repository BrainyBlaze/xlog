...
| Targeted native Python CI | Self-hosted CUDA runner after pushes to `main`, or a maintainer dispatch from `main` for a reviewed full commit SHA | Installed-wheel relation provenance and membership-mask behavior on a real GPU without exposing the persistent host to automatic pull-request code. |
...
self-hosted job exercises a focused native Python surface after merge or an
