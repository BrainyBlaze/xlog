# W4.2 Nested-Loop Join Operator — Plan (iteration 3 canonical)

**Plan iteration:** 3 (amendment after iteration-2 review surfaced F-W42-7..12 — 3 blocking + 2 major + 1 minor).
...
## Direction (locked, iteration 3 canonical)
...
| **D3** | **Provider API surface (per F-W42-2, F-W42-5, F-W42-7, F-W42-9, F-W42-11).** | Add `pub fn nested_loop_join_v2_inner_u32_1key(left, right, left_key, right_key) -> Result<CudaBuffer>` on `CudaKernelProvider`. **Implementation site (per F-W42-7): inside `crates/xlog-cuda/src/provider/relational.rs`** alongside `hash_join_v2_*` — NOT a new sibling module — because the W4.2 fn must call `gather_buffer_by_indices` (currently private at `relational.rs:2394`) and `combine_schemas`/`buffer_from_columns`, which are private impl methods on `CudaKernelProvider` defined in `relational.rs`. Keeping the new fn in the same file avoids a visibility-change step and matches the convention that all join-family provider methods live together. Body sequence: (1) validate eligibility — 1 key col, types match per D1, key cols within arity, sizes within threshold (per F-W42-5: return `Err(XlogError::Kernel)` if caller violates threshold; per F-W42-10: type equality strictly checked); (2) read row counts (`device_row_count`); (3) **empty-input fast path (per F-W42-9)**: if `num_left == 0 \|\| num_right == 0`, return `self.create_empty_buffer(self.combine_schemas(left.schema(), right.schema()))` BEFORE any allocation or launch — mirrors `hash_join_inner_v2`'s empty handling at `relational.rs:3546-3552`; (4) compute `upper_bound = num_left * num_right` in `u64`, validated against `NESTED_LOOP_TOTAL_THRESHOLD`; (5) allocate `output_left_idx, output_right_idx: TrackedCudaSlice<u32>` of length `upper_bound`; allocate counter `TrackedCudaSlice<u32>` of length 1, zero via `memset_zeros`; (6) launch kernel — **per F-W42-11: pass `left.column(left_key)?` and `right.column(right_key)?` (`&CudaColumn`) directly to the launch tuple, NOT a `CudaColumn::Owned`-specific accessor**. cudarc's launch trait accepts `&CudaColumn` regardless of variant (Owned/Dlpack/ArrowDevice) — same generic pass-through `compare_const_mask` uses at `crates/xlog-cuda/src/provider/filter.rs:574-578`; (7) `device.synchronize()`; (8) D2H counter via `dtoh_scalar_untracked` (single u32); (9) `gather_buffer_by_indices(left, &output_left_idx, output_rows)?` and same for right; (10) concat columns and call `self.buffer_from_columns(result_columns, output_rows as usize, combined_schema)` with `combined_schema = self.combine_schemas(left.schema(), right.schema())`. **No silent capacity clamp** (F-W42-5): within eligibility, allocation is exact and bounded at 32 MB total for both u32 index arrays. NO API entry for fall-back to hash inside the provider — fallback decisions are made by the executor's dispatch site. |
...
1. Validate eligibility (per D1, D3, F-W42-10):
   ...
   - `lt == rt && matches!(lt, Some(ScalarType::U32) | Some(ScalarType::Symbol))` — strictly check type equality AND admitted set.
   - `num_left * num_right <= NESTED_LOOP_TOTAL_THRESHOLD` (per F-W42-5: exceed → `Err(XlogError::Kernel)`).
2. **Empty-input fast path (per F-W42-9)**: if `num_left == 0 || num_right == 0`, return `self.create_empty_buffer(self.combine_schemas(left.schema(), right.schema()))?` BEFORE any allocation or launch. Mirrors `hash_join_inner_v2`'s empty handling at `relational.rs:3546-3552`.
3. `upper_bound = (num_left as u64) * (num_right as u64)`; cast to `u32` after threshold check (4M fits in u32).
4. Allocate `d_output_left_idx: TrackedCudaSlice<u32>` of length `upper_bound`. Same for `d_output_right_idx`. Allocate `d_output_count: TrackedCudaSlice<u32>` of length 1; zero it via `memset_zeros`.
5. **Key-column pointers (per F-W42-11)**: `let left_col = left.column(left_key)?;` and `let right_col = right.column(right_key)?;` — these return `&CudaColumn`. Pass them directly into the launch tuple as `(left_col, right_col, ...)`. Do NOT match on the `CudaColumn` variant; cudarc's launch trait accepts `&CudaColumn` regardless of `Owned` / `Dlpack` / `ArrowDevice` (verified by `compare_const_mask` at `crates/xlog-cuda/src/provider/filter.rs:574-578`).
6. Launch the kernel with grid `(num_left + 255) / 256`, block `256`.
7. `device.synchronize()`.
8. D2H the counter via `self.dtoh_scalar_untracked(&d_output_count, 0)?`.
9. Gather: `let gathered_left = self.gather_buffer_by_indices(left, &d_output_left_idx, output_rows)?;` same for right.
10. Combine: `let combined_schema = self.combine_schemas(left.schema(), right.schema()); self.buffer_from_columns(result_columns, output_rows as usize, combined_schema)`. Output schema is the FULL `[left_cols, right_cols]` concatenation per F-W42-1 + D2.

NO new module file. NO visibility change to `gather_buffer_by_indices`. NO `Owned`-variant assumption.
...
* `crates/xlog-runtime/src/executor/node_dispatch.rs` — `use xlog_cuda::provider::NESTED_LOOP_TOTAL_THRESHOLD;` (import, not redeclare). At the top of `execute_join` (BEFORE the existing adaptive-indexing branch), check `eligible_for_nested_loop(...)` + `(num_left as u64) * (num_right as u64) <= NESTED_LOOP_TOTAL_THRESHOLD`. If both pass, call `provider.nested_loop_join_v2_inner_u32_1key(...)` + increment counter + return. Else fall through to the existing hash path (unchanged).
...
## Acceptance Grid (iteration-3 canonical)
...
## Source-of-Truth References (iteration-3 canonical)
...
## Risk Register (informational, iteration-3 canonical)
...
| Empty-input dispatch path: `num_left == 0 || num_right == 0` reaches the kernel and launches with grid_dim 0 | Per F-W42-9: provider's empty fast path returns `create_empty_buffer(combine_schemas(...))` BEFORE any allocation or launch — mirrors `hash_join_inner_v2` at `relational.rs:3546-3552`. The dispatch site (Step 5) may also short-circuit empties before calling the provider. Cert A/B/C fixtures do not exercise empty inputs; a sixth cert could be added in a future iteration if regression coverage warrants. |
...
This plan is **iteration 3 draft** (iteration 2 had F-W42-7..12 surfaced; live D-table + Step plan + Acceptance Grid + Risk Register rewritten in place; stale "iteration 1" / "iteration 2" labels swept to "iteration 3 canonical"). The agent does NOT advance to Step 2 until the user explicitly states "Iteration 3 is approved" (or equivalent). Subsequent iterations may add further F-W42-N findings; the live D-table + Step plan + Acceptance Grid above are the canonical source of truth.
