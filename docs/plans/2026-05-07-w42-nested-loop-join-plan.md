...
3. Validate eligibility (per D1, D3, F-W42-10, F-W42-14):
   ...
   - **Byte-length match (per F-W42-14)**: assert `left_col.num_bytes() == (num_left as usize) * 4` AND `right_col.num_bytes() == (num_right as usize) * 4` (since u32 = 4 bytes); on mismatch return `Err(XlogError::Kernel)`. Mirrors `compare_const_mask`'s validation at `crates/xlog-cuda/src/provider/filter.rs:545-556`.
