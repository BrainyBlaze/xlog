...
Bench the production provider path (NOT the spike kernel) on multi-col fixtures matching the production eligibility envelope. The bench is provider-direct (`provider.nested_loop_join_v2_inner_u32_1key` and `provider.hash_join_v2`) for envelope-parity perf measurement; executor dispatch wiring is already covered by Cert A and Cert E and is not part of the timed Step 12 region:
...
* Output: `docs/evidence/2026-05-08-w42-production-bench/README.md` with median timings + speedup table.
