# W4.3 Sort-Merge Join Operator — Iteration-1 Plan

**Plan iteration:** 1 (first draft, post-spike).
...
| ID | Lock | Direction |
|----|------|-----------|
| **D1** | **Sortedness detection mechanism**. | **Option B (runtime detection kernel)** per recon. New kernel `check_ascending_sorted_u32` in `crates/xlog-cuda/kernels/sort.cu` — single-pass scan, returns `1` if `keys[i] <= keys[i+1]` for all `i`, else `0`. Provider fn `provider.is_sorted_ascending_u32(buf, key)` wraps the kernel + reads the u32 result via `dtoh_scalar_untracked` (single-u32 D2H, same metadata-only profile as `hash_join_v2`'s row-count reads). The dispatch site at `execute_join` calls this fn ONLY when the join is otherwise eligible for sort-merge (Inner + 1-key + matching U32/Symbol + size-eligible per D3); detection cost is paid only on candidates, NOT on every join. Mirrors the established WCOJ layout fast-path pattern at `crates/xlog-cuda/src/provider/wcoj.rs:3137-3187` (u32) and `:3265-3307` (u64), but checks "sorted ascending" only — duplicates allowed (sort-merge handles run-length). The kernel does NOT check uniqueness. **Out of scope for W4.3**: producer-side metadata tracking (option C) and IR-level annotation (option D); both are larger structural changes that can be considered in v0.6.6+ if benchmark data justifies eliminating the per-dispatch detection cost. |
| **D2** | **Dispatch precedence vs W4.2 nested-loop**. | **Sort-merge takes precedence when sorted**, nested-loop when not sorted (and size-eligible), hash otherwise. Decision tree at `execute_join`:<br>1. Eligible for sort-merge envelope (Inner + 1-key + matching U32/Symbol)?<br>&nbsp;&nbsp;a. Yes AND size-eligible (D3)?<br>&nbsp;&nbsp;&nbsp;&nbsp;i. Both inputs detected sorted via D1 kernel? → **sort-merge**.<br>&nbsp;&nbsp;&nbsp;&nbsp;ii. Either input not sorted? → fall through.<br>&nbsp;&nbsp;b. Not size-eligible? → fall through.<br>2. Eligible for nested-loop envelope (W4.2 D1 + size-eligible)? → **nested-loop**.<br>3. Else → **hash**.<br><br>Spike data justifies this precedence: sort-merge wins ≥ 2.52× over hash on sorted inputs; nested-loop wins ≥ 2.10× over hash on small Cartesian inputs (per W4.2 production bench). Sort-merge's win is larger across the entire tested overlap region, so when both could apply, sort-merge is chosen. The detection kernel cost (single-pass O(L+R) scan) is small relative to either operator's win and is paid only on sort-merge candidates. |
...
## Step-by-Step Execution Plan (12 steps, mirrors W4.2 structure)
...
File: `crates/xlog-cuda/src/provider/relational.rs` (edit, alongside W4.2's nested-loop fn) + `crates/xlog-cuda/src/provider/sort.rs` (edit).

* `pub fn sort_merge_join_v2_inner_u32_1key(left, right, left_key, right_key) -> Result<CudaBuffer>` in `relational.rs`. Mirrors W4.2's `nested_loop_join_v2_inner_u32_1key` literal idioms (per F-W42-13/14/15/16/17): byte-length lower-bound check (`<` failure), `checked_mul` for threshold, no `?` on `create_empty_buffer`, `as u64` for `row_cap`, `combine_schemas` for output schema, `gather_buffer_by_indices` for materialization.
* `pub fn is_sorted_ascending_u32(buf, key_col) -> Result<bool>` in `sort.rs` (or wherever sort provider fns currently live). Wraps the new `check_ascending_sorted_u32` kernel; allocates 1-element u32 output, initializes to 1 on device (`memset` 0x01 byte = wrong; need `htod_sync_copy_into(&[1u32], ...)`); launches kernel; reads result via `dtoh_scalar_untracked`; returns `Ok(result == 1)`.
...
At the top of `execute_join`, BEFORE the W4.2 nested-loop branch, add:
...
if eligible_for_sort_merge(left, right, left_keys, right_keys, join_type) {
    ...
    if in_threshold {
        let lk = left_keys[0];
        let rk = right_keys[0];
        let left_sorted = self.provider.is_sorted_ascending_u32(left, lk)?;
        let right_sorted = self.provider.is_sorted_ascending_u32(right, rk)?;
...
### Step 10 — Cert E + Cert F: Symbol-typed dispatch + duplicate-key run-length
...
Commit subject: `test(w43): cert E + F — Symbol-typed + duplicate-key run-length dispatch`.
...
Bench production kernel + dispatch path on multi-col fixtures matching production eligibility. Provider-direct envelope-parity vs `hash_join_v2`. D7 acceptance #8: ≥ 2× win on eligible cells. Output: `docs/evidence/<YYYY-MM-DD>-w43-production-bench/README.md`.

Commit subject: `feat(w43): add production sort-merge bench + evidence`.
...
| **Post-impl bench** | 1 | `w43_production_sort_merge_bench.rs` (new) | sort-merge wins ≥ 2× vs hash on eligible cells |
| **Workspace pass-count delta** | **+7** | — | 7 new test cells (A through F including D'); D folded as parity tail. |
...
This plan is **iteration 1 draft**. The agent does NOT advance to Step 2 until the user explicitly states "Iteration 1 is approved" (or equivalent). Subsequent iterations may add F-W43-N findings; the live D-table + Step plan + Acceptance Grid above are the canonical source of truth.
...
* Plan length: ~430 lines (intentionally tighter than W4.1's 757-line iteration-7 final and slightly shorter than W4.2's iteration-1 ~370). Subsequent iterations may expand if F-W43-N findings warrant.
* No paper-claim alignment is required — sort-merge join is internal optimization, not paper-grounded closure.
* Spike evidence is treated as **load-bearing input**: the threshold value (4M, shared with W4.2), the post-impl bench acceptance (≥ 2×), and the dispatch-precedence decision (sort-merge > nested-loop) are all derived from spike measurements. If subsequent W4.3 iterations contradict the spike, the spike evidence README is the canonical reference.
