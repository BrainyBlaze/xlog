...
**Plan iteration:** 6 (post-execution amendment capturing F-W43-14 — Step 12 bench rejection of D2 + D7 #8 → operator-only scope; and F-W43-15 — workspace-test gate exception expanded from F-W43-12's single file to enumerate the three evidenced files `test_wcoj_layout_fast_path.rs`, `test_wcoj_layout_u32.rs`, `test_wcoj_layout_u64.rs` after Step 7' surfaced flake instances in the latter two. Sibling files `test_wcoj_layout_sort_*.rs` were verified to pass 82/82 standalone and are NOT exempt — F-W43-15 is by evidenced-file enumeration, not by glob. Iterations 1–4 were pre-execution review; iteration 5 was post-execution alignment of plan-record with executed work (F-W43-11/12/13); iteration 6 is the post-bench scope-redirect amendment + post-Step-7' enumerated-flake-exception amendment.).
...
**Workspace-test gate exception (per F-W43-12 + F-W43-15)**: three evidenced files in `crates/xlog-cuda/tests/` — `test_wcoj_layout_fast_path.rs`, `test_wcoj_layout_u32.rs`, `test_wcoj_layout_u64.rs` — are a **pre-existing flake class** unrelated to W4.3. Iteration 5 (F-W43-12) documented `test_wcoj_layout_fast_path` only; iteration-6 Step 7' verification surfaced additional flake instances in `test_wcoj_layout_u32` and `test_wcoj_layout_u64` (each passes 6/6 standalone-serial but fails non-deterministically when run alongside other CUDA tests in the workspace) — same v0.6.2 WCOJ-fast-path code, same data-corruption signature consistent with stale GPU memory reads from polluted device-context state. F-W43-15 enumerates the three evidenced files **only** — sibling files in the same directory (`test_wcoj_layout_sort_roundtrip.rs`, `test_wcoj_layout_sort_u32.rs`, `test_wcoj_layout_sort_u64.rs`) were verified to pass 82/82 standalone and are NOT exempt; the exception is by evidenced-file enumeration, not by glob pattern. Out-of-W4.3-scope to fix; deferred to follow-up work on the v0.6.2 fast-path code. Step 11's workspace-test gate is satisfied iff:
...
* AND every test outside the three evidenced files (`test_wcoj_layout_fast_path.rs`, `test_wcoj_layout_u32.rs`, `test_wcoj_layout_u64.rs`) in the canonical workspace command exits 0. Sibling files in the same directory must pass.
...
This plan is **iteration 6** (iteration 5 was approved through Step 11; iteration 5's Step 12 bench surfaced the F-W43-2 anticipated counter-finding; iteration 6 amends the W4.3 closure scope to "operator implemented, production dispatch rejected by evidence" per F-W43-14, and expands the workspace-test gate exception to enumerate the three evidenced files per F-W43-15). Iterations 1-4 follow the pre-execution review pattern; iteration 5 captured execution-discovered drift (F-W43-11/12/13); iteration 6 is the post-bench scope-redirect amendment (F-W43-14) + post-Step-7' enumerated-flake-exception amendment (F-W43-15). The live D-table + Step plan + Acceptance Grid above remain the canonical source of truth.
...
Iteration 6 captures two execution-discovered findings: F-W43-14 (the major scope redirect from the Step 12 bench rejection) and F-W43-15 (broadened workspace-test gate exception surfaced during Step 7' verification). F-W43-14 is the primary iteration-6 amendment — anticipated by F-W43-2's iteration-1 instruction *"If the bench shows nested-loop wins on the overlap, iteration-N+ amends D2"* — and reshapes W4.3 closure scope from "production dispatch closure" to "operator implemented, production dispatch rejected by evidence." F-W43-15 is a follow-on to F-W43-12: same root cause (v0.6.2 WCOJ-fast-path GPU-state-pollution flake), broader file-set scope.
...
| **F-W43-15** | Major | Step 7' verification surfaced flake instances in `crates/xlog-cuda/tests/test_wcoj_layout_u32.rs` (`wcoj_layout_u32_already_sorted_deduped_round_trips`) and `crates/xlog-cuda/tests/test_wcoj_layout_u64.rs` (`wcoj_layout_u64_already_sorted_deduped_round_trips`) that fail non-deterministically under workspace concurrency but pass 6/6 standalone-serial. Same v0.6.2 WCOJ-fast-path code as F-W43-12, same data-corruption signature consistent with stale GPU memory reads from polluted device-context state. Three runs surfaced three different victim test files: Step 11-era run hit `test_wcoj_layout_fast_path` (multiple failures); Step 7' run 1 hit `test_wcoj_layout_u64::wcoj_layout_u64_already_sorted_deduped_round_trips`; Step 7' run 2 hit `test_wcoj_layout_u32::wcoj_layout_u32_already_sorted_deduped_round_trips`. The flake is non-deterministic in WHICH test fails within an evidenced file, but only three files have actually flaked. Sibling files in the same directory (`test_wcoj_layout_sort_roundtrip.rs`, `test_wcoj_layout_sort_u32.rs`, `test_wcoj_layout_sort_u64.rs`) were verified during Step 7' review to pass 82/82 standalone — they do NOT flake despite living in the same directory and touching some of the same v0.6.2 WCOJ code. | F-W43-12 named only `test_wcoj_layout_fast_path.rs` in the exception scope; the iteration-5 evidence was correctly narrow at that time (only `test_wcoj_layout_fast_path` had reproduced). | F-W43-12 exception amended to **enumerate** the three evidenced files: `test_wcoj_layout_fast_path.rs`, `test_wcoj_layout_u32.rs`, `test_wcoj_layout_u64.rs`. The exception is by file-enumeration, NOT by glob pattern: sibling `test_wcoj_layout_sort_*.rs` files are NOT exempt and must pass (Step 7' verified 82/82 standalone). The narrow enumeration prevents the exception from accidentally hiding regressions in unrelated sibling files that happen to share the `test_wcoj_layout_` prefix. D7 #12 + Step 11 prose + Step 7' prose all amended in-place. The cert suite (the authoritative gate per MEMORY.md) remains 1/1 — its GPU-context fixture isolation explains why it passes where the workspace test runs flake. Out-of-W4.3-scope to fix; deferred to follow-up work on the v0.6.2 fast-path code (the same deferral F-W43-12 documented). |
...
**F-W43-15 process observation**: F-W43-15 is the same class as F-W43-12 (workspace-test gate exception for v0.6.2 WCOJ-fast-path flakiness). The first iteration-6 draft of F-W43-15 proposed broadening F-W43-12's scope to a glob pattern `test_wcoj_layout_*` on the reasoning that "the next victim is highly likely to be a sibling file with the same root cause." User review during Step 7' approval rejected the glob as **overbroad**: sibling files `test_wcoj_layout_sort_roundtrip.rs`, `test_wcoj_layout_sort_u32.rs`, and `test_wcoj_layout_sort_u64.rs` were independently verified to pass 82/82 standalone, demonstrating that "same directory" and "same prefix" are NOT reliable proxies for "same root cause." Per user direction, F-W43-15's final form enumerates the three evidenced files only. The compounding lesson: **flake exceptions should be enumerated by evidence, not generalized by file-name pattern.** A glob pattern that exceeds evidenced scope hides future regressions in sibling files — the exact harm the gate exists to catch. This is a stronger refinement of the F-W43-13 file-wide-concern lesson: contracts and flake exceptions BOTH require evidence-driven scope, but evidence-driven scope for an exception is narrower (only the actually-failing items), not broader (the whole containing class). Future plan-discipline improvement: when a flake exception is broadened, do NOT generalize via pattern — re-run the gate against sibling files to confirm whether each one actually flakes, then enumerate the actual failures.
...
### Step 7' — Final verification gate (per F-W43-12 + F-W43-15 exception)
...
* `cargo test --workspace --release --exclude pyxlog --exclude xlog-cuda-tests` exits 0 for every test path EXCEPT three evidenced files in `crates/xlog-cuda/tests/`: `test_wcoj_layout_fast_path.rs`, `test_wcoj_layout_u32.rs`, `test_wcoj_layout_u64.rs` (per F-W43-12 + F-W43-15 enumerated-files exception). Sibling files in the same directory (`test_wcoj_layout_sort_roundtrip.rs`, `test_wcoj_layout_sort_u32.rs`, `test_wcoj_layout_sort_u64.rs`) are NOT exempt — they were verified to pass 82/82 standalone and must continue to pass.
