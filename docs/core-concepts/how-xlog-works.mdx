...
inputs to the shared CDCL solver service. Acyclic modal programs use the
Generate-Propagate-Test workspace, positive FAEEL cycles use an ordinary founded
least-fixpoint reduction, supported Gelfond-1991 exact-tuple possibility cycles use
a greatest compatibility fixpoint over concrete tuple snapshots, and supported
cycles through negation use the GPU-backed WFS plan.
