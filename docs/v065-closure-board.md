...
| W2.6 | ROADMAP item #16 | OPEN | — | Selectivity + heat statistics feed into the variable ordering from W2.1. Closes the loop between observed stats and slot ordering. **Scheduled last within Wave 2** — both blockers (W2.1 + W2.4) are now DONE so this transitions OPEN. | Cert: hot relation gets preferred lookup-key slot; cold extensional relation gets iteration-key slot; row-set agreement preserved. |
