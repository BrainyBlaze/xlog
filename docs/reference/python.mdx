...
require five explicit keyword arguments in addition to their admission and
authority inputs:
...
masks, and actor eligibility become part of the task identity checked during
restoration. See the
...
contains the producer's 136-byte header, explicit answer boundary, MASK and
autoregressive supervision, causal-memory retention labels and forest geometry.
Its bytes, dimensions and target coordinates are bound to the original row.

An episode requires authentic native execution evidence. An anchor instead
retains its corpus admission evidence and retention group; it cannot be selected
as native execution replay or confer actor credit. Its record contains exactly
`example_id`, `group`, `training_view`, `training_view_identity` and
`content_sha256`; corpus admission and training geometry do not carry an
invocation's model generation. Neither carrier integrity nor
corpus admission replaces the trusted application's fresh use permissions.
Importing this carrier does not establish that device-side training, Update
publication or restoration has completed.

During proposal replay, the native importer owns final use of its original
policy invocation. It schedules native finalization before asking the Runtime to
retire its temporary aliases; reader retirement then joins the named consumer
streams. Full predecessor and successor comparisons remain required. The Runtime
...
Call `step.reserve_model_work(capacity)` during `prepare_segment`, before capture.
The positive capacity bounds event occurrences, including every saved tensor.
Native reserves the device budget before allocating the host roster and device
buffer. It does not run the model to discover a capacity.
...
retained model inputs. The result is
`(predecessor_identity, successor_identity_or_None, model_generation,
model_geometry_digest, model_numerical_digest, carrier_or_None)`. Each identity has the four
fields returned by `PublishedParent.identity()`. The generation and both
32-byte digests belong to the acquired predecessor, not the final segment bank.
A successful Proposal carrier exposes `numerical_realization`, `logits`, and
`random_state` as native-owner identity plus canonical bytes. Recompute, Update,
Drain, and refused steps return `None` for the carrier; a completed numerical
refusal also has no successor. A skipped step, an uncompleted
segment, or uncertain execution cannot return a binding.
...
`completed_action_projection(consumer_streams=[...])` before retiring the
segment. This typed cold projection returns the authenticated predecessor and
...
and distinct action RNG base and successor materials. The projection does not
manufacture an acquired-theory identity; that value remains an application-owned
assembler input.

The projection contains the admitted slot-zero record, learned lane slots 1 and 2
with their native admission or refusal, exact non-NULL text actions and
...
export does not supply canonical logits, complete RNG state, or complete
