...
state change. Variable ids are the 1-based CNF ids reported by `prob_var_map()`;
only entries with `kind == "fact"` are mutable. Annotated-disjunction choices,
...
non-zero and `cpu_only_probability_recomputations == 0`. The four evidence classes
