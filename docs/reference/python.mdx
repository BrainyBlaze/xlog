...
require four explicit keyword arguments in addition to their admission and
authority inputs:
...
| `task_query_ordinals` | Two zero-based query ordinals, in admitted statement order; both queries must return zero-arity results. |
| `task_scoring` | Six nonnegative `u32` coefficients in order: correct, all-correct, work, improvement, refusal, spent. |
| `admissible_truth_masks` | Two nonzero four-bit masks, one per query; bit `n` permits truth value `n`. |
...
and masks become part of the task identity checked during restoration. See the
...
model_geometry_digest, model_numerical_digest)`. Each identity has the four
...
A completed numerical refusal has no successor. A skipped step, an uncompleted
