...
load, then enforce that role contract on later replacements and manifest imports.
...
different records for the same complete tuple remain distinct. Repeated fact
entries are merged by exact typed tuple, exact duplicate records are collapsed,
and facts and records are returned in deterministic canonical order.

Evidence follows the native relation lifecycle atomically:

- `put_relation_with_provenance` replaces both rows and evidence;
- plain `put_relation` replaces the rows and clears old evidence;
...
- `remove_relation` and `clear_relations` remove the matching evidence.
...
Nullary relations do not accept relation provenance.
...
`relation(name)` returns a native `RelationEvidence` snapshot.
`evidence(name=None)` returns a deterministic `program_hash` and a `relations`
mapping of native snapshots. These are not Python package sidecar records.
...
friendly `tuple`. Every dictionary level rejects missing or unknown fields. If
`metadata_present` is false, both `roles` and `facts` must be empty.

Import validates the manifest, compiled schema, DLPack column shape and types,
row count, and complete-fact membership before replacing the target. Fact and
record order and exact duplicates are normalized deterministically. The DLPack
columns are process-local, single-consumer ownership objects; the manifest may
be serialized as data, but it does not contain the relation columns and is not a
cross-process persistence format. Keep the exported columns alive until import
consumes them. For portable storage, use a host-serialized relation format and
treat native manifest reconstruction as a separate in-process operation.
