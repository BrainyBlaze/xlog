...
friendly `tuple`. Each cell's `hex` value encodes the scalar's exact
little-endian bytes. Every dictionary level rejects missing or unknown fields.
If `metadata_present` is false, both `roles` and `facts` must be empty.
