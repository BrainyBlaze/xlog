...
`evaluate(dlpack_inputs=...)`, the world view it produces here is empty
(`accepted_world_views == 0`), every counter in `result.trace` is `0`, and the
conditioned query silently falls back to its unconditioned prior -- this does not
raise. Check `accepted_world_views` (via `epistemic_evidence()`) or the trace
counters before trusting a result from a program that relies on caller-supplied
facts.
...
`result.log_z_e` is the log-evidence of what is known: the natural log of the
product of the priors of the atoms the world view conditioned on, not of the whole
query program. Measured on GPU: one known atom at prior 0.6 gives `log_z_e ==
ln(0.6)`; conditioning on two known atoms each at prior 0.5 gives `log_z_e ==
ln(0.25)`.
...
program's own source -- a program that depends on a caller-supplied relation
reports `accepted_world_views == 0` and every other counter at `0` here too, not
an error.
