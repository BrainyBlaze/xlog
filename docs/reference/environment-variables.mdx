...
| `CAVIAR_CONTINUOUS_DIR` | the CAVIAR pytest modules | Directory holding local `caviar-train.json` / `caviar-test.json`. `test_caviar_alignment_evidence.py` and `test_caviar_xml_corpus.py` skip themselves unless both files are found there. | path; default `../caviar_data` relative to the test file |
...
The shipped CAVIAR example reads one variable of its own. It is not part of the
runtime.
