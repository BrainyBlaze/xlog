...
description: "Comprehensive reference for XLOG syntax, semantics, data types, rules, negation, aggregations, probabilistic and epistemic logic, and the complete grammar."
---
...
Separate import branches cannot provide competing definitions under one predicate or
function name. Predicate clauses may extend an imported predicate within the same module;
functions have one body and cannot be redefined after import. An imported module that
defines an exported function name more than once is rejected with `error[E0410]`.

Repeated public declarations for one predicate must be identical. A difference in arity,
column names, or resolved types is rejected with `error[E0408]`. Domain-backed types are
compared using the scalar type to which each local alias resolves. Reusing one domain alias
name for different scalar types in the entry program or import closure is `error[E0409]`.
