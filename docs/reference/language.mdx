...
incompatible column types from the declared schemas of the predicates they touch
— whether the conflict is between two body atoms or between a body atom and the
head. The error names the rule, the variable, both types, and where each came
from:
...
before that the same program compiled here and failed later with an internal GPU
schema error that did not name the rule.
...
When a parameter is used as a relational term, its argument must be a variable or
numeric literal. Materialize a compound arithmetic expression with an earlier `is`
binding before passing it. Predicate-bodied calls in conditional result branches are
rejected because their relational literals cannot be hoisted without changing guarded
semantics.

### Recursive definitions

Expansion is eager across both conditional branches rather than data-dependent, so a
cyclic call chain reaches the configured expansion-depth error even when a conditional
branch looks like a base case. Function recursion is not a runtime looping construct.
