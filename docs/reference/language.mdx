...
Compatible public facts and rules for the same predicate are merged across resolved import
branches into one relation. The same union semantics apply when selective imports name
that predicate in multiple modules. Functions have one body: separate branches cannot
...
Repeated public declarations for one predicate must resolve to the same schema. A
difference in arity, column names, or resolved types is rejected with `error[E0408]`.
