...
`E0506` and `E0507` are reserved for an internal function-annotation type-inference
module. That module is not wired into production compilation or the CLI, so these codes
are not CLI behavior.
