...
User-defined functions expand after module resolution. Function expansion reports these
diagnostics before lowering an unsupported or ambiguous call:

| Code | Trigger |
|---|---|
| `E0508` | A function call supplies a different number of arguments than its declaration. |
| `E0509` | An expression-only caller tries to expand a predicate-bodied function without an ordinary rule or constraint body to receive its relational literals. |
| `E0510` | An arithmetic expression other than a variable or numeric literal would have to become a relational term in a predicate body. Bind the arithmetic result first and pass its variable. |
