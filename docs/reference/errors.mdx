...
| `E0402` | Distinct providers in separate resolved import branches supply the same predicate or function name, or a module redefines a function imported within its own branch. |
...
| `E0408` | The entry program or imported modules declare the same predicate with different arity, column names, or resolved types. |
...
concatenation step applies analogous `Concat: ... too large` caps. These are
fail-closed declines, never silent truncation: a relation column past 4 GiB
stops the run instead of scattering rows.
...
|---|---|
| `ValueError` | Invalid parameters: an unknown `prob_engine` (expected `exact_ddnnf` or `mc`), an unknown `sampling_method` (expected `rejection` or `evidence_clamping`), `memory_mb=0`, row counts exceeding `u32` range, neural input shape or value validation, and exporting provenance for an unstored relation. |
