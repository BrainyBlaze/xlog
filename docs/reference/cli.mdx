...
| `--stats` | Emit per-stratum timing and memory statistics to stderr, followed by a `WCOJ dispatch:` line carrying the triangle, 4-cycle, group-by-fusion, free-join, and factorized-delta dispatch counts plus the decline count. Under `--stats-format json` those same counts appear in a `wcoj` object as `triangle_dispatch`, `four_cycle_dispatch`, `groupby_fusion_dispatch`, `free_join_dispatch`, `factorized_delta_dispatch`, and `error_decline`. | off |
...
<Warning>
Imported rules have taken part in probabilistic evaluation only since 0.12.0.
Before 0.12.0, `--module-path` on `xlog prob` checked that the imported modules
could be loaded, but their rules were never merged into the program that was
actually compiled and evaluated. A program that imports deterministic rules can
therefore report different probabilities on 0.12.0 than it did on 0.11.0. The
0.12.0 numbers are the correct ones.
</Warning>
...
stderr naming the module and the dropped directive (once per source file) and ignore
the pragma; the exit code is unaffected. This warning is new in 0.12.0 — before
that the pragma was dropped with no diagnostic at all. See
