...
`SemanticTaskScoring` coefficients, and two truth-eligibility masks. XLOG does
not prescribe the statement predicate, arguments, task answers, or reward
weights. The resident transition ABI still has two query slots and two candidate
...
for a self-contained XLOG source program with two selected zero-arity queries.
...
Scoring maximizes `correct_weight * correct_queries + all_correct_weight *
all_correct - work_weight * measured_work` among eligible candidates, retaining
the earlier candidate on ties. Return scoring separately prices improvement,
refusals, and spent work. All six coefficients are required nonnegative `u32`
values; combinations that can overflow the signed return are rejected.
...
bytes, expected truths, scoring, masks, and admitted record selections.
...
bytes and checks numerical ABI version 1: FP32 vectors of width 128, 18 fields,
the field and adjoint layouts, and the forward and derivative operation
signatures. These are explicit supported-ABI constraints, not support for an
...
not establish numerical correctness or GPU equivalence.
