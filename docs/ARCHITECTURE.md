...
XLOG is a **GPU-native logic programming language for unified symbolic reasoning**. Its compiler and runtime span four reasoning paradigms — deterministic Datalog evaluation, probabilistic inference via knowledge compilation, SAT/MaxSAT verification, and differentiable neural-symbolic training — on a single CUDA runtime with zero host–device transfers in production paths. A shared language frontend lowers source programs through a staged IR pipeline (AST $\to$ RIR for relational execution, with PIR and the XGCF circuit format for probabilistic inference; a solver IR is planned for SAT/MaxSAT) to GPU-resident execution plans. The four backends are coordinated by a single kernel provider and a common GPU residency model rather than assembled as independent tools.
