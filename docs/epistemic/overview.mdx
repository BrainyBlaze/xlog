...
description: "Reason about what is known and what is possible across the stable models of a program — modal operators, world-view semantics, FAEEL and Gelfond-1991 modes, and GPU-backed execution paths."
---
...
boundary. Positive FAEEL cycles use an ordinary founded reduction and run to their
least fixpoint. Supported exact-tuple G91 possibility cycles run a GPU upper-bound
pass followed by descending frozen-snapshot refinements to the greatest compatible
tuple fixpoint. Supported cycles through negation use the GPU-backed well-founded
plan.
...
literal doubles this search. Positive-cycle and WFS reductions do not enumerate
this single-pass candidate lattice.

A per-reduction cap, `MAX_MODELS_PER_REDUCTION = 1024`, bounds how many models a
Generate-Propagate-Test reduction may produce.

Positive modal dependency cycles are not blanket rejection cases: FAEEL evaluates
them through a founded least fixpoint, and supported exact-tuple Gelfond-1991
possibility cycles use the greatest compatibility fixpoint. The number of
compatibility refinements is bounded by `#pragma max_recursion_depth`. An
unrelated negative cycle can execute through the GPU-backed well-founded evaluator
inside those passes, but recursive negation or aggregation in the compatibility
component itself is rejected because the descending operator would not be
monotone. XLOG returns a typed diagnostic when no supported route can preserve
the program's semantics, such as a recursive epistemic program that also contains
a modal integrity constraint or a cyclic WFS shape outside the supported
negated-modal plan. The complete list is in the
[language reference](/reference/language#epistemic-support-and-boundaries).

<Note>
The Generate-Propagate-Test bound is not a fixed candidate ceiling. There is no
`k <= 24` limit and no `50000`-candidate limit. Its candidate space is
`2^(number of epistemic literals)`, with at most
`MAX_MODELS_PER_REDUCTION = 1024` models per reduction.
</Note>

## Reference

<Card title="How XLOG works" icon="diagram-project" href="/core-concepts/how-xlog-works">
  Epistemic reasoning reuses the shared parser, normalization, type and dependency
  analysis, and CUDA runtime; EIR then selects its route-specific execution plan — see
  how a program travels from source to GPU.
</Card>
