...
through the `pyxlog` PyPI wheel. The additions described under “New in the 0.10.0
wheel” have been available since 0.10.0. This is a Python-first API; the crates
published on crates.io do not expose it, so install `pyxlog` to use anything on
this page.
</Note>
...
These additions have been available in `pyxlog` since 0.10.0.

- **Existential-join trainable bodies.** A trainable body may join a
