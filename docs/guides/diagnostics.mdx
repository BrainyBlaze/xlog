...
relation, and the retained runtime version does not advance. A preparation failure
preserves authoritative rows and evidence but discards the derived cache and retained
runtime, so the next evaluation rebuilds them.
...
complete typed tuple and exact duplicate records collapse.
...
leave evidence for canceled inserts. Plain replacement clears prior evidence, and
relation removal or a session clear removes it. Validation failures change neither data
nor evidence.
...
`session.evidence(name)` raise `KeyError` when the relation is not stored. The native
`RelationEvidence` type, this exception, and the session operations require the bundled
`pyxlog._native` extension.
