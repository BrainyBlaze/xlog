...
All three rows below carry a measured speedup. Of the runs behind them, only the
neural-symbolic cache ablation's artifact is committed to this repository today —
the two WCOJ artifacts are expected to be added from the machine that produced
them.

The harnesses are a separate question. `wcoj_triangle_bench` ships, so the
super-hub row reproduces with one `cargo bench` invocation. No bench target for the
...
The numbers above are what XLOG measured on the fixtures named beside them. If you
need a number for a specific deployment, run the reproduce step on that hardware
and cite what you get.
</Note>
