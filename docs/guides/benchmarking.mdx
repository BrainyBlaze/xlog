...
Exactly one published speedup in this repository is backed by a committed,
source-verified run: the neural-symbolic cache ablation. Its per-seed timings and
confidence interval sit in a checked-in JSON file you can open and recompute from.

The two WCOJ rows are different. The comparison each one describes is real and the
harness (where one exists) ships in this repository — but **XLOG publishes no
speedup figure for them**, because no committed run backs one and the result swings
with GPU, fixture size, and key width. Each row tells you what to run so you can
measure the number on your own hardware.

| Result | Published speedup | Fixture / setup | How you get a number |
|---|---|---|---|
| WCOJ super-hub | None — measure locally | Deterministic super-hub triangle fixtures, `crates/xlog-integration/benches/wcoj_triangle_bench.rs` | Run `cargo bench -p xlog-integration --bench wcoj_triangle_bench -- wcoj_triangle/superhub`, then divide the `off` cell's time by the `force` cell's time at the same width and row count (for example `u32-50K-off` over `u32-50K-force`). |
| Aggregate-fused WCOJ | None — measure locally | Your own skewed hub data; no benchmark for this route ships in the repository | Time your own count/sum/min/max-by-root query over a triangle body twice: once normally, and once with `XLOG_DISABLE_WCOJ_GROUPBY_FUSION=1`, which forces the materialize-then-group-by path. |
| Neural-symbolic cache ablation | `2.74x` | `01_minimal` MNIST addition, end-to-end | Committed evidence: `examples/neural/results/evidence/cache_ablation_20260218.json`. Circuit caching vs no caching; 95% CI `[2.29, 3.18]`, measured 2026-02-18. |
...
see its cost is to turn it off and compare — which is what the row above asks you
to do.
...
<Note>
Do not quote a WCOJ speedup from this page or from XLOG's release notes as if it
were a measured result you can rely on. Numbers that appeared in earlier notes came
from runs whose evidence is not in this repository, so no one — including us — can
re-derive them. Measure on the hardware and fixture you actually care about, and
cite that.
</Note>

### Neural-symbolic training detail

Two different committed runs sit behind the neural-symbolic numbers on this page.
They used different settings, so read each number against its own artifact rather
than mixing them.
...
**Where the time goes within a run.** A separate committed run — 512 images, 5
epochs, `batch_size=64`, 2 seeds, on the RTX PRO 3000 Blackwell laptop GPU —
records the per-epoch shape:

| Metric | seed 7 | seed 42 | What it measures |
|---|---|---|---|
| `compile_api_sec` | 0.012 s | 0.013 s | The `pyxlog` compile call itself, before training starts |
| `first_epoch_sec` | 83.12 s | 77.66 s | Epoch 1, which pays the circuit compile and verify |
| `steady_epoch_sec_mean` | 2.90 s | 2.92 s | Mean of epochs 2–5, after the circuit is cached |
| `total_train_sec` | 94.72 s | 89.33 s | All five epochs end to end |
| `per_query_ms` | 74.0 ms | 69.8 ms | Total training time ÷ (queries × epochs) — an amortized figure, not the cost of one forward pass |

Evidence:
`examples/neural/results/track_a/20260218T_postfix_v040alpha_batched/01_minimal/seed_7/metrics.json`
and the matching `seed_42/metrics.json`.

The first epoch dominates because XLOG compiles the logic program into a
**Decision-DNNF** — a circuit form that makes exact probability counting cheap to
evaluate — and verifies it. Once that circuit is cached, each later epoch costs
about 3 seconds instead of 80. That gap is what the cache ablation measures.
...
`crates/xlog-integration/benches/wcoj_triangle_bench.rs` is the harness you run to
measure WCOJ for yourself. It compares the GPU 3-way worst-case-optimal join
against the ordinary binary-join chain on identical fixtures across `u32`, `u64`,
and a `Symbol` sanity case.
...
is the one where WCOJ has the most to gain, so it is the family to time first. It
runs at 10K and 50K rows per relation by default; set `WCOJ_BENCH_FULL=1` to add
the 100K and 250K sizes. Criterion names each cell `<width>-<rows>-<mode>`, for
example `u32-50K-off` and `u32-50K-force`, so the ratio of those two reported times
is your local speedup. Nothing about that ratio is portable: report it with the GPU
and the fixture size you ran it on.
