...
ownership objects. Each capsule can be consumed once; do not first pass the exported
columns to another DLPack consumer and then try to import them. The capsules retain the
device buffers even if the source session is released, but they are not cross-process or
on-disk persistence. Use Arrow IPC when portability matters, accepting its host copy.
</Warning>
