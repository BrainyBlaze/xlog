...
zero-copy. Persistent session replacement is deliberately different: it makes one
device-to-device copy into session-owned storage. Neither path uses host row data.
XLOG's CUDA backend (`crates/xlog-cuda`) also offers the Arrow C Device interface and
host-serialized Arrow IPC.
