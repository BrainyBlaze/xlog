# Supervisor Report — Epistemic Executor Completion

**Date:** 2026-05-31
**Author:** implementation session (direct development + isolated-worktree subagents)
**Scope:** the epistemic executor completion (7 executor-completion bundles) and the
semantic completion (the determined-modal family).
**Branch:** epistemic semantic-completion branch @ `3c33ac81` (base `4b91e911`, the
executor-completion tip; 30 commits).
**Release status:** NOT pushed, NOT tagged — the release tag is user-gated.
**Superseded by:** the final June 2 epistemic closure record for the package/release
status and the exact GPU-backed WFS contract.
...
non-trivial modal programs" (before executor completion) to **a load-bearing surface where every modal
program is either soundly executed or fails closed for a genuinely-undefined reason —
and nothing in between.**

- **Executor completion** turned the bounded executor into a real surface: 7 implementation bundles (candidate
...
- **Semantic completion** closed the semantic gaps: mixed-literal membership, invariant recursive
...
Gelfond 1991 compatibility **5**, CLI `run_cli_tests` **9**, prob accepted-evidence **31**, prob production-reuse
**7**; `cargo fmt --check` clean; workspace builds. Epistemic showcase examples: **01–31**.

---

## Part I — Epistemic Executor Completion

**Goal:** turn the bounded epistemic executor into a load-bearing surface per the 7-bundle
epistemic executor goal-bundle plan.
...
| Bundle | What landed |
|---|---|
| Value-level membership | Fixed a REAL soundness bug: the global membership gate was a near-no-op letting all rows through for ground/anon/nullary modal literals; now requires the accepted candidate's assumption bit. |
| Candidate enumeration | Runtime already enumerated the 2^N candidate lattice on-device from EIR; the "limitation" was the CPU fixture layer. Added device pilots incl. empty-world-view ≠ failure (mutation-tested for non-vacuity). |
| FAEEL foundedness | Per-tuple-key founded self-support; hardened diagnostics + 9 production-path pilots. |
| Ground constraints | NEW GPU kernel `epistemic_validate_constraints_u8`; ground epistemic constraints prune world views; constraints dropped from the reduced ordinary program (no RIR rewrite). The constraint-specific rejection-index gap closed separately (`e39bcd33`). |
| Safe split | Merge-reason enum + paired split-vs-unsplit equivalence pilot. |
| Joint multi-epistemic | Distinct-predicate multi-epistemic rules solved jointly (reuses candidate enumeration, value-level membership, and the world-view kernel). |
| Nested-modal | Typed fail-closed for `know possible p()`; stabilized diagnostics for `not`-interspersed nested forms. |
...
1. **Nullary EDB facts materialized 0 rows** (`pred().` → empty buffer → asserted-absent
   everywhere). Fix: `create_zero_arity_buffer` (1 unit-tuple row). The membership fix
...
2. **Recursive epistemic programs silently wrong** (epistemic base + ordinary recursive
   ...
   fail-closed guard at the planning chokepoint (later *resolved*, not just guarded, by
   the invariant-recursion and stratified semantic completion paths).
...
## Part II — Semantic Completion (the determined-modal family)

The semantic-completion charter was three honest gaps: (1) mixed-literal modal membership,
(2) invariant recursive epistemic fixpoint, (3) cross-component coupling — "solve jointly OR
...
2. **Invariant recursive epistemic fixpoint** — recursive ordinary predicates whose modal
...
the stratified-partial experiment branch and **proven wrong by bisection: 29 regressions across 3
...
- A *new* `LogicExecutionPlan::EpistemicStratified` materializes the gated lower-stratum
  ...
  *single* existing membership filter.
...
- **NON-DETERMINED / recursive-boundary cases** are resolved by their defined
  semantics when one exists: positive founded recursion executes to the
  founded least fixpoint, FAEEL-unfounded self-support executes to the defined
  empty founded extension, explicit Gelfond 1991 compatibility self-support accepts, finite nested modal
  ...
  contract. Only forms with no founded, Gelfond 1991 compatibility, WFS, finite-key, or safe-variable
...
2. **Genuinely-undefined cases remain fail-closed by design** — this is correct semantics,
   ...
   self-support would require a *defined* semantics for them (e.g. Gelfond 1991 compatibility already accepts the
   circular `possible` form), not a patch.

3. **Executor-completion-era goal-mandated rejections stand** (aggregate/compound/list/predref modal
...
1. **Device pilots ≠ production-path evidence.** Three executor-completion bugs and one semantic-completion
...
The epistemic semantic-completion branch is functionally and semantically complete at
...
rule-body) and the over-broadening gate set; (c) push and tag at the supervisor's
discretion. The stratified experiment branches are retained as bisection evidence for the failed
