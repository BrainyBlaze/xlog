...
Arguments used as relational terms must already be variables or numeric literals. Bind
a compound arithmetic argument first, then pass the bound variable:
...
Every expansion is bounded by `#pragma max_recursion_depth`. Function normalization
expands both conditional branches before execution; it does not evaluate a
data-dependent base case while expanding. A cyclic call chain therefore fails at the
configured depth instead of acting as a runtime loop, even when its syntax contains a
conditional branch that looks like a base case.
