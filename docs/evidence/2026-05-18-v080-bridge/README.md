...
| `crates/pyxlog/src/neural.rs` | Bridge helper implementation, registered-network top-k/deterministic mode reuse, and circuit-cache hit/miss telemetry. |
...
| Command | Result |
|---------|--------|
| `pytest -q python/tests/test_v080_bridge_source.py` | exit 0; 4 passed |
| `cargo check -p pyxlog` | exit 0 |
| `maturin develop -m crates/pyxlog/Cargo.toml` in `/tmp/xlog-v080-cert-venv` | exit 0; rebuilt editable `pyxlog-0.7.0` |
| registered-mode direct neural probes | exit 0; `det=True` target loss `-0.0` with finite grads; `k=2` kept loss `1.313262`, filtered loss `87.49823`, finite grads |
...
| M080_BRIDGE.4 deterministic top-k | fixed seed and tie inputs produce stable ordered results; registered-network `k`/`det` config is consumed by training paths | PASS | `runtime_probe.json`: `deterministic_topk_indices=[2,0,1]` for tied 0.5 scores; `test_v080_bridge_source.py` guards `apply_network_output_mode(...)` in direct, complex, and batched forward/backward paths |
...
- `register_network(..., k=N, det=True)` reuses the existing `NetworkHandle`
  configuration and applies the configured output mode before direct NLL loss
  and cached circuit probability import. Deterministic mode uses top-1 with a
