...
| `runtime_probe.json` | Branch-local pyxlog 0.7.0 runtime probe: 100 fixed-session replays, max tracked host transfers after reset+evaluate, and CUDA Graph counters. |
...
| `/tmp/xlog-v080-cert-venv/bin/python` branch-local runtime probe | exit 0; pyxlog `0.7.0`, 100/100 bit-exact replays, max host-transfer stats all zero |
...
branch's locally built `pyxlog-0.7.0-cp310-cp310-linux_x86_64.whl`.
