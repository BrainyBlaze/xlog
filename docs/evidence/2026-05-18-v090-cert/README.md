...
| M090_CERT.8 GPU-native evidence | GPU launch counts, kernel timings, and zero CPU fallback counters | BLOCKED | GPU-plan, workspace, preflight, counter-guard, and reduced-plan trace contracts exist, but no epistemic GPU launch/timing evidence exists yet. |
