...
| `cargo test -p xlog-integration --test test_epistemic_gpu_wcoj_execution -- --nocapture` | PASS, 27 passed, 0 failed |
...
| M090_CERT.2 solver tests | 100 percent pass for GPU-native solver scope | PARTIAL | `gpu_solver_production_reuse` plus accepted runtime integration prove SAT/UNSAT, workspace-backed UNSAT, one-record and two-record bounded lifecycle, learned-clause arena publication, same-device-CNF learned-clause import/reuse, distinct-CNF learned-clause import rejection, bounded MaxSAT candidate, and bounded SAT/MaxSAT/UNKNOWN/TIMEOUT portfolio production-adapter reuse of `GpuCdclSolver` plus status propagation; broader status-aware lifecycle coverage remains missing. |
...
| M090_CERT.11 solver production reuse | certification includes traces proving accepted SAT/MaxSAT work used existing GPU solver production paths | PARTIAL | `GpuSolverProductionAdapter` requires accepted GPU runtime evidence before calling existing `GpuCdclSolver` SAT/UNSAT, reusable workspace-backed UNSAT, learned-clause arena publication, same-device-CNF learned-clause import/reuse, single- and multi-candidate bounded lifecycle, bounded MaxSAT candidate, and bounded SAT/MaxSAT portfolio APIs; it rejects distinct-CNF learned-clause import before GPU arena reuse, propagates UNKNOWN/TIMEOUT portfolio statuses with zero CPU search counters, and `GpuSolverProductionTrace::require_production_metric_eligibility` rejects CPU-oracle-only metric traces. Broader status-aware lifecycle coverage remains missing. |
