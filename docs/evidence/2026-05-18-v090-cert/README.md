...
| `cargo test -p xlog-runtime --test test_epistemic_gpu_workspace` | PASS, 49 passed, 0 failed |
...
| M090_CERT.9 WCOJ evidence | at least one WCOJ-eligible epistemic reduction proves WCOJ planner/runtime dispatch | PASS | `test_epistemic_gpu_wcoj_execution` compiles epistemic K5, K6, K7, and K8 rules with `know gate()`, registers `EpistemicExecutablePlan::relation_ids`, executes the reduced production runtime plans, observes `EpistemicGpuRuntimeWcojCertification::Certified`, requires `wcoj_clique5_dispatch_count >= 1`, `wcoj_clique6_dispatch_count >= 1`, `wcoj_clique7_dispatch_count >= 1`, and `wcoj_clique8_dispatch_count >= 1`, and materializes one final device-row from each accepted world-view path. The K5 certified trace requires `certified_edge_permutation_slots == 10`, `certified_stream_groups == 1`, `certified_sorted_layout_requirements == 1`, and `certified_helper_split_specs == 1`; the K6 certified trace requires full edge-permutation count `15`, nonzero stream-group/helper/sorted-layout counts, `observed_metadata_builds >= 1`, and `observed_metadata_build_nanos >= 1`; K7/K8 fixtures prove epistemic reductions reuse the G39 W6.4 K-clique planner surface with `kclique_wcoj_max_arity`, full edge-permutation counts, and stream-group counts before production dispatch. |
