...
| M090_CERT.2 solver tests | 100 percent pass for GPU-native solver scope | PARTIAL | `gpu_solver_production_reuse` plus accepted runtime integration prove SAT/UNSAT, workspace-backed UNSAT, bounded lifecycle, bounded MaxSAT candidate, and bounded SAT/MaxSAT/UNKNOWN/TIMEOUT portfolio production-adapter reuse of `GpuCdclSolver` plus status propagation; broader learned-clause lifecycle coverage remains missing. |
...
- broader WCOJ planner/layout/scheduling/helper-splitting coverage beyond the
  accepted K5 fixture;
- broader accepted SAT learned-clause lifecycle traces and MaxSAT coverage;
