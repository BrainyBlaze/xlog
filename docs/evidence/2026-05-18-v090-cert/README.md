...
| M090_CERT.10 nonzero-arity membership | certification includes GPU tuple-key membership evidence for arity >= 1 predicates | PARTIAL | Runtime source and trace tests require arity-one and arity-two tuple-key kernels over existing relation columns; semantic parity fixtures are still missing. |
