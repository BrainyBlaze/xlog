...
| M090_CERT.2 solver tests | 100 percent pass for GPU-native solver scope | BLOCKED | `gpu_solver_production_reuse` proves SAT/UNSAT, workspace-backed UNSAT, and bounded lifecycle production-adapter reuse of `GpuCdclSolver`, but `production_capabilities` explicitly blocks GPU-native MaxSAT and SAT/MaxSAT portfolio execution. |
...
- broader WCOJ planner/layout/scheduling/helper-splitting coverage beyond the
  accepted K5 fixture;
- GPU-native MaxSAT/portfolio evidence and broader accepted SAT learned-clause
  lifecycle traces;
