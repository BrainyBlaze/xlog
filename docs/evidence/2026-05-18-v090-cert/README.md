...
| M090_CERT.2 solver tests | 100 percent pass for GPU-native solver scope | PARTIAL | `gpu_solver_production_reuse` plus accepted runtime integration prove SAT/UNSAT, workspace-backed UNSAT, bounded lifecycle, learned-clause arena publication, bounded MaxSAT candidate, and bounded SAT/MaxSAT/UNKNOWN/TIMEOUT portfolio production-adapter reuse of `GpuCdclSolver` plus status propagation; cross-candidate learned-clause import/reuse remains missing. |
...
- broader accepted SAT cross-candidate learned-clause import/reuse and MaxSAT
  coverage;
