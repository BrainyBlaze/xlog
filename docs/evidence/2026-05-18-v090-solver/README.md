...
counters,
accepted split-batch lifecycle and portfolio dispatch with batch/component evidence counters,
...
portfolio dispatch plus split-batch portfolio dispatch over the existing GPU
...
|---|---|---|---|
| M090_SOLVER.1 interface | trait/API documented and tested | PARTIAL | CPU facade API exists, and `GpuSolverProductionAdapter` exposes GPU CDCL SAT/UNSAT, accepted-runtime SAT/UNSAT and workspace-backed UNSAT gates, bounded single-, multi-candidate, and split-batch accepted lifecycle APIs, accepted split-batch/component trace counters, accepted G91/default FAEEL mode-specific trace counters, accepted operator-family evidence counters, bounded single- and multi-candidate MaxSAT candidate APIs, single-result and two-record MaxSAT search-pruning APIs, single-result and two-record weighted MaxSAT encoding/search APIs, heterogeneous MaxSAT scheduler APIs, and single-result, two-record, plus split-batch SAT/MaxSAT/UNKNOWN/TIMEOUT portfolio APIs; broader solver coverage is still missing. |
