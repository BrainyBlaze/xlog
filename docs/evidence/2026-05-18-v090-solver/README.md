...
| M090_SOLVER.9 production solver reuse | accepted SAT/MaxSAT fixtures execute through existing GPU CNF/CDCL/solver production APIs or thin adapters over them | PARTIAL | SAT/UNSAT adapter calls `GpuCdclSolver::new`, `solve_expect_sat`, `solve_expect_unsat`, and workspace UNSAT; MaxSAT, portfolio, and accepted candidate integration are missing. |
| M090_SOLVER.10 fixture isolation | CPU semantic-oracle solver facade is gated so it cannot satisfy closure metrics | PARTIAL | Evidence docs mark `SolverService` as oracle-only and the production adapter source test rejects `SolverService`; an automated closure gate is still missing. |
