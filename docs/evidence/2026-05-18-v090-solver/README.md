...
- `GpuSolverProductionAdapter`, a thin production-path adapter over the existing
  ...
  gate, bounded weighted MaxSAT encoding/search gate, bounded status-aware
  portfolio gate, learned-clause arena publication gate,
...
inside a bounded MaxSAT search candidate set plus bounded weighted soft-clause
selection encoding into GPU CNF candidates, but it still lacks full generalized
MaxSAT scheduler coverage and full solver release closure.
...
| M090_SOLVER.8 CPU search ban | accepted solver path records zero CPU exhaustive assignment enumeration | PARTIAL | Accepted runtime SAT/UNSAT, workspace-backed UNSAT, single- and multi-candidate bounded lifecycle, lifecycle UNKNOWN/TIMEOUT propagation, learned-clause arena, same-device-CNF learned-clause reuse, two-record learned-clause reuse, distinct-CNF learned-clause rejection, bounded single- and multi-candidate MaxSAT, bounded MaxSAT search pruning, bounded weighted MaxSAT encoding/search, and bounded portfolio gates route GPU evidence into GPU CDCL or a fail-closed/status propagation path and record zero CPU assignment/MaxSAT enumeration and learned-clause transfer counters; generalized MaxSAT scheduler coverage remains missing. |
| M090_SOLVER.9 production solver reuse | accepted SAT/MaxSAT fixtures execute through existing GPU CNF/CDCL/solver production APIs or thin adapters over them | PARTIAL | Accepted SAT/UNSAT/lifecycle/status-aware-lifecycle/multi-candidate-lifecycle/learned-arena/reuse/multi-candidate-learned-reuse/rejection/MaxSAT/multi-candidate-MaxSAT/MaxSAT-search-prune/weighted-MaxSAT-encoding/portfolio fixtures call `GpuCdclSolver::new`, `GpuCnf::from_host`, `solve_expect_sat`, `solve_expect_sat_with_branch_limit`, `solve_expect_unsat`, `solve_expect_unsat_with_branch_limit_ws`, and `solve_expect_unsat_with_branch_limit_ws_importing_learned`; accepted lifecycle and portfolio fixtures also propagate UNKNOWN/TIMEOUT without CPU search. Generalized MaxSAT scheduler coverage remains missing. |
...
- The production adapter is partial accepted-runtime SAT, UNSAT,
  ...
  multi-candidate MaxSAT, bounded MaxSAT search pruning, bounded weighted
  MaxSAT encoding/search, and bounded status-aware portfolio reuse evidence
  only.
- Generalized MaxSAT scheduler coverage remains required before v0.9.0 can
  close.
