...
- `GpuSolverProductionAdapter`, a thin production-path adapter over the existing
  ...
  without using the CPU fixture service. Its accepted-runtime SAT/UNSAT gates
  consume an accepted `EpistemicGpuExecutionResult` before dispatching to GPU
  CDCL.
...
| Accepted runtime SAT/UNSAT gates | `solve_expect_sat_with_gpu_execution_result` and `solve_expect_unsat_with_gpu_execution_result` validate stable tuple-source membership, GPU kernel traces, zero hot-path transfers, and non-empty final device output before calling `GpuCdclSolver` SAT/UNSAT paths. |
...
|---|---|---|---|
| M090_SOLVER.1 interface | trait/API documented and tested | PARTIAL | CPU facade API exists, and `GpuSolverProductionAdapter` exposes GPU CDCL SAT/UNSAT plus accepted-runtime SAT/UNSAT gates; full incremental accepted-candidate API is missing. |
| M090_SOLVER.2 incremental SAT | add/retract assumption fixtures pass on GPU-native path | PARTIAL | CPU oracle fixture exists, and accepted GPU runtime evidence can gate SAT/UNSAT dispatch through `GpuCdclSolver`; add/retract incremental epistemic assumptions are not wired to the GPU path. |
...
| M090_SOLVER.8 CPU search ban | accepted solver path records zero CPU exhaustive assignment enumeration | PARTIAL | Accepted runtime SAT/UNSAT gates route GPU evidence into GPU CDCL and record zero CPU assignment/MaxSAT enumeration counters; MaxSAT/portfolio and multi-candidate lifecycle traces are missing. |
| M090_SOLVER.9 production solver reuse | accepted SAT/MaxSAT fixtures execute through existing GPU CNF/CDCL/solver production APIs or thin adapters over them | PARTIAL | Accepted SAT/UNSAT fixtures call `GpuCdclSolver::new`, `solve_expect_sat`, and `solve_expect_unsat`; capability report blocks MaxSAT and portfolio. |
