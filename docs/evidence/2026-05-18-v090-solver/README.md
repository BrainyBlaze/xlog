...
- `GpuSolverProductionAdapter`, a thin production-path adapter over the existing
  ...
  gate, same-device-CNF learned-clause reuse gate, and distinct-CNF
  learned-clause import rejection guard consume an accepted
...
CPU learned-clause transfers, but it still lacks broader multi-candidate
assumption lifecycle traces, complete status-aware
...
| M090_SOLVER.7 assumption lifecycle | push, solve, retract, and reuse trace proves no assumption leak between candidates | PARTIAL | CPU lifecycle fixture exists; accepted GPU candidate evidence now gates one SAT/UNSAT push/solve/retract sequence and records balanced pushes/retractions, workspace reuse, learned-clause arena publication, bounded same-device-CNF learned-clause import/reuse, and distinct-CNF learned-clause import rejection. Broader multi-candidate lifecycle traces are missing. |
| M090_SOLVER.8 CPU search ban | accepted solver path records zero CPU exhaustive assignment enumeration | PARTIAL | Accepted runtime SAT/UNSAT, workspace-backed UNSAT, bounded lifecycle, learned-clause arena, same-device-CNF learned-clause reuse, distinct-CNF learned-clause rejection, bounded MaxSAT, and bounded portfolio gates route GPU evidence into GPU CDCL or a fail-closed rejection and record zero CPU assignment/MaxSAT enumeration and learned-clause transfer counters; broader lifecycle traces are missing. |
| M090_SOLVER.9 production solver reuse | accepted SAT/MaxSAT fixtures execute through existing GPU CNF/CDCL/solver production APIs or thin adapters over them | PARTIAL | Accepted SAT/UNSAT/lifecycle/learned-arena/reuse/rejection/MaxSAT/portfolio fixtures call `GpuCdclSolver::new`, `solve_expect_sat`, `solve_expect_sat_with_branch_limit`, `solve_expect_unsat`, `solve_expect_unsat_with_branch_limit_ws`, and `solve_expect_unsat_with_branch_limit_ws_importing_learned`; the accepted portfolio fixture also propagates UNKNOWN/TIMEOUT without CPU search. Broader multi-candidate learned-clause lifecycle coverage remains missing. |
...
- The production adapter is partial accepted-runtime SAT, UNSAT,
  ...
  evidence only.
- Broader multi-candidate lifecycle traces, status-aware lifecycle coverage,
  and MaxSAT coverage remain required before v0.9.0 can close.
