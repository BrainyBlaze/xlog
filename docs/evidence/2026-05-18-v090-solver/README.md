...
| M090_SOLVER.2 incremental SAT | add/retract assumption fixtures pass on GPU-native path | PARTIAL | CPU oracle fixture exists, and accepted GPU runtime evidence can gate single- and multi-record SAT/UNSAT push/solve/retract sequences through `GpuCdclSolver`, including mixed unary and binary `possible`/`not possible` plus binary `not know` accepted operator results, plus bounded UNKNOWN/TIMEOUT lifecycle propagation; broader incremental epistemic candidate assumptions are not fully wired. |
