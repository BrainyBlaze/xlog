...
- `GpuSolverProductionAdapter`, a thin production-path adapter over the existing
  ...
  gate, two-record multi-candidate MaxSAT gate, bounded MaxSAT search-pruning
  gate, single-result and two-record weighted MaxSAT encoding/search gates,
  bounded status-aware portfolio gate, learned-clause arena publication gate,
  two-record multi-candidate
  push/solve/retract lifecycle gate,
  two-record multi-candidate learned-clause reuse gate, same-device-CNF
...
propagation plus two-record learned-clause publication/import reuse and
two-record/two-CNF MaxSAT candidate-set execution plus GPU-CDCL UNSAT pruning
inside a bounded MaxSAT search candidate set plus single-result and two-record
weighted soft-clause selection encoding into GPU CNF candidates, but it still
lacks full generalized MaxSAT scheduler coverage and full solver release
closure.
...
|---|---|---|---|
| M090_SOLVER.1 interface | trait/API documented and tested | PARTIAL | CPU facade API exists, and `GpuSolverProductionAdapter` exposes GPU CDCL SAT/UNSAT, accepted-runtime SAT/UNSAT and workspace-backed UNSAT gates, bounded single- and multi-candidate accepted lifecycle APIs, bounded single- and multi-candidate MaxSAT candidate APIs, bounded MaxSAT search-pruning APIs, single-result and two-record weighted MaxSAT encoding/search APIs, and bounded SAT/MaxSAT/UNKNOWN/TIMEOUT portfolio API; broader solver coverage is still missing. |
| M090_SOLVER.2 incremental SAT | add/retract assumption fixtures pass on GPU-native path | PARTIAL | CPU oracle fixture exists, and accepted GPU runtime evidence can gate single- and two-record SAT/UNSAT push/solve/retract sequences through `GpuCdclSolver`, plus bounded UNKNOWN/TIMEOUT lifecycle propagation; broader incremental epistemic candidate assumptions are not fully wired. |
...
- The production adapter is partial accepted-runtime SAT, UNSAT,
  ...
  multi-candidate MaxSAT, bounded MaxSAT search pruning, single-result and
  two-record weighted MaxSAT encoding/search, and bounded status-aware
  portfolio reuse evidence only.
- Full generalized MaxSAT scheduler coverage remains required before v0.9.0 can
  close.
