...
- `GpuSolverProductionAdapter`, a thin production-path adapter over the existing
  ...
  workspace-backed UNSAT gates consume an accepted `EpistemicGpuExecutionResult`
  before dispatching to GPU CDCL.
...
|---|---|---|---|
| M090_SOLVER.1 interface | trait/API documented and tested | PARTIAL | CPU facade API exists, and `GpuSolverProductionAdapter` exposes GPU CDCL SAT/UNSAT plus accepted-runtime SAT/UNSAT and workspace-backed UNSAT gates; full incremental accepted-candidate API is missing. |
| M090_SOLVER.2 incremental SAT | add/retract assumption fixtures pass on GPU-native path | PARTIAL | CPU oracle fixture exists, and accepted GPU runtime evidence can gate SAT/UNSAT and reusable workspace-backed UNSAT dispatch through `GpuCdclSolver`; add/retract incremental epistemic assumptions are not wired to the GPU path. |
| M090_SOLVER.3 learned clauses | transfer observable in GPU trace or test double | PARTIAL | CPU test double observes transfer, and the accepted production adapter can reuse GPU CDCL workspaces after accepted runtime evidence; learned transfer trace from epistemic candidates is missing. |
...
| M090_SOLVER.7 assumption lifecycle | push, solve, retract, and reuse trace proves no assumption leak between candidates | PARTIAL | CPU lifecycle fixture exists, and accepted GPU candidate evidence is counted before SAT, UNSAT, and workspace-backed UNSAT dispatch; push/retract/reuse traces across multiple accepted candidates are missing. |
| M090_SOLVER.8 CPU search ban | accepted solver path records zero CPU exhaustive assignment enumeration | PARTIAL | Accepted runtime SAT/UNSAT and workspace-backed UNSAT gates route GPU evidence into GPU CDCL and record zero CPU assignment/MaxSAT enumeration counters; MaxSAT/portfolio and multi-candidate lifecycle traces are missing. |
| M090_SOLVER.9 production solver reuse | accepted SAT/MaxSAT fixtures execute through existing GPU CNF/CDCL/solver production APIs or thin adapters over them | PARTIAL | Accepted SAT/UNSAT fixtures call `GpuCdclSolver::new`, `solve_expect_sat`, `solve_expect_unsat`, and `solve_expect_unsat_with_branch_limit_ws`; capability report blocks MaxSAT and portfolio. |
...
- The production adapter is partial accepted-runtime SAT, UNSAT, and
  workspace-backed UNSAT reuse evidence only.
