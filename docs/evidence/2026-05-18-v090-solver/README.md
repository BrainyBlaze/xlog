...
- `GpuSolverProductionAdapter`, a thin production-path adapter over the existing
  ...
  gate, and bounded status-aware portfolio gate consume an accepted
...
status-aware SAT/UNSAT/UNKNOWN/TIMEOUT lifecycle coverage beyond the bounded
portfolio adapter, and full solver release closure.
...
| GPU production adapter | `GpuSolverProductionAdapter` exports SAT/UNSAT calls over `GpuCdclSolver`, including workspace-backed UNSAT reuse, bounded MaxSAT candidate solving, bounded SAT/MaxSAT portfolio dispatch, and UNKNOWN/TIMEOUT portfolio status propagation. |
...
| M090_SOLVER.9 production solver reuse | accepted SAT/MaxSAT fixtures execute through existing GPU CNF/CDCL/solver production APIs or thin adapters over them | PARTIAL | Accepted SAT/UNSAT/lifecycle/MaxSAT/portfolio fixtures call `GpuCdclSolver::new`, `solve_expect_sat`, `solve_expect_sat_with_branch_limit`, `solve_expect_unsat`, and `solve_expect_unsat_with_branch_limit_ws`; the accepted portfolio fixture also propagates UNKNOWN/TIMEOUT without CPU search. Broader learned-clause lifecycle coverage remains missing. |
...
- Broader multi-candidate learned-clause lifecycle traces and MaxSAT coverage
