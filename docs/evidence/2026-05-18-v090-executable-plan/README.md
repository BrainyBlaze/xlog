...
| FAEEL foundedness guard | Default FAEEL executable-plan lowering rejects unsupported self-supported `possible` rules before reduced runtime compilation, allows self-`possible` rules with independent ordinary support, and explicit G91 compatibility mode still lowers the same bounded self-support fixture. |
