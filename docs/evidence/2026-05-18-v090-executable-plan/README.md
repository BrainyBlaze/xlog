...
| M090_GPU.2 WCOJ eligibility | at least one epistemic reduction uses the WCOJ planner/path where eligible | PASS | Reduced fixtures reach `RirNode::MultiWayJoin` and K-clique `MultiwayPlan`; accepted runtime evidence executes epistemic K5, K7, and K8 fixtures and observes certified production WCOJ dispatch; K7/K8 preflight evidence proves broader G39 K-clique planner metadata is reused. |
...
The next runtime slice must broaden accepted semantic parity beyond the K5/K7/K8
