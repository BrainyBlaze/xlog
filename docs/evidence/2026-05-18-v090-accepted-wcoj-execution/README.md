...
K-clique planner coverage. The K5 and K7 fixtures prove WCOJ dispatch; the K8
...
| Production WCOJ dispatch | `test_epistemic_gpu_wcoj_execution::accepted_epistemic_k5_execution_certifies_production_wcoj_dispatch` and `accepted_epistemic_k7_execution_certifies_production_wcoj_dispatch` compile epistemic K5/K7 fixtures with `know gate()`, register returned relation IDs, execute the reduced production plans, assert `EpistemicGpuRuntimeWcojCertification::Certified`, and observe `wcoj_clique5_dispatch_count >= 1` / `wcoj_clique7_dispatch_count >= 1`. |
| Existing K-clique path reuse | The accepted K5 fixture requires `kclique_wcoj_plan_count == 1`, `sorted_layout_requirement_count == 1`, `helper_split_spec_count == 1`, and stable tuple-source model membership. The accepted K7 fixture requires `kclique_wcoj_max_arity == 7`, full edge-permutation count `21`, nonzero sorted-layout requirements, certified runtime WCOJ counters, and final tuple materialization. Both use the existing K-clique relation layouts, stats snapshots, WCOJ planner metadata, and runtime counter paths from the G38-B/G39 production evidence chain. |
| K7/K8 planner-surface reuse | `test_epistemic_gpu_wcoj_execution::epistemic_k7_k8_reductions_reuse_g39_kclique_planner_preflight_surface` compiles generated epistemic K7 and K8 reductions with `know gate()`, supplies production-style stats snapshots, and requires `kclique_wcoj_max_arity == 7/8`, full `kclique_wcoj_edge_permutation_count` of 21/28, nonzero sorted-layout requirements, and zero planned-hash/CPU-fallback counters. K7 now also has accepted runtime dispatch evidence; K8 remains compile/preflight evidence only. |
...
- This satisfies accepted WCOJ dispatch evidence for K5 and K7 epistemic
  reductions.
- It adds K8 planner/preflight reuse evidence for the G39 W6.4 K-clique
  template surface, without claiming K8 accepted runtime dispatch.
