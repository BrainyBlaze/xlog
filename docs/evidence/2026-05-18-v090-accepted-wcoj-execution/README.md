...
existing production runtime paths. The K5 fixture proves WCOJ dispatch; the
unary fixture proves variable-bound nonzero-arity membership filters final rows
on device. This is not a closure claim for `G090_GPU`, `G090_CERT`, or
`G090_CLOSE`.
...
| Accepted nonzero-arity membership | `test_epistemic_gpu_wcoj_execution::accepted_nonzero_arity_membership_filters_final_rows_by_bound_tuple_key` runs `accepted(X) :- node(X), know edge(X)` with `node = [1, 2]` and `edge = [1]`; final device output is `[1]`, proving bound tuple-key membership filters rows rather than accepting every reduced tuple. |
...
- It satisfies one accepted unary variable-bound nonzero-arity membership
  fixture.
