...
| Solver | Existing `xlog-solve` GPU CNF/CDCL production path | `GpuSolverProductionAdapter` wraps `GpuCdclSolver::new`, validates accepted `EpistemicGpuExecutionResult` evidence through `solve_expect_sat_with_gpu_execution_result` and `solve_expect_unsat_with_gpu_execution_result`, then calls existing SAT/UNSAT APIs while exposing zero CPU assignment and MaxSAT enumeration counters. | GPU-native MaxSAT, SAT/MaxSAT portfolio production paths, and full multi-candidate assumption lifecycle traces are explicitly blocked. |
