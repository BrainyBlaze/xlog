...
It is closure evidence for `G090_CERT.13` when read with the final closure
proposal and post-checkpoint gates. The runtime, solver, and probability
accepted-path status rows recorded in earlier drafts are closed for the current
v0.9.0 closure matrix.
...
markers, counters, and explicit closure status, while board, merge, push, and
...
which proves those two single-result quaternary accepted evidences reach the
existing learned-clause reuse, bounded MaxSAT, and status-aware portfolio
adapter paths with two learned-clause arena publications/imports/reused solves,
four workspace-backed UNSAT solves, four GPU CDCL MaxSAT candidate solves, two
MaxSAT optima, two SAT jobs, two MaxSAT jobs, two UNKNOWN jobs, two TIMEOUT
jobs, one accepted `possible` counter, one accepted `not know` counter, eight
...
adapter paths with one accepted `possible` counter, one accepted `not know`
counter, two nonzero-arity evidence consumptions, eight tuple-key column reads,
two direct UNSAT prunes, four encoded candidates, twelve scheduled GPU CDCL
candidate solves, four scheduler UNSAT prunes, UNKNOWN/TIMEOUT scheduler
statuses, and zero CPU search or MaxSAT enumeration.
The source audit now also requires
...
GPU timing audit note: the single-result timing marker
`accepted_gpu_execution_result_records_kernel_timing` proves an accepted
single-result nonzero-arity execution records kernel launches, zero host writes,
and one CUDA-event timing pair for each hot-path phase: candidate generation,
propagation, candidate validation, model membership, world-view validation,
accepted materialization, final-result materialization, and final tuple
materialization. The split-batch quaternary all-operator timing marker
`accepted_split_quaternary_all_operator_batch_records_component_kernel_timing`
extends the same all-phase timing contract across four accepted arity-four
components and the aggregate batch trace.

Final-result transfer audit note: the single-result transfer marker
`accepted_gpu_execution_result_records_final_result_transfer_budget` proves an
accepted single-result execution reports zero tracked hot-path H2D/D2H calls and
...
The quaternary possible/not-know source-gradient marker
`accepted_quaternary_possible_and_not_know_results_gate_source_conditioned_probabilistic_gradients`
proves those same two accepted single-result GPU evidences also route through
`compile_and_evaluate_conditioned_source_with_grads_for_gpu_execution_results`,
recording source-conditioned arity-four `possible` and `not know` evidence,
one negative fact, two source conditioned-gradient evaluations, and zero CPU
probability recomputation.
...
device allocations. It checks buffer lengths against the preflight workspace
layout, device reset byte accounting, four device zero operations, zero reset
host writes, and trace byte counts for generated candidates, propagated world
views, model-membership bytes, and rejection slots.
...
The split-batch all-binary, quaternary possible/not-know, and quaternary
all-operator CPU-fallback markers
`accepted_split_all_binary_operator_batch_rejects_cpu_fallback_counters`,
`accepted_split_quaternary_possible_and_not_know_batch_rejects_cpu_fallback_counters`,
and `accepted_split_quaternary_all_operator_batch_rejects_cpu_fallback_counters`
prove accepted solver and probabilistic batch consumers fail closed when
...
proceed across binary and arity-four split batches.
...
PIR/CNF adapters.
The source probabilistic marker
...
- This audit closes `M090_CERT.13` when read with the final closure proposal and
  post-checkpoint validation gates.
- `M090_GPU.11`, `M090_SOLVER.9`, and `M090_PROB.8` are closed for the current
  v0.9.0 matrix; broader accepted execution traces remain future work.
