...
- bounded final-row map construction that filters accepted unary, possible,
  ...
  binary `not know`,
  quaternary generic-arity `know`, multi-membership, and `not know` nonzero-arity output
...
- device-derived semantic trace accounting that reads bounded rejection-reason
  ...
  including bounded FAEEL, G91, unary operator, binary all-operator, quaternary generic-arity, multi-membership, and
...
- accepted solver production adapters that gate GPU CDCL SAT/UNSAT, reusable
  ...
  accepted split-batch solver lifecycle, learned-clause reuse, and portfolio dispatch with batch/component counters,
  ...
  distinct-CNF learned-clause import rejection, bounded single-
...
- accepted-world-view probabilistic evidence fixtures and production adapter
  ...
  aggregate and source/program-specific operator-conditioned trace counters,
  ...
  PIR/CNF encoding with source/program-specific PIR/CNF counters, query
  evaluation, and single-record, two-record, and split-batch gradient evaluation.
...
- broader nonzero GPU launch counts and kernel timings for actual stable-model
  ...
  binary/quaternary-generic/multi-membership/missing-required and `not know`
  accepted fixtures;
...
- broader accepted-world-view probabilistic coverage beyond the bounded
  split-batch conditioned source/program query/gradient, conditioned query/gradient, and PIR/CNF GPU-native knowledge-compilation
