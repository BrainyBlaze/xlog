...
transfers. The same-rule all-operator mixed-membership
...
and `not know` counters and zero CPU search; the
...
- accepted solver production adapters that gate GPU CDCL SAT/UNSAT, reusable
  ...
  learned-clause reuse, MaxSAT, and portfolio evidence with nonzero-arity
  tuple-key counters,
...
- broader accepted solver semantic integration beyond the current single/multi/split
  ...
  learned-clause/MaxSAT evidence, all-binary split-batch
...
- broader accepted-world-view probabilistic coverage beyond the bounded
  ...
  single-result and two-record source/program PIR/CNF, exact query/gradient, and PIR/CNF
  GPU-native knowledge-compilation fixtures with zero CPU-only probability
  recomputation;
