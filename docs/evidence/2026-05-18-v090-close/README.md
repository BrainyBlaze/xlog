...
probabilistic acceptance matrix, checkpoint SHAs are stable, and the local
release decision is `MERGE_READY`. Push, tag, release-board update, merge, or
main-branch mutation still require separate coordinator authorization.
...
`docs/plans/2026-05-24-v090-closure-proposal.md`. Current closure is no longer
blocked on the previously missing GPU-native code paths or release packaging:

- `G090_CERT`: code/runtime pilots are green for the accepted matrix, including
  post-checkpoint validation.
- `G090_CLOSE`: the local closure table records stable sub-goal commit SHAs and
  a coordinator-facing `MERGE_READY` decision. Explicit coordinator
  authorization is still required before any release workflow mutation.

The dated update paragraphs below preserve their original audit wording,
including local "blocked" statements from before the final checkpoint. The
current gate table and closure proposal supersede those historical statements.
...
| G090_CERT | PASS for current acceptance matrix | Semantic, runtime, solver, probability, integration, compatibility, fmt, and diff checks pass after the stable local checkpoint. |
...
| G090_CLOSE | PASS with `MERGE_READY` decision | Main is integrated, sub-goal commit SHA accounting is stable, and release workflow mutations remain gated on separate coordinator authorization. |

## Historical Gate Table

| Goal | Historical Status | Evidence |
|---|---|---|
| G090_PRE | PASS for inventory | Preflight evidence committed. |
| G090_EIR | SUPERSEDED | EIR is explicit and executable-plan lowering reaches reduced production runtime plans, but accepted epistemic forms still lack production GPU runtime dispatch. |
...
## Release Evidence Superseded By Closure

The current accepted matrix has GPU-native code-path evidence. The stable
checkpoint, final validation bundle, commit-SHA table, and coordinator-facing
`MERGE_READY` decision are recorded in
`docs/plans/2026-05-24-v090-closure-proposal.md`. Coordinator authorization is
still required before any push, tag, release-board update, merge, or main-branch
mutation.
...
Current release decision: `MERGE_READY`.
