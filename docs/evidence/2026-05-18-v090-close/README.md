...
- runtime preflight that rejects nonzero CPU fallback counters and records
  ...
  arity, live edge-permutation counts, and distinct stream-group scheduling
  counts;
- runtime counter guard that refuses to certify WCOJ evidence from preflight
  ...
  world-view staging; K5 certifies edge-permutation, stream-group,
  sorted-layout, and helper-split counts inside the dispatch-certified trace,
  and K6 certifies G38-B helper-split plus runtime histogram metadata-build
  counters;
