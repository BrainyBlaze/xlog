...
| G090_SOLVER | BLOCKED | Accepted GPU runtime evidence can gate GPU CDCL SAT/UNSAT, reusable workspace-backed UNSAT, one-record and two-record bounded push/solve/retract lifecycles, lifecycle UNKNOWN/TIMEOUT propagation, learned-clause arena publication, same-device-CNF learned-clause import/reuse, two-record learned-clause reuse, distinct-CNF learned-clause import rejection, bounded single- and multi-candidate MaxSAT solving, single-result and two-record MaxSAT search pruning, single-result and two-record weighted soft-clause selection encoding through existing GPU CNF/CDCL paths, and bounded SAT/MaxSAT portfolio dispatch with UNKNOWN/TIMEOUT status propagation, but full generalized MaxSAT scheduler coverage remains incomplete. |
