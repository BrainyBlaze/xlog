...
narrows the production-reuse gap, but it is not a closure proposal.
...
- device-derived semantic trace accounting that reads bounded rejection-reason
  ...
  ternary specialized-arity, quaternary generic-arity, multi-membership,
  split-component, and all-binary-operator split GPU-vs-GPT oracle trace
  parity fixtures;
...
- accepted-world-view probabilistic evidence fixtures and production adapter
  ...
  quaternary source plus parsed-program nonzero-arity max-arity evidence,
  ...
  source/program query and gradient fixtures,
...
- broader accepted solver semantic integration beyond the current single/multi/split
  combined lifecycle, all-binary split-batch learned-clause/MaxSAT evidence,
