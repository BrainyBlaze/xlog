...
| G090_GPT | PARTIAL | CPU trace fixtures pass; GPU-resident candidate generation, propagation staging, and candidate-buffer validation exist, but stable-model world-view validation is missing. |
| G090_SPLIT | PARTIAL | CPU split/recompose fixtures pass; GPU split execution is missing. |
| G090_GPU | BLOCKED | GPU-plan, reduced-runtime-plan, workspace allocation/reset, bounded candidate-generation, propagation, and candidate-validation kernels, runtime-preflight, counter-guard, and reduced-plan trace contracts exist, but no production epistemic stable-model world-view validation/materialization dispatch, full semantic kernel buffer use, or timing evidence exists. |
