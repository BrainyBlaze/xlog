...
learned-clause reuse, bounded MaxSAT, and status-aware portfolio adapter paths
with three accepted `not possible` evidence consumptions and zero CPU search; the
...
- accepted solver production adapters that gate GPU CDCL SAT/UNSAT, reusable
  ...
  and portfolio evidence with nonzero-arity tuple-key counters,
