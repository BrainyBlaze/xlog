...
- runtime counter guard that refuses to certify WCOJ evidence from preflight
  metadata unless production WCOJ counters advance, while accepted K5/K7
...
- K8 preflight evidence, plus K7 runtime evidence, that generated epistemic
  reductions reuse the G39 K-clique template planner metadata, with complete
  21/28 edge-permutation counts and zero planned-hash/CPU-fallback counters;
...
- broader WCOJ-eligible epistemic reductions proving successful runtime
  dispatch beyond the current accepted K5/K7 fixtures and K8 preflight-only
  planner coverage, including layout and helper-splitting evidence where
  applicable;
- broader accepted SAT/UNSAT multi-candidate solver lifecycle coverage with
