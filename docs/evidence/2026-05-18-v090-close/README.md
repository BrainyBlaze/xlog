...
- runtime preflight that rejects nonzero CPU fallback counters and records
  ...
  counts, helper-split specs, and production helper relation rule/scan counts;
- runtime counter guard that refuses to certify WCOJ evidence from preflight
  ...
  edge-permutation, stream-group, sorted-layout, helper-split, helper-rule, and
  WCOJ helper input counts inside the dispatch-certified trace, and K6 certifies
...
- G91 and FAEEL fixture evaluators plus a default FAEEL executable-plan
  foundedness guard and an independently founded self-`possible` accepted
  GPU runtime fixture;
