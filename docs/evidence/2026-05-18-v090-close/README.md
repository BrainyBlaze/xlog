...
| G090_SPLIT | PARTIAL | CPU split/recompose fixtures pass, valid split components lower through GPU executable subplans, and accepted split components execute through `execute_epistemic_gpu_execution_batch` while matching simple component output oracles, per-component GPT trace/candidate-index oracles, and the absent `possible` vs true `not know` world-view oracle with zero CPU candidate/world-view fallback counters; full accepted-runtime semantic parity is still missing. |
...
- accepted-world-view probabilistic evidence fixtures and production adapter
  ...
  operator-specific trace counters,
