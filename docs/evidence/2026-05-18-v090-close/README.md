...
- device-derived semantic trace accounting that reads bounded rejection-reason
  ...
  accepted, rejected, and typed rejection-reason counts with zero CPU
  candidate/world-view fallback counters, including one bounded GPU-vs-GPT
  oracle trace parity fixture;
