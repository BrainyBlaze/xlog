...
| G090_SOLVER | BLOCKED | Accepted GPU runtime evidence can gate GPU CDCL SAT/UNSAT, but MaxSAT, portfolio execution, and full assumption lifecycle traces are not wired to epistemic candidates. |
| G090_PROB | BLOCKED | Accepted GPU runtime evidence can gate exact compilation and gradient evaluation through the existing GPU-native path, but broader probabilistic query, PIR, and knowledge-compilation execution on accepted world views is incomplete. |
...
- bounded final-row map construction that filters accepted unary and binary
  nonzero-arity output rows by bound tuple-key membership on device;
...
- GPU-native MaxSAT/portfolio assumption lifecycle evidence plus broader
  accepted SAT lifecycle traces with distinct SAT, UNSAT, UNKNOWN, and TIMEOUT
  handling;
- accepted-world-view evidence flowing through broader GPU-native
  exact/provenance/PIR/knowledge-compilation query/evaluation with zero
  CPU-only probability recomputation;
