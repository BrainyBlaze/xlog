...
but this plan-contract evidence alone was not a standalone closure for `G090_GPU`.
...
| M090_GPU.3 GPU buffers | candidate, world-view, and rejection state have GPU-resident representations | SUPERSEDED | Buffer categories are explicit; later runtime evidence allocates/resets them and uses bounded candidate/propagation/validation/arity 0-3 tuple-source model-membership/candidate-assumption-aware world-view-validation/materialization/final-result flag/final tuple kernels. |
| M090_GPU.4 kernel coverage | GPU kernels cover candidate generation, propagation, validation, and materialization hot paths | SUPERSEDED | Later runtime evidence launches candidate-generation, propagation-staging, candidate-buffer validation, tuple-source model-membership staging with fixed arity-one/two/three and generic arity-N row-scoped ground and variable-bound comparison, candidate-assumption-aware world-view-validation staging, materialization-staging, final-result flag, final-row map, and final tuple materialization kernels; broader semantic parity remains missing. |
| M090_GPU.5 CPU fallback ban | accepted execution trace records zero CPU candidate enumeration/world-view validation fallbacks | SUPERSEDED | Plan counters initialize to zero and runtime preflight rejects nonzero counters; tuple-source staging has device relation-column, source-term metadata, and row-scoped ground plus variable-bound key byte-comparison evidence, while broader accepted semantics remain missing. |
| M090_GPU.6 launch evidence | certification logs include nonzero GPU launch counts and kernel timing for epistemic execution | SUPERSEDED | Later runtime traces record candidate-generation, propagation, candidate-validation, tuple-source model-membership staging with fixed arity-one/two/three and generic arity-N row-scoped comparison, candidate-assumption-aware world-view-validation, accepted-candidate materialization, final-result flag, final-row map, and final tuple materialization launches with CUDA-event elapsed timing; accepted semantic parity timing is incomplete. |
| M090_GPU.7 parity | GPU output matches semantic oracle on all G91, FAEEL, GPT, and splitting fixtures | SUPERSEDED | Bounded final-output device buffers exist, accepted membership fixtures match tuple-key oracles, accepted split component execution now matches simple component output oracles, split GPU runtime proves absent `possible` rejects while `not know` accepts over the same tuple source, and a bounded `know edge(X)` GPU trace matches GPT oracle counters with a typed unsatisfied-membership rejection reason, but full G91/FAEEL/GPT/splitting semantic parity coverage is not proven yet. |
| M090_GPU.8 transfer budget | host-device transfers are bounded and reported; no per-candidate host round trip in hot path | SUPERSEDED | Later runtime evidence records hot-path provider transfer snapshots and rejects tracked H2D/D2H deltas, then accounts final-result rows, columns, payload bytes, row-count metadata reads, and zero accepted-path data-plane D2H calls after the hot path; broader semantic-matrix transfer coverage remains missing. |
