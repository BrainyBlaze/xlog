...
| Command | Result |
|---------|--------|
| `pytest -q python/tests/test_v080_pyapi_source.py` | exit 0; 3 passed |
...
| M080_PYAPI.3 memory override tests | per-call limit accepted and enforced; over-limit failure is typed | PASS | `runtime_probe.json`: 512 MiB accepted; 1 MiB over-limit raised `MemoryError` with `allocated_bytes=3200012` |
