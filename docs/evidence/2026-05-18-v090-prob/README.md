...
conditioned paths.
Accepted split-batch runtime evidence can now gate conditioned source exact
evaluation through `EpistemicProbGpuBatchExecutionEvidence`, validating the
...
| Accepted split-batch conditioning gate | `compile_and_evaluate_conditioned_source_for_gpu_batch_execution_result` consumes `EpistemicProbGpuBatchExecutionEvidence`, validates the aggregate split-batch trace for one GPU runtime execution per component plus zero CPU recomposition, zero CPU candidate/world-view fallback, zero tracked D2H calls, and zero per-candidate host-round-trip counters, then evaluates each component's accepted assumptions through the existing conditioned source `ExactDdnnfProgram` path while recording batch and component evidence counters. |
...
- The production adapter is partial source/program exact-compile,
  ...
  source/program batch reuse, split-batch conditioned source reuse,
