...
exact/provenance machinery. A bounded zero-arity conditioning path now compiles
accepted epistemic assumptions into exact `evidence(atom(), value)` statements
before evaluating through the GPU exact path, but broader probabilistic
knowledge-compilation coverage over accepted world views is not complete.
...
- The production adapter is partial source/program exact-compile,
  ...
  source evaluation, and source/program bounded compile/evaluate reuse
  evidence only.
