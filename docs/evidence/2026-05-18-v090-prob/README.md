...
epistemic assumptions, including unary and binary true `possible`, unary and
binary false `not possible`, and unary and binary false `not know` operator
evidence, into exact
...
mode-specific accepted G91 and default FAEEL evidence consumptions.
...
- The production adapter is partial source/program exact-compile,
  ...
  operator-level unary and binary possible/not-possible plus binary-not-know conditioned source evaluation,
