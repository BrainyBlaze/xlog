...
production exact compilation plus GPU exact query and gradient evaluation, but
broader end-to-end probabilistic provenance, PIR, and knowledge-compilation
...
| M090_PROB.7 CPU recompute ban | accepted probabilistic epistemic path records zero CPU-only probability recomputation | PARTIAL | Production trace records zero CPU-only recomputation and zero fixture-circuit counters for accepted runtime source-compile, parsed-program compile, query-evaluation, and gradient-evaluation paths; broader probabilistic execution traces are missing. |
| M090_PROB.8 production prob reuse | accepted probabilistic fixtures execute through existing GPU exact/provenance/PIR/knowledge-compilation APIs | PARTIAL | Source guard and integration fixtures prove accepted GPU runtime evidence compiles source and parsed programs, evaluates query probabilities, and evaluates gradients through the existing exact/provenance path; broader PIR, knowledge-compilation, and full query/evaluation coverage is missing. |
...
- The production adapter is partial source/program exact-compile,
  query-evaluation, and gradient-evaluation reuse evidence only.
