...
`EpistemicGpuBatchExecutionTrace` before each component's accepted world-view
assumptions are appended as parsed evidence, with
`accepted_gpu_batch_evidence_consumed`,
`accepted_gpu_batch_component_evidence_consumed`, and source/program-specific
conditioned gradient trace counters.
...
- The production adapter is partial source/program exact-compile,
  ...
  source/program batch reuse, split-batch conditioned source/program query and
  gradient reuse,
