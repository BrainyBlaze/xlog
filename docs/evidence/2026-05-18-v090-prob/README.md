...
epistemic assumptions into exact `evidence(atom(...), value)` statements before
evaluating through the GPU exact path, including two-record conditioned source
and parsed-program batches whose query probabilities differ by accepted
world-view evidence. Conditioned source and parsed-program gradient APIs now
run through the same accepted-world-view evidence boundary, including a
two-record parsed-program gradient batch. Broader probabilistic coverage and
release certification remain incomplete.
...
- The production adapter is partial source/program exact-compile,
  ...
  nonzero-arity conditioned source/program evaluation, source/program bounded
  compile/evaluate reuse, two-record accepted source batch reuse, and
