...
conditioned paths.
Accepted split-batch runtime evidence can now gate conditioned source and
parsed-program exact evaluation through `EpistemicProbGpuBatchExecutionEvidence`, validating the
aggregate `EpistemicGpuBatchExecutionTrace` before each component's accepted
world-view assumptions are appended as parsed evidence, with
`accepted_gpu_batch_evidence_consumed` and
`accepted_gpu_batch_component_evidence_consumed` trace counters.
...
- The production adapter is partial source/program exact-compile,
  single-record and two-record PIR/CNF with source/program-specific PIR/CNF
  trace counters, source/program-specific exact-query trace counters,
  single-record and two-record query-evaluation,
  single-record and two-record gradient-evaluation, zero-arity and concrete
  ...
  source/program batch reuse, split-batch conditioned source/program reuse,
