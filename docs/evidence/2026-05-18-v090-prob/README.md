...
conditioned paths. Ternary and quaternary source-conditioned evidence plus
quaternary parsed-program accepted probabilistic evidence now record aggregate
...
batch now conditions source queries with one true `know`, one true `possible`,
one false `possible`/`not possible`, and one false `know`/`not know` accepted
assumption while preserving aggregate split-batch zero CPU recomposition and
zero probability recomputation counters. The same accepted split-batch evidence
now also gates source and parsed-program PIR/CNF encoding plus already-compiled
exact query and gradient evaluation through the existing GPU exact/provenance
APIs, recording batch/component evidence counters and zero CPU recomputation.
...
- The production adapter is partial source/program exact-compile,
  ...
  all-binary-operator split-batch conditioned source query reuse,
