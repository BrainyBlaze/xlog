...
Current closure status: `G090_PROB` is PASS for the v0.9.0 accepted
GPU-production matrix at local closure commit `cf91091d` plus the subsequent
release-surface reconciliation. The bounded `EpistemicCircuit` fixture remains
oracle-only and cannot satisfy release metrics; closure evidence comes from
accepted `EpistemicProbProductionAdapter`, `epistemic_prob_gpu_accepted_evidence`,
`epistemic_prob_production_reuse`, and cross-crate
...
This is accepted closure evidence for the current v0.9.0 matrix. The accepted epistemic runtime can now feed a
...
zero CPU recomputation. A four-component quaternary all-operator split batch now
conditions source query probabilities with one accepted `know`, `possible`,
`not possible`, and `not know` arity-four assumption while recording accepted
batch/component evidence, two negative evidence facts, and zero CPU probability
recomputation. The same quaternary all-operator split batch now also conditions
parsed-program queries plus source and parsed-program gradients while preserving
source/program arity-four evidence counters, operator-family counters, and zero
CPU probability recomputation. The same quaternary all-operator split batch now
also gates source and parsed-program PIR/CNF encoding plus already-compiled
exact query and gradient evaluation through the existing GPU exact/provenance
APIs. The same accepted split-batch
...
The final closure proposal records the compatibility and certification gates
that close the current probabilistic release surface. Additional probabilistic
matrix expansion remains future work.
...
2026-05-20 addendum: the `M090_PROB.5` through `M090_PROB.8` closure rows
below also include
...
which validate a four-component arity-four split batch through conditioned
source/program exact-query, source/program gradient, source/program PIR-CNF, and
already-compiled exact query/gradient paths with one accepted `know`,
`possible`, `not possible`, and `not know` evidence record,
source/program-conditioned operator-family counters, two negative evidence
facts, batch/component evidence counters, and zero CPU probability recomputation.

## Metric Status

| Metric | Target | Status | Evidence |
|---|---|---|---|
| M090_PROB.1 semantic contract | documented interaction between epistemic and probabilistic layers | PASS | `AcceptedWorldViewEvidence` and architecture docs. |
| M090_PROB.2 incremental circuit fixture | changed assumption updates circuit without full rebuild where supported | PASS | `evidence_conditioning_consumes_accepted_world_view` and `changed_assumption_replaces_active_evidence_without_rebuilding_circuit`. |
| M090_PROB.3 compiler adapter | at least one alternative compiler adapter design or implementation | PASS | `external_ddnnf_text_compiler_adapter_is_explicitly_represented` and `c2d_and_minic2d_compiler_adapters_are_explicitly_represented`. |
...
- The production adapter is accepted source/program exact-compile,
  ...
  split-batch quaternary parsed-program not-possible evidence reuse plus
	  ...
	  source/program gradients, PIR/CNF, and exact query/gradient evaluation,
  split-batch quaternary all-operator conditioned source/program exact-query
  and source/program gradient evidence plus source/program PIR-CNF and
  already-compiled exact query/gradient evaluation,
  ...
  evidence for the current closure matrix.
