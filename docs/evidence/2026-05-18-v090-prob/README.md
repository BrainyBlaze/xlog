...
epistemic assumptions, including true `possible` and false `not possible`
operator evidence plus unary and binary false assumptions from `not know`, into exact
...
- The production adapter is partial source/program exact-compile,
  ...
  operator-level possible/not-possible/binary-not-know conditioned source evaluation,
