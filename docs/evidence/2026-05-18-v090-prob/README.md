...
evaluating through the GPU exact path, including a two-record conditioned source
batch whose query probabilities differ by accepted world-view evidence. The
full query-conditioned probabilistic matrix over accepted world views is not
complete.
...
- The production adapter is partial source/program exact-compile,
  ...
  two-record conditioned source query batch evidence only.
