...
recomputation. The same accepted split-batch
...
which validates a four-component arity-four split batch through the conditioned
source exact-query path with one accepted `know`, `possible`, `not possible`,
and `not know` evidence record, source-conditioned operator-family counters,
two negative evidence facts, batch/component evidence counters, and zero CPU
...
- The production adapter is partial source/program exact-compile,
  ...
  split-batch quaternary parsed-program not-possible evidence reuse plus
	  ...
	  source/program gradients, PIR/CNF, and exact query/gradient evaluation,
  split-batch quaternary all-operator conditioned source exact-query evidence,
