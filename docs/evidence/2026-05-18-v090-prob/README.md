...
CPU probability recomputation. The same accepted split-batch
...
source/program exact-query and source/program gradient paths with one accepted
`know`, `possible`, `not possible`, and `not know` evidence record,
source/program-conditioned operator-family counters, two negative evidence
facts, batch/component evidence counters, and zero CPU probability
recomputation.
...
- The production adapter is partial source/program exact-compile,
  ...
  and source/program gradient evidence,
