...
| Model-membership tuple-source API | `Executor::populate_epistemic_gpu_model_membership_from_tuple_sources` launches zero-arity plus arity-one/arity-two tuple-source kernels once per certified binding and reads named reduced stable-model relation row-count scalars and key columns on device. |
...
| Stable-model membership gate | `EpistemicGpuModelMembershipTrace::require_stable_model_tuple_source` rejects row-count-only staging; the runtime path uses stable tuple-source traces for arity 0-2 bindings and fails closed beyond the fixed-arity kernels. |
...
|---|---|---|---|
| M090_GPU.1 production lowering | accepted epistemic fixture runs through production runtime dispatch | PARTIAL | Runtime API launches candidate generation, propagation, and candidate validation before reduced production-plan execution with counter tracing, then launches arity 0-2 tuple-source-backed model-membership staging, world-view validation, accepted-candidate materialization, final-result flag staging, and device-side final tuple materialization; complete tuple-key matching and full accepted semantics remain missing. |
...
| M090_GPU.3 GPU buffers | candidate, world-view, and rejection state have GPU-resident representations | PARTIAL | Runtime workspace uses `TrackedCudaSlice` handles, device-side reset, bounded candidate-assumption kernel writes, propagation staging writes, candidate validation writes, arity 0-2 tuple-source model-membership staging from existing relation buffers, bounded world-view validation reads/writes, accepted-candidate materialization writes, final-result flag writes, and device-resident final-output tuple buffers; complete stable-model tuple matching is missing. |
| M090_GPU.4 kernel coverage | GPU kernels cover candidate generation, propagation, validation, and materialization hot paths | PARTIAL | Candidate generation, propagation staging, candidate-buffer validation, arity 0-2 tuple-source model-membership staging, bounded world-view validation staging, accepted-candidate materialization staging, final-result flag staging, and final tuple materialization have CUDA kernels; complete value-level tuple matching remains missing. |
| M090_GPU.5 CPU fallback ban | accepted execution trace records zero CPU candidate enumeration/world-view validation fallbacks | PARTIAL | Runtime preflight rejects nonzero forbidden CPU fallback counters, and candidate/propagation/validation/model-membership/world-view-validation/materialization/final-result traces record zero host writes; arity 0-2 tuple-source staging reads device relation columns, while complete tuple-matching evidence remains missing. |
| M090_GPU.6 launch evidence | certification logs include nonzero GPU launch counts and kernel timing for epistemic execution | PARTIAL | Candidate-generation, propagation, candidate-validation, arity 0-2 tuple-source model-membership staging, world-view-validation, accepted-candidate materialization, final-result flag, and final tuple traces each record kernel launches with CUDA-event elapsed timing; complete stable-model membership timing evidence is missing. |
| M090_GPU.9 nonzero-arity membership | at least two fixtures with arity >= 1 check stable-model tuple membership on GPU over existing relation layouts | PARTIAL | Source and trace tests require arity-one and arity-two tuple-source kernels over existing relation columns; semantic GPU parity fixtures are still missing. |
...
tuple-key matching, use that membership when producing final query results,
then produce a complete accepted-execution trace with semantic parity, WCOJ
