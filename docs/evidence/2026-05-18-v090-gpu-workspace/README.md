...
adds accepted K5/K7 WCOJ execution evidence through the production runtime path,
K8 K-clique planner-surface reuse through runtime preflight, and accepted
...
| Accepted WCOJ execution fixtures | `test_epistemic_gpu_wcoj_execution` compiles and executes epistemic K5 and K7 rules with `know gate()`, observes certified production K5/K7 WCOJ dispatch, and materializes one final accepted row for each. The K5 fixture also requires helper-split preflight metadata; the K7 fixture requires max arity 7, edge-permutation count 21, and nonzero sorted-layout metadata. |
| K7/K8 preflight fixture | `test_epistemic_gpu_wcoj_execution::epistemic_k7_k8_reductions_reuse_g39_kclique_planner_preflight_surface` compiles epistemic K7/K8 rules with `know gate()`, confirms `kclique_wcoj_max_arity` and full edge-permutation counts of 21/28 through `EpistemicGpuRuntimePreflight`, and keeps planned-hash/CPU-fallback counters at zero. K8 remains preflight-only evidence. |
...
| M090_GPU.2 WCOJ eligibility | at least one epistemic reduction uses the WCOJ planner/path where eligible | PASS | `test_epistemic_gpu_wcoj_execution` compiles epistemic K5 and K7 reductions through the production K-clique planner surface, executes the reduced runtime plans, observes certified `wcoj_clique5_dispatch_count >= 1` and `wcoj_clique7_dispatch_count >= 1`, and materializes accepted final rows. The K5 fixture also requires helper-split preflight metadata; the K8 preflight fixture proves broader G39 K-clique planner-surface reuse without claiming accepted runtime dispatch. |
...
| M090_GPU.11 production path reuse | source audit and runtime counters prove existing RIR/runtime/WCOJ/K-clique/helper-split paths executed where eligible | PARTIAL | Source audit plus accepted K5/K7 execution prove reuse of reduced RIR lowering, production relation IDs, `execute_plan`, K-clique WCOJ counters, sorted-layout metadata, existing relation buffers, and final tuple materialization for accepted WCOJ-eligible reductions; K5 additionally proves helper-split planner metadata, and K8 preflight proves the G39 K-clique template planner metadata reaches epistemic reductions with complete edge-permutation counts. Accepted-runtime solver SAT/UNSAT plus workspace-backed UNSAT, bounded lifecycle, learned-clause arena publication, same-device-CNF learned-clause import/reuse, distinct-CNF learned-clause import rejection, bounded MaxSAT, and bounded status-aware portfolio route through GPU CDCL or fail closed before unsafe reuse, and accepted-runtime probabilistic source/program exact compile, source/program bounded compile/evaluate, zero-arity evidence conditioning, PIR/CNF encoding, query-evaluation, and gradient-evaluation route through existing prob APIs; broader multi-candidate solver lifecycle plus broader probabilistic knowledge-compilation coverage remain incomplete. |
...
evidence where needed, and add broader multi-candidate solver lifecycle plus
probabilistic knowledge-compilation coverage with zero CPU fallback counters.
