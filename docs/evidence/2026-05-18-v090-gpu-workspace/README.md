...
| Model-membership tuple-source API | `Executor::populate_epistemic_gpu_model_membership_from_tuple_sources` launches `epistemic_populate_model_membership_from_tuple_source_u8` once per certified binding and reads named reduced stable-model relation row-count scalars on device. |
...
| Stable-model membership gate | `EpistemicGpuModelMembershipTrace::require_stable_model_tuple_source` rejects row-count-only staging; the runtime path uses stable tuple-source traces for arity-zero bindings and fails closed for nonzero-arity bindings until tuple-key matching metadata exists. |
...
| M090_GPU.3 GPU buffers | candidate, world-view, and rejection state have GPU-resident representations | PARTIAL | Runtime workspace uses `TrackedCudaSlice` handles, device-side reset, bounded candidate-assumption kernel writes, propagation staging writes, candidate validation writes, arity-zero tuple-source model-membership writes, bounded world-view validation reads/writes, accepted-candidate materialization writes, final-result flag writes, and device-resident final-output tuple buffers; nonzero-arity stable-model tuple matching is missing. |
| M090_GPU.4 kernel coverage | GPU kernels cover candidate generation, propagation, validation, and materialization hot paths | PARTIAL | Candidate generation, propagation staging, candidate-buffer validation, arity-zero tuple-source model-membership, bounded world-view validation staging, accepted-candidate materialization staging, final-result flag staging, and final tuple materialization have CUDA kernels; nonzero-arity semantic tuple matching remains missing. |
...
| M090_GPU.6 launch evidence | certification logs include nonzero GPU launch counts and kernel timing for epistemic execution | PARTIAL | Candidate-generation, propagation, candidate-validation, arity-zero tuple-source model-membership, world-view-validation, accepted-candidate materialization, final-result flag, and final tuple traces each record kernel launches with CUDA-event elapsed timing; nonzero-arity stable-model membership timing evidence is missing. |
...
use that membership when producing final query results, then produce a complete
accepted-execution trace with WCOJ dispatch evidence and zero CPU fallback
counters.
