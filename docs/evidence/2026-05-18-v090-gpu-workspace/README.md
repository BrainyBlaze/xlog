...
K7/K8 K-clique planner-surface reuse through runtime preflight, and accepted
...
| Accepted WCOJ execution fixture | `test_epistemic_gpu_wcoj_execution` compiles and executes an epistemic K5 rule with `know gate()`, requires sorted-layout and helper-split preflight metadata, observes certified production K5 WCOJ dispatch, and materializes one final accepted row. |
| K7/K8 preflight fixture | `test_epistemic_gpu_wcoj_execution::epistemic_k7_k8_reductions_reuse_g39_kclique_planner_preflight_surface` compiles epistemic K7/K8 rules with `know gate()`, confirms `kclique_wcoj_max_arity` and full edge-permutation counts of 21/28 through `EpistemicGpuRuntimePreflight`, and keeps planned-hash/CPU-fallback counters at zero. |
...
| M090_GPU.2 WCOJ eligibility | at least one epistemic reduction uses the WCOJ planner/path where eligible | PASS | `test_epistemic_gpu_wcoj_execution` compiles an epistemic K5 reduction through the production K-clique planner/helper-split surface, requires sorted-layout and helper-split preflight metadata, executes the reduced runtime plan, observes certified `wcoj_clique5_dispatch_count >= 1`, and materializes the accepted final row. The K7/K8 preflight fixture also proves broader G39 K-clique planner-surface reuse without claiming accepted runtime dispatch. |
