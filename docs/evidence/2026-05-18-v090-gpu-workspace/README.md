...
|---|---|---|---|
| M090_GPU.1 production lowering | accepted epistemic fixture runs through production runtime dispatch | PARTIAL | Runtime API launches candidate generation, propagation, and candidate validation before reduced production-plan execution with counter tracing, then launches ground-key tuple-source-backed model-membership staging with row-scoped comparison through specialized arity-one/two/three kernels or the generic arity-N kernel, world-view validation, accepted-candidate materialization, final-result flag staging, and device-side final tuple materialization; bound-variable tuple-key matching and full accepted semantics remain missing. |
...
| M090_GPU.3 GPU buffers | candidate, world-view, and rejection state have GPU-resident representations | PARTIAL | Runtime workspace uses `TrackedCudaSlice` handles, device-side reset, bounded candidate-assumption kernel writes, propagation staging writes, candidate validation writes, ground-key tuple-source model-membership staging from existing relation buffers with encoded expectations and device metadata arrays for generic arity-N keys, bounded world-view validation reads/writes, accepted-candidate materialization writes, final-result flag writes, and device-resident final-output tuple buffers; bound-variable stable-model tuple matching is missing. |
| M090_GPU.4 kernel coverage | GPU kernels cover candidate generation, propagation, validation, and materialization hot paths | PARTIAL | Candidate generation, propagation staging, candidate-buffer validation, tuple-source model-membership staging with specialized arity-one/two/three and generic arity-N row-scoped ground key comparison, bounded world-view validation staging, accepted-candidate materialization staging, final-result flag staging, and final tuple materialization have CUDA kernels; bound-variable tuple matching remains missing. |
...
| M090_GPU.6 launch evidence | certification logs include nonzero GPU launch counts and kernel timing for epistemic execution | PARTIAL | Candidate-generation, propagation, candidate-validation, tuple-source model-membership staging with specialized arity-one/two/three and generic arity-N row-scoped ground key comparison, world-view-validation, accepted-candidate materialization, final-result flag, and final tuple traces each record kernel launches with CUDA-event elapsed timing; accepted semantic parity timing evidence is missing. |
| M090_GPU.9 nonzero-arity membership | at least two fixtures with arity >= 1 check stable-model tuple membership on GPU over existing relation layouts | PARTIAL | Source and trace tests require arity-one, arity-two, arity-three, and generic arity-N tuple-source kernels over existing relation columns, encoded expected tuple-key bits/type codes, device column-pointer/width arrays for wider keys, and device cell-byte comparison for ground keys; semantic GPU parity fixtures are still missing. |
...
The next slice must extend ground-key matching to bound-value tuple keys, use
that membership when producing final query results, then produce a complete
accepted-execution trace with semantic parity, WCOJ dispatch evidence, and zero
