...
adds accepted K5/K7/K8 WCOJ execution evidence through the production runtime
path, K7/K8 K-clique planner-surface reuse through runtime preflight, and accepted
...
| Accepted WCOJ execution fixtures | `test_epistemic_gpu_wcoj_execution` compiles and executes epistemic K5, K7, and K8 rules with `know gate()`, observes certified production K5/K7/K8 WCOJ dispatch, and materializes one final accepted row for each. The K5 fixture also requires helper-split preflight metadata; the K7/K8 fixtures require max arity 7/8, edge-permutation counts 21/28, and nonzero sorted-layout metadata. |
...
| M090_GPU.2 WCOJ eligibility | at least one epistemic reduction uses the WCOJ planner/path where eligible | PASS | `test_epistemic_gpu_wcoj_execution` compiles epistemic K5, K7, and K8 reductions through the production K-clique planner surface, executes the reduced runtime plans, observes certified `wcoj_clique5_dispatch_count >= 1`, `wcoj_clique7_dispatch_count >= 1`, and `wcoj_clique8_dispatch_count >= 1`, and materializes accepted final rows. The K5 fixture also requires helper-split preflight metadata; the K7/K8 fixtures prove broader G39 K-clique planner-surface reuse. |
...
| M090_GPU.11 production path reuse | source audit and runtime counters prove existing RIR/runtime/WCOJ/K-clique/helper-split paths executed where eligible | PARTIAL | Source audit plus accepted K5/K7/K8 execution prove reuse of reduced RIR lowering, production relation IDs, `execute_plan`, K-clique WCOJ counters, sorted-layout metadata, existing relation buffers, and final tuple materialization for accepted WCOJ-eligible reductions; K5 additionally proves helper-split planner metadata, and K7/K8 prove the G39 K-clique template planner metadata reaches epistemic reductions with complete edge-permutation counts before production dispatch. Accepted-runtime solver SAT/UNSAT plus workspace-backed UNSAT, bounded lifecycle, learned-clause arena publication, same-device-CNF learned-clause import/reuse, distinct-CNF learned-clause import rejection, bounded MaxSAT, and bounded status-aware portfolio route through GPU CDCL or fail closed before unsafe reuse, and accepted-runtime probabilistic source/program exact compile, source/program bounded compile/evaluate, zero-arity evidence conditioning, PIR/CNF encoding, query-evaluation, and gradient-evaluation route through existing prob APIs; broader multi-candidate solver lifecycle plus broader probabilistic knowledge-compilation coverage remain incomplete. |

## Remaining Blocker

The next slice must broaden accepted-execution parity beyond the K5/K7/K8 WCOJ
fixtures and add broader helper/skew runtime evidence, multi-candidate solver
lifecycle coverage, and probabilistic knowledge-compilation coverage with zero
