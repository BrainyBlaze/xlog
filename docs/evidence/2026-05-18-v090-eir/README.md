...
| M090_EIR.3 lowering boundary | EIR-to-GPU-executable boundary documented | PARTIAL | Current docs identify the boundary and plan contract, but runtime GPU lowering is missing. |
...
| M090_EIR.6 production route | accepted epistemic forms have a production lowering route; rejected forms are explicit | PARTIAL | `plan_epistemic_gpu_execution` builds a GPU-plan contract; direct RIR/runtime lowering still rejects epistemic literals. |
