...
|---|---|
| Epistemic guide | `docs/epistemic-solver-semantics-guide.md` explains EIR, G91, FAEEL, GPT, splitting including aggregate split-batch zero CPU recomposition tracing, split binary operator GPT parity, split all-binary-operator GPT parity, all-binary-operator split-batch probabilistic source/program query and gradient conditioning evidence, world-view fixtures, partial accepted GPU runtime evidence, and the corrected GPU-native blocker. |
