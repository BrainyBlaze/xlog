...
| M090_DOC.5 GPU/WCOJ execution | guide documents the production GPU-native and WCOJ-backed epistemic execution path | PARTIAL | Guide documents the current bounded accepted GPU/WCOJ runtime path, K5/K6/K7/K8 dispatch evidence, GPU tuple-membership/final-materialization path, solver/probability production adapter slices including binary `possible`/`not possible`/`not know` evidence, and remaining semantic-parity/rebase blockers. |
