...
| WCOJ route metadata inspected | Preflight records reduced rule count, `MultiWayJoin` count, K-clique WCOJ plan count, K-clique max arity, live edge-permutation slot count, distinct stream-group scheduling count, planned-hash count, sorted-layout requirement count, helper-split spec count, tuple-membership binding count, and explicit `know`/`possible`/`not know`/`not possible` operator counts. Certified WCOJ traces now carry the edge-permutation, stream-group, sorted-layout, and helper-split counts only after production K-clique runtime counters advance and a required layout sort or layout fast-path counter is observed. |
