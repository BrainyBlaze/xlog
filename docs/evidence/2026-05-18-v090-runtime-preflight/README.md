...
| M090_GPU.6 launch evidence | nonzero GPU launch counts and timings | PARTIAL | Candidate-generation, propagation, candidate-validation, arity 0-2 tuple-source model-membership, world-view-validation, accepted-candidate materialization, final-result flag, and final tuple traces record nonzero launches with CUDA-event elapsed timing; arbitrary-arity stable-model membership timing evidence is still missing. |
