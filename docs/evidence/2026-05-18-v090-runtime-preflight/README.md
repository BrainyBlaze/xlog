...
|---|---|---|---|
| M090_GPU.1 production lowering | accepted epistemic fixture runs through production runtime dispatch | PARTIAL | Runtime API wraps reduced production-plan execution with counter tracing; candidate generation has a bounded GPU kernel; accepted full Generate-Propagate-Test dispatch is still missing. |
...
| M090_GPU.3 GPU buffers | candidate, world-view, and rejection state have GPU-resident representations | PARTIAL | Prepare API combines preflight with workspace allocation and device-side reset; candidate-assumption buffers can be populated by a bounded CUDA kernel; world-view/model/rejection semantic population is still missing. |
| M090_GPU.4 kernel coverage | GPU kernels cover candidate generation, propagation, validation, and materialization hot paths | PARTIAL | Candidate generation has `epistemic_generate_candidate_assumptions_u8`; propagation, validation, and materialization kernels are missing. |
...
The next slice must move from allocation/reset/candidate generation/counter
tracing to actual epistemic runtime dispatch: propagate candidate assumptions,
validate world views, materialize accepted results, and emit full launch
counters/timing plus zero CPU fallback counters.
