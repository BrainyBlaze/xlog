...
| WCOJ route metadata inspected | Preflight records reduced rule count, `MultiWayJoin` count, K-clique WCOJ plan count, K-clique max arity, live edge-permutation slot count, planned-hash count, sorted-layout requirement count, helper-split spec count, and tuple-membership binding count. Certified WCOJ traces now carry the edge-permutation, sorted-layout, and helper-split counts only after production K-clique runtime counters advance. |
...
| Stable-model membership gate | `EpistemicGpuModelMembershipTrace::require_stable_model_tuple_source` still rejects row-count-only staging; the accepted runtime path uses stable tuple-source traces for arity 0-3 and generic arity-N bindings, including bound-value buffers and `not know` polarity. |
...
| M090_GPU.9 nonzero-arity membership | at least two fixtures with arity >= 1 check stable-model tuple membership on GPU over existing relation layouts | PASS for fixture count | Plan/runtime tests require identity key-column metadata, source tuple terms, encoded expected tuple-key bits/type codes, arity-one/arity-two/arity-three and generic arity-N tuple-source kernels over existing `CudaBuffer` columns, plus binding polarity for negated epistemic literals; accepted unary, binary, multi-membership, and `not know` variable-bound final-row filtering fixtures pass. |
| M090_GPU.10 row-count guard | nonzero-arity membership fails closed if only row-count metadata is available | PARTIAL | `EpistemicTupleMembershipBinding::key_columns` and `bound_output_columns` validation rejects invalid key metadata, row-count-only certification remains a negative fixture, and accepted unary/binary/multi-membership/not-know fixtures prove final output is filtered by tuple key rather than row count alone; complete accepted-execution fixture coverage is still missing. |
