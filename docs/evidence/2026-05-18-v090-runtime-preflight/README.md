...
a device-side workspace reset trace, adds a certification guard tying WCOJ
evidence to actual production counter deltas, and exposes a reduced-plan
execution trace around `execute_plan`. It is still pre-kernel plumbing for the
epistemic hot path and does not close `G090_GPU`.
...
| M090_GPU.3 GPU buffers | candidate, world-view, and rejection state have GPU-resident representations | PARTIAL | Prepare API combines preflight with workspace allocation and device-side reset; kernels do not populate buffers with semantic state. |
...
The next slice must move from allocation/reset/counter tracing to actual
epistemic runtime dispatch: populate workspace buffers, launch
Generate-Propagate-Test kernels or GPU-backed adapters, and emit launch
