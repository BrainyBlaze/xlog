...
| WCOJ route metadata inspected | Preflight records reduced rule count, `MultiWayJoin` count, K-clique WCOJ plan count, K-clique max arity, live edge-permutation slot count, planned-hash count, sorted-layout requirement count, helper-split spec count, tuple-membership binding count, and explicit `know`/`possible`/`not know`/`not possible` operator counts. Certified WCOJ traces now carry the edge-permutation, sorted-layout, and helper-split counts only after production K-clique runtime counters advance. |
...
| Stable-model membership gate | `EpistemicGpuModelMembershipTrace::require_stable_model_tuple_source` still rejects row-count-only staging; the accepted runtime path uses stable tuple-source traces for arity 0-3 and generic arity-N bindings, including bound-value buffers, `possible` operator metrics, and `not know` polarity. |
...
| Runtime WCOJ counter snapshot | `Executor::epistemic_gpu_runtime_counters` snapshots existing production WCOJ, layout-sort, and K-clique metadata count/timing counters. |
...
| Accepted WCOJ dispatch | `test_epistemic_gpu_wcoj_execution` proves accepted K5, K6, K7, and K8 epistemic reductions reach certified production WCOJ dispatch and final row materialization. The K6 fixture additionally proves G38-B helper-split specs compose with runtime K-clique histogram metadata builds and timing counters under accepted epistemic execution. |
...
|---|---|---|---|
| M090_GPU.1 production lowering | accepted epistemic fixture runs through production runtime dispatch | PARTIAL | Runtime API launches candidate generation, propagation, and candidate validation before reduced production-plan execution with counter tracing, then launches arity 0-3 and generic arity-N tuple-source-backed model-membership staging with row-scoped ground and variable-bound key comparison plus `possible` operator preflight metrics and `not know` polarity, world-view validation, accepted-candidate materialization, final-result flag staging, final-row map construction, and final tuple materialization; broader full accepted semantics remain missing. |
| M090_GPU.2 WCOJ eligibility | at least one epistemic reduction uses the WCOJ planner/path where eligible | PASS | Accepted K5 fixture records WCOJ/K-clique/helper metadata, passes the runtime WCOJ certification gate with `certified_edge_permutation_slots == 10`, `certified_sorted_layout_requirements == 1`, and `certified_helper_split_specs == 1`, observes production K5 dispatch counters, and materializes one final accepted row. Accepted K6 records full edge-permutation count `15`, nonzero helper/sorted-layout metadata, `wcoj_clique6_dispatch_count >= 1`, `kclique_metadata_build_count >= 1`, and `kclique_metadata_build_nanos >= 1`, proving the G38-B helper/histogram path under the accepted epistemic boundary. Accepted K7 and K8 fixtures record K-clique max-arity/edge-permutation metadata, pass the runtime WCOJ certification gate, observe production K7/K8 dispatch counters, and materialize one final accepted row each. |
...
| M090_GPU.6 launch evidence | nonzero GPU launch counts and timings | PARTIAL | Candidate-generation, propagation, candidate-validation, tuple-source model-membership staging with fixed arity-one/two/three and generic arity-N row-scoped comparison, world-view-validation, accepted-candidate materialization, final-result flag, final-row map, and final tuple traces record nonzero launches with CUDA-event elapsed timing; semantic summary counts are device-derived from rejection metadata after the hot-path budget; accepted semantic parity timing evidence is still incomplete. |
| M090_GPU.9 nonzero-arity membership | at least two fixtures with arity >= 1 check stable-model tuple membership on GPU over existing relation layouts | PASS for fixture count | Plan/runtime tests require identity key-column metadata, source tuple terms, encoded expected tuple-key bits/type codes, arity-one/arity-two/arity-three and generic arity-N tuple-source kernels over existing `CudaBuffer` columns, explicit operator counts for `know`, `possible`, `not know`, and `not possible`, plus binding polarity for negated epistemic literals; accepted unary, possible, binary, multi-membership, and `not know` variable-bound final-row filtering fixtures pass. |
| M090_GPU.10 row-count guard | nonzero-arity membership fails closed if only row-count metadata is available | PARTIAL | `EpistemicTupleMembershipBinding::key_columns` and `bound_output_columns` validation rejects invalid key metadata, row-count-only certification remains a negative fixture, and accepted unary/possible/binary/multi-membership/not-know fixtures prove final output is filtered by tuple key rather than row count alone; complete accepted-execution fixture coverage is still missing. |
