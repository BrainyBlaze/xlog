# v0.9.0 G090_GPU Runtime Preflight And Counter-Guard Evidence
...
This slice connects `EpistemicExecutablePlan` to `xlog-runtime` preflight and
adds a certification guard tying WCOJ evidence to actual production counter
deltas. It is still pre-kernel plumbing and does not close `G090_GPU`.
...
The next slice must move from counter-guarded preflight to actual runtime
dispatch: populate workspace buffers, launch Generate-Propagate-Test kernels or
GPU-backed adapters, and emit launch counters/timing plus zero CPU fallback
counters.
