...
adds bounded candidate-assumption-aware world-view
validation/materialization staging, adds final-result flag staging from
reduced-output device row-count metadata, adds final tuple materialization into
a device-resident output buffer, adds a
...
| WCOJ route metadata inspected | Preflight records reduced rule count, `MultiWayJoin` count, K-clique WCOJ plan count, K-clique max arity, live edge-permutation slot count, distinct stream-group scheduling count, `kclique_skew_scheduled_plan_count`, planned-hash count, sorted-layout requirement count, helper-split spec count, production helper relation rule count, production WCOJ helper input scan count, tuple-membership binding count, and explicit `know`/`possible`/`not know`/`not possible` operator counts. Certified WCOJ traces now carry the edge-permutation, stream-group, `certified_skew_scheduled_plans`, sorted-layout, helper-split, helper-rule, and WCOJ helper input counts only after production K-clique runtime counters advance and a required layout sort or layout fast-path counter is observed. |
...
| World-view validation staging kernel | `epistemic_validate_world_views_u8` checks staged candidate-assumption and model-membership bytes against active world-view slots, rejects candidates unless every required epistemic literal has tuple-source support, and updates rejection codes. |
...
| Device semantic summary trace | `EpistemicGpuSemanticTrace` reads bounded device rejection-reason metadata after the hot-path transfer-budget window, decodes typed `EpistemicGpuRejectionReason` values, and reports generated, propagated, tested, reduced-model-slot, accepted-world-view, rejected-candidate, accepted/rejected candidate indices, and rejection-reason counts with zero CPU candidate/world-view fallback counters. |
...
| Helper metadata-only evidence rejected | `EpistemicGpuRuntimePreflight::for_executable_plan` returns typed `UnsupportedEpistemicConstruct` with `epistemic GPU helper-split certification` when K-clique helper-split specs exist without compiler-created `__w37_helper_*` relation rules and WCOJ input scans of those helper relations. A separate negative fixture proves a helper scan outside the WCOJ plan does not satisfy the helper-split gate. |
...
| Reduced-plan execution trace | `Executor::execute_epistemic_gpu_execution` prepares workspace, launches candidate generation, propagation, and candidate validation, executes the reduced production runtime plan with `execute_plan`, captures before/after counter deltas in `EpistemicGpuRuntimeTrace`, requires WCOJ certification, clones the named reduced output relation, then launches model-membership, candidate-assumption-aware world-view validation, accepted-candidate materialization, final-result flag staging, and final tuple materialization. |
| Accepted WCOJ dispatch | `test_epistemic_gpu_wcoj_execution` proves accepted K5, K6, K7, and K8 epistemic reductions reach certified production WCOJ dispatch and final row materialization. The K5/K6/K7/K8 fixtures require `kclique_stream_group_count == 1`, the K5/K6 fixtures require `kclique_skew_scheduled_plan_count == 1`, and the certified K5/K6 traces carry nonzero `certified_skew_scheduled_plans`. The K5 fixture requires one production helper rule and one WCOJ helper input scan; the K6 fixture additionally proves G38-B skew-scheduled helper-split specs compose with production helper relation rewrites plus runtime K-clique histogram metadata builds and timing counters under accepted epistemic execution. |
...
| `cargo test -p xlog-integration --test test_epistemic_gpu_wcoj_execution -- --nocapture` | PASS, 75 passed, 0 failed |
...
| M090_GPU.6 launch evidence | nonzero GPU launch counts and timings | PARTIAL | Candidate-generation, propagation, candidate-validation, tuple-source model-membership staging with fixed arity-one/two/three and generic arity-N row-scoped comparison, candidate-assumption-aware world-view validation, accepted-candidate materialization, final-result flag, final-row map, and final tuple traces record nonzero launches with CUDA-event elapsed timing plus row-filter polarity counts; WCOJ traces carry stream-group and skew-scheduled helper certification counts plus K6 metadata-build timing; semantic summary counts are device-derived from rejection metadata after the hot-path budget; accepted semantic parity timing evidence is still incomplete. |
| M090_GPU.9 nonzero-arity membership | at least two fixtures with arity >= 1 check stable-model tuple membership on GPU over existing relation layouts | PASS for fixture count | Plan/runtime tests require identity key-column metadata, source tuple terms, encoded expected tuple-key bits/type codes, arity-one/arity-two/arity-three and generic arity-N tuple-source kernels over existing `CudaBuffer` columns, explicit operator counts for `know`, `possible`, `not know`, and `not possible`, plus binding polarity counts for negated epistemic literals; accepted unary, possible, not possible, binary, multi-membership, missing-required multi-membership, and `not know` variable-bound final-row filtering fixtures pass. |
| M090_GPU.10 row-count guard | nonzero-arity membership fails closed if only row-count metadata is available | PARTIAL | `EpistemicTupleMembershipBinding::key_columns` and `bound_output_columns` validation rejects invalid key metadata, row-count-only certification remains a negative fixture, and accepted unary/possible/not-possible/binary/multi-membership/missing-required/not-know fixtures prove final output and world-view acceptance are filtered by tuple key rather than row count alone; complete accepted-execution fixture coverage is still missing. |
