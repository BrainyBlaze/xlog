...
reject final output rows on device. The current slice broadens accepted
generic arity-N tuple-key parity with a quaternary `know fact4(A, B, C, D)`
fixture whose accepted GPU trace and candidate-index fields match the bounded
GPT oracle. It is not a closure claim for `G090_GPU`, `G090_CERT`, or
`G090_CLOSE`.
...
- This covers unary, binary, and quaternary variable-bound `know` membership fixtures,
  unary and binary variable-bound `possible` and `not possible` membership
  metrics with operator-level GPT trace/candidate-index oracle parity, generic
