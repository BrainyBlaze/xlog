...
device. The current slice broadens same-rule negated mixed membership with
`not know edge(X), not possible blocked(X)` against bounded GPT
trace/candidate-index oracles. It is not a closure claim for `G090_GPU`,
...
- This covers unary, binary, and quaternary variable-bound `know` membership fixtures,
  ...
  fixture, all-`know`, mixed `know`/`possible`, and negated
  `not know`/`not possible` multi-membership acceptance with two-literal GPT
  trace/candidate-index oracle parity,
...
- Multiple-epistemic-literal final-row filters now have focused positive,
  negated, and missing-required fixtures, but broader semantic parity still
