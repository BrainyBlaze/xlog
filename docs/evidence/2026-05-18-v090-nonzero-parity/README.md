...
This artifact records accepted runtime fixtures proving `know`, `possible`, and
negated `not know` variable-bound nonzero-arity tuple membership can filter
final output rows on device. It is not a closure claim for `G090_GPU`,
...
| `cargo test -p xlog-runtime --test test_epistemic_gpu_workspace -- --nocapture` | PASS, 48 passed, 0 failed |
...
- This covers unary and binary variable-bound `know` membership fixtures,
  unary variable-bound `possible` membership metrics, and unary variable-bound
  `not know` absent-key filtering.
