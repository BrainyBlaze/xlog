...
| Solver | `SolverService` is a CPU fixture facade with SAT/UNSAT/UNKNOWN/TIMEOUT/Optimal statuses; `GpuSolverProductionAdapter` is a thin adapter over the existing `GpuCdclSolver` production path with accepted-runtime SAT/UNSAT, reusable workspace-backed UNSAT, single- and multi-candidate bounded lifecycle, learned-clause arena publication, same-device-CNF learned-clause import/reuse, distinct-CNF learned-clause import rejection, bounded MaxSAT candidate, bounded SAT/MaxSAT portfolio gates, UNKNOWN/TIMEOUT portfolio status propagation, and zero CPU search counters; `production_capabilities` reports those GPU-backed adapters available while disallowing the CPU oracle for production metrics; `GpuSolverProductionTrace::require_production_metric_eligibility` rejects CPU-oracle-only traces. | PARTIAL for accepted-runtime SAT, UNSAT, workspace-backed UNSAT, bounded lifecycle, two-record candidate lifecycle, learned-clause arena publication, same-device-CNF learned-clause import/reuse, distinct-CNF learned-clause import rejection, bounded MaxSAT, and bounded status-aware portfolio production reuse; BLOCKED until broader status-aware lifecycle and multi-candidate learned-clause coverage are wired to epistemic candidates. |
| Probabilistic | `AcceptedWorldViewEvidence` guards evidence conditioning in fixtures; `EpistemicProbProductionAdapter` can construct evidence from an accepted `EpistemicGpuExecutionResult`, route source and parsed programs into the existing `ExactDdnnfProgram` GPU exact/provenance compile path, source/program bounded compile/evaluate paths, two-record accepted source batch compile/evaluate, zero-arity conditioned source evaluation through parsed `Evidence` AST entries, `GpuPirGraph`/`GpuPirRoots` upload plus `encode_cnf_gpu`, and query/gradient-evaluation paths, and record zero CPU recompute counters; `EpistemicProbProductionTrace::require_production_metric_eligibility` rejects fixture-only metric traces. | PARTIAL for production exact compile/PIR-CNF/evaluation reuse; BLOCKED until broader nonzero/query-conditioned probabilistic coverage over accepted runtime world views exists. |
...
3. Lower accepted EIR into production runtime plans instead of the current
   ...
   registration metadata, accepted K5/K7/K8 WCOJ dispatch fixtures, and K7/K8
...
5. Route WCOJ-eligible reductions through existing planner/layout/dispatch
   ...
   accepted K5/K7/K8 dispatch plus K7/K8 planner/preflight reuse; broader
...
6. Replace CPU solver fixture search in accepted execution with GPU-native
   ...
   learned-clause import/reuse, distinct-CNF learned-clause import rejection,
   bounded MaxSAT, and bounded status-aware portfolio reuse through
   ...
   `solve_portfolio_with_gpu_execution_result`; broader accepted
   status-aware lifecycle and multi-candidate learned-clause coverage remain
   open.
7. Feed accepted world-view evidence into the existing GPU-native
   ...
   nonzero/query-conditioned probabilistic coverage remains open.
