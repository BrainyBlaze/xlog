...
| GPT | CPU fixture records guesses, reduced models, accepted world views, and rejection reasons. | PARTIAL: candidate generation, propagation staging, candidate-buffer validation, model-membership staging, and materialization staging use GPU-resident buffers; stable-model world-view validation still does not. |
...
4. Add GPU-resident candidate/world-view/rejection buffer population and launch
   ...
   staging, candidate-buffer validation, model-membership staging,
   materialization staging, and CUDA-event elapsed timing for those five
   staging launches; stable-model world-view validation and accepted
