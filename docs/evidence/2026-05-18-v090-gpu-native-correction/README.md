...
| GPU workspace | `EpistemicGpuWorkspace` maps required buffer categories to runtime `TrackedCudaSlice` handles; `EpistemicGpuRuntimePreflight` consumes executable plans and records WCOJ/helper route metadata. | PARTIAL until kernels populate and consume those buffers. |
