...
| Solver | `SolverService` is a CPU fixture facade with SAT/UNSAT/UNKNOWN/TIMEOUT/Optimal statuses; `GpuSolverProductionAdapter` is a thin adapter over the existing `GpuCdclSolver` production path with accepted-runtime SAT/UNSAT, reusable workspace-backed UNSAT, single-, multi-candidate, and split-batch bounded lifecycle through `GpuSolverProductionBatchExecutionEvidence`, accepted split-batch learned-clause reuse, accepted split-batch/component counters, accepted G91/default FAEEL mode-specific solver trace counters, mixed unary and binary `possible`/`not possible` plus binary `not know` operator-result lifecycle, lifecycle UNKNOWN/TIMEOUT propagation, learned-clause arena publication, same-device-CNF learned-clause import/reuse, two-record learned-clause reuse, distinct-CNF learned-clause import rejection, bounded single- and multi-candidate MaxSAT, single-result and two-record MaxSAT search pruning through GPU CDCL UNSAT, single-result and two-record weighted MaxSAT selection encoding/search through existing GPU CNF/CDCL paths, heterogeneous MaxSAT scheduler reuse, single-result, two-record, and split-batch bounded SAT/MaxSAT portfolio gates, UNKNOWN/TIMEOUT scheduler/portfolio status propagation, and zero CPU search counters; `production_capabilities` reports those GPU-backed adapters available while disallowing the CPU oracle for production metrics; `GpuSolverProductionTrace::require_production_metric_eligibility` rejects CPU-oracle-only traces. | PARTIAL for accepted-runtime SAT, UNSAT, workspace-backed UNSAT, bounded lifecycle, two-record candidate lifecycle, accepted split-batch lifecycle, learned-clause reuse, and portfolio dispatch, accepted G91/default FAEEL mode-specific solver trace counters, mixed unary and binary `possible`/`not possible` plus binary `not know` operator-result lifecycle, lifecycle UNKNOWN/TIMEOUT propagation, learned-clause arena publication, same-device-CNF learned-clause import/reuse, two-record and accepted split-batch learned-clause reuse, distinct-CNF learned-clause import rejection, bounded single- and multi-candidate MaxSAT, single-result and two-record MaxSAT search pruning, single-result and two-record weighted MaxSAT encoding/search, heterogeneous MaxSAT scheduler reuse, and single-result plus two-record status-aware portfolio production reuse; BLOCKED until broader solver semantic integration and post-v0.8 certification are complete. |
...
4. Add GPU-resident candidate/world-view/rejection buffer population and launch
   ...
   unary/possible/not-possible/not-know/binary/quaternary-generic/multi-membership variable-bound final-row
...
6. Replace CPU solver fixture search in accepted execution with GPU-native
   ...
   learned-clause reuse, and split-batch portfolio dispatch,
...
| `cargo test -p xlog-integration --test test_epistemic_gpu_wcoj_execution -- --nocapture` | PASS, 83 passed, 0 failed |
