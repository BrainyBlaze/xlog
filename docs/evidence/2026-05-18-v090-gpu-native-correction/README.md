...
| GPT | CPU fixture records guesses, reduced models, accepted world views, and rejection reasons. | PARTIAL: candidate generation, propagation staging, candidate-buffer validation, arity-zero tuple-source model-membership staging, bounded world-view validation staging, accepted-candidate materialization staging, final-result flag staging, and final tuple materialization use GPU-resident buffers; nonzero-arity tuple matching still does not. |
...
4. Add GPU-resident candidate/world-view/rejection buffer population and launch
   ...
   staging, candidate-buffer validation, arity-zero tuple-source model-membership staging, bounded
   ...
   nonzero-arity stable-model tuple matching and accepted
