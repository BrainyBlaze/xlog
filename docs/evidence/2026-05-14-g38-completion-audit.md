...
past G_INT M_INT.5 after restoring the W2.5 selector cert, and is now awaiting
G_INT M_INT.6.
...
| G_INT M_INT.6-M_INT.11 | G38 plan S_INT.3 after M_INT.5 pass | PENDING. Resume at M_INT.6 cached-kernel resolution. |
...
G_PURGE has not started because G_INT still has unrun M_INT.6-M_INT.11 gates.
...
- G_INT M_INT.6: cached-kernel resolution has not run yet.

Per S_INT.3, G_INT should resume at M_INT.6. G_PURGE and G_CLOSE should not be
treated as complete until M_INT.5-M_INT.11 are green.

## Next Steps

1. Run G_INT M_INT.6 cached-kernel resolution and document it in
   `docs/evidence/2026-05-14-g38-int-cached-kernel-resolution.md`.
2. Continue M_INT.7-M_INT.11 sequentially under S_INT.3, stopping on the first
   failing gate.
