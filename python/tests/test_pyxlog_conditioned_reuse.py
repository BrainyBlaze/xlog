...
def test_prepared_conditioned_program_reuses_one_circuit_across_priors(
    ...
    for prior in (0.5, 0.9, 0.1):
        ...
        fresh_source = PROB_SOURCE.replace("0.5::target().", f"{prior}::target().")
        ...
        variable_map = prepared.prob_var_map()
        assert variable_map[target_var]["prob"] == pytest.approx(prior)
...
def test_prepared_conditioned_program_rejects_invalid_updates_atomically() -> None:
    ...
    other_var = next(
        index for index, entry in enumerate(variable_map) if entry["kind"] == "other"
    )
...
def test_prepared_prob_var_map_releases_gil_while_waiting_for_shared_state() -> None:
    ...
    setter_errors: list[BaseException] = []

    def update_all_facts() -> None:
        try:
            setter_entered.set()
            prepared.set_fact_probabilities(updates)
        except BaseException as exc:  # pragma: no cover - surfaced on test thread
