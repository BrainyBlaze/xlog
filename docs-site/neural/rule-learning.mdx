...
description: "The dILP training subsystem: a GPU-accelerated differentiable Inductive Logic Programming engine that learns Datalog rules from examples via gradient descent."
---
...
A higher-level training entry point handles sources that mix neural predicates and
...
- [Python Bindings — ILP Training API](/reference/python) — user-facing API reference
- [GPU Execution](/architecture/gpu-execution) — mask DAG evaluation, stream compaction
- [Probabilistic Engines](/probabilistic/engines) — XGCF circuits, provenance (shared infrastructure)
- [Arrow, DLPack, and cuDF Interop](/guides/interop) — DLPack details
