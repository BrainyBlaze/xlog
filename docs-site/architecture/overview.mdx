...
description: "Contributor-facing deep reference for XLOG's crate decomposition, IR stack, memory residency model, and GPU execution pipeline."
---
...
The companion whitepaper explains the language framing and evaluation results;
this document focuses on how the repository is structured today.
...
For release and version context around technical terms used throughout this document (SCC, RIR, PIR, CNF, XGCF, WMC, Decision-DNNF, DLPack, Arrow IPC, Arrow C Data Interface, Semi-Naive, HISA, etc.), see the [Roadmap](/architecture/roadmap).
...
[GPU Execution](/architecture/gpu-execution), probabilistic
inference in [Probabilistic Engines](/probabilistic/engines), solver
services in [Solver Services](/architecture/solver-services),
and neural/dILP execution in
[Rule Learning](/neural/rule-learning).
...
| `xlog-induce` | Bounded exact-induction engine — scores all `(left, right)` candidate pairs across four fixed 2-body topologies (chain/star/fanout/fanin) in one batched GPU pass via `ilp_exact` kernel; top-K per topology with structured candidate metadata. See [Bounded Exact Induction](/neural/exact-induction) |
...
recorded variants. Default `new` constructors are unchanged.
  ...
  * Architecture: [Device Runtime](/architecture/device-runtime).
...
- [Roadmap](/architecture/roadmap) — Feature status, planned development, glossary

### Architecture Documents

| Document | Description |
|----------|-------------|
| [GPU Execution](/architecture/gpu-execution) | GPU-resident filter, groupby, and arithmetic evaluation |
| [Query Optimizer](/architecture/query-optimizer) | Cost-based join ordering, predicate pushdown, statistics |
| [Arithmetic Expressions](/architecture/arithmetic) | `is` syntax, type inference, GPU evaluation |
| [Probabilistic Engines](/probabilistic/engines) | Exact inference (Decision-DNNF/XGCF) and Monte Carlo sampling |
| [Rule Learning](/neural/rule-learning) | Differentiable ILP trainer architecture and the RFC-backed execution model |
| [Solver Services](/architecture/solver-services) | GPU CDCL verifier (zero host reads) + CLS SAT/MaxSAT services |
| [Adaptive Indexing](/architecture/adaptive-indexing) | HISA-based heat tracking and index selection |
| [Multi-GPU Joins](/architecture/multi-gpu) | Distributed join design (planned) |
| [Data Interoperability](/guides/interop) | Arrow IPC/C Data and DLPack integration |
| [CUDA Certification](/architecture/certification) | PTX kernel test suite (140 tests, 24 categories) |
| [CLI Reference](/reference/cli) | `xlog run` and `xlog prob` commands |
| [Python Bindings](/reference/python) | PyO3 + DLPack API (`pyxlog` module) |
