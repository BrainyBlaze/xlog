---
title: "Roadmap"
description: "Version-oriented development roadmap for XLOG, covering shipped releases through v0.9.2 and planned work through v0.10.0."
---
...
- [x] [Language reference](/reference/language) for the v0.3.2 language surface.
...
- [x] Document the v0.6 device runtime stack
      ...
      → [Device Runtime](/architecture/device-runtime);
      linked from the [System Architecture](/architecture/overview) Memory Management
      section.
- [x] Add migration guidance for operator authors:
      ...
      (Recorded-launch migration guidance, part of the internal
      architecture docs.)
...
- [x] **Gate held: no WCOJ or fully GPU-resident binary-join
      ...
      ongoing operator-author contract documented in the
      recorded-launch migration guidance.
...
<Note>All four blockers closed. v0.6.0 has no outstanding gates.</Note>
...
3. ~~Operator-author migration docs + runtime-stack docs.~~
   **DONE — commit `1b267dbf`.** Both items in the v0.6.0
   ...
   The [Device Runtime](/architecture/device-runtime) page covers the runtime
   stack; the recorded-launch migration guidance
   covers the operator-author checklist. Linked from the
...
- [x] Integrate WCOJ into semi-naive recursive evaluation.
      **Done in the WCOJ expansion pack — `Executor::execute_wcoj_or_fallback_node`
...
- [x] Preserve deterministic mixed execution across WCOJ and
      ...
      **Done in the WCOJ expansion pack — `MultiWayJoin.fallback` identity
...
- [x] Add deterministic WCOJ kernels for 4-way conjunctive joins.
      **Done in the WCOJ expansion pack — `wcoj_4cycle_count` / `wcoj_4cycle_materialize`
...
- [x] Add dedicated WCOJ architecture guide. Architecture reference:
      [Worst-Case Optimal Joins](/architecture/wcoj).
- [x] Document WCOJ eligibility, fallback, and performance tuning in a
      ...
      User guide reference: [WCOJ tuning](/guides/wcoj-tuning).
...
- [x] Add a canonical external consumer certification pack in xlog that replays
      ...
      external consumer pilot by default. (internal certification evidence)
...
- [x] Refresh the [language reference](/reference/language) to the v0.8.5 language contract,
...
- [x] Add `the language architecture record` with parser, term, probability,
      CLI, and v0.9.0 handoff contracts. (internal documentation evidence)
...
- [x] Add finite list syntax and list built-ins. (internal documentation evidence)
...
- [x] Add negation-as-failure syntax and semantics where it is distinct from
      existing probabilistic WFS support. (internal documentation evidence)
- [x] Add magic sets transformation for safe bound recursive queries.
      (internal documentation evidence)

### Probabilistic And Approximate Inference

- [x] Add aggregate support in probabilistic programs. (internal documentation evidence)
- [x] Add aggregate lifting for small domains. (internal documentation evidence)
- [x] Add approximate inference engine pragmas, CLI configuration, and
      confidence reporting. (internal documentation evidence)

### Incremental Parsing And CLI Developer Experience

- [x] Add incremental parsing for interactive use. (internal documentation evidence)
- [x] Add interactive REPL.
- [x] Add watch mode.
- [x] Add CLI explain/plan visualization. (internal documentation evidence)

### Examples And Certification

- [x] Add at least 10 advanced v0.8.5 `.xlog` examples covering every feature
      node and at least 5 feature interactions. (internal documentation evidence)
- [x] Add `scripts/validate_v085_examples.py` or equivalent validation with
      committed evidence JSON. (internal validation evidence)
...
- [x] Add adaptive query re-optimization during execution when runtime
      ...
      deterministic replay and rollback gates. (internal benchmark evidence)
...
- [x] Add G91 semantics as a compatibility mode for classic epistemic logic.
      Evidence: `cargo test -p xlog-logic --test test_epistemic_g91`.
- [x] Add FAEEL semantics as the default Founded Autoepistemic Equilibrium Logic mode.
      Evidence: `cargo test -p xlog-logic --test test_epistemic_faeel`.
...
- [x] Add epistemic semantics guide. Evidence:
      [Epistemic overview](/epistemic/overview).
...
engines, typed fail-closed, real runtime/device pilots).
...
faithful guard-only shortcut; no surrogate scoring path. Design and evidence are
internal; `[Unreleased]` CHANGELOG entries carry the surface
details.
...
- Driver-gated follow-up (no engine prerequisite for the guard-only path):
      ...
      concrete neural-body driver. Feasibility read is internal.
...
(2026-06-12 requirements package). The highest-priority engine defects
