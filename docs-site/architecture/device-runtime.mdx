...
description: "The per-CUDA-ordinal runtime allocator and stream-dependency stack that tracks cross-stream allocation lifetimes and enforces a process-wide byte budget."
---
...
records per-allocation events that the `LaunchRecorder`
...
All of these implement the `DeviceMemoryResource` trait
(`crates/xlog-cuda/src/device_runtime/resource.rs`).
The trait is the seam: every allocation /
...
Hardening" in the [Roadmap](/architecture/roadmap). The v0.6.0 release gate is
