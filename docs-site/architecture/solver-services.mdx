...
description: "XLOG's GPU-native SAT/UNSAT solver services, built around a complete on-device CDCL verifier that returns device-resident results."
---
...
- The whitepaper discusses the GPU CDCL verifier in the compiled reasoning pipeline.
- [Probabilistic Engines](/probabilistic/engines) — probabilistic tier that consumes the verifier
