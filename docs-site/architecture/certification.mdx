...
description: "The XLOG CUDA/PTX kernel certification suite validating correctness, safety, determinism, portability, and performance pathologies across all GPU kernels."
---
...
| `xlog_ilp_exact` | `kernels/ilp_exact.cu` | External-consumer bounded exact-induction scorer (`ilp_exact_score`). See [Bounded Exact Induction](/neural/exact-induction). |
...
- [System Architecture](/architecture/overview#cuda-kernels) — Kernel documentation
- [GPU Execution](/architecture/gpu-execution) — Runtime execution model
