...
Source pages are MDX files under `docs-site/`. The GitHub workflow
`.github/workflows/docs-site.yml` runs whenever `docs-site/**` changes on a pull
...
docs, edit the MDX in `docs-site/`, then let validation, export, and the workflow
publish `docs-dist`.
