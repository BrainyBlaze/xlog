...
correct results. Most non-default settings fall into two families: *tuning knobs*
that trade memory for speed, and *kill switches* that force a fused or factorized
route back onto its baseline path so the two can be compared A/B. Boolean knobs
generally accept `1`/`true` to enable and `0`/`false` to disable; per-variable
parsing is noted where it differs.
...
Read by the `xlog-cuda` build script (`crates/xlog-cuda/build.rs`) at compile time,
or by the kernel loader at startup.
...
These exist so a fused or factorized route can be forced back onto its baseline
path for parity and performance comparison. All of them default to the production
route (switch off).
...
The recorded-launch device-runtime stack is opt-in: the legacy launch paths remain
the production default until the runtime stack is certified end-to-end. The
per-operator flags below parse as "set and non-empty and not `0`" and each is
also implied by the umbrella `XLOG_USE_RECORDED_OPS`.
