...
XLOG fails closed. When a program asks for something an engine cannot run soundly
on the device, the engine returns a typed rejection that names the violated rule —
it does not silently fall back to a slower or semantically different path. This
page catalogs the error types, the exit codes, and the specific rejections you are
most likely to encounter.

## Error types

All Rust-level errors are variants of `XlogError` (defined in
...
| `CompileCapacityExceeded` | The D4 knowledge-compilation phase declined a CNF too large to compile safely — the fixed-capacity emit buffers would overrun and fail with a context-poisoning CUDA launch error. "Too big to compile", catchable, distinct from the verify-phase signal. **(unreleased)** |
| `VerifyBudgetExceeded` | The GPU CDCL equivalence verifier declined: the per-verify conflict budget ran out before a definite answer. The indeterminate result is never trusted as a proof. **(unreleased)** |
...
The production Monte Carlo engine is GPU-resident and bounded. At compile time it
checks every rule and fact against its device model; anything outside that model is
rejected with a typed `ResidentRejection` (surfaced as a `Compilation` error of the
form `resident MC engine rejected program [kind=...] construct=... context=...`).
...
The exact engine (`exact_ddnnf`) lifts aggregates over finite probabilistic
domains with dynamic programming, and that lifting is capped per aggregate group:
...
On `main` (not yet in a published release), the D4 compile and verify phases
decline oversized instances instead of risking a CUDA launch failure that would
poison the GPU context for the rest of the process:

- A CNF whose variable or clause capacity exceeds `XLOG_D4_VERIFY_MAX_VARS` /
  `XLOG_D4_VERIFY_MAX_CLAUSES` declines **before any kernel launch** with
  `CompileCapacityExceeded`. Both bounds default to unbounded.
- A verify whose CDCL search exhausts `XLOG_D4_VERIFY_MAX_CONFLICTS` declines
...
| `IlpConfigError` (a `ValueError` subclass) | `induce_exact(backend="python")` without `XLOG_ALLOW_PYTHON_ILP_REFERENCE=1` — the Python reference scorer is parity-only, never a production path. |
