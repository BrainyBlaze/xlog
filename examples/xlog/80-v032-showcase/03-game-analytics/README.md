...
| Feature | Usage |
|---------|-------|
| **symbol type** | Player names, item names, region names, achievement identifiers |
...
| **sum aggregation** | Total kills, experience points, guild power, inventory value |
| **Comparisons** | Top fraggers (kills >= 20), high achievers (points >= 100) |
| **Arithmetic** | Kill/death/assist ratio calculations, inventory value |

## Key Predicates

### Base Data
```xlog
pred player(symbol, symbol, u32).         // player, username, level
pred player_stats(symbol, u32, u32, u32). // player, total experience, games played, hours
pred match_result(symbol, symbol, u32, u32, u32). // match, player, kills, deaths, assists
pred achievement(symbol, symbol, u32).    // achievement, name, points
...
total_kills(Player, sum(Kills)) :- match_result(_, Player, Kills, _, _).

pred player_achievement_points(symbol, u64).
player_achievement_points(Player, sum(Points)) :-
    player_achievement(Player, Achievement),
    achievement(Achievement, _, Points).

// Guild analytics
pred guild_power(symbol, u64).
guild_power(Guild, sum(Level)) :-
    guild_member(Guild, Player),
    player(Player, _, Level).
```
...
4. **Active regions**: Regions with 50000+ total experience points
