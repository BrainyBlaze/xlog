...
| Negated modal through recursion (GPU WFS) | `33-negated-modal-through-recursion-wfs.xlog` | a negated modal `not know reach` over a relation that co-evolves with recursion reduces to a non-monotone ordinary SCC. The sound semantics is WFS, and the accepted fixture routes through the GPU-backed alternating-fixpoint path: `reach={(1,2)}` while undefined tuples are absent. Convergence may be host-orchestrated, but execution does not fall back to the former host WFS solver |
...
| Negated modal through recursion WFS plus EDB negation | `33b-negated-modal-through-recursion-wfs-with-edb-negation.xlog` | mixed-negation case: the cyclic WFS component contains both `not know reach(X,Y)` and ordinary `not banned(Y)`. The accepted fixture proves the GPU-backed WFS path handles ordinary EDB anti-joins inside the same reduced non-monotone SCC: seed-derived `reach={(1,2)}` is emitted, non-banned unseeded pairs remain undefined, and banned target-column pairs are false/absent. Convergence may be host-orchestrated without using the former host WFS solver |
...
| Negated modal over recursive (stratified) | `37-negated-modal-over-recursive-stratified.xlog` | a negated modal `not know reach` over a recursive relation that is stratified (reach is in a strictly-lower stratum, not co-evolving with the negation) executes on GPU: `not know reach == not reach` (anti-join) once reach is materialized -> `unreachable = node^2 - reach = {(1,1),(2,1),(2,2),(3,1),(3,2),(3,3)}`. Contrast `33-negated-modal-through-recursion-wfs.xlog` (genuine negation cycle -> GPU-backed WFS alternating fixpoint with possible host orchestration, but no fallback to the former host WFS solver) |
...
| G91 possible recursion self-support | `43-g91-possible-recursion-self-support.xlog` | G91 recursive compatibility starts with the relaxed upper bound and then reevaluates against frozen snapshots of the preceding `reach` relation. Every tuple in `linked = vertex x vertex` for `{1,2,3}` survives because `reach(X,Y) :- linked(X,Y)` returns that same concrete tuple to the snapshot read; the full `xlog run` path returns and tests all nine tuples |

Notes: acyclic examples with one epistemic output head use the single-pass plan;
acyclic examples with independent epistemic output heads route through split GPU
execution from `xlog run`. Recursive dependencies are classified first and use
the FAEEL founded least-fixpoint reduction, a descending Gelfond-1991
greatest compatibility fixpoint for a supported exact-tuple positive `possible`
cycle, or GPU-backed WFS for a supported negative cycle. A Gelfond-1991 pass may
use WFS for an unrelated nonmonotone component; the host sequences upper-bound
and frozen-snapshot passes and checks GPU relation-set convergence. A
coalesced acyclic component with more than one
epistemic output head sharing a base modal predicate is solved jointly with
multi-output materialization; each head is materialized against one shared
accepted world view (examples 18 and 21).
...
Current fail-closed boundaries include unsafe unbound negated modal variables,
unbounded or untyped modal tuple keys, and recursive programs that also carry
modal integrity constraints; the complete list is in the
[language reference](../../docs/reference/language.mdx#epistemic-support-and-boundaries).
Positive FAEEL dependency cycles execute through the founded least-fixpoint
reduction: founded predecessors propagate, while an unseeded tuple cycle produces
the empty extension. Other recursive modal semantics with a defined G91 positive
`possible` or WFS interpretation execute through their respective reductions:
