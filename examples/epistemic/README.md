...
| FAEEL-unfounded self-support (rejected) | `31-faeel-unfounded-self-support-rejected.xlog` | v0.9.2 genuinely-undefined boundary (over-broadening gate): `p() :- possible p()` with no independent founded support is UNFOUNDED under default FAEEL and fails closed → typed `UnsupportedEpistemicConstruct` `FAEEL foundedness guard` (names `self-supported possible p/0`). Asserts the permissive determined-closure does NOT leak a non-empty answer for a non-determined self-supported modal. Contrast example 11 (founded → accepted); G91 mode accepts the circular form |
