...
| Device resource management | Provider-owned stream-aware allocator stack with byte budgets and deferred frees. | [Device Runtime](/architecture/device-runtime) |
...
- Unsupported optimizer or kernel shapes record a decline and use the existing
  ordinary GPU route. That route must return the same row set (row-set parity).
