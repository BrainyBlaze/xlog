...
If you just want to turn these speedups on and confirm they fired, start with the
user guide: [Worst-case-optimal join tuning](/guides/wcoj-tuning).
</Note>
...
XLOG has a family of WCOJ routes. The right-hand column records the release line
where each route first became available.
...
| --- | --- | --- |
| Triangle WCOJ | Available since 0.6.2 | Dedicated route for recognized triangle bodies over supported key widths. |
| 4-cycle WCOJ | Available since 0.7.0 | Dedicated route for recognized 4-cycle bodies. |
| K-clique WCOJ | Available since 0.7.0 | Dedicated clique routes with variable-order metadata and helper-split support. |
| Aggregate-fused WCOJ | Available since 0.10.0 | Computes selected grouped aggregates without materializing the full join output. |
| Free Join | Available since 0.10.0 | Generalized GPU multiway route for eligible bodies that do not match a dedicated shape. |
| Factorized recursive deltas | Available since 0.10.0 | Computes novel recursive tuples without materializing witness-multiplied delta joins. |
...
- actual fallback counts by attempted route: chain, dedicated multiway,
  Free Join, planned hash, factorized delta, and group-by fusion;
- WCOJ pipeline-error decline count, reported separately from ordinary
  eligibility or cost-gate fallbacks;
- aggregate-fused groupby dispatch count;
- Free Join dispatch count;
- factorized recursive-delta dispatch count.
...
table. It has been available since 0.10.0.
...
triangle or 4-cycle kernel. It has been available since 0.10.0.
...
This route speeds up recursive rules shaped like transitive closure. It has been
...
- Free Join and factorized recursive deltas have been available since 0.10.0.
...
- [Worst-case-optimal join tuning](/guides/wcoj-tuning) — the user guide: turning
  these routes on, forcing one off, and confirming which one fired.
- [Factorized Execution](/architecture/factorized-execution) — contributor
  overview of the factorized routes.
- [Factorized Execution Internals](/architecture/factorized-execution-internals) —
  implementation details for the factorized routes.
