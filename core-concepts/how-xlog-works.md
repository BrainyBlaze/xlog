...
A shared compiler and provider-owned CUDA runtime support four reasoning paradigms — here is the path a program takes from source to GPU.
...
symbolic computation with CUDA-backed execution. SAT/MaxSAT verification is available
through a shared solver service. You write in one typed language, and a shared
compiler plus provider-owned CUDA runtime services support all four user-facing
paradigms.

Device result buffers can be handed straight to a PyTorch training loop instead of
being copied through host memory. Some routes still use host orchestration and bounded
control receipts; residency claims therefore name the measured execution region. This
page follows the path a program takes from source text to running GPU code.
...
<Steps>
  ...
  <Step title="Optimize">
    Lowering chooses a deterministic greedy join order, and the optimizer pushes
    filters closer to the data so less work reaches later stages. Shape-specific
    selectivity rewrites improve recognized triangle and four-cycle plans; a general
    dynamic-programming join optimizer is not active.
  ...
  <Step title="Execute">
    The ordinary executor orchestrates the plan on the host and dispatches CUDA kernels
    over relations in GPU memory. Eligible recursive query plans are selected
    automatically for a certified resident conditional-graph route; that route runs
    its bounded fixpoint inside one recorded launch and returns one final control
    receipt after synchronization.
...
## Device result handoff

Query-output and gradient buffers remain on the GPU when handed to tensor code. XLOG
exposes them through DLPack capsules and Arrow — two standard formats for sharing
tensors and tables between GPU libraries without copying the data plane through host
memory.

Because of that, a downstream tensor computation in PyTorch, JAX, or cuDF reads those
device buffers directly. Host-orchestrated routes may still observe bounded status or
metadata, so a final tensor's device location is not by itself evidence that the
entire preceding execution had zero transfers.
